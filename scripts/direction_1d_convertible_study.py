"""转债独立研究CLI；旧研究包只读、候选固定、真实预测与在用模型不变。"""
...
def main() -> None:
    ...
    if args.command == "freeze":
        if not all((args.base_run, args.data_dir, args.mapping)):
            parser.error("freeze需要--base-run、--data-dir、--mapping")
        result = study.freeze(args.base_run, args.data_dir, args.mapping, args.run_dir)

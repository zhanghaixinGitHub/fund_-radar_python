...
from app.services import direction_1d_sprint_market_nonnegative as us_etf  # noqa: E402

from scripts import direction_1d_sprint_market_nasdaq_relative as existing  # noqa: E402
...
def run():
    ...
    for name, action in (("existing_through_round80", existing.run), ("us_etf", us_etf.tick)):
        try:
            result[name] = action()
...
def main():
    ...
    lock_path = base.ROOT / "run.lock" if action in ("run", "tick", "report") else base.ROOT / "round81-training.lock"

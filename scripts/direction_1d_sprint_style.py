"""十轮执行入口：保留旧九轮流程，新增市场风格模型最早用于9月16日。"""
...
from app.services import direction_1d_sprint_style as style  # noqa: E402

from scripts import direction_1d_sprint_exposure as existing  # noqa: E402
...
def run():
    ...
    for name, action in (("existing_nine_rounds", existing.run), ("style", style.tick)):
        try:
            result[name] = action()
        except Exception as exc:
            errors[name] = failure(exc, "run")
    if errors:
        base.save(style.root() / "branch-errors.json", errors, replace=True)
...
def main():
    ...
    with (base.ROOT / "run.lock").open("a+b") as lock:
        ...
        try:
            ...
            return
        try:
            result = run() if action == "run" else getattr(style, action)()
            print(json.dumps(result, ensure_ascii=True))
            if action in ("run", "tick"):
                base.save(
                    style.root() / "last-run.json",
                    {"at": base.now().isoformat(), "status": "SUCCEEDED"},
                    replace=True,
                )
        except Exception as exc:
            error = failure(exc, action)
            base.save(style.root() / "last-error.json", error, replace=True)

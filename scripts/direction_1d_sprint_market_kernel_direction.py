...
from app.services import direction_1d_sprint_market_kernel_direction as us_etf  # noqa: E402

from scripts import direction_1d_sprint_market_fund_bias as existing  # noqa: E402
...
def run():
    ...
    for name, action in (("existing_through_round89", existing.run), ("us_etf", us_etf.tick)):
        try:
            result[name] = action()
...
def main():
    ...
    parser.add_argument("action", choices=("plan", "prepare", "preflight", "tick", "run", "report"))
    action = parser.parse_args().action
    lock_path = base.ROOT / "run.lock" if action in ("run", "tick", "report") else base.ROOT / "round90-prepare.lock"

"""第四十八轮入口：对比美股ETF两种价格变化对下一交易日方向的增量。"""
...
from app.services import direction_1d_sprint_us_etf as us_etf  # noqa: E402
...
def run():
    ...
    for name, action in (("existing_through_round47", existing.run), ("us_etf", us_etf.tick)):
        try:
            result[name] = action()
...
def main():
    ...
    lock_path = base.ROOT / "run.lock" if action in ("run", "tick", "report") else base.ROOT / "round48-training.lock"

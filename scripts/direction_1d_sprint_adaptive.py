"""八轮执行入口：保留旧七轮流程，新时间适应模型最早用于9月16日。"""
...
from app.services import direction_1d_sprint_adaptive as adaptive  # noqa: E402

from scripts import direction_1d_sprint_return_target as existing  # noqa: E402
...
def run():
    ...
    for name, action in (("existing_seven_rounds", existing.run), ("adaptive", adaptive.tick)):
        try:
            result[name] = action()
        except Exception as exc:
            errors[name] = failure(exc, "run")
    if errors:
        base.save(adaptive.root() / "branch-errors.json", errors, replace=True)
...
def main():
    ...
    with (base.ROOT / "run.lock").open("a+b") as lock:
        ...
        try:
            ...
            return
        try:
            result = run() if action == "run" else getattr(adaptive, action)()
            print(json.dumps(result, ensure_ascii=True))
            if action in ("run", "tick"):
                base.save(
                    adaptive.root() / "last-run.json",
                    {"at": base.now().isoformat(), "status": "SUCCEEDED"},
                    replace=True,
                )
        except Exception as exc:
            error = failure(exc, action)
            base.save(adaptive.root() / "last-error.json", error, replace=True)

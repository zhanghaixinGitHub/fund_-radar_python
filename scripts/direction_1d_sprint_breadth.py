"""十一轮执行入口：保留旧十轮流程，新增国内同类基金特征最早用于9月16日。"""
...
from app.services import direction_1d_sprint_breadth as breadth  # noqa: E402

from scripts import direction_1d_sprint_style as existing  # noqa: E402
...
def run():
    ...
    for name, action in (("existing_ten_rounds", existing.run), ("breadth", breadth.tick)):
        try:
            result[name] = action()
        except Exception as exc:
            errors[name] = failure(exc, "run")
    if errors:
        base.save(breadth.root() / "branch-errors.json", errors, replace=True)
...
def main():
    ...
    with (base.ROOT / "run.lock").open("a+b") as lock:
        ...
        try:
            ...
            return
        try:
            result = run() if action == "run" else getattr(breadth, action)()
            print(json.dumps(result, ensure_ascii=True))
            if action in ("run", "tick"):
                base.save(
                    breadth.root() / "last-run.json",
                    {"at": base.now().isoformat(), "status": "SUCCEEDED"},
                    replace=True,
                )
        except Exception as exc:
            error = failure(exc, action)
            base.save(breadth.root() / "last-error.json", error, replace=True)

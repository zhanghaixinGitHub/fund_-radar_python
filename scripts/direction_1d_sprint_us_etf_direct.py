...
from app.services import direction_1d_sprint_us_etf_direct as us_etf  # noqa: E402

from scripts import direction_1d_sprint_us_etf as existing  # noqa: E402
...
def run():
    ...
    for name, action in (("existing_through_round48", existing.run), ("us_etf", us_etf.tick)):
        try:
            result[name] = action()
...
def main():
    ...
    lock_path = base.ROOT / "run.lock" if action in ("run", "tick", "report") else base.ROOT / "round49-training.lock"

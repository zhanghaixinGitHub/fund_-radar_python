        ...
        rows.append(
            {
                ...
                "market": {
                    ...
                    "dollar_etf": {
                        "available": True,
                        "intraday_log_pct": direction * 0.5,
...
def test_four_dimensional_training_on_synthetic_rows_preserves_original_weights(synthetic_training):
    rows = synthetic_training
    head = s.fit(rows, s.CANDIDATES[0], "2025-01-01")
    assert s.validate_head(head, s.CANDIDATES[0]).coef_.shape == (1, 4)
...
def test_invalid_training_input_stops_before_fit(synthetic_training, kind):
    if kind == "immature":
        ...
        synthetic_training[-1]["market"]["dollar_etf"]["available"] = False
...
def test_missing_extra_keeps_corresponding_parent_answer():
    values = [
        {"available": True, "dollar_etf": {"available": False}},
        {"available": False, "dollar_etf": {"available": True}},
    ]
...
@pytest.fixture
def identity_input():
    rows = [{"code": "001021", "y": 1, "market": {"original": 2, "dollar_etf": {"intraday_log_pct": 0.1}}}]
...
def test_actual_question_or_source_changes_rejected(identity_input, kind):
    ...
    if kind == "source_hash":
        ...
        rows[0]["market"]["dollar_etf"]["intraday_log_pct"] = 0.2
    else:
...
def test_missing_new_feature_falls_back_to_sign_control_not_raw(monkeypatch):
    ...
    result = s.batch([{"available": True, "dollar_etf": {"available": False}}], {}, [])
...
def test_training_missing_signal_is_zero_but_keeps_original_three_inputs(monkeypatch):
    ...
    assert s.vector({"dollar_etf": {"available": False, "intraday_log_pct": 0.0}}) == [1.0, -1.0, 1.0, 0.0]
    with pytest.raises(ValueError, match="MISSING_INPUT_NOT_ZERO"):
        s.vector({"dollar_etf": {"available": False, "intraday_log_pct": 1.0}})

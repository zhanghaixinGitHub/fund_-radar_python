...
from app.services import direction_1d_sprint_sector_joint as s
...
def test_changed_saved_input_is_rejected(ready, monkeypatch):
    ...
    with pytest.raises(ValueError, match="ROUND_39_VECTOR_CHANGED"):
        s.report()


def test_prior_failure_still_attempts_sector_joint_branch(monkeypatch):
    from scripts import direction_1d_sprint_sector_joint as entry

    assert entry.existing.__name__ == "scripts.direction_1d_sprint_sector_exposure"
    monkeypatch.setattr(entry.existing, "run", lambda: (_ for _ in ()).throw(ValueError("OLD_FAILED")))
    calls = []
    monkeypatch.setattr(entry.sector_joint, "tick", lambda: calls.append(1))
...
def test_malformed_feature_shape_or_nonfinite_is_rejected(values):
    with pytest.raises(ValueError, match="JOINT_INPUT_INVALID"):
...
def test_single_error_class_cannot_be_trained(monkeypatch):
    ...
    with pytest.raises(ValueError, match="JOINT_MODEL_ERROR_LABELS_INSUFFICIENT"):
...
def test_code_freeze_covers_context_and_prototype(monkeypatch):
    ...
    assert len(files) == 6
    assert "app/services/direction_1d_sprint_sector_joint_context.py" in files
    assert any(path.endswith("round-39/context-feasibility.py") for path in files)
...
def test_rehashed_context_cannot_replace_model_bound_snapshot(tmp_path, monkeypatch):
    ...
    p = {"current_fit_cutoff": "2026-09-15", "input_hashes": {"round-39/current-context-feasibility.json": "reference"}}

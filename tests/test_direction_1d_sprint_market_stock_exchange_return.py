"""交易所相对强弱模型的固定四维、时间边界与缺失回退；不跑真实基金成绩。"""
...
def test_only_exchange_return_is_read_from_the_shared_two_feature_source(monkeypatch):
    monkeypatch.setattr(s.baseline, "market_vector", lambda _: np.array([1.0, -2.0, 0.0]))
    market = {"stock_exchange_spread": {"available": True, "breadth_spread": -1.0, "return_spread": 3.0}}
    expected = [1.0, -2.0, 0.0, 3.0]
    assert s.vector(market) == expected
    market["stock_exchange_spread"]["breadth_spread"] = 1.0
    assert s.vector(market) == expected


def test_sign_three_inputs_keeps_exchange_spread_continuous():
    x = [[2.0, -0.1, 0.0, 0.5]]
    assert np.allclose(s.transform(x, [2.0, 2.0, 2.0, 0.5], True), [[1.0, -1.0, 0.0, 1.0]])
    assert np.allclose(s.transform(x, [2.0, 2.0, 2.0, 0.5], False), [[1.0, -0.05, 0.0, 1.0]])
...
@pytest.mark.parametrize("basis", [float("nan"), 41.0, True])
...
def synthetic_training(monkeypatch):
    """只用人工504行走实际拟合函数，验证四维矩阵和样本权重；不读取基金成绩。"""
    rows = []
    for i in range(504):

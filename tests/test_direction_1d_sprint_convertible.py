...
from app.services import direction_1d_sprint_convertible as s

from test_direction_1d_sprint_convertible_data import points
...
def trained(score):
    return {
        ...
        "convertible": {"model": ConstantModel(score), "mean": [0.0] * 16, "scale": [1.0] * 16},
    }


def test_nonconvertible_predictions_remain_exact_hk_even_if_convertible_model_changes():
    z = [-1.0] + [0.0] * 16
...
def test_only_convertible_route_uses_strict_error_gate(move):
    z = [move] + [0.0] * 15 + [1.0]
    ...
    with pytest.raises(ValueError, match="GROUP_ROUTE"):
        s.answer(z, s.CANDIDATES[0], trained(0.6) | {"convertible": None})
...
def test_live_and_historical_features_use_one_day_target():
    z = s.vector([0.0] * 32, "005284", "2026-09-14", "2026-09-15", markets(), points())
    ...
    with pytest.raises(ValueError, match="NOT_ADJACENT"):
        s.vector([0.0] * 32, "005284", "2026-09-14", "2026-09-16", markets(), points())


def test_future_and_other_fund_rows_cannot_change_convertible_fit(monkeypatch):
    monkeypatch.setattr(s, "active", lambda: None)
    rows = [r | {"code": "005284", "z": r["z"] + [0.0] * 4 + [1.0]} for r in historical_rows()]
...
def test_feature_ablation_keeps_convertible_route_and_drops_only_four_features():
...
def ready(hk_ready, monkeypatch):  # noqa: F811
    ...
    input_value = {
        ...
        "target": "2026-09-15",
        "rows": points(),
        "source": s.convertible_data.SOURCE,
    }
    monkeypatch.setattr(s.convertible_data, "capture", lambda at: input_value)
    monkeypatch.setattr(s.convertible_data, "load", lambda target: input_value)
    return hk_ready
...
def test_changed_saved_fxi_value_is_rejected(ready, monkeypatch):
    ...
    with pytest.raises(ValueError, match="ROUND_47_VECTOR_CHANGED"):
        s.report()


def test_prior_failure_still_attempts_fxi_branch(monkeypatch):
    from scripts import direction_1d_sprint_convertible as entry

    assert entry.existing.__name__ == "scripts.direction_1d_sprint_food"
    monkeypatch.setattr(entry.existing, "run", lambda: (_ for _ in ()).throw(ValueError("OLD_FAILED")))
    calls = []
    monkeypatch.setattr(entry.convertible, "tick", lambda: calls.append(1))

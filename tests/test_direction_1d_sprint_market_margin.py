...
import numpy as np
import pytest
from app.services import direction_1d_sprint_market_margin as s
...
def test_immature_and_unavailable_rows_cannot_change_fitted_model(monkeypatch, name):
    ...
    assert before["model"].fit_intercept is False and before["model"].n_features_in_ == 3
...
def test_new_entry_runs_independent_branch_even_when_old_branch_fails(monkeypatch):
    from scripts import direction_1d_sprint_market_margin as entry

    assert entry.existing.__name__ == "scripts.direction_1d_sprint_market_intercept"
...
def test_margin_is_not_limited_to_probability_and_zero_means_up():
    class Margin:
        def decision_function(self, x):
            return np.array([-2.0, 0.0, 2.0])

        def predict_proba(self, x):
            pytest.fail("SVM margin must not be presented as calibrated probability")

    head = {"model": Margin(), "mean": [0.0] * 3, "scale": [1.0] * 3, "signed": False}
    result = s.batch_answers([market()] * 3, s.CANDIDATES[0], head)
    assert [r["prediction"] for r in result] == [0, 1, 1]
    assert [r["research_score"] for r in result] == [-2, 0, 2]
    assert {r["kind"] for r in result} == {"UNCALIBRATED_DIRECTION_MARGIN"}

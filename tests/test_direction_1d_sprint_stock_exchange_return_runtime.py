...
import pytest
from app.services import direction_1d_sprint_stock_exchange_return_runtime as s
...
def test_completed_parent_and_training_bind_once_without_changing_training(runtime_fixture, monkeypatch):
    ...
    frozen = s.plan()
    assert frozen["artifact_hashes"]["round-118/result.json"] == s.base.digest(prior)

...
import pytest
from app.integrations import tushare_sprint_stock_breadth_v2 as s
...
def test_missing_quote_values_only_outside_fixed_scope_do_not_remove_sh_sz():
    v = body()
    expected = s.parse(json.dumps(v).encode(), "2026-09-15", minimum_included=3)
    v["data"]["items"][-1][3] = None
    v["data"]["items"][-1][4] = None
    actual = s.parse(json.dumps(v).encode(), "2026-09-15", minimum_included=3)
    assert actual == expected | {"excluded_bj_nonfinite_rows": 1}
    v["data"]["items"][0][4] = None

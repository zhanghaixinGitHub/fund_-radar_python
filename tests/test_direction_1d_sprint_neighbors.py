"""相似历史交易日：日期去重、家族权重、成熟边界及提前答案完整性。"""
...
from app.services import direction_1d_sprint_neighbors as s
...
def test_future_labels_and_features_cannot_change_fitting(monkeypatch, candidate):
    ...
    np.testing.assert_allclose(
        before["model"].predict_proba([np.zeros(9)]), after["model"].predict_proba([np.zeros(9)])
    )


def test_error_labels_keep_natural_date_prior_and_fixed_neighbors(monkeypatch):
    monkeypatch.setattr(s, "active", lambda: None)
    rows = historical_rows()
    ...
    assert fitted["model"].predict_proba([np.zeros(9)])[0, 1] == pytest.approx(fitted["model"].errors[:32].mean())
    assert fitted["training_target"] == "NEXT_DAY_SPX_BASELINE_ERROR"
    assert fitted["neighbors"] == 32
    assert len(fitted["model"].points) == len(set(fitted["neighbor_dates"])) == 180
    assert s.answer([0.0] * 12, s.CANDIDATES[0], fitted)["prediction"] == 0


def trained(score):
    return {"model": ConstantModel(score), "mean": [0.0] * 9, "scale": [1.0] * 9}
...
def test_changed_saved_input_is_rejected(ready, monkeypatch):
    ...
    with pytest.raises(ValueError, match="ROUND_29_VECTOR_CHANGED"):
        s.report()


def test_prior_failure_still_attempts_neighbors_branch(monkeypatch):
    from scripts import direction_1d_sprint_neighbors as entry

    assert entry.existing.__name__ == "scripts.direction_1d_sprint_boost"
    monkeypatch.setattr(entry.existing, "run", lambda: (_ for _ in ()).throw(ValueError("OLD_FAILED")))
    calls = []
    monkeypatch.setattr(entry.neighbors, "tick", lambda: calls.append(1))
...
def test_malformed_feature_shape_or_nonfinite_is_rejected(values):
    with pytest.raises(ValueError, match="NEIGHBOR_INPUT_INVALID"):
...
def test_single_error_class_cannot_be_trained(monkeypatch):
    ...
    with pytest.raises(ValueError, match="NEIGHBOR_ERROR_LABELS_INSUFFICIENT"):
        s.fit(rows, s.CANDIDATES[0], "2023-07-01")


def test_duplicate_share_rows_do_not_increase_date_or_family_vote(monkeypatch):
    monkeypatch.setattr(s, "active", lambda: None)
    rows = historical_rows()
    before = s.fit(rows, s.CANDIDATES[0], "2023-07-01")
    duplicates = [r | {"code": r["code"] + "_copy"} for r in rows if r["family"] == "a"]
    after = s.fit(rows + duplicates * 5, s.CANDIDATES[0], "2023-07-01")
    assert before["fit_rows"] < after["fit_rows"]
    assert before["date_aggregation_hash"] == after["date_aggregation_hash"]
    np.testing.assert_array_equal(before["model"].errors, after["model"].errors)
    np.testing.assert_array_equal(before["model"].points, after["model"].points)


def test_exact32_distinct_neighbors_and_oldest_ties_are_deterministic():
    model = s.MarketDayNeighbors(np.zeros((40, 9)), np.r_[np.ones(32), np.zeros(8)])
    assert model.predict_proba(np.zeros((1, 9)))[0, 1] == 1
    assert model.predict_proba(np.zeros((300, 9))).shape == (300, 2)
    with pytest.raises(ValueError, match="TRAINING_POINTS_INVALID"):
        s.MarketDayNeighbors(np.zeros((31, 9)), np.zeros(31))


def test_conflicting_same_date_market_inputs_fail_closed(monkeypatch):
    monkeypatch.setattr(s, "active", lambda: None)
    rows = historical_rows()
    rows[1]["z"][6] += 1
    with pytest.raises(ValueError, match="SAME_DATE_MARKET_MISMATCH"):
        s.fit(rows, s.CANDIDATES[0], "2023-07-01")

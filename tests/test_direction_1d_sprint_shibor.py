"""Shibor利率的一日目标、原输入保留、成熟时点及未来证据绑定。"""
...
from app.services import direction_1d_sprint_shibor as s
...
from test_direction_1d_sprint_shibor_data import body


def test_code_freeze_covers_real_shibor_client_and_data_files(monkeypatch):
    monkeypatch.setattr(s.previous_round, "fingerprint", lambda: {"code": {}})
    code = s.fingerprint()["code"]
    assert len(code) == 6
    assert "app/integrations/tushare_sprint_shibor.py" in code
    assert "app/services/direction_1d_sprint_shibor_data.py" in code


def test_shibor_appends_eight_known_inputs_and_target_stays_one_day():
    x = [0.0] * 32
    points = s.shibor_data.parse(body())
    z = s.vector(x, "2026-09-14", "2026-09-15", markets(), points)
    assert len(z) == 20 and z[:12] == s.hk.vector(x, "2026-09-14", "2026-09-15", markets())
    assert z[-8:] == s.shibor_data.features("2026-09-14", "2026-09-15", points)
    with pytest.raises(ValueError, match="NOT_ADJACENT"):
        s.vector(x, "2026-09-14", "2026-09-16", markets(), points)


def training_rows():
    return [r | {"z": r["z"] + [1.0, 0.1] * 4} for r in historical_rows()]
...
def test_future_labels_and_input_changes_do_not_change_fitting(monkeypatch):
    ...
    np.testing.assert_allclose(
        before["model"].predict_proba([np.zeros(20)]), after["model"].predict_proba([np.zeros(20)]), atol=1e-12
    )
...
def trained(score):
    return {"model": ConstantModel(score), "mean": [0.0] * 20, "scale": [1.0] * 20}
...
def ready(hk_ready, monkeypatch):  # noqa: F811
    ...
    input_value = {
        ...
        "base": "2026-09-14",
        "target": "2026-09-15",
        "rows": s.shibor_data.parse(body()),
        "source": s.shibor_data.SOURCE,
    }
    monkeypatch.setattr(s.shibor_data, "capture", lambda at: input_value)
    monkeypatch.setattr(s.shibor_data, "load", lambda target: input_value)
    return hk_ready
...
def test_live_input_order_and_announcements_are_checked(ready):
    ...
    with pytest.raises(ValueError, match="SEQUENCE_LIVE_DATES_INVALID"):
        s.live_vector(source, original | {"inputs": original["inputs"][::-1]}, markets(), s.shibor_data.parse(body()))
    original["inputs"][0]["ann_date"] = "2026-09-16"
    with pytest.raises(ValueError, match="SEQUENCE_LIVE_DATES_INVALID"):
        s.live_vector(source, original, markets(), s.shibor_data.parse(body()))
...
def test_changed_saved_shibor_value_is_rejected(ready, monkeypatch):
    monkeypatch.setattr(s, "FIRST_TARGET", "2026-09-15")
    assert s.tick()["verified_forecasts"] == 1
    path = s.root() / "forward/2026-09-15/001000.json"
    value = b.read(path)
    ...
    with pytest.raises(ValueError, match="ROUND_30_VECTOR_CHANGED"):
        s.report()


def test_prior_failure_still_attempts_shibor_branch(monkeypatch):
    from scripts import direction_1d_sprint_shibor as entry

    assert entry.existing.__name__ == "scripts.direction_1d_sprint_neighbors"
    monkeypatch.setattr(entry.existing, "run", lambda: (_ for _ in ()).throw(ValueError("OLD_FAILED")))
    calls = []
    monkeypatch.setattr(entry.shibor, "tick", lambda: calls.append(1))

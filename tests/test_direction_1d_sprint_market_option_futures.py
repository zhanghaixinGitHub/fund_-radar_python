...
import numpy as np
import pytest
from app.services import direction_1d_sprint_market_option_futures as s
...
def test_missing_extra_keeps_corresponding_parent_answer():
    values = [
        {"available": True, "option_futures": {"available": False}},
        {"available": False, "option_futures": {"available": True}},
    ]
        ...
        s.vector(
            {
                "option_futures": {
                    ...
                    "close_location": 0.2,
                    "log_put_call_volume_change": 0.1,
                    "log_put_call_oi_change": 0.2,
                }
            }
        )
...
def actual_forecast(monkeypatch):
    ...
    history = {
        ...
        "t": "2026-09-16",
        "u": "2026-09-17",
        "change_source_hash": "change-source",
    ...
    value = parent | {
        ...
        "model_hash": "model",
        "plan_hash": "plan",
        "r99_forecast_hash": s.base.digest(extra),
    ...
    proofs[s.base.ROOT / "round-99/forward/2026-09-17/001021.json"] = {"option_change_source_hash": "change-source"}
...
def test_r99_parent_must_finish_readback_first(actual_forecast):
    ...
    with pytest.raises(ValueError, match="R99_PARENT_READBACK_LATE_OR_CHANGED"):
        forward.validate(*args)


@pytest.fixture
def identity_input():
    rows = [{"code": "001021", "y": 1, "market": {"original": 2, "option_futures": {"intraday_return_pct": 0.1}}}]
...
def test_actual_question_or_source_changes_rejected(identity_input, kind):
    ...
    if kind == "source_hash":
        ...
        rows[0]["market"]["option_futures"]["intraday_return_pct"] = 0.2
    else:
...
def test_missing_new_feature_falls_back_to_sign_control_not_raw(monkeypatch):
    ...
    result = s.batch([{"available": True, "option_futures": {"available": False}}], {}, [])
...
def test_intraday_parent_source_hash_must_match_prior_prediction(actual_forecast):
    args, _ = actual_forecast
    args[-1]["change_source_hash"] = "different-source"
    args[0]["joint_source_hash"] = s.base.digest(args[-1])
    args[1]["forecast_hash"] = s.base.digest(args[0])
...
def test_all_eighteen_branches_retained_after_two_new_parent_levels(monkeypatch):
    ...
    records = {
        ...
        for n in (92, 94, 95, 96, 97, 98)
    }
    monkeypatch.setattr(forward.base, "read", lambda path: records[path])
    merged = forward.combined_answers(
        parent, {"answers": {"r99": {"prediction": 1}}}, {"answers": {"r100": {"prediction": 0}}}
    )
    assert len(merged) == 18 and all(merged[k] == v for k, v in parent["answers"].items())
    assert merged["r92-1"] == {"prediction": 1} and merged["r100"] == {"prediction": 0}

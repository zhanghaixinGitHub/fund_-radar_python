...
from app.services import direction_1d_sprint_market_futures_activity as s
...
def test_missing_extra_keeps_corresponding_parent_answer():
    values = [
        {"available": True, "futures_activity": {"available": False}},
        {"available": False, "futures_activity": {"available": True}},
    ]
...
def actual_forecast(monkeypatch):
    ...
    value = parent | {
        ...
        "model_hash": "model",
        "plan_hash": "plan",
        "r97_forecast_hash": s.base.digest(extra),
        "activity_source_hash": s.base.digest(history),
        "answers": answers,
    ...
    proofs[s.base.ROOT / "round-95/forward/2026-09-17/001021.json"] = {
        "plan_hash": "parent-plan",
        "basis_source_hash": "parent-source",
    }
...
def test_late_new_futures_snapshot_rejected_even_if_hashes_recomputed(actual_forecast):
    ...
    args[0]["activity_source_hash"] = s.base.digest(args[-1])
...
def test_r97_parent_must_finish_readback_first(actual_forecast):
    ...
    with pytest.raises(ValueError, match="R97_PARENT_READBACK_LATE_OR_CHANGED"):
        forward.validate(*args)


@pytest.fixture
def identity_input():
    rows = [{"code": "001021", "y": 1, "market": {"original": 2, "futures_activity": {"intraday_return_pct": 0.1}}}]
...
def test_actual_question_or_source_changes_rejected(identity_input, kind):
    ...
    if kind == "source_hash":
        ...
        rows[0]["market"]["futures_activity"]["intraday_return_pct"] = 0.2
    else:
...
def test_missing_new_feature_falls_back_to_sign_control_not_raw(monkeypatch):
    ...
    result = s.batch([{"available": True, "futures_activity": {"available": False}}], {}, [])
...
def test_old_historical_snapshot_cannot_stand_in_for_actual_capture(actual_forecast):
    ...
    args[0]["activity_source_hash"] = s.base.digest(args[-1])
    args[1]["forecast_hash"] = s.base.digest(args[0])
    with pytest.raises(ValueError, match="BASIS_SOURCE_NOT_ACTUALLY_CAPTURED_IN_WINDOW"):
        forward.validate(*args)
...
def test_intraday_parent_source_hash_must_match_prior_prediction(actual_forecast):
    args, _ = actual_forecast
    args[-1]["parent_source_hash"] = "different-source"
    args[0]["activity_source_hash"] = s.base.digest(args[-1])
    args[1]["forecast_hash"] = s.base.digest(args[0])
    with pytest.raises(ValueError, match="BASIS_SOURCE_SCOPE_CHANGED"):
        forward.validate(*args)


def test_all_sixteen_branches_retained_after_two_new_parent_levels(monkeypatch):
    ...
    records = {
        ...
        for n in (92, 94, 95, 96)
    }
    monkeypatch.setattr(forward.base, "read", lambda path: records[path])
    merged = forward.combined_answers(
        parent, {"answers": {"r97": {"prediction": 1}}}, {"answers": {"r98": {"prediction": 0}}}
    )
    assert len(merged) == 16 and all(merged[k] == v for k, v in parent["answers"].items())
    assert merged["r92-1"] == {"prediction": 1} and merged["r98"] == {"prediction": 0}

...
from app.services import direction_1d_sprint_etf_decay as s
...
def test_new_entry_preserves_verified_etf_runtime(monkeypatch):
    from scripts import direction_1d_sprint_etf_decay as entry

    assert entry.existing.__name__ == "scripts.direction_1d_sprint_ashare_residual"
...
def test_half_life_uses_observed_dates_and_does_not_balance_labels():
    from datetime import date, timedelta

    rows = [{"u": str(date(2024, 1, 1) + timedelta(days=i * 3)), "family": "A", "y": 1} for i in range(64)]
    weights = s.decay_weights(rows)
    assert weights[0] / weights[-1] == pytest.approx(0.5)
    assert weights.sum() == pytest.approx(64)
    duplicate = rows + [dict(rows[-1])]
    other = s.decay_weights(duplicate)
    assert other[-1] + other[-2] == pytest.approx(weights[-1])
    assert other.sum() == pytest.approx(weights.sum())


def test_recent_loss_weights_keep_original_training_scales(monkeypatch):
    monkeypatch.setattr(s, "active", lambda: None)
    ...
    assert new["fit_hash"] == old["fit_hash"]
    np.testing.assert_array_equal(new["scale"], old["scale"])
    assert new["loss_half_life_observed_dates"] == 63
    chosen = s.adaptive.training_rows([r for r in rows if s.available(r["z"])], "2023-06-10", "MONTHLY_BAL504")
    assert new["weight_hash"] == b.digest(s.decay_weights(chosen).tolist())

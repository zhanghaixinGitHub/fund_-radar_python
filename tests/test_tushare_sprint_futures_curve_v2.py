...
import pytest
from app.integrations import tushare_sprint_futures_curve_v2 as s
...
def test_exact_contract_calendar_and_prices():
    assert s.parse(raw([row()]), "IF2501.CFX", ["2025-01-02"], "2025-01-17")["2025-01-02"]["close"] == 3819.0
...
def test_wrong_contract_date_duplicate_or_empty_rejected(items):
    with pytest.raises(ValueError):
        s.parse(raw(items), "IF2501.CFX", ["2025-01-02"], "2025-01-17")
...
def test_invalid_prices_and_liquidity_rejected(field, value):
    ...
    item[field] = value
    with pytest.raises(ValueError):
        s.parse(raw([item]), "IF2501.CFX", ["2025-01-02"], "2025-01-17")


def test_missing_day_not_interpolated():
    with pytest.raises(ValueError, match="MISSING_DATES"):
        s.parse(raw([row()]), "IF2501.CFX", ["2025-01-02", "2025-01-03"], "2025-01-17")
...
def test_zero_oi_only_allowed_on_expiry_with_positive_volume():
    item = row(day="20250117")
    item[7] = 0
    assert s.parse(raw([item]), "IF2501.CFX", ["2025-01-17"], "2025-01-17")["2025-01-17"]["oi"] == 0
    with pytest.raises(ValueError, match="OI_INVALID"):
        s.parse(raw([item]), "IF2501.CFX", ["2025-01-17"], "2025-01-18")
    item[6] = 0
    with pytest.raises(ValueError, match="PRICE_OR_LIQUIDITY_INVALID"):
        s.parse(raw([item]), "IF2501.CFX", ["2025-01-17"], "2025-01-17")

...
import numpy as np
import pytest
from app.services import direction_1d_sprint_market_stock_turnover as s
from app.services import direction_1d_sprint_stock_turnover_forward as forward
from sklearn.linear_model import LogisticRegression


def test_sign_three_inputs_keeps_turnover_distribution_continuous():
...
def test_missing_extra_keeps_corresponding_parent_answer():
    values = [
        {"available": True, "stock_turnover": {"available": False}},
        {"available": False, "stock_turnover": {"available": True}},
    ]
...
def test_bad_turnover_value_not_silently_normalized(monkeypatch, basis):
    monkeypatch.setattr(s.baseline, "market_vector", lambda _: [1, 2, 3])
    with pytest.raises(ValueError, match="STOCK_TURNOVER_INVALID"):
        s.vector(
            {
                "stock_turnover": {
                    "available": True,
                    "turnover_breadth": basis,
                    "turnover_return_pct": 0.2,
                }
            }
        )
...
def actual_forecast(monkeypatch):
    ...
    extra = {
        ...
        "stock_breadth_source_hash": "parent-source",
    }
    history = {
    ...
    value = parent | {
        ...
        "model_hash": "model",
        "plan_hash": "plan",
        "r109_forecast_hash": s.base.digest(extra),
        "stock_turnover_source_hash": s.base.digest(history),
        "answers": answers,
...
def test_late_new_turnover_snapshot_rejected_even_if_hashes_recomputed(actual_forecast):
    args, _ = actual_forecast
    args[-1]["at"] = "2026-09-17T08:17:00+08:00"
    args[0]["stock_turnover_source_hash"] = s.base.digest(args[-1])
    args[1]["forecast_hash"] = s.base.digest(args[0])
    with pytest.raises(ValueError, match="TURNOVER_SOURCE_CHANGED_OR_LATE"):
        forward.validate(*args)


def test_r109_parent_must_finish_readback_first(actual_forecast):
    ...
    with pytest.raises(ValueError, match="R109_PARENT_READBACK_LATE_OR_CHANGED"):
        forward.validate(*args)
    ...
    rows = [
        {
            "code": "001021",
            "y": 1,
            "market": {"original": 2, "stock_turnover": {"turnover_breadth": 0.1}},
        }
    ]
...
def test_actual_question_or_source_changes_rejected(identity_input, kind):
    ...
    if kind == "source_hash":
        ...
        rows[0]["market"]["stock_turnover"]["turnover_breadth"] = 0.2
    else:
...
def test_missing_new_feature_falls_back_to_sign_control_not_raw(monkeypatch):
    ...
    result = s.batch([{"available": True, "stock_turnover": {"available": False}}], {}, [])
...
def test_old_historical_snapshot_cannot_stand_in_for_actual_capture(actual_forecast):
    ...
    args[0]["stock_turnover_source_hash"] = s.base.digest(args[-1])
    args[1]["forecast_hash"] = s.base.digest(args[0])
    with pytest.raises(ValueError, match="TURNOVER_SOURCE_NOT_ACTUALLY_CAPTURED_IN_WINDOW"):
        forward.validate(*args)
...
def test_turnover_source_must_match_new_model_plan(actual_forecast):
    args, _ = actual_forecast
    args[-1]["parent_plan_hash"] = "different-plan"
    args[0]["stock_turnover_source_hash"] = s.base.digest(args[-1])
    args[1]["forecast_hash"] = s.base.digest(args[0])
    with pytest.raises(ValueError, match="TURNOVER_SOURCE_SCOPE_CHANGED"):
        forward.validate(*args)


def test_all_twenty_eight_branches_retained_after_new_parent_level(monkeypatch):
    ...
    records = {
        ...
        for n in (92, 94, 95, 96, 97, 98, 99, 100, 101, 102, 103, 104, 105, 106, 107, 108)
    }
    monkeypatch.setattr(forward.base, "read", lambda path: records[path])
    merged = forward.combined_answers(
        parent, {"answers": {"r109": {"prediction": 1}}}, {"answers": {"r110": {"prediction": 0}}}
    )
    assert len(merged) == 28 and all(merged[k] == v for k, v in parent["answers"].items())
    assert merged["r92-1"] == {"prediction": 1} and merged["r110"] == {"prediction": 0}
...
def test_actual_raw_derived_features_cannot_change_with_rehashed_envelope(actual_forecast):
    ...
    args[0]["stock_turnover_source_hash"] = s.base.digest(args[-1])
    args[1]["forecast_hash"] = s.base.digest(args[0])
    with pytest.raises(ValueError, match="ACTUAL_TURNOVER_SOURCE_CHANGED"):
        forward.validate(*args)

...
import pytest
from app.integrations import sina_sprint_ashare_etf as client
from app.services import direction_1d_sprint as b
from app.services import direction_1d_sprint_ashare_data as d
...
def points(t="2026-09-14", u="2026-09-15"):
    days = d.required(t, u)
    return {
        "ASHR": {days[0]: quote(100, 100), days[1]: quote(105, 110)},
    }
...
@pytest.mark.parametrize(
    "raw",
    [b"", b'var KLC_K2_ASHR="abc";process.exit(0);', b'var KLC_K2_FXI="abc";', b"x" * 200001],
...
def test_row_quality_and_futures_are_checked_after_decoding(monkeypatch):
    ...
    result = client.parse(b"ignored", "ASHR", "2026-09-14")
    ...
    with pytest.raises(ValueError, match="FUTURE_ROW"):
        client.parse(b"ignored", "ASHR", "2023-12-12")
    monkeypatch.setattr(client, "decode", lambda *a: [row, row])
    with pytest.raises(ValueError, match="DUPLICATE"):
        client.parse(b"ignored", "ASHR")
...
def test_live_capture_is_once_per_slot_and_rechecks_original_bytes(live, monkeypatch):
    ...
    assert calls == ["ASHR"] and value["source"] == d.SOURCE
    d.capture(b.now())
    assert len(calls) == 1
    path = d.root() / "2026-09-15/ASHR/raw/0700.bin"
...
def test_rehashed_parsed_value_cannot_replace_original_response(live):
    ...
    value = b.read(path)
    value["rows"]["ASHR"]["2026-09-14"]["close"] = 52
...
def test_stock_split_does_not_create_overnight_session_jump():
    p = points()
    p["ASHR"]["2026-09-11"] = quote(300, 300)
    p["ASHR"]["2026-09-14"] = quote(100, 100)

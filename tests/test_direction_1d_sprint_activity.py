"""国内市场活跃特征一日纠错：目标语义、自然错误权重、成熟边界及提前答案完整性。"""
...
from app.services import direction_1d_sprint_activity as s
...
def historical_rows():
    return [r | {"z": r["z"] + s.activity_features(r["x"])} for r in original_rows()]


def test_activity_feature_units_and_source_positions():
    x = [0.0] * 32
    x[21], x[26], x[22], x[27] = 0.012, 0.023, 0.4, -0.2
    assert s.activity_features(x) == pytest.approx([1.2, 2.3, 0.4, -0.2])
    x[20], x[25] = 999.0, -999.0
    assert s.activity_features(x) == pytest.approx([1.2, 2.3, 0.4, -0.2])


@pytest.mark.parametrize("index,value", [(21, -0.01), (26, -0.01), (22, -1), (27, -2)])
def test_invalid_volatility_or_turnover_is_rejected(index, value):
    x = [0.0] * 32
    x[index] = value
    with pytest.raises(ValueError, match="ACTIVITY_INPUT_UNIT_INVALID"):
        s.activity_features(x)
...
def test_code_freeze_covers_new_source_runner_and_tests(monkeypatch):
    ...
    assert len(files) == 3 and all("direction_1d_sprint_activity" in path for path in files)
...
def test_original_inputs_and_next_trading_day_label_are_kept():
    x = [0.0] * 32
    assert (
        s.vector(x, "2026-09-14", "2026-09-15", markets())
        == s.hk.vector(x, "2026-09-14", "2026-09-15", markets()) + [0.0] * 4
    )
...
def test_future_labels_and_features_cannot_change_fitting(monkeypatch, candidate):
    ...
    for row in rows:
        if row["mature"] >= "2023-06-10":
            row["z"], row["y"] = [float("nan")] * 16, 1 - row["y"]
...
def test_error_labels_keep_natural_prior(monkeypatch):
    ...
    for i, row in enumerate(rows):
        ...
        row["z"], row["y"] = [0.0] * 16, int(i % 5 == 0)
    fitted = s.fit(rows, s.CANDIDATES[0], "2023-07-01")
    assert fitted["weighted_error_rate"] == pytest.approx(0.8)
    assert fitted["model"].predict_proba([np.zeros(16)])[0, 1] == pytest.approx(0.8)
    assert s.answer([0.0] * 16, s.CANDIDATES[0], fitted)["prediction"] == 0


def trained(score):
    return {"model": ConstantModel(score), "mean": [0.0] * 16, "scale": [1.0] * 16}
...
def test_changed_saved_input_is_rejected(ready, monkeypatch):
    ...
    with pytest.raises(ValueError, match="ROUND_33_VECTOR_CHANGED"):
        s.report()


def test_prior_failure_still_attempts_activity_branch(monkeypatch):
    from scripts import direction_1d_sprint_activity as entry

    assert entry.existing.__name__ == "scripts.direction_1d_sprint_regime"
    monkeypatch.setattr(entry.existing, "run", lambda: (_ for _ in ()).throw(ValueError("OLD_FAILED")))
    calls = []
    monkeypatch.setattr(entry.activity, "tick", lambda: calls.append(1))
...
@pytest.mark.parametrize("values", [[0.0] * 15, [0.0] * 17, [float("nan")] * 16])
def test_malformed_feature_shape_or_nonfinite_is_rejected(values):
    with pytest.raises(ValueError, match="ACTIVITY_INPUT_INVALID"):
...
def test_single_error_class_cannot_be_trained(monkeypatch):
    ...
    with pytest.raises(ValueError, match="ACTIVITY_MODEL_ERROR_LABELS_INSUFFICIENT"):

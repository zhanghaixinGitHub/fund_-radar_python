"""随机森林一日纠错：目标语义、自然错误权重、成熟边界及提前答案完整性。"""
...
from app.services import direction_1d_sprint_forest as s
...
def test_error_labels_keep_natural_prior_and_fixed_forest_recipe(monkeypatch):
    monkeypatch.setattr(s, "active", lambda: None)
    rows = historical_rows()
    ...
    assert params["bootstrap"] is True and params["max_samples"] is None
    assert params["n_estimators"] == 256 and params["max_depth"] == 6
    assert params["min_samples_leaf"] == 80 and params["max_features"] == 1.0
...
def test_changed_saved_input_is_rejected(ready, monkeypatch):
    ...
    with pytest.raises(ValueError, match="ROUND_34_VECTOR_CHANGED"):
        s.report()


def test_prior_failure_still_attempts_forest_branch(monkeypatch):
    from scripts import direction_1d_sprint_forest as entry

    assert entry.existing.__name__ == "scripts.direction_1d_sprint_activity"
    monkeypatch.setattr(entry.existing, "run", lambda: (_ for _ in ()).throw(ValueError("OLD_FAILED")))
    calls = []
    monkeypatch.setattr(entry.forest, "tick", lambda: calls.append(1))
...
def test_malformed_feature_shape_or_nonfinite_is_rejected(values):
    with pytest.raises(ValueError, match="FOREST_INPUT_INVALID"):
...
def test_single_error_class_cannot_be_trained(monkeypatch):
    ...
    with pytest.raises(ValueError, match="FOREST_MODEL_ERROR_LABELS_INSUFFICIENT"):
...
def test_code_freeze_covers_new_source_runner_and_tests(monkeypatch):
    ...
    assert len(files) == 3 and all("direction_1d_sprint_forest" in path for path in files)

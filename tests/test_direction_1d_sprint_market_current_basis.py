...
from app.services import direction_1d_sprint_market_current_basis as s
...
def actual_forecast(monkeypatch):
    ...
    value = parent | {
        ...
        "model_hash": "model",
        "plan_hash": "plan",
        "r94_forecast_hash": s.base.digest(extra),
        "basis_source_hash": s.base.digest(history),
        "answers": answers,
...
def test_late_new_futures_snapshot_rejected_even_if_hashes_recomputed(actual_forecast):
    ...
    args[0]["basis_source_hash"] = s.base.digest(args[-1])
...
def test_r94_parent_must_finish_readback_first(actual_forecast):
    ...
    with pytest.raises(ValueError, match="R94_PARENT_READBACK_LATE_OR_CHANGED"):
        forward.validate(*args)


@pytest.fixture
def identity_input():
    rows = [{"code": "001021", "y": 1, "market": {"original": 2, "futures_basis": {"basis_pct": 0.1}}}]
...
def test_actual_question_or_source_changes_rejected(identity_input, kind):
    ...
    if kind == "source_hash":
        ...
        rows[0]["market"]["futures_basis"]["basis_pct"] = 0.2
    else:
...
def test_missing_new_feature_falls_back_to_sign_control_not_raw(monkeypatch):
    ...
    result = s.batch([{"available": True, "futures_basis": {"available": False}}], {}, [])

...
from app.services import direction_1d_sprint_etf_transform as s
...
def trained(name=None):
    return {
        ...
        "us_etf": {
            ...
            "mean": [0.0] * 3,
            "scale": [1.0] * 3,
            "transform": name or s.CANDIDATES[0],
        },
    }
...
def test_new_entry_preserves_verified_etf_runtime(monkeypatch):
    from scripts import direction_1d_sprint_etf_transform as entry

    assert entry.existing.__name__ == "scripts.direction_1d_sprint_etf_pooled"
...
def test_tanh_saturates_extremes_and_preserves_zero_and_odd_symmetry():
    positive = s.transform([[1.0, 2.0, 100.0]], [1.0, 2.0, 1.0], s.CANDIDATES[0])
    negative = s.transform([[-1.0, -2.0, -100.0]], [1.0, 2.0, 1.0], s.CANDIDATES[0])
    np.testing.assert_allclose(positive, -negative)
    np.testing.assert_allclose(positive[0, :2], np.tanh([1.0, 1.0]))
    assert 0.99 < positive[0, 2] <= 1
    np.testing.assert_array_equal(s.transform([[0.0, 0.0, 0.0]], [1.0, 1.0, 1.0], s.CANDIDATES[0]), [[0.0, 0.0, 0.0]])


def test_sign_discards_magnitude_and_uses_majority_zero_tie():
    name = s.CANDIDATES[1]
    np.testing.assert_array_equal(s.transform([[0.0, 0.01, -100.0]], [1.0, 1.0, 1.0], name), [[1.0, 1.0, -1.0]])
    np.testing.assert_array_equal(s.transform([[0.0, 100.0, -0.01]], [5.0, 2.0, 3.0], name), [[1.0, 1.0, -1.0]])


@pytest.mark.parametrize("scale", [[0.0, 1.0, 1.0], [float("nan"), 1.0, 1.0], [1.0, 1.0]])
def test_invalid_scale_rejected(scale):
    with pytest.raises(ValueError, match="SCALE_INVALID"):
        s.transform([[1.0, 2.0, 3.0]], scale, s.CANDIDATES[0])


def test_candidate_model_cannot_be_mixed_up():
    with pytest.raises(ValueError, match="MODEL_SCHEMA_CHANGED"):
        s.answer(vector((1.0, -1.0, 1.0)), s.CANDIDATES[1], trained(s.CANDIDATES[0]))


def test_actual_inference_uses_the_candidates_transformed_values():
    class Probe:
        def predict_proba(self, values):
            score = 0.5 + 0.25 * values[:, 0]
            return np.column_stack((1 - score, score))

    for name, expected in zip(s.CANDIDATES, (0.5 + 0.25 * np.tanh(1), 0.75), strict=True):
        model = trained(name)
        model["us_etf"]["model"] = Probe()
        assert s.answer(vector((1, -1, 100)), name, model)["research_score"] == pytest.approx(expected)


@pytest.mark.parametrize("name", s.CANDIDATES)
def test_fitting_keeps_raw_rows_and_original_three_training_scales(monkeypatch, name):
    monkeypatch.setattr(s, "active", lambda: None)
    monkeypatch.setattr(s.reference, "active", lambda: None)
    rows = [r | {"z": r["z"] + [0.2, 0.0, 0.0, 0.0, 1.0, -0.3, 0.0, 1.0]} for r in historical_rows()]
    old = s.reference.fit(rows, s.reference.LEARNED[0], "2023-06-10")
    new = s.fit(rows, name, "2023-06-10")
    assert old["fit_hash"] == new["fit_hash"] and old["scale"] == new["scale"] and new["transform"] == name

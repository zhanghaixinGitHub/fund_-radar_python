...
import pytest
from app.services import direction_1d_sprint_stock_exchange_return_live as s
...
def actual_source(monkeypatch, tmp_path):
    ...
    s.b.save(tmp_path / "round-119/plan.json", {"model": "exchange"})
    ...
    row = {"available": True, "date": t, "source_distribution_hash": s.b.digest(original), "breadth_spread": 0.2}
    calls = []
    ...
    return source, row, calls, t, u, s.b.digest({"model": "exchange"})
...
def test_zero_exchange_amount_is_unavailable_even_when_parent_quote_exists(actual_source):
    source, row, _, t, u, ph = actual_source
    row.update(available=False, unavailable_reason="MISSING_EXCHANGE_OR_ZERO_AMOUNT")
    got = s.load_live(t, u, ph)
    assert source["available"] and not got["available"] and got["points"] == {t: row}

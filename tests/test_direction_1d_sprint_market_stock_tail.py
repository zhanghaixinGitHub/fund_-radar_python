"""极端涨跌比例模型的固定五维、时间边界与缺失回退；不跑真实基金成绩。"""
...
def test_sign_three_inputs_keeps_tail_distribution_continuous():
...
def test_missing_extra_keeps_corresponding_parent_answer():
    values = [
        {"available": True, "stock_tail": {"available": False}},
        {"available": False, "stock_tail": {"available": True}},
    ]
...
def test_bad_tail_value_not_silently_normalized(monkeypatch, basis):
    monkeypatch.setattr(s.baseline, "market_vector", lambda _: [1, 2, 3])
    with pytest.raises(ValueError, match="STOCK_TAIL_INVALID"):
        s.vector(
            {
                "stock_tail": {
                    "available": True,
    ...
    rows = [
        {
            "code": "001021",
            "y": 1,
            "market": {"original": 2, "stock_tail": {"tail_balance": 0.1}},
        }
    ]
...
def test_actual_question_or_source_changes_rejected(identity_input, kind):
    ...
    if kind == "source_hash":
        ...
        rows[0]["market"]["stock_tail"]["tail_balance"] = 0.2
    else:
...
def test_missing_new_feature_falls_back_to_sign_control_not_raw(monkeypatch):
    ...
    result = s.batch([{"available": True, "stock_tail": {"available": False}}], {}, [])
...
def test_invalid_training_window_rejected_before_fit(synthetic_training, bad):
    ...
    if bad == "maturity":
        ...
        rows[-1]["market"]["stock_tail"]["available"] = False
    else:
        rows.pop()

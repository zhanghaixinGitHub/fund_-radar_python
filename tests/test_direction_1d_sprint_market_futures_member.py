"""IF会员多空排名模型的固定五维、时间边界与缺失回退；不跑真实基金成绩。"""
...
import numpy as np
import pytest
from app.services import direction_1d_sprint_market_futures_member as s
from sklearn.linear_model import LogisticRegression


def test_sign_three_inputs_keeps_member_continuous():
...
def test_missing_extra_keeps_corresponding_parent_answer():
    values = [
        {"available": True, "futures_member": {"available": False}},
        {"available": False, "futures_member": {"available": True}},
    ]
...
def test_bad_member_value_not_silently_normalized(monkeypatch, basis):
    monkeypatch.setattr(s.baseline, "market_vector", lambda _: [1, 2, 3])
    with pytest.raises(ValueError, match="IF_MEMBER_INVALID"):
        s.vector(
            {
                "futures_member": {
                    "available": True,
                    "level_imbalance": basis,
                    "change_imbalance": 0.2,
                }
            }
        )
    ...
    rows = [
        {
            "code": "001021",
            "y": 1,
            "market": {"original": 2, "futures_member": {"level_imbalance": 0.1}},
        }
    ]
...
def test_actual_question_or_source_changes_rejected(identity_input, kind):
    ...
    if kind == "source_hash":
        ...
        rows[0]["market"]["futures_member"]["level_imbalance"] = 0.2
    else:
...
def test_missing_new_feature_falls_back_to_sign_control_not_raw(monkeypatch):
    ...
    result = s.batch([{"available": True, "futures_member": {"available": False}}], {}, [])
        ...
        rows.append(
            {
                ...
                "market": {
                    ...
                    "futures_member": {
                        ...
                        "level_imbalance": sign / 3,
                        "change_imbalance": 0.5,
                    },
                },
            }
        )
...
def test_invalid_training_window_rejected_before_fit(synthetic_training, bad):
    ...
    if bad == "maturity":
        ...
        rows[-1]["market"]["futures_member"]["available"] = False
    else:
        rows.pop()

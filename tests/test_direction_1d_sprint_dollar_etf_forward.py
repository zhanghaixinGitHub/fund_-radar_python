...
import pytest
from app.services import direction_1d_sprint_dollar_etf_forward as f
from app.services import direction_1d_sprint_market_dollar_etf as s
...
def future_record(monkeypatch):
    ...
    extra = {"answers": {"r110": {"prediction": 1}}}
    ...
    value = parent | {
        ...
        "model_hash": "model",
        "plan_hash": "plan",
        "r110_forecast_hash": s.base.digest(extra),
        "dollar_etf_source_hash": s.base.digest(history),
        "answers": answers,
...
def test_new_history_with_recomputed_hash_cannot_replace_actual_source(future_record):
    ...
    args[0]["dollar_etf_source_hash"] = s.base.digest(args[-1])
    args[1]["forecast_hash"] = s.base.digest(args[0])
    with pytest.raises(ValueError, match="ACTUAL_DOLLAR_SOURCE_CHANGED"):
        f.validate(*args)
...
def test_parent_readback_must_finish_before_new_prediction(future_record):
    ...
    with pytest.raises(ValueError, match="R110_PARENT_READBACK_LATE_OR_CHANGED"):
        f.validate(*args)
...
def test_late_source_rejected_even_with_recomputed_hashes(future_record):
    ...
    args[0]["dollar_etf_source_hash"] = s.base.digest(args[-1])
    args[1]["forecast_hash"] = s.base.digest(args[0])
    with pytest.raises(ValueError, match="DOLLAR_SOURCE_CHANGED_OR_LATE"):
        f.validate(*args)


def test_all_twenty_nine_branches_preserved(monkeypatch):
    ...
    records = {
        ...
        for n in (92, 94, 95, 96, 97, 98, 99, 100, 101, 102, 103, 104, 105, 106, 107, 108, 109)
    }
    monkeypatch.setattr(f.base, "read", lambda p: records[p])
    merged = f.combined_answers(
        parent, {"answers": {"r110": {"prediction": 1}}}, {"answers": {"r111": {"prediction": 0}}}
    )
    assert len(merged) == 29 and merged["r111"]["prediction"] == 0 and merged["r92-1"]["prediction"] == 1
...
def test_old_source_cannot_be_relabelled_as_actual_future(future_record, monkeypatch):
    ...
    with pytest.raises(ValueError, match="DOLLAR_SOURCE_NOT_ACTUALLY_CAPTURED_IN_WINDOW"):
        f.validate(*args)

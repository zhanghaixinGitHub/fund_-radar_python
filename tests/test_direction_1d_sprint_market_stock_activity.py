"""成交活跃程度模型的固定五维、时间边界与缺失回退；不跑真实基金成绩。"""
...
def test_missing_extra_keeps_corresponding_parent_answer():
    values = [
        {"available": True, "stock_activity": {"available": False}},
        {"available": False, "stock_activity": {"available": True}},
    ]
...
def test_bad_turnover_value_not_silently_normalized(monkeypatch, basis):
    ...
    with pytest.raises(ValueError, match="STOCK_ACTIVITY_INVALID"):
        s.vector(
            {
                "stock_activity": {
                    "available": True,
                    "log_activity": basis,
                    "concentration_change": 0.2,
                }
            }
        )
    ...
    rows = [
        {
            "code": "001021",
            "y": 1,
            "market": {"original": 2, "stock_activity": {"log_activity": 0.1}},
        }
    ]
...
def test_actual_question_or_source_changes_rejected(identity_input, kind):
    ...
    if kind == "source_hash":
        ...
        rows[0]["market"]["stock_activity"]["log_activity"] = 0.2
    else:
...
def test_missing_new_feature_falls_back_to_sign_control_not_raw(monkeypatch):
    ...
    result = s.batch([{"available": True, "stock_activity": {"available": False}}], {}, [])
...
def test_invalid_training_window_rejected_before_fit(synthetic_training, bad):
    ...
    if bad == "maturity":
        ...
        rows[-1]["market"]["stock_activity"]["available"] = False
    else:
        rows.pop()

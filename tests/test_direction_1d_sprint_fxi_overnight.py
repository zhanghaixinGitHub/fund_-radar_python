"""最新现金日期FXI净值口径差的一日目标、原输入保留、成熟时点及未来证据绑定。"""
...
from app.services import direction_1d_sprint_fxi_overnight as s
...
def test_code_freeze_covers_real_fxi_client_and_data_files(monkeypatch):
    ...
    assert len(code) == 6
    assert ".local-runs/direction-1d-sprint-20260914/round-42/feature-feasibility.py" in code
    assert "app/services/direction_1d_sprint_fxi_overnight_data.py" in code


def test_fxi_appends_three_known_inputs_and_target_stays_one_day():
    x = [0.0] * 32
    points = s.fxi_overnight_data.parse(body())
    z = s.vector(x, "2026-09-14", "2026-09-15", markets(), points)
    assert len(z) == 15 and z[:12] == s.hk.vector(x, "2026-09-14", "2026-09-15", markets())
    assert z[-3:] == s.fxi_overnight_data.features("2026-09-14", "2026-09-15", points)
...
def ready(hk_ready, monkeypatch):  # noqa: F811
    ...
    input_value = {
        ...
        "base": "2026-09-14",
        "target": "2026-09-15",
        "rows": s.fxi_overnight_data.parse(body()),
        "source": s.fxi_overnight_data.SOURCE,
    }
    monkeypatch.setattr(s.fxi_overnight_data, "capture", lambda at: input_value)
    monkeypatch.setattr(s.fxi_overnight_data, "load", lambda target: input_value)
    return hk_ready
...
def test_live_input_order_and_announcements_are_checked(ready):
    ...
    with pytest.raises(ValueError, match="SEQUENCE_LIVE_DATES_INVALID"):
        s.live_vector(
            source, original | {"inputs": original["inputs"][::-1]}, markets(), s.fxi_overnight_data.parse(body())
        )
    original["inputs"][0]["ann_date"] = "2026-09-16"
    with pytest.raises(ValueError, match="SEQUENCE_LIVE_DATES_INVALID"):
        s.live_vector(source, original, markets(), s.fxi_overnight_data.parse(body()))
...
def test_changed_saved_fxi_value_is_rejected(ready, monkeypatch):
    ...
    with pytest.raises(ValueError, match="ROUND_42_VECTOR_CHANGED"):
        s.report()


def test_prior_failure_still_attempts_fxi_branch(monkeypatch):
    from scripts import direction_1d_sprint_fxi_overnight as entry

    assert entry.existing.__name__ == "scripts.direction_1d_sprint_fxi_shares"
    monkeypatch.setattr(entry.existing, "run", lambda: (_ for _ in ()).throw(ValueError("OLD_FAILED")))
    calls = []
    monkeypatch.setattr(entry.fxi_overnight, "tick", lambda: calls.append(1))

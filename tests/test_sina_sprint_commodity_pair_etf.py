"""黄金和铜期货ETF来源的独立白名单、包络安全、原始质量标记和时间边界。"""

import pytest
from app.integrations import sina_sprint_commodity_pair_etf as s
...
def test_known_commodity_pair_symbols_and_no_arbitrary_endpoint():
...
@pytest.mark.parametrize("raw", [b'var KLC_K2_GLD="abc";evil();', b'var KLC_K2_FXI="abc";', b"<html>error</html>"])
...
def test_original_ohlc_not_repaired(monkeypatch):
    ...
    row = s.parse(b"raw", "GLD")["2025-01-02"]

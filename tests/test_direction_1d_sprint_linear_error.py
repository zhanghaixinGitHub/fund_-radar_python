...
from app.services import direction_1d_sprint_linear_error as s
...
def test_changed_saved_input_is_rejected(ready, monkeypatch):
    ...
    with pytest.raises(ValueError, match="ROUND_35_VECTOR_CHANGED"):
        s.report()


def test_prior_failure_still_attempts_linear_branch(monkeypatch):
    from scripts import direction_1d_sprint_linear_error as entry

    assert entry.existing.__name__ == "scripts.direction_1d_sprint_forest"
    monkeypatch.setattr(entry.existing, "run", lambda: (_ for _ in ()).throw(ValueError("OLD_FAILED")))
    calls = []
    monkeypatch.setattr(entry.linear_error, "tick", lambda: calls.append(1))
...
def test_malformed_feature_shape_or_nonfinite_is_rejected(values):
    with pytest.raises(ValueError, match="LINEAR_ERROR_INPUT_INVALID"):
...
def test_single_error_class_cannot_be_trained(monkeypatch):
    ...
    with pytest.raises(ValueError, match="LINEAR_ERROR_MODEL_ERROR_LABELS_INSUFFICIENT"):
...
def test_code_freeze_covers_new_source_runner_and_tests(monkeypatch):
    ...
    assert len(files) == 3 and all("direction_1d_sprint_linear_error" in path for path in files)


def test_zero_hk_features_match_original_eight_input_linear_error_model(monkeypatch):
    monkeypatch.setattr(s, "active", lambda: None)
    monkeypatch.setattr(s.correction, "active", lambda: None)
    rows = historical_rows()
    assert all(row["z"][8:] == [0.0] * 4 for row in rows)
    actual = s.fit(rows, s.CANDIDATES[0], "2023-06-10")
    control = s.correction.fit([row | {"z": row["z"][:8]} for row in rows], "LR8_ERR504", "2023-06-10")
    for row in rows[40:45]:
        expected = s.correction.answer(row["z"][:8], "LR8_ERR504", control)
        answer = s.answer(row["z"], s.CANDIDATES[0], actual)
        assert answer["prediction"] == expected["prediction"]
        assert answer["research_score"] == pytest.approx(expected["research_score"], abs=1e-10)

...
from app.services import direction_1d_sprint_fund_pooling as s
...
def vector(index=0, values=(2, -3, 4)):
    return parent_vector(values) + [index]


def test_interaction_uses_shared_scale_then_quarter_strength():
    matrix = s.design([vector(0), vector(1)], [2, 3, 4])
    assert matrix.shape == (2, 93)
    np.testing.assert_allclose(matrix[:, :3], [[1, -1, 1], [1, -1, 1]])
    np.testing.assert_allclose(matrix[0, 3:6], [0.25, -0.25, 0.25])
    np.testing.assert_allclose(matrix[1, 6:9], [0.25, -0.25, 0.25])
    assert not matrix[0, 6:].any() and not matrix[1, 3:6].any() and not matrix[1, 9:].any()
...
def test_mature_only_fit_keeps_three_scales_and_no_intercept(monkeypatch):
    ...
    for i, r in enumerate(historical_rows()):
        index = i % 2
        rows.append(r | {"code": s.CODE_ORDER[index], "z": r["z"] + [0.2, 0, 0, 0, 1, -0.3, 0, 1, index]})
    name = s.CANDIDATES[0]
    before = s.fit(rows, name, "2023-06-10")
    for r in rows:
        if r["mature"] >= "2023-06-10":
            r["z"], r["y"] = [float("nan")] * 21, 1 - r["y"]
    after = s.fit(rows, name, "2023-06-10")
    assert before["fit_hash"] == after["fit_hash"] and len(after["scale"]) == 3
    assert after["model"].n_features_in_ == 93 and after["model"].fit_intercept is False
...
def test_future_answer_is_bound_to_original_fund_and_unchanged(ready, monkeypatch):
    ...
    value = b.read(path)
    assert value["z"][20] == 0 and s.tick()["verified_forecasts"] == 1 and before == path.read_bytes()
    b.save(
...
def test_entry_keeps_previous_round_and_attempts_new_after_failure(monkeypatch):
    from scripts import direction_1d_sprint_fund_pooling as entry

    assert entry.existing.__name__ == "scripts.direction_1d_sprint_margin_balance"

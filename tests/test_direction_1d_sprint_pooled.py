"""合并训练的同题范围、资产类型输入、成熟边界与一日提前预测隔离。"""
...
from app.services import direction_1d_sprint_pooled as s
...
def test_group_flags_are_known_and_target_stays_one_day():
    x = [0.0] * 32
    for i, group in enumerate(s.GROUPS):
        z = s.vector(x, "2026-09-14", "2026-09-15", markets(), group)
        assert len(z) == 15 and z[-3:].count(1) == 1 and z[12 + i] == 1
        assert z[:12] == s.hk.vector(x, "2026-09-14", "2026-09-15", markets())
    with pytest.raises(ValueError, match="POOLED_GROUP_UNKNOWN"):
        s.group_vector("UNKNOWN")
    with pytest.raises(ValueError, match="TARGET_NOT_ADJACENT"):
        s.vector(x, "2026-09-14", "2026-09-16", markets(), "CN_EQUITY")
    with pytest.raises(ValueError, match="POOLED_GROUP_VECTOR_INVALID"):
        s.selected_features([0.0] * 15, s.CANDIDATES[0])
    ...
    for group in s.GROUPS:
        for row in historical_rows():
            rows.append(
                ...
                | {
                    "code": group + row["code"],
                    "family": group + row["family"],
                    "group": group,
                    "z": row["z"] + s.group_vector(group),
                }
            )
    return sorted(rows, key=lambda r: (r["u"], r["family"], r["code"]))


def test_pooled_selection_is_exact_union_of_existing_group_selections():
    rows = training_rows()
    chosen = s.pooled_training_rows(rows, "2023-06-10")
    for group in s.GROUPS:
        expected = s.adaptive.training_rows([r for r in rows if r["group"] == group], "2023-06-10", "MONTHLY_BAL504")
        assert [r for r in chosen if r["group"] == group] == expected
    with pytest.raises(ValueError, match="POOLED_GROUP_COVERAGE_INVALID"):
        s.pooled_training_rows([r for r in rows if r["group"] != "CN_BOND"], "2023-06-10")
    rows[0]["z"][-3:] = s.group_vector("CN_EQUITY")
    with pytest.raises(ValueError, match="POOLED_TRAINING_GROUP_CHANGED"):
        s.pooled_training_rows(rows, "2023-06-10")
...
def test_future_labels_and_input_changes_do_not_change_fitting(monkeypatch):
    ...
    assert set(before["group_rows"]) == set(s.GROUPS) and len(before["mean"]) == 15
    np.testing.assert_allclose(
        before["model"].predict_proba([np.zeros(15)]), after["model"].predict_proba([np.zeros(15)]), atol=1e-12
    )
...
def trained(score):
    return {"model": ConstantModel(score), "mean": [0.0] * 15, "scale": [1.0] * 15}


@pytest.mark.parametrize("move", [-0.01, 0, 0.01])
def test_correction_threshold_is_unchanged(move):
    z = [move] + [0.0] * 11 + s.group_vector("CN_EQUITY")
    baseline = int(move >= 0)
...
def ready(hk_ready, monkeypatch):  # noqa: F811
    ...
    bundle = {s.CANDIDATES[0]: {"POOLED": trained(0.3)}}
...
def test_changed_saved_group_value_is_rejected(ready, monkeypatch):
    monkeypatch.setattr(s, "FIRST_TARGET", "2026-09-15")
    assert s.tick()["verified_forecasts"] == 1
    path = s.root() / "forward/2026-09-15/001000.json"
    value = b.read(path)
    ...
    with pytest.raises(ValueError, match="POOLED_GROUP_VECTOR_INVALID|ROUND_20_VECTOR_CHANGED"):
        s.report()


def test_prior_failure_still_attempts_pooled_branch(monkeypatch):
    from scripts import direction_1d_sprint_pooled as entry

    assert entry.existing.__name__ == "scripts.direction_1d_sprint_technical"
    monkeypatch.setattr(entry.existing, "run", lambda: (_ for _ in ()).throw(ValueError("OLD_FAILED")))
    calls = []
    monkeypatch.setattr(entry.pooled, "tick", lambda: calls.append(1))

...
from app.services import direction_1d_sprint_etf_joint as s
...
def test_fixed_rules_are_majority_and_cn_gap_not_fitted_probabilities(values):
    z = vector(values)
    model = trained() | {"us_etf": None}
    vote = s.answer(z, s.CANDIDATES[0], model)
    assert vote["prediction"] == int(sum(v >= 0 for v in values) >= 2)
    assert vote["kind"] == "FIXED_DIRECTION_VOTE_FRACTION"
    assert s.answer(z, s.CANDIDATES[2], model)["prediction"] == int(values[2] >= 0)
    assert s.fit([], s.CANDIDATES[0], "2025-01-01") is None
...
def test_new_entry_preserves_verified_etf_runtime(monkeypatch):
    from scripts import direction_1d_sprint_etf_joint as entry

    assert entry.existing.__name__ == "scripts.direction_1d_sprint_etf_runtime_v2"

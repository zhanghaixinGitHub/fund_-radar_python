...
import numpy as np
import pytest
from app.services import direction_1d_sprint_sector_exposure_context as s
...
def test_fixed_covariance_formula_and_last126_dates():
    ...
    assert prior["beta"] == pytest.approx([1 / 1.25, 2 / 4.25, 3 / 9.25, 4 / 16.25, 5 / 25.25])
...
def test_zero_market_variance_and_extreme_sector_exposure_are_bounded():
    values = rows()
    for row in values:
        row["x"][7] *= 1000

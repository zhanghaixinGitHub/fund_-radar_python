...
from app.services import direction_1d_sprint_market_futures_intraday as s
...
def test_missing_extra_keeps_corresponding_parent_answer():
    values = [
        {"available": True, "futures_intraday": {"available": False}},
        {"available": False, "futures_intraday": {"available": True}},
    ]
...
def test_bad_intraday_return_not_silently_normalized(monkeypatch, basis):
    ...
    with pytest.raises(ValueError, match="INTRADAY_INVALID"):
        s.vector({"futures_intraday": {"available": True, "intraday_return_pct": basis, "close_location": 0.2}})
...
def actual_forecast(monkeypatch):
    ...
    value = parent | {
        ...
        "model_hash": "model",
        "plan_hash": "plan",
        "r95_forecast_hash": s.base.digest(extra),
        "intraday_source_hash": s.base.digest(history),
        "answers": answers,
...
def test_late_new_futures_snapshot_rejected_even_if_hashes_recomputed(actual_forecast):
    ...
    args[0]["intraday_source_hash"] = s.base.digest(args[-1])
...
def test_r95_parent_must_finish_readback_first(actual_forecast):
    ...
    with pytest.raises(ValueError, match="R95_PARENT_READBACK_LATE_OR_CHANGED"):
        forward.validate(*args)


@pytest.fixture
def identity_input():
    rows = [{"code": "001021", "y": 1, "market": {"original": 2, "futures_intraday": {"intraday_return_pct": 0.1}}}]
...
def test_actual_question_or_source_changes_rejected(identity_input, kind):
    ...
    if kind == "source_hash":
        ...
        rows[0]["market"]["futures_intraday"]["intraday_return_pct"] = 0.2
    else:
...
def test_missing_new_feature_falls_back_to_sign_control_not_raw(monkeypatch):
    ...
    result = s.batch([{"available": True, "futures_intraday": {"available": False}}], {}, [])

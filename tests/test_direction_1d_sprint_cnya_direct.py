...
from app.services import direction_1d_sprint_cnya_direct as s
...
def test_code_freeze_covers_real_fxi_client_and_data_files(monkeypatch):
    ...
    assert len(code) == 4
    assert ".local-runs/direction-1d-sprint-20260914/round-44/feature-feasibility.py" in code
    assert "app/services/direction_1d_sprint_cnya_direct.py" in code
...
def test_future_labels_and_input_changes_do_not_change_fitting(monkeypatch):
    ...
    np.testing.assert_allclose(
        before["model"].predict_proba([np.zeros(3)]), after["model"].predict_proba([np.zeros(3)]), atol=1e-12
    )
...
def test_direct_up_labels_natural_prior_and_fixed_recipe(monkeypatch):
    ...
    assert fitted["model"].n_features_in_ == 3
    params = fitted["model"].get_params()
    assert params["C"] == 0.1 and params["max_iter"] == 1500 and params["class_weight"] is None
    assert s.answer([0.0] * 15, s.CANDIDATES[0], fitted)["prediction"] == 0


def trained(score):
    return {"model": ConstantModel(score), "mean": [0.0] * 3, "scale": [1.0] * 3}
...
def test_only_three_selected_inputs_reach_estimator():
    z = list(range(15))
    assert s.selected_features(z, s.CANDIDATES[0]) == [0, 12, 14]
    z[1:12] = [999] * 11
    z[13] = -999
    assert s.selected_features(z, s.CANDIDATES[0]) == [0, 12, 14]
...
def test_changed_saved_fxi_value_is_rejected(ready, monkeypatch):
    ...
    with pytest.raises(ValueError, match="ROUND_44_VECTOR_CHANGED"):
        s.report()


def test_prior_failure_still_attempts_fxi_branch(monkeypatch):
    from scripts import direction_1d_sprint_cnya_direct as entry

    assert entry.existing.__name__ == "scripts.direction_1d_sprint_cnya"
    monkeypatch.setattr(entry.existing, "run", lambda: (_ for _ in ()).throw(ValueError("OLD_FAILED")))
    calls = []
    monkeypatch.setattr(entry.cnya_direct, "tick", lambda: calls.append(1))

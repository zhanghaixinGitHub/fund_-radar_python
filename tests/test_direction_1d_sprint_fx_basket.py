"""多币种参考汇率的一日目标、原输入保留、成熟时点及未来证据绑定。"""
...
from app.services import direction_1d_sprint_fx_basket as s

from test_direction_1d_sprint_fx_basket_data import body
...
def test_ecb_appends_only_eight_known_inputs_and_target_stays_one_day():
    x = [0.0] * 32
    points = s.basket_data.parse(body())
    z = s.vector(x, "2026-09-14", "2026-09-15", markets(), points)
    assert len(z) == 20 and z[:12] == s.hk.vector(x, "2026-09-14", "2026-09-15", markets())
    assert z[-8:] == s.basket_data.features("2026-09-14", "2026-09-15", points)
...
def ready(hk_ready, monkeypatch):  # noqa: F811
    ...
    input_value = {
        ...
        "base": "2026-09-14",
        "target": "2026-09-15",
        "rows": s.basket_data.parse(body()),
        "source": s.basket_data.SOURCE,
    }
    monkeypatch.setattr(s.basket_data, "capture", lambda at: input_value)
    monkeypatch.setattr(s.basket_data, "load", lambda target: input_value)
    return hk_ready
...
def test_live_input_order_and_announcements_are_checked(ready):
    ...
    with pytest.raises(ValueError, match="SEQUENCE_LIVE_DATES_INVALID"):
        s.live_vector(source, original | {"inputs": original["inputs"][::-1]}, markets(), s.basket_data.parse(body()))
    original["inputs"][0]["ann_date"] = "2026-09-16"
    with pytest.raises(ValueError, match="SEQUENCE_LIVE_DATES_INVALID"):
        s.live_vector(source, original, markets(), s.basket_data.parse(body()))
...
def test_changed_saved_ecb_value_is_rejected(ready, monkeypatch):
    monkeypatch.setattr(s, "FIRST_TARGET", "2026-09-15")
    assert s.tick()["verified_forecasts"] == 1
    path = s.root() / "forward/2026-09-15/001000.json"
    value = b.read(path)
    ...
    with pytest.raises(ValueError, match="ROUND_27_VECTOR_CHANGED"):
        s.report()


def test_prior_failure_still_attempts_ecb_branch(monkeypatch):
    from scripts import direction_1d_sprint_fx_basket as entry

    assert entry.existing.__name__ == "scripts.direction_1d_sprint_ecb"
    monkeypatch.setattr(entry.existing, "run", lambda: (_ for _ in ()).throw(ValueError("OLD_FAILED")))
    calls = []
    monkeypatch.setattr(entry.basket, "tick", lambda: calls.append(1))

...
from app.services import direction_1d_sprint_ashare as s
...
def test_entry_keeps_previous_round_and_attempts_new_after_failure(monkeypatch):
    from scripts import direction_1d_sprint_ashare as entry

    assert entry.existing.__name__ == "scripts.direction_1d_sprint_china_internet_tree"
...
def test_rehashed_sector_input_rejected(ready, monkeypatch):
    ...
    with pytest.raises(ValueError, match="ASHARE_INPUT_CHANGED"):
        s.report()


def test_fixed_sign_uses_only_available_session_direction_without_fitting(monkeypatch):

...
from app.services.tushare_fund_sync import (
    ...
    SyncOutcome,
    _build_focused_nav_incremental_windows,
    _normalize_focused_nav_incremental_records,
)
...
def test_incremental_windows_reject_missing_history_baseline() -> None:
    ...
    with pytest.raises(FocusedNavIncrementalPreconditionError, match="baseline is missing"):
        _build_focused_nav_incremental_windows(
            ts_codes=("002112.OF", "010710.OF"),
...
def test_incremental_normalization_rejects_another_fund_and_window_outside_record() -> None:
    ...
    records, invalid_count = _normalize_focused_nav_incremental_records(
        (
...
def test_beat_schedule_is_configurable_and_can_be_disabled() -> None:
    ...
    enabled = Settings(
        tushare_focused_incremental_enabled=True,
        tushare_focused_incremental_hour=21,
        tushare_focused_incremental_minute=5,
    )
    schedule = build_beat_schedule(enabled)

    trigger = schedule["focused-nav-incremental-weekdays"]["schedule"]
    assert schedule["focused-nav-incremental-weekdays"]["task"] == "fund_ai.tushare.sync_focused_nav_incremental"
...
def test_incremental_task_uses_configured_codes_and_optional_as_of_date(monkeypatch) -> None:
    ...
    class StubService:
        def sync_focused_nav_incremental(self, ts_codes: tuple[str, ...], *, as_of_date: date | None) -> SyncOutcome:
    ...
    monkeypatch.setattr(tasks, "get_settings", lambda: SimpleNamespace(focused_fund_ts_codes=("002112.OF",)))

    payload = tasks.sync_focused_nav_incremental.run("2026-08-27")
    ...
    assert payload["sync_type"] == "FOCUSED_NAV_INCREMENTAL"

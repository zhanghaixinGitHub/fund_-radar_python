...
from app.services import direction_1d_sprint_stock_exchange_spread_forward as f
...
def future_record(monkeypatch):
    ...
    extra = {"answers": {"r117": {"prediction": 1}}}
    ...
    value = parent | {
        ...
        "model_hash": "model",
        "plan_hash": "plan",
        "r117_forecast_hash": s.base.digest(extra),
...
def test_parent_readback_must_finish_before_new_prediction(future_record):
    ...
    with pytest.raises(ValueError, match="R117_PARENT_READBACK_LATE_OR_CHANGED"):
        f.validate(*args)
...
def test_all_thirty_seven_branches_preserved(monkeypatch):
    ...
    merged = f.combined_answers(
        parent,
        {"answers": {"r117": {"prediction": 1}}},
        {"answers": {"r118": {"prediction": 0}}},
    )
    assert len(merged) == 37 and merged["r118"]["prediction"] == 0 and merged["r92-1"]["prediction"] == 1

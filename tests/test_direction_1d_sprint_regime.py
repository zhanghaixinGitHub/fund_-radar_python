"""SPX正负分场景一日纠错：目标语义、自然错误权重、成熟边界及提前答案完整性。"""
...
from app.services import direction_1d_sprint_regime as s
...
def test_each_leg_keeps_its_natural_error_prior(monkeypatch):
    monkeypatch.setattr(s, "active", lambda: None)
    rows = historical_rows()
    for i, row in enumerate(rows):
        leg = int(row["z"][0] >= 0)
        error = int(i % 5 != 0) if leg else int(i % 5 == 0)
        row["y"] = leg ^ error
    fitted = s.fit(rows, s.CANDIDATES[0], "2023-07-01")
    scores = fitted["model"].predict_proba([[-1.0] + [0.0] * 11, [1.0] + [0.0] * 11])[:, 1]
    for leg, score in enumerate(scores):
        assert score == pytest.approx(fitted["leg_audits"][str(leg)]["weighted_error_rate"])
    assert scores[0] < 0.3 and scores[1] > 0.7
    assert fitted["training_target"] == "NEXT_DAY_SPX_BASELINE_ERROR"
    assert fitted["estimator_fits"] == 2
    assert sum(part["fit_rows"] for part in fitted["leg_audits"].values()) == fitted["fit_rows"]


def test_raw_sign_routes_zero_to_nonnegative_before_normalization():
    parts = {
        0: {"model": ConstantModel(0.2), "mean": [100.0] * 12, "scale": [2.0] * 12},
        1: {"model": ConstantModel(0.8), "mean": [-100.0] * 12, "scale": [3.0] * 12},
    }
    model = s.SignSpecificClassifier(parts)
    assert model.predict_proba([[-0.01] + [0.0] * 11, [0.0] * 12, [0.01] + [0.0] * 11])[:, 1] == pytest.approx(
        [0.2, 0.8, 0.8]
    )


def test_partition_union_is_exact_original504_and_never_extends_a_leg():
    chosen = s.training_rows(historical_rows(), "2024-01-01")
    partitions = s.split_training(chosen)
    assert sorted(row["u"] for part in partitions.values() for row in part) == sorted(row["u"] for row in chosen)
    assert sum(len(part) for part in partitions.values()) == 504
    assert all(len(part) == 252 for part in partitions.values())


def test_insufficient_regime_fails_before_any_estimator_fit(monkeypatch):
    monkeypatch.setattr(s, "active", lambda: None)
    rows = historical_rows()
    for row in rows:
        row["z"][0] = 1.0
    monkeypatch.setattr(s, "ExtraTreesClassifier", lambda **kwargs: pytest.fail("fitting incomplete partitions"))
    with pytest.raises(ValueError, match="REGIME_DATES_INSUFFICIENT"):
...
def test_code_freeze_covers_new_source_runner_and_tests(monkeypatch):
    ...
    assert len(files) == 3 and all("direction_1d_sprint_regime" in path for path in files)
...
def test_changed_saved_input_is_rejected(ready, monkeypatch):
    ...
    with pytest.raises(ValueError, match="ROUND_32_VECTOR_CHANGED"):
        s.report()


def test_prior_failure_still_attempts_regime_branch(monkeypatch):
    from scripts import direction_1d_sprint_regime as entry

    assert entry.existing.__name__ == "scripts.direction_1d_sprint_fxi_gap"
    monkeypatch.setattr(entry.existing, "run", lambda: (_ for _ in ()).throw(ValueError("OLD_FAILED")))
    calls = []
    monkeypatch.setattr(entry.regime, "tick", lambda: calls.append(1))
...
def test_malformed_feature_shape_or_nonfinite_is_rejected(values):
    with pytest.raises(ValueError, match="REGIME_INPUT_INVALID"):
...
def test_single_error_class_cannot_be_trained(monkeypatch):
    ...
    with pytest.raises(ValueError, match="REGIME_ERROR_LABELS_INSUFFICIENT"):
        s.fit(rows, s.CANDIDATES[0], "2023-07-01")

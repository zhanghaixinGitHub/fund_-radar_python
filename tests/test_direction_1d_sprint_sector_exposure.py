"""个体行业敏感度一日纠错：目标语义、自然错误权重、成熟边界及提前答案完整性。"""
...
from app.services import direction_1d_sprint_sector_exposure as s
...
def empty_prior():
    return {"beta": [0.0] * 5, "count": 0, "available_fraction": 0.0, "max_u": None, "max_mature": None}


def historical_rows():
    return [r | {"z": r["z"] + [0.0] * 11} for r in original_rows()]
...
def test_original_inputs_and_next_trading_day_label_are_kept():
    x = [0.0] * 32
    assert (
        s.vector(x, "2026-09-14", "2026-09-15", markets(), points(), empty_prior())
        == s.hk.vector(x, "2026-09-14", "2026-09-15", markets()) + [0.0] * 11
    )
    with pytest.raises(ValueError, match="TARGET_NOT_ADJACENT"):
        s.vector(x, "2026-09-14", "2026-09-16", markets(), points(), empty_prior())
...
def test_future_labels_and_features_cannot_change_fitting(monkeypatch, candidate):
    ...
    for row in rows:
        if row["mature"] >= "2023-06-10":
            row["z"], row["y"] = [float("nan")] * 23, 1 - row["y"]
    ...
    np.testing.assert_allclose(
        before["model"].predict_proba([np.zeros(23)]), after["model"].predict_proba([np.zeros(23)])
    )
...
def test_error_labels_keep_natural_prior(monkeypatch):
    ...
    for i, row in enumerate(rows):
        ...
        row["z"], row["y"] = [0.0] * 23, int(i % 5 == 0)
    fitted = s.fit(rows, s.CANDIDATES[0], "2023-07-01")
    assert fitted["weighted_error_rate"] == pytest.approx(0.8)
    assert fitted["model"].predict_proba([np.zeros(23)])[0, 1] == pytest.approx(0.8)
    assert s.answer([0.0] * 23, s.CANDIDATES[0], fitted)["prediction"] == 0


def trained(score):
    return {"model": ConstantModel(score), "mean": [0.0] * 23, "scale": [1.0] * 23}
...
def ready(hk_ready, monkeypatch):  # noqa: F811
    ...
    sector_input = {"at": "2026-09-15T07:15:00+08:00", "base": "2026-09-14", "target": "2026-09-15", "rows": points()}
    monkeypatch.setattr(s.sector_data, "capture", lambda at: sector_input)
    monkeypatch.setattr(s.sector_data, "load", lambda target: sector_input)
    return hk_ready
...
def test_live_input_order_and_announcements_are_checked(ready):
    ...
    with pytest.raises(ValueError, match="SEQUENCE_LIVE_DATES_INVALID"):
        s.live_vector(
            source, original | {"inputs": original["inputs"][::-1]}, markets(), points(), s.current_contexts()
        )
    original["inputs"][0]["ann_date"] = "2026-09-16"
    with pytest.raises(ValueError, match="SEQUENCE_LIVE_DATES_INVALID"):
        s.live_vector(source, original, markets(), points(), s.current_contexts())
...
def test_changed_saved_input_is_rejected(ready, monkeypatch):
    ...
    with pytest.raises(ValueError, match="ROUND_38_VECTOR_CHANGED"):
        s.report()


def test_prior_failure_still_attempts_sector_exposure_branch(monkeypatch):
    from scripts import direction_1d_sprint_sector_exposure as entry

    assert entry.existing.__name__ == "scripts.direction_1d_sprint_sector"
    monkeypatch.setattr(entry.existing, "run", lambda: (_ for _ in ()).throw(ValueError("OLD_FAILED")))
    calls = []
    monkeypatch.setattr(entry.sector_exposure, "tick", lambda: calls.append(1))
...
@pytest.mark.parametrize("values", [[0.0] * 22, [0.0] * 24, [float("nan")] * 23])
def test_malformed_feature_shape_or_nonfinite_is_rejected(values):
    with pytest.raises(ValueError, match="EXPOSURE_INPUT_INVALID"):
...
def test_single_error_class_cannot_be_trained(monkeypatch):
    ...
    with pytest.raises(ValueError, match="EXPOSURE_MODEL_ERROR_LABELS_INSUFFICIENT"):
...
def test_code_freeze_covers_context_and_prototype(monkeypatch):
    ...
    assert len(files) == 6
    assert "app/services/direction_1d_sprint_sector_exposure_context.py" in files
    assert any(path.endswith("round-38/context-feasibility.py") for path in files)


def test_missing_fund_or_future_context_is_rejected(ready):
    _, original, source = ready
    with pytest.raises(ValueError, match="CONTEXT_FUND_MISSING"):
        s.live_vector(source, original, markets(), points(), {"cutoff": "2026-09-15", "contexts": {}})
    with pytest.raises(ValueError, match="CONTEXT_FUTURE_CUTOFF"):
        s.live_vector(source, original, markets(), points(), {"cutoff": "2026-09-16", "contexts": {}})


def test_rehashed_context_cannot_replace_model_bound_snapshot(tmp_path, monkeypatch):
    monkeypatch.setattr(b, "ROOT", tmp_path)
    p = {"current_fit_cutoff": "2026-09-15", "input_hashes": {"round-38/current-context-feasibility.json": "reference"}}

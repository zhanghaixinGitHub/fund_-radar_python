"""ECB参考汇率的一日目标、原输入保留、成熟时点及未来证据绑定。"""
...
from app.services import direction_1d_sprint_ecb as s

from test_direction_1d_sprint_ecb_data import body
...
def test_ecb_appends_only_two_known_inputs_and_target_stays_one_day():
    x = [0.0] * 32
    points = s.ecb_data.parse(body())
    z = s.vector(x, "2026-09-14", "2026-09-15", markets(), points)
    assert len(z) == 14 and z[:12] == s.hk.vector(x, "2026-09-14", "2026-09-15", markets())
    assert z[-2:] == s.ecb_data.features("2026-09-14", "2026-09-15", points)
...
def ready(hk_ready, monkeypatch):  # noqa: F811
    ...
    input_value = {
        ...
        "base": "2026-09-14",
        "target": "2026-09-15",
        "rows": s.ecb_data.parse(body()),
        "source": s.ecb_data.SOURCE,
    }
    monkeypatch.setattr(s.ecb_data, "capture", lambda at: input_value)
    monkeypatch.setattr(s.ecb_data, "load", lambda target: input_value)
    return hk_ready
...
def test_live_input_order_and_announcements_are_checked(ready):
    ...
    with pytest.raises(ValueError, match="SEQUENCE_LIVE_DATES_INVALID"):
        s.live_vector(source, original | {"inputs": original["inputs"][::-1]}, markets(), s.ecb_data.parse(body()))
    original["inputs"][0]["ann_date"] = "2026-09-16"
    with pytest.raises(ValueError, match="SEQUENCE_LIVE_DATES_INVALID"):
        s.live_vector(source, original, markets(), s.ecb_data.parse(body()))
...
def test_changed_saved_ecb_value_is_rejected(ready, monkeypatch):
    ...
    with pytest.raises(ValueError, match="ROUND_26_VECTOR_CHANGED"):
        s.report()


def test_prior_failure_still_attempts_ecb_branch(monkeypatch):
    from scripts import direction_1d_sprint_ecb as entry

    assert entry.existing.__name__ == "scripts.direction_1d_sprint_direct"
    monkeypatch.setattr(entry.existing, "run", lambda: (_ for _ in ()).throw(ValueError("OLD_FAILED")))
    calls = []
    monkeypatch.setattr(entry.ecb, "tick", lambda: calls.append(1))

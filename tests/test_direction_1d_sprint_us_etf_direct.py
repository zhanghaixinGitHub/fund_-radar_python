...
def trained(score):
    return {
        ...
        "us_etf": {"model": ConstantModel(score), "mean": [0.0], "scale": [1.0]},
    }
...
def test_direct_up_boundary_does_not_depend_on_spx_sign(move):
    z = [move] + [0.0] * 15 + [1.0]
    for score in (0.4999999, 0.5):
        answer = s.answer(z, s.CANDIDATES[0], trained(score))
        expected = int(move >= 0)
        assert answer["prediction"] == int(score >= 0.5)
        assert answer["flipped"] == (answer["prediction"] != expected)
...
def test_future_and_unavailable_rows_cannot_change_etf_fit(monkeypatch):
    ...
    assert after["model"].n_features_in_ == 1
    np.testing.assert_allclose(
        before["model"].predict_proba([np.zeros(1)]), after["model"].predict_proba([np.zeros(1)])
    )


def test_ablation_selects_spx_and_fxi_without_route_or_cn_features():
    z = [float(i) for i in range(16)] + [1.0]
    assert s.selected_features(z, s.CANDIDATES[0]) == [0.0]
    assert s.selected_features(z, s.CANDIDATES[1]) == [0.0, 12.0]
...
def ready(hk_ready, monkeypatch):  # noqa: F811
    ...
    bundle = {}
    for i, n in enumerate(s.CANDIDATES, start=1):
        bundle[n] = {"CN_EQUITY": model | {"us_etf": model["us_etf"] | {"mean": [0.0] * i, "scale": [1.0] * i}}}
...
def test_changed_saved_fxi_value_is_rejected(ready, monkeypatch):
    ...
    with pytest.raises(ValueError, match="ROUND_49_VECTOR_CHANGED"):
        s.report()


def test_prior_failure_still_attempts_fxi_branch(monkeypatch):
    from scripts import direction_1d_sprint_us_etf_direct as entry

    assert entry.existing.__name__ == "scripts.direction_1d_sprint_us_etf"
...
@pytest.mark.parametrize("name,dimensions", [(s.CANDIDATES[0], 1), (s.CANDIDATES[1], 2)])
def test_real_direct_fit_has_no_bias_and_is_odd_around_zero(monkeypatch, name, dimensions):
    monkeypatch.setattr(s, "active", lambda: None)
    rows = [r | {"code": "001632", "z": r["z"] + [0.3 * r["z"][0], 0.0, 0.0, 0.0, 1.0]} for r in historical_rows()]
    fitted = s.fit(rows, name, "2023-06-10")
    assert fitted["model"].fit_intercept is False
    assert fitted["mean"] == [0.0] * dimensions
    assert fitted["model"].n_features_in_ == dimensions
    assert fitted["training_target"] == "raw_NAV_UP"
    model = {"control": trained(0.5)["control"], "us_etf": fitted}
    zero = [0.0] * 16 + [1.0]
    assert s.answer(zero, name, model)["research_score"] == pytest.approx(0.5)
    positive = zero.copy()
    positive[0], positive[12] = 1.0, 0.2
    negative = zero.copy()
    negative[0], negative[12] = -1.0, -0.2
    assert s.answer(positive, name, model)["research_score"] + s.answer(negative, name, model)[
        "research_score"
    ] == pytest.approx(1.0)

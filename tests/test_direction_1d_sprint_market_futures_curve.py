...
from app.services import direction_1d_sprint_market_futures_curve as s
from sklearn.linear_model import LogisticRegression


def test_sign_three_inputs_keeps_intraday_continuous():
    x = [[2.0, -0.1, 0.0, -0.8, 0.5, 0.4]]
    assert np.allclose(s.transform(x, [2.0, 2.0, 2.0, 0.4, 0.5, 0.2], True), [[1.0, -1.0, 0.0, -2.0, 1.0, 2.0]])
    assert np.allclose(s.transform(x, [2.0, 2.0, 2.0, 0.4, 0.5, 0.2], False), [[1.0, -0.05, 0.0, -2.0, 1.0, 2.0]])
...
def test_missing_extra_keeps_corresponding_parent_answer():
    values = [
        {"available": True, "futures_curve": {"available": False}},
        {"available": False, "futures_curve": {"available": True}},
    ]
...
def actual_forecast(monkeypatch):
    ...
    value = parent | {
        ...
        "model_hash": "model",
        "plan_hash": "plan",
        "r101_forecast_hash": s.base.digest(extra),
        "curve_combined_source_hash": s.base.digest(history),
        "answers": answers,
    ...
    monkeypatch.setattr(s.data, "load_live", lambda *_: {"source": "real_curve"})
...
def test_late_new_futures_snapshot_rejected_even_if_hashes_recomputed(actual_forecast):
    ...
    args[0]["curve_combined_source_hash"] = s.base.digest(args[-1])
...
def test_r101_parent_must_finish_readback_first(actual_forecast):
    ...
    with pytest.raises(ValueError, match="R101_PARENT_READBACK_LATE_OR_CHANGED"):
        forward.validate(*args)


@pytest.fixture
def identity_input():
    rows = [{"code": "001021", "y": 1, "market": {"original": 2, "futures_curve": {"intraday_return_pct": 0.1}}}]
...
def test_actual_question_or_source_changes_rejected(identity_input, kind):
    ...
    if kind == "source_hash":
        ...
        rows[0]["market"]["futures_curve"]["intraday_return_pct"] = 0.2
    else:
...
def test_missing_new_feature_falls_back_to_sign_control_not_raw(monkeypatch):
    ...
    result = s.batch([{"available": True, "futures_curve": {"available": False}}], {}, [])
...
def test_all_twenty_branches_retained_after_two_new_parent_levels(monkeypatch):
    ...
    records = {
        ...
        for n in (92, 94, 95, 96, 97, 98, 99, 100)
    }
    monkeypatch.setattr(forward.base, "read", lambda path: records[path])
    merged = forward.combined_answers(
        parent, {"answers": {"r101": {"prediction": 1}}}, {"answers": {"r102": {"prediction": 0}}}
    )
    assert len(merged) == 20 and all(merged[k] == v for k, v in parent["answers"].items())
    assert merged["r92-1"] == {"prediction": 1} and merged["r102"] == {"prediction": 0}

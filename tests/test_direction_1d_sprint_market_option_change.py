...
import numpy as np
import pytest
from app.services import direction_1d_sprint_market_option_change as s
...
def test_missing_extra_keeps_corresponding_parent_answer():
    values = [
        {"available": True, "option_change": {"available": False}},
        {"available": False, "option_change": {"available": True}},
    ]
...
def test_bad_option_ratio_not_silently_normalized(monkeypatch, basis):
    ...
    with pytest.raises(ValueError, match="OPTION_CHANGE_INVALID"):
        s.vector(
            {"option_change": {"available": True, "log_put_call_volume_change": basis, "log_put_call_oi_change": 0.2}}
        )
...
def actual_forecast(monkeypatch):
    ...
    value = parent | {
        ...
        "model_hash": "model",
        "plan_hash": "plan",
        "r98_forecast_hash": s.base.digest(extra),
        "option_change_source_hash": s.base.digest(history),
        "answers": answers,
...
def test_late_new_option_snapshot_rejected_even_if_hashes_recomputed(actual_forecast):
    ...
    args[0]["option_change_source_hash"] = s.base.digest(args[-1])
...
def test_r98_parent_must_finish_readback_first(actual_forecast):
    ...
    with pytest.raises(ValueError, match="R98_PARENT_READBACK_LATE_OR_CHANGED"):
        forward.validate(*args)


@pytest.fixture
def identity_input():
    rows = [{"code": "001021", "y": 1, "market": {"original": 2, "option_change": {"log_put_call_volume_change": 0.1}}}]
...
def test_actual_question_or_source_changes_rejected(identity_input, kind):
    ...
    if kind == "source_hash":
        ...
        rows[0]["market"]["option_change"]["log_put_call_volume_change"] = 0.2
    else:
...
def test_missing_new_feature_falls_back_to_sign_control_not_raw(monkeypatch):
    ...
    result = s.batch([{"available": True, "option_change": {"available": False}}], {}, [])
...
def test_all_seventeen_branches_retained_after_new_parent_level(monkeypatch):
    ...
    records = {
        ...
        for n in (92, 94, 95, 96, 97)
    }
    monkeypatch.setattr(forward.base, "read", lambda path: records[path])
    merged = forward.combined_answers(
        parent, {"answers": {"r98": {"prediction": 1}}}, {"answers": {"r99": {"prediction": 0}}}
    )
    assert len(merged) == 17 and all(merged[k] == v for k, v in parent["answers"].items())
    assert merged["r92-1"] == {"prediction": 1} and merged["r99"] == {"prediction": 0}

...
from app.services import direction_1d_sprint_market_futures_style as s
from sklearn.linear_model import LogisticRegression


def test_sign_three_inputs_keeps_intraday_continuous():
    x = [[2.0, -0.1, 0.0, -0.8, 0.5, 0.4, -0.2]]
    assert np.allclose(
        s.transform(x, [2.0, 2.0, 2.0, 0.4, 0.5, 0.2, 0.1], True), [[1.0, -1.0, 0.0, -2.0, 1.0, 2.0, -2.0]]
    )
    assert np.allclose(
        s.transform(x, [2.0, 2.0, 2.0, 0.4, 0.5, 0.2, 0.1], False), [[1.0, -0.05, 0.0, -2.0, 1.0, 2.0, -2.0]]
    )
...
def test_missing_extra_keeps_corresponding_parent_answer():
    values = [
        {"available": True, "futures_style": {"available": False}},
        {"available": False, "futures_style": {"available": True}},
    ]
...
def head():
    ...
    m.fit(
        np.array(
            [
                [1.0, 1.0, 1.0, 0.1, 0.2, 0.1, 0.2],
                [-1.0, -1.0, -1.0, -0.1, -0.2, -0.1, -0.2],
                [1.0, -1.0, 1.0, 0.2, 0.1, 0.2, 0.1],
                [-1.0, 1.0, -1.0, -0.2, -0.1, -0.2, -0.1],
            ]
        ),
        [1, 0, 1, 0],
...
def actual_forecast(monkeypatch):
    ...
    history = {
        ...
        "t": "2026-09-16",
        "u": "2026-09-17",
        "curve_source_hash": s.base.digest({"source": "real_curve"}),
        "intraday_source_hash": "intraday-source",
    ...
    value = parent | {
        ...
        "model_hash": "model",
        "plan_hash": "plan",
        "r102_forecast_hash": s.base.digest(extra),
        "style_source_hash": s.base.digest(history),
        "answers": answers,
    ...
    proofs[s.base.ROOT / "round-96/forward/2026-09-17/001021.json"] = {"intraday_source_hash": "intraday-source"}
    monkeypatch.setattr(s.base, "read", lambda p: proofs[p])
    proofs[forward.prior_model.root() / "plan.json"] = {"p": "actual-parent"}
    monkeypatch.setattr(s.data.current, "load_live", lambda *_: {"source": "real_curve"})
...
def test_late_new_futures_snapshot_rejected_even_if_hashes_recomputed(actual_forecast):
    ...
    args[0]["style_source_hash"] = s.base.digest(args[-1])
    args[1]["forecast_hash"] = s.base.digest(args[0])
    with pytest.raises(ValueError, match="CURVE_SOURCE_CHANGED_OR_LATE"):
        forward.validate(*args)


def test_r102_parent_must_finish_readback_first(actual_forecast):
    ...
    with pytest.raises(ValueError, match="R102_PARENT_READBACK_LATE_OR_CHANGED"):
        forward.validate(*args)


@pytest.fixture
def identity_input():
    rows = [{"code": "001021", "y": 1, "market": {"original": 2, "futures_style": {"intraday_return_pct": 0.1}}}]
...
def test_actual_question_or_source_changes_rejected(identity_input, kind):
    ...
    if kind == "source_hash":
        ...
        rows[0]["market"]["futures_style"]["intraday_return_pct"] = 0.2
    else:
...
def test_missing_new_feature_falls_back_to_sign_control_not_raw(monkeypatch):
    ...
    result = s.batch([{"available": True, "futures_style": {"available": False}}], {}, [])
...
def test_old_historical_snapshot_cannot_stand_in_for_actual_capture(actual_forecast):
    ...
    args[0]["style_source_hash"] = s.base.digest(args[-1])
    args[1]["forecast_hash"] = s.base.digest(args[0])
    with pytest.raises(ValueError, match="CURVE_SOURCE_NOT_ACTUALLY_CAPTURED_IN_WINDOW"):
        forward.validate(*args)
...
def test_intraday_parent_source_hash_must_match_prior_prediction(actual_forecast):
    args, _ = actual_forecast
    args[-1]["curve_source_hash"] = "different-source"
    args[0]["style_source_hash"] = s.base.digest(args[-1])
    args[1]["forecast_hash"] = s.base.digest(args[0])
    with pytest.raises(ValueError, match="CURVE_SOURCE_SCOPE_CHANGED"):
        forward.validate(*args)


def test_all_twenty_one_branches_retained_after_two_new_parent_levels(monkeypatch):
    ...
    records = {
        ...
        for n in (92, 94, 95, 96, 97, 98, 99, 100, 101)
    }
    monkeypatch.setattr(forward.base, "read", lambda path: records[path])
    merged = forward.combined_answers(
        parent, {"answers": {"r102": {"prediction": 1}}}, {"answers": {"r103": {"prediction": 0}}}
    )
    assert len(merged) == 21 and all(merged[k] == v for k, v in parent["answers"].items())
    assert merged["r92-1"] == {"prediction": 1} and merged["r103"] == {"prediction": 0}

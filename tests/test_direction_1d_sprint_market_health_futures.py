...
import numpy as np
import pytest
from app.services import direction_1d_sprint_health_futures_forward as f
from app.services import direction_1d_sprint_market_health_futures as s
...
def test_missing_extra_keeps_corresponding_parent_answer():
    values = [
        {"available": True, "health_futures": {"available": False}},
        {"available": False, "health_futures": {"available": True}},
    ]
...
@pytest.fixture
def identity_input():
    rows = [{"code": "001021", "y": 1, "market": {"original": 2, "health_futures": {"response_signal": 0.1}}}]
...
def test_actual_question_or_source_changes_rejected(identity_input, kind):
    ...
    if kind == "source_hash":
        ...
        rows[0]["market"]["health_futures"]["response_signal"] = 0.2
    else:
...
def test_missing_new_feature_falls_back_to_sign_control_not_raw(monkeypatch):
    ...
    result = s.batch([{"available": True, "health_futures": {"available": False}}], {}, [])
    assert result[s.CANDIDATES[0]] == controls[s.CONTROLS[1]]
    assert result[s.CANDIDATES[0]] != controls[s.CONTROLS[0]]


def test_training_missing_response_keeps_actual_if_inputs(monkeypatch):
    ...
    assert s.vector({"health_futures": value}) == [1.0, -1.0, 1.0, 0.2, 0.3, 0.0]
    value["response_signal"] = 1.0
    with pytest.raises(ValueError, match="MISSING_INPUT_NOT_ZERO"):
        s.vector({"health_futures": value})
...
def future_record(monkeypatch):
    ...
    extra = {"answers": {"r104": {"prediction": 1}}, "plan_hash": "prior-plan", "response_source_hash": "health-source"}
    history = {
        ...
        "health_points": {},
        "futures_points": {},
        "intraday_source_hash": "if-source",
        "response_source_hash": "health-source",
    }
    ...
    value = parent | {
        ...
        "model_hash": "model",
        "plan_hash": "plan",
        "r105_forecast_hash": s.base.digest(extra),
        "joint_source_hash": s.base.digest(history),
        "answers": answers,
    ...
    proofs[s.base.ROOT / "round-96/forward/2026-09-17/001021.json"] = {"intraday_source_hash": "if-source"}
...
def test_new_history_with_recomputed_hash_cannot_replace_actual_source(future_record):
    ...
    args[0]["joint_source_hash"] = s.base.digest(args[-1])
...
def test_parent_readback_must_finish_before_new_prediction(future_record):
    ...
    with pytest.raises(ValueError, match="R105_PARENT_READBACK_LATE_OR_CHANGED"):
        f.validate(*args)
...
def test_all_twenty_four_branches_preserved(monkeypatch):
    ...
    records = {
        ...
        for n in (92, 94, 95, 96, 97, 98, 99, 100, 101, 102, 103, 104)
    }
    monkeypatch.setattr(f.base, "read", lambda p: records[p])
    merged = f.combined_answers(
        parent, {"answers": {"r105": {"prediction": 1}}}, {"answers": {"r106": {"prediction": 0}}}
    )
    assert len(merged) == 24 and merged["r106"]["prediction"] == 0 and merged["r92-1"]["prediction"] == 1


def test_if_parent_source_cannot_be_replaced_by_health_source(future_record):
    args, proofs = future_record
    proofs[s.base.ROOT / "round-96/forward/2026-09-17/001021.json"]["intraday_source_hash"] = "different"
    with pytest.raises(ValueError, match="ACTUAL_IF_SOURCE_CHANGED"):
        f.validate(*args)

...
import pytest
from app.integrations import sina_sprint_china_internet_etf as client
from app.services import direction_1d_sprint as b
from app.services import direction_1d_sprint_china_internet_data as d
...
def points(t="2026-09-14", u="2026-09-15"):
    days = d.required(t, u)
    return {
        "KWEB": {days[0]: quote(100, 100), days[1]: quote(105, 110)},
    }


def test_us_session_move_is_distinct_from_close_to_close():
    assert d.features("2026-09-14", "2026-09-15", points()) == pytest.approx([100 * 5 / 105, 1])
...
def test_no_new_us_close_uses_old_hk_route():
    ...
    assert d.features("2025-01-09", "2025-01-10", points("2025-01-09", "2025-01-10")) == [0] * 2


def test_invalid_ohlc_is_preserved_and_routes_to_control():
    p = points()
    row = p["KWEB"]["2026-09-14"]
    row.update(close=60, available=False, unavailable_reason="OHLC_RANGE_OR_NO_TRADE")
    assert d.features("2026-09-14", "2026-09-15", p) == [0] * 2
    assert row["close"] == 60
    row["available"] = True
...
@pytest.mark.parametrize(
    "raw",
    [b"", b'var KLC_K2_KWEB="abc";process.exit(0);', b'var KLC_K2_FXI="abc";', b"x" * 200001],
...
def test_row_quality_and_futures_are_checked_after_decoding(monkeypatch):
    ...
    result = client.parse(b"ignored", "KWEB", "2026-09-14")
    ...
    with pytest.raises(ValueError, match="FUTURE_ROW"):
        client.parse(b"ignored", "KWEB", "2023-12-12")
    monkeypatch.setattr(client, "decode", lambda *a: [row, row])
    with pytest.raises(ValueError, match="DUPLICATE"):
        client.parse(b"ignored", "KWEB")
...
def test_live_capture_is_once_per_slot_and_rechecks_original_bytes(live, monkeypatch):
    ...
    assert calls == ["KWEB"] and value["source"] == d.SOURCE
    d.capture(b.now())
    assert len(calls) == 1
    path = d.root() / "2026-09-15/KWEB/raw/0700.bin"
...
def test_rehashed_parsed_value_cannot_replace_original_response(live):
    ...
    value = b.read(path)
    value["rows"]["KWEB"]["2026-09-14"]["close"] = 52
...
def test_stock_split_does_not_create_overnight_session_jump():
    p = points()
    p["KWEB"]["2026-09-11"] = quote(300, 300)
    p["KWEB"]["2026-09-14"] = quote(100, 100)

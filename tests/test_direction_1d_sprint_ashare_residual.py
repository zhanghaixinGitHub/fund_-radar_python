"""验证新增沪深300海外价格确实参与一日推断，成熟训练与未来证据保持原题。"""
...
from app.services import direction_1d_sprint_ashare_residual as s
...
def test_four_scaled_inputs_include_residual_signal():
...
def test_mature_only_fit_keeps_four_scales_and_no_intercept(monkeypatch):
    ...
    assert after["model"].n_features_in_ == 4 and after["model"].fit_intercept is False
...
def ready(joint_ready, monkeypatch):  # noqa: F811
    ...
    from test_direction_1d_sprint_ashare_data import points

    sector_input = {"rows": points() | {"cash": {}}}
...
def test_entry_keeps_previous_round_and_attempts_new_after_failure(monkeypatch):
    from scripts import direction_1d_sprint_ashare_residual as entry

    assert entry.existing.__name__ == "scripts.direction_1d_sprint_ashare"
...
def test_rehashed_sector_input_rejected(ready, monkeypatch):
    ...
    with pytest.raises(ValueError, match="ASHARE_RESIDUAL_INPUT_CHANGED"):
        s.report()


def test_fixed_sign_uses_only_available_residual_direction_without_fitting(monkeypatch):
    monkeypatch.setattr(s, "active", lambda: pytest.fail("fixed rule cannot fit"))
    name = s.CANDIDATES[1]
    assert s.fit([], name, "2025-01-01") is None
    for value, expected in ((-0.01, 0), (0.0, 1), (0.01, 1)):
        z = vector()
        z[20] = value
        choice = s.answer(z, name, {"control": trained()["control"], "us_etf": None})
        assert choice["prediction"] == expected and choice["kind"] == "FIXED_DIRECTION_RULE"

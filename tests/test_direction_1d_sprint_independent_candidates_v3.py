...
import pytest
from app.services import direction_1d_sprint_independent_candidates_v3 as m


def test_branches_preserved_without_untrained_placeholder():
    old = {f"old{i}": {"prediction": i % 2} for i in range(35)}
    new = {f"r{i}": {"prediction": i % 2} for i in (118, 119, 120, 121, 122)}
    assert m.combine(old, new) == old | new
    assert "r117" not in m.combine(old, new)


@pytest.mark.parametrize("old_count,new_count,collide", [(34, 5, False), (35, 4, False), (35, 5, True)])
...
def test_model_output_and_missing_fallback_used_without_changes(monkeypatch):
    ...
    for n in m.ROUNDS:
        ...
        def extend(market, t, u, points, number=n, **kwargs):
    ...
    histories[122]["frozen_current"] = {}
    monkeypatch.setattr(m, "SPECS", specs)
    values = m.candidate_answers(
        {"market": {"original": 1}}, {"t": "t", "u": "u", "group": "g", "code": "synthetic"}, histories, loaded
    )
    assert len(seen) == 5 and values["r120"]["fallback"]

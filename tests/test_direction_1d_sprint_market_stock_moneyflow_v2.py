"""订单分类输入模型的固定五维、时间边界与缺失回退；不跑真实基金成绩。"""
...
import numpy as np
import pytest
from app.services import direction_1d_sprint_market_stock_moneyflow_v2 as s
...
def test_bad_moneyflow_value_not_silently_normalized(monkeypatch, basis):
    ...
    with pytest.raises(ValueError, match="STOCK_MONEYFLOW_INVALID"):
        s.vector(
            {
                "stock_moneyflow": {
                    "available": True,

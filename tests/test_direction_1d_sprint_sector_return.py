...
from app.services import direction_1d_sprint_sector_return as s
...
def test_same_market_can_predict_different_funds_and_zero_is_neutral():
    ...
    assert neutral["research_score"] == 0 and neutral["prediction"] == 0
...
def test_mature_only_fit_keeps_three_scales_and_no_intercept(monkeypatch):
    ...
    monkeypatch.setattr(s, "target_values", lambda rows: np.asarray([1.0 if r["y"] else -1.0 for r in rows]))
    rows = []
...
def test_entry_keeps_previous_round_and_attempts_new_after_failure(monkeypatch):
    from scripts import direction_1d_sprint_sector_return as entry

    assert entry.existing.__name__ == "scripts.direction_1d_sprint_sector_pooling"
...
def test_rehashed_sector_input_rejected(ready, monkeypatch):
    ...
    with pytest.raises(ValueError, match="SECTOR_RETURN_INPUT_CHANGED"):
        s.report()


def test_continuous_target_uses_original_nav_prior_vol_and_preserves_direction(monkeypatch):
    history = {
        "funds": [
            {"fund_code": "001000", "rows": [{"date": "2025-01-02", "nav": 1.0}, {"date": "2025-01-03", "nav": 1.01}]}
        ]
    }
    monkeypatch.setattr(b, "read", lambda path: history)
    x = [0.0] * 32
    x[3] = 0.01
    rows = [{"code": "001000", "t": "2025-01-02", "u": "2025-01-03", "x": x, "y": 1}]
    assert s.target_values(rows).tolist() == pytest.approx([1.0])
    rows[0]["x"][3] = 0
    assert s.target_values(rows).tolist() == [5.0]
    rows[0]["y"] = 0
    with pytest.raises(ValueError, match="RAW_DIRECTION_CHANGED"):
        s.target_values(rows)


def test_regression_score_can_exceed_probability_range_and_zero_is_non_up():
    name = s.CANDIDATES[0]
    result = s.answer(vector(0, (4, -3, 2)), name, trained(name))
    assert result["research_score"] == 4 and result["kind"] == "NORMALIZED_RETURN_RESEARCH_SCORE"
    assert s.answer(vector(0, (0, 0, 0)), name, trained(name))["prediction"] == 0

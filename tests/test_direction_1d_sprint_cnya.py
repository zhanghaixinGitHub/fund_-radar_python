...
from app.services import direction_1d_sprint_cnya as s
...
def test_code_freeze_covers_real_fxi_client_and_data_files(monkeypatch):
    ...
    assert len(code) == 7
    assert "app/integrations/ishares_sprint_cnya.py" in code
    assert "app/services/direction_1d_sprint_cnya_data.py" in code
...
def test_changed_saved_fxi_value_is_rejected(ready, monkeypatch):
    ...
    with pytest.raises(ValueError, match="ROUND_43_VECTOR_CHANGED"):
        s.report()


def test_prior_failure_still_attempts_fxi_branch(monkeypatch):
    from scripts import direction_1d_sprint_cnya as entry

    assert entry.existing.__name__ == "scripts.direction_1d_sprint_fxi_overnight"
    monkeypatch.setattr(entry.existing, "run", lambda: (_ for _ in ()).throw(ValueError("OLD_FAILED")))
    calls = []
    monkeypatch.setattr(entry.cnya, "tick", lambda: calls.append(1))

...
def test_immature_and_unavailable_rows_cannot_change_fitted_model(monkeypatch, name):
    ...
    assert before["model"].fit_intercept is False and before["model"].n_features_in_ == 5
...
def test_new_entry_runs_independent_branch_even_when_old_branch_fails(monkeypatch):
    from scripts import direction_1d_sprint_market_gap_delta as entry

    assert entry.existing.__name__ == "scripts.direction_1d_sprint_market_margin"

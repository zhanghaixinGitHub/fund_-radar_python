...
import numpy as np
import pytest
from app.services import direction_1d_sprint_china_health_forward as f
from app.services import direction_1d_sprint_market_china_health as s
...
def test_missing_extra_keeps_corresponding_parent_answer():
    values = [
        {"available": True, "china_health": {"available": False}},
        {"available": False, "china_health": {"available": True}},
    ]
...
@pytest.fixture
def identity_input():
    rows = [{"code": "001021", "y": 1, "market": {"original": 2, "china_health": {"intraday_log_pct": 0.1}}}]
...
def test_actual_question_or_source_changes_rejected(identity_input, kind):
    ...
    if kind == "source_hash":
        ...
        rows[0]["market"]["china_health"]["intraday_log_pct"] = 0.2
    else:
...
def test_missing_new_feature_falls_back_to_sign_control_not_raw(monkeypatch):
    ...
    result = s.batch([{"available": True, "china_health": {"available": False}}], {}, [])
...
def test_training_missing_signal_is_zero_but_keeps_original_three_inputs(monkeypatch):
    ...
    assert s.vector({"china_health": {"available": False, "intraday_log_pct": 0.0}}) == [1.0, -1.0, 1.0, 0.0]
    with pytest.raises(ValueError, match="MISSING_INPUT_NOT_ZERO"):
        s.vector({"china_health": {"available": False, "intraday_log_pct": 1.0}})
...
def future_record(monkeypatch):
    ...
    extra = {"answers": {"r103": {"prediction": 1}}}
    ...
    value = parent | {
        ...
        "model_hash": "model",
        "plan_hash": "plan",
        "r103_forecast_hash": s.base.digest(extra),
        "health_source_hash": s.base.digest(history),
        "answers": answers,
...
def test_new_history_with_recomputed_hash_cannot_replace_actual_source(future_record):
    ...
    args[0]["health_source_hash"] = s.base.digest(args[-1])
    args[1]["forecast_hash"] = s.base.digest(args[0])
    with pytest.raises(ValueError, match="ACTUAL_HEALTH_SOURCE_CHANGED"):
        f.validate(*args)
...
def test_parent_readback_must_finish_before_new_prediction(future_record):
    ...
    with pytest.raises(ValueError, match="R103_PARENT_READBACK_LATE_OR_CHANGED"):
        f.validate(*args)
...
def test_all_twenty_two_branches_preserved(monkeypatch):
    ...
    records = {
        ...
        for n in (92, 94, 95, 96, 97, 98, 99, 100, 101, 102)
    }
    monkeypatch.setattr(f.base, "read", lambda p: records[p])
    merged = f.combined_answers(
        parent, {"answers": {"r103": {"prediction": 1}}}, {"answers": {"r104": {"prediction": 0}}}
    )
    assert len(merged) == 22 and merged["r104"]["prediction"] == 0 and merged["r92-1"]["prediction"] == 1

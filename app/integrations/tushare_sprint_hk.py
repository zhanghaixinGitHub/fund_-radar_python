"""三天研究的恒生指数和恒生科技指数只读客户端，独立于已冻结的已有指数客户端。
...
def fetch_hk(code: str, start: date, end: date) -> bytes:
    if code not in ("HSI", "HKTECH") or not 0 <= (end - start).days <= 366:
        raise ValueError("HK_QUERY_RANGE_INVALID")
    ...
    if (endpoint.scheme, endpoint.hostname, endpoint.path) != ("https", "api.tushare.pro", "") or any(
        ...
        raise ValueError("HK_OFFICIAL_ENDPOINT_REQUIRED")
    token = settings.tushare_token.get_secret_value()
    if not token:
        raise ValueError("HK_TOKEN_MISSING")
    began = monotonic()

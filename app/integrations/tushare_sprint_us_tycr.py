"""三天研究的美国国债收益率只读客户端，固定官方接口、字段和请求上限。
...
FIELDS = ["date", "m3", "y2", "y5", "y10", "y30"]
MAX_BYTES = 262144
URL = "https://api.tushare.pro"


def fetch_us_tycr(start: date, end: date) -> bytes:
    """最多读取一个自然年的利率；无重定向、无自动重试，不输出凭据或原始异常响应。"""
    if not 0 <= (end - start).days <= 366:
        raise ValueError("US_TREASURY_QUERY_RANGE_INVALID")
    ...
    if (endpoint.scheme, endpoint.hostname, endpoint.path) != ("https", "api.tushare.pro", "") or any(
        ...
        raise ValueError("US_TREASURY_OFFICIAL_ENDPOINT_REQUIRED")
    token = settings.tushare_token.get_secret_value()
    if not token:
        raise ValueError("US_TREASURY_TOKEN_MISSING")
    began = monotonic()
    with httpx.Client(timeout=httpx.Timeout(20, connect=5), follow_redirects=False) as client:
        with client.stream(
            ...
            if response.status_code != 200:
                raise ValueError("US_TREASURY_PROVIDER_HTTP_FAILED")
            body = bytearray()
            for chunk in response.iter_bytes():
                body.extend(chunk)
                if len(body) > MAX_BYTES or monotonic() - began > 30:
                    raise ValueError("US_TREASURY_RESPONSE_LIMIT")
    if token.encode() in body:
        raise ValueError("US_TREASURY_RESPONSE_CONTAINS_CREDENTIAL")
    value = json.loads(body)
    if not isinstance(value, dict) or type(value.get("code")) is not int or value["code"] != 0:
        raise ValueError("US_TREASURY_PROVIDER_BUSINESS_FAILED")
    ...
    if (
        ...
        raise ValueError("US_TREASURY_RESPONSE_FIELDS_OR_ROWS_INVALID")
    return bytes(body)

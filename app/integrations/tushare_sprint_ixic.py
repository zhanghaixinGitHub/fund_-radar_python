"""三天研究的纳斯达克指数只读客户端，独立于已冻结的SPX客户端。

固定官方地址、指数和字段；最多一年270行，单次请求无自动重试。调用者负责
...
def fetch_ixic(start: date, end: date) -> bytes:
    if not 0 <= (end - start).days <= 366:
        raise ValueError("IXIC_QUERY_RANGE_INVALID")
    ...
    if (endpoint.scheme, endpoint.hostname, endpoint.path) != ("https", "api.tushare.pro", "") or any(
        ...
        raise ValueError("IXIC_OFFICIAL_ENDPOINT_REQUIRED")
    token = settings.tushare_token.get_secret_value()
    if not token:
        raise ValueError("IXIC_TOKEN_MISSING")
    began = monotonic()
    with httpx.Client(timeout=httpx.Timeout(20, connect=5), follow_redirects=False) as client:
        with client.stream(
            ...
            if response.status_code != 200:
                raise ValueError("IXIC_PROVIDER_HTTP_FAILED")
            body = bytearray()
            for chunk in response.iter_bytes():
                body.extend(chunk)
                if len(body) > 262144 or monotonic() - began > 30:
                    raise ValueError("IXIC_RESPONSE_LIMIT")
    if token.encode() in body:
        raise ValueError("IXIC_RESPONSE_CONTAINS_CREDENTIAL")
    ...
    if data.get("fields") != FIELDS or not isinstance(data.get("items"), list) or len(data["items"]) > 270:
        raise ValueError("IXIC_RESPONSE_FIELDS_OR_ROWS_INVALID")
    return bytes(body)

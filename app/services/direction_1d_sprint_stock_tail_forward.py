"""第116轮复用 CORE3 输入，复用R109的实际沪深股票响应，不调用已退役的旧实验链。"""
...
from app.services import direction_1d_sprint_market_stock_activity as prior_model
from app.services import direction_1d_sprint_market_stock_tail as model
from app.services import direction_1d_sprint_stock_activity_forward as prior_forward
from app.services import direction_1d_sprint_stock_tail_live as live
from app.services import direction_1d_sprint_stock_tail_runtime as runtime
...
def validate(value, receipt, parent, source, manifest, bundle, extra, history):
    ...
    model.require(value["r115_forecast_hash"] == base.digest(extra), "R115_PARENT_CHANGED")
    ...
    model.require(
        ...
        "R115_PARENT_READBACK_LATE_OR_CHANGED",
    )
    model.require(
        value["stock_tail_source_hash"] == base.digest(history) and datetime.fromisoformat(history["at"]) <= created,
        "STOCK_TAIL_SOURCE_CHANGED_OR_LATE",
    )
    model.require(
        history == live.load_live(parent["t"], parent["u"], manifest["plan_hash"]),
        "ACTUAL_STOCK_TAIL_SOURCE_CHANGED",
    )
    model.require(
        live.in_window(parent["t"], parent["u"], datetime.fromisoformat(history["at"])),
        "STOCK_TAIL_SOURCE_NOT_ACTUALLY_CAPTURED_IN_WINDOW",
    )
...
def run():
    ...
    for key, p in parents.items():
        ...
        value = {k: p[k] for k in ("code", "family", "group", "t", "u")} | {
            ...
            "r115_forecast_hash": base.digest(extra),
            "stock_tail_source_hash": base.digest(history[p["u"]]),
        }
...
def combined_answers(parent, extra, value):
    """明确合并全部35个同题分支，避免新增中间模型后递归层级漂移丢掉旧对照。"""
    ...
    model.require(len(combined) == 35, "FORWARD_BRANCH_COUNT_CHANGED")
    return combined

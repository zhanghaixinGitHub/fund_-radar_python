...
from app.services import direction_1d_sprint_health_response_data as data
from app.services import direction_1d_sprint_market_china_health as previous
...
CANDIDATES = ("SIGN3_KURE_RESPONSE4_LR504",)
CONTROLS = previous.CONTROLS
BRANCHES = CANDIDATES + CONTROLS
FIRST_TARGET = "2026-09-17"
PROPOSAL_HASH = "616af2419694cd72fec9b4abf480355a80b7792a98532c9b8d4b296df40d9def"


def root():
    return base.ROOT / "round-105"


def require(condition, reason):
    if not condition:
        raise ValueError("KURE_RESPONSE_" + reason)


def fingerprint():
    code = dict(base.read(base.ROOT / "round-104" / "plan.json")["fingerprint"]["code"])
    for name, expected in code.items():
        require(core.sha(base.PROJECT / name) == expected, "OLD_CODE_CHANGED")
    for name in (
        "app/services/direction_1d_sprint_market_health_response.py",
        "app/services/direction_1d_sprint_health_response_forward.py",
        "scripts/direction_1d_sprint_market_health_response.py",
        "tests/test_direction_1d_sprint_market_health_response.py",
        "app/services/direction_1d_sprint_health_response_data.py",
        "tests/test_direction_1d_sprint_health_response_data.py",
        "app/services/direction_1d_sprint_health_response_context.py",
        "tests/test_direction_1d_sprint_health_response_context.py",
        ".local-runs/direction-1d-sprint-20260914/china-health-response-feasibility-v1/check.py",
    ):
...
def plan():
    ...
    require(proposal["r104_result_hash"] == base.digest(base.read(previous.root() / "result.json")), "SOURCE_CHANGED")
    require(
...
def vector(value):
    ...
    feature = value["health_response"]
    require(type(feature["available"]) is bool, "INTRADAY_INVALID")
    extra = feature["response_signal"]
    require(type(extra) in (int, float) and np.isfinite(extra) and abs(extra) <= 60, "INTRADAY_INVALID")
...
def fit(rows, name, cutoff):
    ...
    return {
        ...
        "max_feature_date": max(d for r in chosen for d in r["market"]["health_response"]["new_us_dates"]),
        "training_feature_mean_not_subtracted": mean,
        "max_prior_mature": max(
            r["market"]["health_response"]["max_prior_mature"]
            for r in chosen
            if r["market"]["health_response"]["max_prior_mature"] is not None
        ),
        "latest_context_cutoff": max(r["market"]["health_response"]["context_cutoff"] for r in chosen),
        "masked_training_rows": sum(not r["market"]["health_response"]["available"] for r in chosen),
...
def candidate_answers(values, name, head, fallback):
    ...
    indices = [i for i, v in enumerate(values) if v["available"] and v["health_response"]["available"]]
    if indices:
...
def preflight():
    from app.services import direction_1d_sprint_health_response_forward as forward

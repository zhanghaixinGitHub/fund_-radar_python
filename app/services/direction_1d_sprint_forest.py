"""第三十四轮：使用优化切分和有放回采样的随机森林学习下一日方向规则的错误。

完全保留第17轮HK12输入、504个成熟日期及0.55改判阈值。
此轮检验一套事前固定的随机森林配方，不将改善归因于其中某个单独算法设置。
"""
...
from app.services import direction_1d_sprint_activity as previous_round
...
def fingerprint():
    ...
    for name in (
        "app/services/direction_1d_sprint_forest.py",
        "scripts/direction_1d_sprint_forest.py",
        "tests/test_direction_1d_sprint_forest.py",
    ):
...
def selected_features(z, name):
    if name not in CANDIDATES or len(z) != 12 or not np.isfinite(z).all():
        raise ValueError("FOREST_INPUT_INVALID")
    return z
...
def plan():
    ...
    if path.exists():
        ...
        if value["fingerprint"] != fingerprint() or value["calendar_hash"] != base.calendar()[1]:
            raise ValueError("ROUND_34_CODE_OR_CALENDAR_CHANGED")
        return value
    active()
    if base.read(previous_round.root() / "result.json")["fingerprint"] != previous_round.fingerprint():
        raise ValueError("FOREST_PREVIOUS_FINGERPRINT_CHANGED")
    parent, _ = hk.models()
    value = {
        ...
        "hypothesis": (
            "Optimized split thresholds plus bootstrap may learn HK12 baseline errors missed by random splits"
        ),
        "motivation": "Compare fixed RandomForest recipe with original ExtraTrees; no parameter or threshold search",
...
def fit(rows, name, cutoff):
    """保留第17轮输入及样本；固定256棵有放回采样的树，每棵树按训练目标选择切分。"""
    if name not in CANDIDATES:
        raise ValueError("FOREST_MODEL_RECIPE_INVALID")
    ...
    if len(Counter(y)) != 2 or min(Counter(y).values()) < 20:
        raise ValueError("FOREST_MODEL_ERROR_LABELS_INSUFFICIENT")
...
def fit_checkpoint(rows, name, cutoff, label):
    ...
    if receipt.exists():
        ...
        if (
            ...
            raise ValueError("FOREST_MODEL_CHECKPOINT_CHANGED")
        return joblib.load(path)
    if attempt.exists():
        raise ValueError("FOREST_MODEL_PREVIOUS_FIT_INTERRUPTED")
    ...
    if base.digest(short) != expected:
        raise ValueError("FOREST_CONTROL_TRAINING_ROWS_CHANGED")
...
def train():
    ...
    for name, expected in p["input_hashes"].items():
        if base.digest(base.read(base.ROOT / name)) != expected:
            raise ValueError("ROUND_34_INPUT_CHANGED")
    rows, proofs = dataset()
    ...
    with threadpool_limits(limits=2):
        ...
        if len(expected) != 5670 or any(
            ...
            raise ValueError("ROUND_34_COMMON_EXAM_CHANGED")
...
def models():
    ...
    if (
        ...
        raise ValueError("ROUND_34_MODEL_OR_CODE_CHANGED")

"""第二十二轮：滞后VIX风险信息对一日港股纠错模型的增量影响。

保留原港股12项输入，增加基准日15:00前已收盘的VIX水平与前一日变化。
...
from app.services import direction_1d_sprint_kernel as previous_round
...
def fingerprint():
    ...
    for name in (
        "app/services/direction_1d_sprint_vix.py",
        "scripts/direction_1d_sprint_vix.py",
        "tests/test_direction_1d_sprint_vix.py",
        "app/integrations/cboe_sprint_vix.py",
        "app/services/direction_1d_sprint_vix_data.py",
        "tests/test_direction_1d_sprint_vix_data.py",
    ):
...
def selected_features(z, name):
    if name not in CANDIDATES or len(z) != 14 or not np.isfinite(z).all():
        raise ValueError("VIX_MODEL_INPUT_INVALID")
    return z


def dataset():
    rows, proof = hk.dataset()
    points = vix_data.history()
    cache, output = {}, []
    for row in rows:
        key = (row["t"], row["u"])
        if key not in cache:
            cache[key] = vix_data.features(*key, points)
        output.append(row | {"z": row["z"] + cache[key]})
    return output, proof | {
        "at": base.now().isoformat(),
        "vix_rows": len(points),
        "vix_raw_source_hash": base.digest(base.read(base.ROOT / "vix-data-feasibility-v1/result.json")),
        "vix_input_time": "last US cash close <= CN baseT15:00 and previous US cash close",
    }
...
def plan():
    ...
    if path.exists():
        ...
        if value["fingerprint"] != fingerprint() or value["calendar_hash"] != base.calendar()[1]:
            raise ValueError("ROUND_22_CODE_OR_CALENDAR_CHANGED")
        return value
    active()
    if base.read(previous_round.root() / "result.json")["fingerprint"] != previous_round.fingerprint():
        raise ValueError("VIX_PREVIOUS_FINGERPRINT_CHANGED")
    parent, _ = hk.models()
    vix_data.history()
    value = {
        "at": base.now().isoformat(),
        ...
        "hypothesis": "Known lagged volatility may add risk information beyond SPX direction and HK relative moves",
        ...
        "feature_order": "HK12; VIX cashclose<=T15:00 /20; previouscashclose percent change clipped+-100",
        "vix_source": vix_data.SOURCE,
        "vix_raw_url": vix_data.URL,
        "source_timing": "Last-Modified09:50 does not prove08:30 first arrival; use lagged cash-close anchor",
        "future_evidence": "Raw CSV, request/receive, required2rows, hashes, prediction/readback<U08:30",
        "live_budget": {
            "slots": ["0700", "0730", "0800"],
            "max_requests_per_target": 3,
            "max_bytes": vix_data.MAX_BYTES,
            "redirects": False,
            "automatic_retries": 0,
        },
        "extra_trees": {
        ...
        | {
            ...
            for name in (
                "vix-data-feasibility-v1/result.json",
                "vix-data-feasibility-v1/response.json",
                "vix-data-feasibility-v1/lagged-feasibility-result.json",
            )
        },
...
def fit(rows, name, cutoff):
    """严格沿用第17轮树配方；只追加两个已知VIX输入，记录原训练样本摘要。"""
    if name not in CANDIDATES:
        raise ValueError("VIX_MODEL_RECIPE_INVALID")
    ...
    if len(Counter(y)) != 2 or min(Counter(y).values()) < 20:
        raise ValueError("VIX_MODEL_ERROR_LABELS_INSUFFICIENT")
...
def fit_checkpoint(rows, name, cutoff, label):
    ...
    if receipt.exists():
        ...
        if (
            ...
            raise ValueError("VIX_MODEL_CHECKPOINT_CHANGED")
        return joblib.load(path)
    if attempt.exists():
        raise ValueError("VIX_MODEL_PREVIOUS_FIT_INTERRUPTED")
    ...
    if base.digest([r | {"z": r["z"][:12]} for r in chosen]) != expected:
        raise ValueError("VIX_CONTROL_TRAINING_ROWS_CHANGED")
...
def train():
    ...
    for name, expected in p["input_hashes"].items():
        if base.digest(base.read(base.ROOT / name)) != expected:
            raise ValueError("ROUND_22_INPUT_CHANGED")
    rows, proofs = dataset()
    ...
    with threadpool_limits(limits=2):
        ...
        if len(expected) != 5670 or any(
            ...
            raise ValueError("ROUND_22_COMMON_EXAM_CHANGED")
...
def models():
    ...
    if (
        ...
        raise ValueError("ROUND_22_MODEL_OR_CODE_CHANGED")
...
def report():
    ...
    for path in (root() / "forward").glob("*/*.json"):
        ...
        if not np.allclose(
            ...
            raise ValueError("ROUND_22_VECTOR_CHANGED")
        good += 1

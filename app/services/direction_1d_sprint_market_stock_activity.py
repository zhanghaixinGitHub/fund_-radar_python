"""第115轮：T日相对过去20交易日的成交活跃程度与集中变化，保持原标签、训练样本和对照。"""
...
from app.services import direction_1d_sprint_market_stock_interaction as previous
...
from app.services import direction_1d_sprint_stock_activity_data as data

original, active, reference = previous.original, previous.active, previous.reference
CANDIDATES = ("SIGN3_STOCK_ACTIVITY5_LR504",)
CONTROLS = previous.CONTROLS
BRANCHES = CANDIDATES + CONTROLS
FIRST_TARGET = "2026-09-17"
PROPOSAL_HASH = "86b938e14864d31dbaa6bcb9cf0d461e18c0905177b4e6c1f533b6df51c42657"


def root():
    return base.ROOT / "round-115"
...
def fingerprint():
    code = dict(base.read(base.ROOT / "round-114" / "plan.json")["fingerprint"]["code"])
    ...
    for name, expected in source_plan["code"].items():
        ...
        code[name] = expected
    for name in (
        "app/services/direction_1d_sprint_market_stock_activity.py",
        "scripts/direction_1d_sprint_market_stock_activity.py",
        "tests/test_direction_1d_sprint_market_stock_activity.py",
    ):
...
def plan():
    ...
    require(proposal["r114_result_hash"] == base.digest(base.read(previous.root() / "result.json")), "SOURCE_CHANGED")
    require(
    ...
    require(base.digest(source) == proposal["source_qualification_hash"], "ACTIVITY_SOURCE_CHANGED")
    coverage = base.read(root() / "feature-feasibility.json")
    require(
        coverage["source_history_hash"] == source["source_proof"]["source_history_hash"]
        and all(x["activity_available"] == x["questions"] for x in coverage["exam"].values())
...
def reconstruct_dataset():
    """按共享市场日期缓存新增沪深成交活跃程度及集中变化，避免为同日30基金重复解析整份交易日历。"""
    ...
    for row in rows:
        key = row["t"], row["u"]
        if key not in cache:
            cache[key] = data.extend({}, *key, h["snapshot"]["rows"])["stock_activity"]
        extra.append(row | {"market": row["market"] | {"stock_activity": cache[key]}})
    require([data.original_row(r) for r in extra] == rows, "ORIGINAL_ROWS_CHANGED")
    proof = proof | {"stock_activity_history_hash": base.digest(h)}
    return extra, proof
...
def vector(value):
    ...
    feature = value["stock_activity"]
    require(feature["available"], "STOCK_ACTIVITY_MISSING")
    extra = [feature[k] for k in data.KEYS]
    require(
        all(type(v) in (int, float) and np.isfinite(v) for v in extra) and abs(extra[0]) <= 3 and abs(extra[1]) <= 1,
        "STOCK_ACTIVITY_INVALID",
    )
    return [*x, *extra]
...
def fit(rows, name, cutoff):
    ...
    require(
        ...
        and all(
            r["mature"] < cutoff and r["u"] < cutoff and r["market"]["stock_activity"]["available"] for r in chosen
        ),
    ...
    return {
        ...
        "max_feature_date": max(r["market"]["stock_activity"]["date"] for r in chosen),
...
def candidate_answers(values, name, head, fallback):
    ...
    indices = [i for i, v in enumerate(values) if v["available"] and v["stock_activity"]["available"]]
    if indices:

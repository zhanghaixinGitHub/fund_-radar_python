"""第五十轮：比较三信号固定投票、CNYA单规则和学习三项权重的一日方向模型。

规则不训练权重，学习方案使用零截距逻辑回归；缺少任一来源时复用原HK答案。
...
from app.services import direction_1d_sprint_us_etf_direct as previous_round

CANDIDATES = ("SPX_FXI_CNYA_MAJORITY3", "SPX_FXI_CNYA_LR3_ZERO504", "CNYA_GAP_SIGN_WITH_HK")
...
def fingerprint():
    ...
    for name in (
        "app/services/direction_1d_sprint_etf_joint.py",
        "scripts/direction_1d_sprint_etf_joint.py",
        "tests/test_direction_1d_sprint_etf_joint.py",
        ".local-runs/direction-1d-sprint-20260914/round-50/feature-feasibility.py",
    ):
        value["code"][name] = hashlib.sha256((base.PROJECT / name).read_bytes()).hexdigest()
    return value


def vector(x, code, t, u, markets, points, cnys):
    """保持相邻国内交易日标签，追加同一美国现金收盘的CNYA净值口径差。"""
    return previous_round.vector(x, code, t, u, markets, points) + cnya_data.features(t, u, cnys)


def live_vector(source, original, markets, points, cnys):
    sequence.live_vector(source, original)
    return vector(source["x"], original["code"], original["base"], original["u"], markets, points, cnys)


def available(z):
    return z[16] == 1 and z[19] == 1


def selected_features(z, name):
    """统一三项百分比输入；可用标记只用于回退，不能成为分类器的常量偏置。"""
    if name not in CANDIDATES or len(z) != 20 or not np.isfinite(z).all() or z[19] not in (0, 1):
        raise ValueError("ETF_JOINT_INPUT_INVALID")
    previous_round.selected_features(z[:17], previous_round.CANDIDATES[0])
    if z[19] == 0 and any(z[17:19]):
        raise ValueError("ETF_JOINT_MISSING_GAP_NONZERO")
    return [z[0], z[12], z[17]]


def dataset():
    rows, proof = previous_round.dataset()
    points = cnya_data.history()
    context = {(r["t"], r["u"]): None for r in rows}
    for key in context:
        context[key] = cnya_data.features(*key, points)
    expanded = [r | {"z": r["z"] + context[r["t"], r["u"]]} for r in rows]
    proposal, feasible = (base.read(root() / n) for n in ("proposal-before-training.json", "input-feasibility.json"))
    if (
        feasible["proposal_hash"] != base.digest(proposal)
        or feasible["rows_hash"] != base.digest(expanded)
        or feasible["cnya_hash"] != base.digest(points)
        ...
        raise ValueError("ETF_JOINT_PROTOTYPE_CHANGED")
...
def plan():
    ...
    if path.exists():
        ...
        if value["fingerprint"] != fingerprint() or value["calendar_hash"] != base.calendar()[1]:
            raise ValueError("ROUND_50_CODE_OR_CALENDAR_CHANGED")
        return value
    active()
    parent, _ = hk.models()
    ...
    value = {
        ...
        "hypothesis": (
            "CNYA domestic-A valuation signal resolves SPX/offshore FXI disagreement; "
            "fixed vote versus learned combination"
        ),
        "target": "Raw unit NAV T to adjacent CN U UP/NON_UP; flat separate",
        "feature_order": (
            "Full20 evidence; model indices0,12,17 SPX,FXI_USsession,CNYA_gap; "
            "availability16,19 only routing"
        ),
        "routing": "Same30funds5670questions; either source unavailable exactHK fallback",
        "recipe": (
            "OneLR3 C.1 max_iter1500 lbfgs L2 seed17 no intercept; train-only scale "
            "without centering; no convergence retry"
        ),
        "rules": "MAJORITY3>=2 of3 nonnegative signs, CNYA_GAP_SIGN>=0; zero fitting for both controls",
        "training": "Last504 mature joint-source-available dates/group; natural date/productfamily UP weights",
        ...
        "expected_dates": 199,
        "selection": "One learned combination and two fixed rules; no weight or threshold search",
        ...
        | {
            ...
            for name in (
                ...
                "round-50/proposal-before-training.json",
                "round-50/input-feasibility.json",
            )
        },
...
def batch_answers(values, name, trained):
    ...
    if indices:
        if name in LEARNED:
            ...
            if us_etf is None:
                raise ValueError("ETF_JOINT_MODEL_MISSING")
            x = (np.asarray([selected[i] for i in indices]) - us_etf["mean"]) / us_etf["scale"]
            if not np.isfinite(x).all():
                raise ValueError("ETF_JOINT_NORMALIZED_INPUT_INVALID")
            scores = us_etf["model"].predict_proba(x)[:, 1]
            kind = "UNCALIBRATED_UP_SCORE"
        elif name == CANDIDATES[0]:
            scores = np.asarray([sum(v >= 0 for v in selected[i]) / 3 for i in indices])
            kind = "FIXED_DIRECTION_VOTE_FRACTION"
        else:
            scores = np.asarray([float(selected[i][2] >= 0) for i in indices])
...
def train():
    ...
    for name, expected in p["input_hashes"].items():
        if base.digest(base.read(base.ROOT / name)) != expected:
            raise ValueError("ROUND_50_INPUT_CHANGED")
    rows, proofs = dataset()
    ...
    with threadpool_limits(limits=2):
        ...
        if len(expected) != 5670 or any(
            ...
            raise ValueError("ROUND_50_COMMON_EXAM_CHANGED")
...
def models():
    ...
    if (
        ...
        raise ValueError("ROUND_50_MODEL_OR_CODE_CHANGED")
...
def report():
    ...
    for path in (root() / "forward").glob("*/*.json"):
        ...
        if (
            ...
            raise ValueError("ROUND_50_PARENT_OR_MODEL_CHANGED")
        ...
        if not np.allclose(
            ...
            raise ValueError("ROUND_50_VECTOR_CHANGED")
        ...
        if not runtime.answers_match(value["answers"], expected_answers):
            raise ValueError("ROUND_50_SAVED_ANSWER_CHANGED")
        good += 1

"""第96轮复用 CORE3 输入，并复用已实际取得T日原始期货行情，不调用已退役的旧实验链。"""
...
from app.services import direction_1d_sprint_current_basis_forward as prior_forward
from app.services import direction_1d_sprint_market_current_basis as prior_model
from app.services import direction_1d_sprint_market_futures_intraday as model
...
def validate(value, receipt, parent, source, manifest, bundle, extra, history):
    ...
    model.require(value["r95_forecast_hash"] == base.digest(extra), "R95_PARENT_CHANGED")
    ...
    model.require(
        ...
        "R95_PARENT_READBACK_LATE_OR_CHANGED",
    )
    model.require(
        value["intraday_source_hash"] == base.digest(history) and datetime.fromisoformat(history["at"]) <= created,
        "FUTURES_SOURCE_CHANGED_OR_LATE",
    )
    model.require(
        history["t"] == parent["t"]
        and history["u"] == parent["u"]
        and history["parent_plan_hash"] == extra["plan_hash"]
        and history["parent_source_hash"] == extra["basis_source_hash"],
...
def run():
    # 先完成已有CORE3及R95分支，再在同一实际输入上增加独立日内走势模型。
    prior_forward.run()
    ...
    for key, p in parents.items():
        ...
        value = {k: p[k] for k in ("code", "family", "group", "t", "u")} | {
            ...
            "r95_forecast_hash": base.digest(extra),
            "intraday_source_hash": base.digest(history[p["u"]]),
        }
...
def report(loaded=None):
    ...
    for path in sorted((model.root() / "forward").glob("*/*.json")):
        ...
        prior94 = base.read(prior_forward.prior_model.root() / "forward" / p["u"] / path.name)
        prior92 = base.read(prior_forward.prior_forward.prior_model.root() / "forward" / p["u"] / path.name)
        for name, answer in (
            p["answers"]
            | prior92["answers"]
            | prior94["answers"]
            | extras[p["code"], p["u"]]["answers"]
            | value["answers"]
        ).items():

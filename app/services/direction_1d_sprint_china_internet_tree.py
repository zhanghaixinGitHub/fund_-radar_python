"""第六十二轮：检验中国互联网板块隔夜价格能否改善一日方向模型。

固定四输入树与原三输入树作同成熟题对照，仅增加KWEB美国时段涨跌；该代理不是基金持仓映射。
...
from app.services import direction_1d_sprint_china_internet as previous_round
from app.services import direction_1d_sprint_china_internet_data as sectors
...
from app.services import direction_1d_sprint_etf_fund_state as tree_control
...
CANDIDATES = ("DIRECT_ETF_KWEB4_ET504",)
...
def fingerprint():
    ...
    for name in (
        "app/services/direction_1d_sprint_china_internet_tree.py",
        "scripts/direction_1d_sprint_china_internet_tree.py",
        "tests/test_direction_1d_sprint_china_internet_tree.py",
        "app/services/direction_1d_sprint_china_internet_data.py",
        "tests/test_direction_1d_sprint_china_internet_data.py",
        "app/integrations/sina_sprint_china_internet_etf.py",
        "app/integrations/sina_sprint_china_internet_decode.cjs",
        "tests/test_sina_sprint_china_internet_etf.py",
    ):
...
def selected_features(z, name):
    if (
        ...
        raise ValueError("CHINA_INTERNET_INPUT_INVALID")
...
def dataset():
    ...
    if sorted({r["code"] for r in rows}) != list(CODE_ORDER):
        raise ValueError("CHINA_INTERNET_SCOPE_CHANGED")
    points = sectors.history()
    context = {key: sectors.features(*key, points) for key in {(r["t"], r["u"]) for r in rows}}
    if any(majority.available(r["z"]) and context[r["t"], r["u"]][1] != 1 for r in rows):
        raise ValueError("CHINA_INTERNET_COMMON_ROWS_CHANGED")
...
def plan():
    ...
    if path.exists():
        ...
        if value["fingerprint"] != fingerprint() or value["calendar_hash"] != base.calendar()[1]:
            raise ValueError("ROUND_62_CODE_OR_CALENDAR_CHANGED")
        return value
    active()
    ...
    if proposal["candidates"] != list(CANDIDATES) or proposal["budget"] != {
        ...
        raise ValueError("CHINA_INTERNET_PROPOSAL_CHANGED")
    value = {
        ...
        "features": "R50three +KWEB_USsessionpct;four continuousinputs, nofundinteractions",
        "normalization": "Rawfourinputs, mean0scale1;exactR55treeinputconvention",
        ...
        | {
            "round-62/proposal-before-implementation.json": base.digest(proposal),
            **{
                f"china-internet-etf-source-v1/{name}": base.digest(
                    base.read(base.ROOT / "china-internet-etf-source-v1" / name)
                )
                for name in ("qualification-plan.json", "qualification-result.json", "history.json")
            },
        },
        ...
        "source_timing": "LatestclosedUSsessionafterT15 andbeforeU0830;actualfutureHTTP+readbackreceipts",
        "live_budget": "ReuseR61KWEBshared3GETbudget,targetincremental0;oldquotecapturesreused",
        "limits": "Internetproxy notholdings;historicalfirstpublicationunverified;source samplecheckedagainstissuer",
    }
...
def fit(rows, name, cutoff):
    ...
    if name not in CANDIDATES:
        raise ValueError("CHINA_INTERNET_RECIPE_INVALID")
    ...
    if len({r["u"] for r in chosen}) < 120:
        raise ValueError("CHINA_INTERNET_TRAINING_SHORT")
    ...
    if len(Counter(y)) != 2 or min(Counter(y).values()) < 20:
        raise ValueError("CHINA_INTERNET_CLASSES_INSUFFICIENT")
    weights = regression.weights(chosen)
    training_mean = np.average(core, axis=0, weights=weights).tolist()
    scale = [1.0] * 4
    x = design([r["z"] for r in chosen], scale, name)
    active()
    with warnings.catch_warnings(), threadpool_limits(limits=2):
        warnings.simplefilter("error", ConvergenceWarning)
        # 与原三输入树使用同一固定结构，四输入均不做缩放，避免混入其他训练规则变化。
...
def batch_answers(values, name, trained):
    ...
    if indices:
        ...
        if (
            ...
            raise ValueError("CHINA_INTERNET_MODEL_SCHEMA_CHANGED")
        ...
        if not np.isfinite(scores).all() or min(scores) < 0 or max(scores) > 1:
            raise ValueError("CHINA_INTERNET_SCORE_INVALID")
...
def fit_checkpoint(rows, name, cutoff, label):
    ...
    if (
        ...
        raise ValueError("CHINA_INTERNET_PRIOR_CHECKPOINT_CHANGED")
    ...
    if base.digest([r | {"z": r["z"][:20]} for r in shared]) != prior_head["fit_hash"]:
        raise ValueError("CHINA_INTERNET_LR3_CONTROL_ROWS_CHANGED")
    # 额外校验第55轮三输入树的同一训练题，作为已存在的非线性对照，不再重训。
    tree_path = tree_control.root() / "checkpoints" / f"{label.replace(name, tree_control.CANDIDATES[0])}.joblib"
    tree_receipt = base.read(tree_path.with_suffix(".json"))
    if tree_receipt["sha256"] != hashlib.sha256(tree_path.read_bytes()).hexdigest():
        raise ValueError("INTERNET_TREE_CONTROL_CHANGED")
    if joblib.load(tree_path)["us_etf"]["fit_hash"] != prior_head["fit_hash"]:
        raise ValueError("INTERNET_TREE_CONTROL_ROWS_CHANGED")
...
def train():
    ...
    for name, expected in p["input_hashes"].items():
        if base.digest(base.read(base.ROOT / name)) != expected:
            raise ValueError("ROUND_62_INPUT_CHANGED")
    rows, proofs = dataset()
    ...
    with threadpool_limits(limits=2):
        for q in range(1, 5):
            ...
            for group in groups:
                ...
                parent_candidate = tree_control.CANDIDATES[0]
                output[parent_candidate].extend(
                    base.read(tree_control.root() / f"folds/{q}-{group}-{parent_candidate}.json")
                )
        ...
        if len(expected) != 5670 or any(
            ...
            raise ValueError("ROUND_62_COMMON_EXAM_CHANGED")
...
def models():
    ...
    if (
        ...
        raise ValueError("ROUND_62_MODEL_OR_CODE_CHANGED")
...
def report():
    ...
    for path in (root() / "forward").glob("*/*.json"):
        ...
        if (
            ...
            raise ValueError("ROUND_62_PARENT_OR_MODEL_CHANGED")
        ...
        if value["sector_input_hash"] != base.digest(sector_input):
            raise ValueError("CHINA_INTERNET_INPUT_CHANGED")
        ...
        if not np.allclose(
            ...
            raise ValueError("ROUND_62_VECTOR_CHANGED")
        ...
        if not runtime.answers_match(value["answers"], expected_answers):
            raise ValueError("ROUND_62_SAVED_ANSWER_CHANGED")
        good += 1

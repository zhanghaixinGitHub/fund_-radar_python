"""第三十三轮：基准日国内市场活跃程度对一日纠错模型的增量影响。

原HK12增加沪深300、中证500的20日波动及成交额相对前20日均值的偏离。
完全复用既有基准日行情，原始下一日标签、共同样本和纠错门槛保持不变。
"""
...
from app.services import direction_1d_sprint_regime as previous_round
...
CANDIDATES = ("ACTIVITY_HK16_ERR504",)
LEARNED = CANDIDATES
...
def fingerprint():
    ...
    for name in (
        "app/services/direction_1d_sprint_activity.py",
        "scripts/direction_1d_sprint_activity.py",
        "tests/test_direction_1d_sprint_activity.py",
    ):
        value["code"][name] = hashlib.sha256((base.PROJECT / name).read_bytes()).hexdigest()
    return value


def activity_features(x):
    """读取已冻结父输入中的国内指数特征，波动乘100为百分数，成交额使用无量纲偏离。

    指数波动来自基准日及以前21个收盘；成交额分母仅取此前20日，不使用目标日行情。
    """
    if len(x) != 32 or not np.isfinite(x).all():
        raise ValueError("ACTIVITY_PARENT_INPUT_INVALID")
    values = [x[21] * 100, x[26] * 100, x[22], x[27]]
    if min(values[:2]) < 0 or min(values[2:]) <= -1:
        raise ValueError("ACTIVITY_INPUT_UNIT_INVALID")
    return values


def vector(x, t, u, markets):
    return hk.vector(x, t, u, markets) + activity_features(x)


def live_vector(source, original, markets):
    """原HK12与净值时点保留，增加四项既有国内行情特征，复用已核验的港股响应，不新增供应商请求。"""
...
def dataset():
    ...
    return [r | {"z": r["z"] + activity_features(r["x"])} for r in rows], proof | {
        "activity_source": "existing parentx32 indices21,26,22,27; T and earlier only",
    }
...
def plan():
    ...
    if path.exists():
        ...
        if value["fingerprint"] != fingerprint() or value["calendar_hash"] != base.calendar()[1]:
            raise ValueError("ROUND_33_CODE_OR_CALENDAR_CHANGED")
        return value
    active()
    if base.read(previous_round.root() / "result.json")["fingerprint"] != previous_round.fingerprint():
        raise ValueError("ACTIVITY_PREVIOUS_FINGERPRINT_CHANGED")
    parent, _ = hk.models()
    value = {
        ...
        "hypothesis": (
            "Domestic turnover activity and volatility may identify SPX baseline errors beyond existing HK12"
        ),
        "motivation": "Earlier direct models used another target; test four activity inputs in fixed HK12 correction",
        ...
        "input_features": "HK12 +x21*100,x26*100,x22,x27 from parentx32; no new source requests or clipping",
        "training_dates": 504,
...
def fit(rows, name, cutoff):
    """严格沿用第17轮树配方；只追加四项国内市场活跃信息，记录原训练样本摘要。"""
    if name not in CANDIDATES:
        raise ValueError("ACTIVITY_MODEL_RECIPE_INVALID")
    ...
    if len(Counter(y)) != 2 or min(Counter(y).values()) < 20:
        raise ValueError("ACTIVITY_MODEL_ERROR_LABELS_INSUFFICIENT")
...
def fit_checkpoint(rows, name, cutoff, label):
    ...
    if receipt.exists():
        ...
        if (
            ...
            raise ValueError("ACTIVITY_MODEL_CHECKPOINT_CHANGED")
        return joblib.load(path)
    if attempt.exists():
        raise ValueError("ACTIVITY_MODEL_PREVIOUS_FIT_INTERRUPTED")
    ...
    if base.digest([r | {"z": r["z"][:12]} for r in short]) != expected:
        raise ValueError("ACTIVITY_CONTROL_TRAINING_ROWS_CHANGED")
...
def train():
    ...
    for name, expected in p["input_hashes"].items():
        if base.digest(base.read(base.ROOT / name)) != expected:
            raise ValueError("ROUND_33_INPUT_CHANGED")
    rows, proofs = dataset()
    ...
    with threadpool_limits(limits=2):
        ...
        if len(expected) != 5670 or any(
            ...
            raise ValueError("ROUND_33_COMMON_EXAM_CHANGED")
...
def models():
    ...
    if (
        ...
        raise ValueError("ROUND_33_MODEL_OR_CODE_CHANGED")

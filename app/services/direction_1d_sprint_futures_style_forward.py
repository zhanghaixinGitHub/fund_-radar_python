"""第103轮复用 CORE3 输入，只复用R102已实际取得的T日全CFFEX行情，不调用已退役的旧实验链。"""
...
from app.services import direction_1d_sprint_futures_curve_forward as prior_forward
from app.services import direction_1d_sprint_market_futures_curve as prior_model
from app.services import direction_1d_sprint_market_futures_style as model
...
def validate(value, receipt, parent, source, manifest, bundle, extra, history):
    ...
    model.require(value["r102_forecast_hash"] == base.digest(extra), "R102_PARENT_CHANGED")
    ...
    model.require(
        ...
        "R102_PARENT_READBACK_LATE_OR_CHANGED",
    )
    model.require(
        value["style_source_hash"] == base.digest(history) and datetime.fromisoformat(history["at"]) <= created,
        "CURVE_SOURCE_CHANGED_OR_LATE",
    )
    curve_source = model.data.current.load_live(
        parent["t"], parent["u"], base.digest(base.read(prior_model.root() / "plan.json"))
    )
    model.require(
        history["t"] == parent["t"]
    ...
    model.require(
        model.data.current.in_window(parent["t"], parent["u"], datetime.fromisoformat(history["at"])),
...
def run():
    # 先完成已有CORE3及R102分支，在已实际取得的同日行情上增加IH/IC相对走势模型。
    prior_forward.run()
    ...
    for key, p in parents.items():
        ...
        value = {k: p[k] for k in ("code", "family", "group", "t", "u")} | {
            ...
            "r102_forecast_hash": base.digest(extra),
            "style_source_hash": base.digest(history[p["u"]]),
        }
...
def combined_answers(parent, extra, value):
    """明确合并全部21个同题分支，避免新增中间模型后递归层级漂移丢掉旧对照。"""
    combined = dict(parent["answers"])
    for number in (92, 94, 95, 96, 97, 98, 99, 100, 101):
    ...
    model.require(len(combined) == 21, "FORWARD_BRANCH_COUNT_CHANGED")
    return combined

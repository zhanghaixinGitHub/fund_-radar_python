"""第98轮复用 CORE3 输入，并复用已实际取得T日原始期货行情，不调用已退役的旧实验链。"""
...
from app.services import direction_1d_sprint_market_futures_activity as model
from app.services import direction_1d_sprint_market_option_position as prior_model
from app.services import direction_1d_sprint_option_position_forward as prior_forward
...
def validate(value, receipt, parent, source, manifest, bundle, extra, history):
    ...
    model.require(value["r97_forecast_hash"] == base.digest(extra), "R97_PARENT_CHANGED")
    ...
    model.require(
        ...
        "R97_PARENT_READBACK_LATE_OR_CHANGED",
    )
    model.require(
        value["activity_source_hash"] == base.digest(history) and datetime.fromisoformat(history["at"]) <= created,
        "FUTURES_SOURCE_CHANGED_OR_LATE",
    )
    source_parent = base.read(base.ROOT / "round-95" / "forward" / parent["u"] / f"{parent['code']}.json")
    model.require(
        history["t"] == parent["t"]
        and history["u"] == parent["u"]
        and history["parent_plan_hash"] == source_parent["plan_hash"]
        and history["parent_source_hash"] == source_parent["basis_source_hash"],
    ...
    for key, p in candidates.items():
        ...
        if p["u"] not in history:
            history[p["u"]] = model.data.live(
                p["t"], p["u"], base.digest(base.read(base.ROOT / "round-95" / "plan.json"))
            )
...
def run():
    # 先完成已有CORE3及R97分支，再在同一实际输入上增加独立日内活动交互模型。
    prior_forward.run()
    ...
    for key, p in parents.items():
        ...
        value = {k: p[k] for k in ("code", "family", "group", "t", "u")} | {
            ...
            "r97_forecast_hash": base.digest(extra),
            "activity_source_hash": base.digest(history[p["u"]]),
        }
...
def combined_answers(parent, extra, value):
    """明确合并全部16个同题分支，避免新增中间模型后递归层级漂移丢掉旧对照。"""
    combined = dict(parent["answers"])
    for number in (92, 94, 95, 96):
    ...
    model.require(len(combined) == 16, "FORWARD_BRANCH_COUNT_CHANGED")
    return combined

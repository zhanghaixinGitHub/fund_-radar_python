"""第120轮前向运行独立绑定：不改已冻结训练代码，待R119完成后一次性冻结接入关系。"""
...
from app.services import direction_1d_sprint_market_stock_exchange_return as parent
from app.services import direction_1d_sprint_stock_exchange_return_runtime as parent_runtime
...
def plan():
    """R120训练与R119运行链均完成后才绑定；父运行层未完成时拒绝占位接入。"""
    intent = verify_intent()
    ...
    context = {
        ...
        "code_hashes": code,
        "artifact_hashes": {
            "round-120/plan.json": trained["plan_hash"],
            "round-120/result.json": base.digest(trained),
            "round-119/plan.json": prior["plan_hash"],
            "round-119/result.json": base.digest(prior),
            "round-119/runtime-plan.json": base.digest(parent_run),
        },
    }

"""第102轮复用 CORE3 输入，并独立取得T日具体合约行情，不调用已退役的旧实验链。"""
...
from app.services import direction_1d_sprint_futures_equal_forward as prior_forward
from app.services import direction_1d_sprint_market_futures_curve as model
from app.services import direction_1d_sprint_market_futures_equal as prior_model
...
def validate(value, receipt, parent, source, manifest, bundle, extra, history):
    ...
    model.require(value["r101_forecast_hash"] == base.digest(extra), "R101_PARENT_CHANGED")
    ...
    model.require(
        ...
        "R101_PARENT_READBACK_LATE_OR_CHANGED",
    )
    model.require(
        value["curve_combined_source_hash"] == base.digest(history)
        and datetime.fromisoformat(history["at"]) <= created,
        "CURVE_SOURCE_CHANGED_OR_LATE",
    )
    curve_source = model.data.load_live(parent["t"], parent["u"], manifest["plan_hash"])
    model.require(
        history["t"] == parent["t"]
        and history["u"] == parent["u"]
        and history["curve_source_hash"] == base.digest(curve_source),
        "CURVE_SOURCE_SCOPE_CHANGED",
    )
    intraday_parent = base.read(base.ROOT / "round-96" / "forward" / parent["u"] / f"{parent['code']}.json")
    model.require(
        history["intraday_source_hash"] == intraday_parent["intraday_source_hash"], "INTRADAY_SOURCE_SCOPE_CHANGED"
    )
    model.require(
        model.data.in_window(parent["t"], parent["u"], datetime.fromisoformat(history["at"])),
        "CURVE_SOURCE_NOT_ACTUALLY_CAPTURED_IN_WINDOW",
    )
...
def context(allow_capture=False):
    ...
    for key, p in candidates.items():
        ...
        prior_forward.validate(
            extra, receipt, p, sources[p["u"]], prior_manifest, prior_bundle, prior_extras[key], prior_sources[key]
        )
        if p["u"] not in history:
            if allow_capture:
                ...
                if captured is None:
                    continue
            elif not (model.data.live_root() / p["u"] / "snapshot.json").exists():
                continue
            history[p["u"]] = model.data.live(
                p["t"], p["u"], manifest["plan_hash"], base.digest(base.read(base.ROOT / "round-95" / "plan.json"))
            )
...
def run():
    # 先完成已有CORE3及R101分支，再取得当日具体合约行情，增加期限价差模型。
    prior_forward.run()
    manifest, bundle, ctx, parents, extras, history = context(allow_capture=True)
    sources = {}
    for key, p in parents.items():
        ...
        value = {k: p[k] for k in ("code", "family", "group", "t", "u")} | {
            ...
            "r101_forecast_hash": base.digest(extra),
            "curve_combined_source_hash": base.digest(history[p["u"]]),
        }
...
def combined_answers(parent, extra, value):
    """明确合并全部20个同题分支，避免新增中间模型后递归层级漂移丢掉旧对照。"""
    combined = dict(parent["answers"])
    for number in (92, 94, 95, 96, 97, 98, 99, 100):
    ...
    model.require(len(combined) == 20, "FORWARD_BRANCH_COUNT_CHANGED")
    return combined
...
def report(loaded=None):
    ...
    value = {
        ...
        "new_branches": 1,
        "same_question_branches": 20,
        "joint_sources": {
            u: {
                "available": v["available"],
                "received_snapshot_at": v["at"],
                "curve_source_hash": v["curve_source_hash"],
                "intraday_source_hash": v["intraday_source_hash"],
        ...
        "new_source_requests": len(list(model.data.live_root().glob("*/request.json"))),
        "new_cost_cny": 0,
        "status": "MODEL_NOT_RELEASED",
    }

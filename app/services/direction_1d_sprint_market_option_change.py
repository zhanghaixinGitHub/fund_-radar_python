"""第99轮：T日期权成交持仓比例的一日变化，保持原标签、训练样本和对照。"""
...
from app.services import direction_1d_sprint_market_futures_activity as previous
from app.services import direction_1d_sprint_market_only as baseline
from app.services import direction_1d_sprint_option_change_data as data
...
CANDIDATES = ("SIGN3_IO_CHANGE5_LR504",)
CONTROLS = previous.CONTROLS
BRANCHES = CANDIDATES + CONTROLS
FIRST_TARGET = "2026-09-17"
PROPOSAL_HASH = "c33f319872dc283a29dfcb8a89464472da94f377bfb52856abc3e6be57794b87"


def root():
    return base.ROOT / "round-99"


def require(condition, reason):
    if not condition:
        raise ValueError("IO_CHANGE_" + reason)


def fingerprint():
    code = dict(base.read(base.ROOT / "round-98" / "plan.json")["fingerprint"]["code"])
    for name, expected in code.items():
        require(core.sha(base.PROJECT / name) == expected, "OLD_CODE_CHANGED")
    for name in (
        "app/services/direction_1d_sprint_market_option_change.py",
        "app/services/direction_1d_sprint_option_change_forward.py",
        "scripts/direction_1d_sprint_market_option_change.py",
        "tests/test_direction_1d_sprint_market_option_change.py",
        "app/services/direction_1d_sprint_option_change_data.py",
        "tests/test_direction_1d_sprint_option_change_data.py",
    ):
...
def plan():
    ...
    require(proposal["r98_result_hash"] == base.digest(base.read(previous.root() / "result.json")), "SOURCE_CHANGED")
    require(
    ...
    require(
        ...
        and all(x["missing_changes"] == 0 for x in coverage["exam"].values())
        and len(coverage["training_windows"]) == 27
        and all(x["missing_changes"] == 0 for x in coverage["training_windows"]),
...
def reconstruct_dataset():
    ...
    for row in rows:
        key = row["t"], row["u"]
        if key not in cache:
            cache[key] = data.extend({}, *key, h["snapshot"]["rows"])["option_change"]
        extra.append(row | {"market": row["market"] | {"option_change": cache[key]}})
    require([data.original_row(r) for r in extra] == rows, "ORIGINAL_ROWS_CHANGED")
    proof = proof | {"option_change_history_hash": base.digest(h)}
    return extra, proof
...
def vector(value):
    ...
    feature = value["option_change"]
    require(feature["available"], "OPTION_CHANGE_MISSING")
    extra = [feature[k] for k in ("log_put_call_volume_change", "log_put_call_oi_change")]
    require(
        all(type(v) in (int, float) and np.isfinite(v) for v in extra) and abs(extra[0]) <= 20 and abs(extra[1]) <= 20,
        "OPTION_CHANGE_INVALID",
    )
    return [*x, *extra]
...
def fit(rows, name, cutoff):
    ...
    require(
        ...
        and all(r["mature"] < cutoff and r["u"] < cutoff and r["market"]["option_change"]["available"] for r in chosen),
    ...
    return {
        ...
        "max_feature_date": max(r["market"]["option_change"]["date"] for r in chosen),
...
def candidate_answers(values, name, head, fallback):
    ...
    indices = [i for i, v in enumerate(values) if v["available"] and v["option_change"]["available"]]
    if indices:
...
def preflight():
    from app.services import direction_1d_sprint_option_change_forward as forward

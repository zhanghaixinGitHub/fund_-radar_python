"""第120轮：T日具体IF主力合约公开会员多空排名，保持原标签、训练样本和对照。"""
...
from app.services import direction_1d_sprint_market_stock_exchange_return as previous
...
CANDIDATES = ("SIGN3_IF_MEMBER5_LR504",)
CONTROLS = previous.CONTROLS
BRANCHES = CANDIDATES + CONTROLS
FIRST_TARGET = "2026-09-17"
PROPOSAL_HASH = "784a0270c16d74b243ca2f8cf42885c9984caca69c6d3fdfc0538c1ffa5b4ace"


def root():
    return base.ROOT / "round-120"


def require(condition, reason):
    if not condition:
        raise ValueError("IF_MEMBER_" + reason)


def fingerprint():
    code = dict(base.read(base.ROOT / "round-119" / "plan.json")["fingerprint"]["code"])
    ...
    for name, expected in source_plan["code"].items():
        ...
        code[name] = expected
    for name in (
        "app/services/direction_1d_sprint_market_futures_member.py",
        "scripts/direction_1d_sprint_market_futures_member.py",
        "tests/test_direction_1d_sprint_market_futures_member.py",
    ):
...
def plan():
    ...
    require(base.digest(intent) == "45d53fe19c58bbdae740c048a806fa0bbdb9cf9fd312243caaeb7326d41421a3", "INTENT_CHANGED")
    ...
    require(proposal["r119_result_hash"] == base.digest(base.read(previous.root() / "result.json")), "SOURCE_CHANGED")
    require(
    ...
    require(base.digest(source) == proposal["source_qualification_hash"], "IF_MEMBER_SOURCE_CHANGED")
    coverage = base.read(root() / "feature-feasibility.json")
    require(
        coverage["source_history_hash"] == source["source_proof"]["source_history_hash"]
        and all(x["member_available"] == x["questions"] for x in coverage["exam"].values())
...
def reconstruct_dataset():
    """按共享市场日期缓存具体IF合约会员多空排名差和变化，避免为同日30基金重复解析整份交易日历。"""
    ...
    for row in rows:
        key = row["t"], row["u"]
        if key not in cache:
            cache[key] = data.extend({}, *key, h["snapshot"]["rows"])["futures_member"]
        extra.append(row | {"market": row["market"] | {"futures_member": cache[key]}})
    require([data.original_row(r) for r in extra] == rows, "ORIGINAL_ROWS_CHANGED")
    proof = proof | {"futures_member_history_hash": base.digest(h)}
    return extra, proof
...
def vector(value):
    ...
    feature = value["futures_member"]
    require(feature["available"], "IF_MEMBER_MISSING")
    extra = [feature[k] for k in data.KEYS]
    require(
        all(type(v) in (int, float) and np.isfinite(v) for v in extra) and abs(extra[0]) <= 1 and abs(extra[1]) <= 1,
        "IF_MEMBER_INVALID",
    )
    return [*x, *extra]
...
def fit(rows, name, cutoff):
    ...
    require(
        ...
        and all(
            r["mature"] < cutoff and r["u"] < cutoff and r["market"]["futures_member"]["available"] for r in chosen
        ),
    ...
    return {
        ...
        "max_feature_date": max(r["market"]["futures_member"]["date"] for r in chosen),
...
def candidate_answers(values, name, head, fallback):
    ...
    indices = [i for i, v in enumerate(values) if v["available"] and v["futures_member"]["available"]]
    if indices:

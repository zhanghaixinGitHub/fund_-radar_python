"""第四十三轮：改用预测早晨前最新美股现金日期的CNYA净值口径差。
...
from app.services import direction_1d_sprint_fxi_overnight as previous_round
...
def fingerprint():
    ...
    for name in (
        "app/services/direction_1d_sprint_cnya.py",
        "scripts/direction_1d_sprint_cnya.py",
        "tests/test_direction_1d_sprint_cnya.py",
        "app/services/direction_1d_sprint_cnya_data.py",
        "tests/test_direction_1d_sprint_cnya_data.py",
        "app/integrations/ishares_sprint_cnya.py",
        ".local-runs/direction-1d-sprint-20260914/cnya-data-feasibility-v1/probe.py",
    ):
...
def selected_features(z, name):
    if name not in CANDIDATES or len(z) != 15 or not np.isfinite(z).all():
        raise ValueError("CNYA_MODEL_INPUT_INVALID")
    return z
...
def dataset():
    ...
    for row in rows:
        key = (row["t"], row["u"])
        if key not in cache:
            cache[key] = cnya_data.features(*key, points)
        output.append(row | {"z": row["z"] + cache[key]})
    expected = base.read(base.ROOT / "cnya-data-feasibility-v1/result.json")["feature_hash"]
    if base.digest([{k: r[k] for k in ("code", "u", "z")} for r in output]) != expected:
        raise ValueError("CNYA_OVERNIGHT_PROTOTYPE_FEATURES_CHANGED")
    return output, proof | {
        "at": base.now().isoformat(),
        "cnya_rows": len(points),
        "cnya_history_hash": base.digest(base.read(base.ROOT / "cnya-data-feasibility-v1/history.json")),
        "cnya_input_time": "latest US cash close<=U08:30 and previous cash date",
    }
...
def plan():
    ...
    if path.exists():
        ...
        if value["fingerprint"] != fingerprint() or value["calendar_hash"] != base.calendar()[1]:
            raise ValueError("ROUND_43_CODE_OR_CALENDAR_CHANGED")
        return value
    active()
    ...
    value = {
        ...
        "hypothesis": (
            "CNYA tracks A-shares more directly; same latest-US-session gap features and fixed recipe as round42 FXI"
        ),
        ...
        "live_budget": {
            ...
            "independent_cnya_file_required": True,
            "shared_across_funds": True,
            "max_bytes": cnya_data.MAX_BYTES,
            "redirects": False,
            "automatic_retries": 0,
        },
        "extra_trees": {
        ...
        "new_historical_provider_calls": 1,
        "new_cost_cny": 0,
...
def fit(rows, name, cutoff):
    ...
    if len(Counter(y)) != 2 or min(Counter(y).values()) < 20:
        raise ValueError("CNYA_MODEL_ERROR_LABELS_INSUFFICIENT")
...
def train():
    ...
    for name, expected in p["input_hashes"].items():
        if base.digest(base.read(base.ROOT / name)) != expected:
            raise ValueError("ROUND_43_INPUT_CHANGED")
    rows, proofs = dataset()
    ...
    with threadpool_limits(limits=2):
        ...
        if len(expected) != 5670 or any(
            ...
            raise ValueError("ROUND_43_COMMON_EXAM_CHANGED")
...
def models():
    ...
    if (
        ...
        raise ValueError("ROUND_43_MODEL_OR_CODE_CHANGED")
...
def report():
    ...
    for path in (root() / "forward").glob("*/*.json"):
        ...
        if (
            ...
            raise ValueError("ROUND_43_PARENT_OR_MODEL_CHANGED")
        ...
        if not np.allclose(
            ...
            raise ValueError("ROUND_43_VECTOR_CHANGED")
        good += 1

"""第四十四轮：以SPX、CNYA口径差及有效标记直接预测下一日原始净值方向。

估计器只读取三项输入；完整15项仍保存供原始数据与时点核验，不另取供应商数据。
...
from app.services import direction_1d_sprint_cnya as previous_round
...
def fingerprint():
    ...
    for name in (
        "app/services/direction_1d_sprint_cnya_direct.py",
        "scripts/direction_1d_sprint_cnya_direct.py",
        "tests/test_direction_1d_sprint_cnya_direct.py",
        ".local-runs/direction-1d-sprint-20260914/round-44/feature-feasibility.py",
    ):
...
def plan():
    ...
    if path.exists():
        ...
        if value["fingerprint"] != fingerprint() or value["calendar_hash"] != base.calendar()[1]:
            raise ValueError("ROUND_44_CODE_OR_CALENDAR_CHANGED")
        return value
    active()
    ...
    value = {
        ...
        "hypothesis": (
            "Simple direct UP logistic model of SPX and CNYA gap may generalize better than baseline-error correction"
        ),
        ...
        "feature_order": (
            "Full15 evidence retained; estimator selects indices0,12,14: SPXpercent,CNYAgapPercent,pairAvailable"
        ),
        ...
        "normalization": "Train-only weighted centered-square scale; three selected inputs",
        ...
        | {
            ...
            for name in (
                ...
                "round-44/proposal-before-training.json",
                "round-44/input-feasibility.json",
            )
        },
...
def fit(rows, name, cutoff):
    """三输入直接学习原始UP，保留原成熟样本和自然权重；不收敛时明确失败，不自动调参。"""
...
def train():
    ...
    for name, expected in p["input_hashes"].items():
        if base.digest(base.read(base.ROOT / name)) != expected:
            raise ValueError("ROUND_44_INPUT_CHANGED")
    rows, proofs = dataset()
    ...
    with threadpool_limits(limits=2):
        ...
        if len(expected) != 5670 or any(
            ...
            raise ValueError("ROUND_44_COMMON_EXAM_CHANGED")
...
def models():
    ...
    if (
        ...
        raise ValueError("ROUND_44_MODEL_OR_CODE_CHANGED")
...
def report():
    ...
    for path in (root() / "forward").glob("*/*.json"):
        ...
        if (
            ...
            raise ValueError("ROUND_44_PARENT_OR_MODEL_CHANGED")
        ...
        if not np.allclose(
            ...
            raise ValueError("ROUND_44_VECTOR_CHANGED")
        good += 1

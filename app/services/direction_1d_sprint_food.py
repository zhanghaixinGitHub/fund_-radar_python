"""第四十六轮：核实食品饮料基金基准，单独检验滞后偏离能否改善下一日方向。

001632使用自己的504成熟日期训练纠错树；另外29只复用原HK模型，仍同题整体评估。
...
from app.services import direction_1d_sprint_cnya_neutral as previous_round
from app.services import direction_1d_sprint_dual_us as dual
from app.services import direction_1d_sprint_food_data as food_data
...
CANDIDATES = ("FOOD_BENCH_HK16_ERR504", "FOOD_ONLY_HK12_ERR504")
LEARNED = CANDIDATES
...
def fingerprint():
    ...
    for name in (
        "app/integrations/tushare_sprint_food.py",
        "app/services/direction_1d_sprint_food_data.py",
        "app/services/direction_1d_sprint_food.py",
        "scripts/direction_1d_sprint_food.py",
        "tests/test_direction_1d_sprint_food.py",
        "tests/test_direction_1d_sprint_food_data.py",
        ".local-runs/direction-1d-sprint-20260914/round-46/feature-feasibility.py",
        ".local-runs/direction-1d-sprint-20260914/food-benchmark-data-v1/acquire.py",
    ):
...
def vector(x, code, t, u, markets, points):
    return hk.vector(x, t, u, markets) + food_data.features(x, code, t, u, points)
...
def selected_features(z, name):
    if name not in CANDIDATES or len(z) != 17 or not np.isfinite(z).all() or z[-1] not in (0.0, 1.0):
        raise ValueError("FOOD_MODEL_INPUT_INVALID")
    if z[-1] == 0 and any(z[12:16]):
        raise ValueError("FOOD_UNMAPPED_FEATURE_NOT_ZERO")
    return list(z[:16] if name == CANDIDATES[0] else z[:12])


def dataset():
    """保留全部原题，特征仅对事先选定的001632适用；不按结果增删基金或日期。"""
    rows, proof = hk.dataset()
    points = food_data.history()
    expanded = [r | {"z": r["z"] + food_data.features(r["x"], r["code"], r["t"], r["u"], points)} for r in rows]
    feasible, proposal = (
    ...
    if (
        ...
        or feasible["history_hash"] != base.digest(base.read(base.ROOT / "food-benchmark-data-v1/history.json"))
        or proposal["script_sha256"] != hashlib.sha256((root() / "feature-feasibility.py").read_bytes()).hexdigest()
    ):
        raise ValueError("FOOD_PROTOTYPE_CHANGED")
    return expanded, proof | {
        "expanded_rows_hash": base.digest(expanded),
        "food_rows": sum(r["code"] == FUND for r in rows),
    }
...
def plan():
    ...
    if path.exists():
        ...
        if value["fingerprint"] != fingerprint() or value["calendar_hash"] != base.calendar()[1]:
            raise ValueError("ROUND_46_CODE_OR_CALENDAR_CHANGED")
        return value
    active()
    parent, _ = hk.models()
    ...
    value = {
        ...
        "hypothesis": "Own benchmark andlagged deviation may inform foodfund next-day error",
        "target": "Raw unit NAV T to adjacent CN U, UP vs NON_UP; keep flat separate",
        "feature_order": "HK12+own index1/5day minusCSI500+fund1/5day minus.95index+explicit food routing flag",
        "routing": "Only001632 new model; other29 reuse exact HK model and answers; no whole-scope exclusion",
        "ablation": "Samefood HK12 model distinguishes individualization from four new features",
        "recipe": "ExtraTrees256 depth6 leaf80 max_features1 bootstrapFalse seed17 threads2",
        "training": "Food only last504 mature dates, natural date/family error weights, train-only weighted scaling",
        ...
        | {
            ...
            for name in (
                "food-benchmark-data-v1/plan.json",
                "food-benchmark-data-v1/identity.json",
                "food-benchmark-data-v1/history.json",
                "food-benchmark-data-v1/result.json",
                "food-benchmark-data-v1/source-review.json",
                "round-46/proposal-before-training.json",
                "round-46/input-feasibility.json",
            )
        },
        ...
        "live_budget": "One index, at most one query per0700/0730/0800 slot; max3requests/target, no retry",
        "benchmark_boundary": "95% benchmark coefficient, notholding; gap omits depositinterest/dividends",
    }
...
def fit(rows, name, cutoff):
    ...
    if name not in CANDIDATES:
        raise ValueError("FOOD_RECIPE_INVALID")
    ...
    if len({r["u"] for r in chosen}) < 120 or any(r["z"][-1] != 1 for r in chosen):
        raise ValueError("FOOD_TRAINING_SCOPE_OR_DATES_INVALID")
    ...
    if len(Counter(y)) != 2 or min(Counter(y).values()) < 20:
        raise ValueError("FOOD_ERROR_CLASSES_INSUFFICIENT")
...
def batch_answers(values, name, trained):
    ...
    if indices:
        food = trained["food"]
        if food is None:
            raise ValueError("FOOD_MODEL_GROUP_ROUTE_INVALID")
        x = (np.asarray([selected[i] for i in indices]) - food["mean"]) / food["scale"]
        if not np.isfinite(x).all():
            raise ValueError("FOOD_NORMALIZED_INPUT_INVALID")
        scores = food["model"].predict_proba(x)[:, 1]
        if not np.isfinite(scores).all() or min(scores) < 0 or max(scores) > 1:
            raise ValueError("FOOD_SCORE_INVALID")
...
def fit_checkpoint(rows, name, cutoff, label):
    """旧HK模型只校验、加载；新食品基金模型每季度和当前版本各拟合一次。"""
    ...
    if receipt.exists():
        ...
        if (
            ...
            raise ValueError("FOOD_CHECKPOINT_CHANGED")
        return joblib.load(path)
    if attempt.exists():
        raise ValueError("FOOD_PREVIOUS_FIT_INTERRUPTED")
    group = rows[0]["group"]
    if label.startswith("current-"):
        ...
        if (
            ...
            raise ValueError("FOOD_HK_CHECKPOINT_CHANGED")
    ...
    if control["cutoff"] != cutoff or base.digest([r | {"z": r["z"][:12]} for r in chosen]) != control["fit_hash"]:
        raise ValueError("FOOD_CONTROL_TRAINING_ROWS_CHANGED")
    base.save(attempt, {"at": base.now().isoformat(), "name": name, "cutoff": cutoff, "new_fit": group == "CN_EQUITY"})
    food = fit(rows, name, cutoff) if group == "CN_EQUITY" else None
    trained = {
        "control": control,
        "food": food,
        "group": group,
        "control_fit_hash": control["fit_hash"],
        "new_fit_count": int(food is not None),
    }
...
def train():
    ...
    for name, expected in p["input_hashes"].items():
        if base.digest(base.read(base.ROOT / name)) != expected:
            raise ValueError("ROUND_46_INPUT_CHANGED")
    rows, proofs = dataset()
    ...
    with threadpool_limits(limits=2):
                        ...
                        if trained:
                            base.save(
                                root() / f"training/{q}-{group}-{name}.json",
                                {
                                    ...
                                    "food": {k: v for k, v in (trained["food"] or {}).items() if k != "model"},
                                },
                            )
        ...
        if len(expected) != 5670 or any(
            ...
            raise ValueError("ROUND_46_COMMON_EXAM_CHANGED")
...
def models():
    ...
    if (
        ...
        raise ValueError("ROUND_46_MODEL_OR_CODE_CHANGED")
...
def tick():
    ...
    food_input = food_data.capture(base.now())
    ...
    with threadpool_limits(limits=2):
        for path in paths:
            ...
            z = live_vector(source, original, hk_input["rows"], food_input["rows"])
            ...
            value = {
                ...
                "food_input_hash": base.digest(food_input),
...
def report():
    ...
    for path in (root() / "forward").glob("*/*.json"):
        ...
        if (
            ...
            raise ValueError("ROUND_46_PARENT_OR_MODEL_CHANGED")
        hk_input = hk_live.load(value["u"])
        food_input = food_data.load(value["u"])
        if value["food_input_hash"] != base.digest(food_input):
            raise ValueError("FOOD_MODEL_LIVE_INPUT_CHANGED")
        if value["hk_input_hash"] != base.digest(hk_input):
            raise ValueError("FOOD_MODEL_LIVE_INPUT_CHANGED")
        if not np.allclose(
            value["z"], live_vector(source, original, hk_input["rows"], food_input["rows"]), rtol=0, atol=1e-12
        ):
            raise ValueError("ROUND_46_VECTOR_CHANGED")
        good += 1

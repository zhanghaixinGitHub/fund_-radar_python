"""第119轮前向运行独立绑定：不改已冻结训练代码，待R118完成后一次性冻结接入关系。"""
...
from app.services import direction_1d_sprint_market_stock_exchange_spread as parent
from app.services import direction_1d_sprint_stock_exchange_spread_runtime as parent_runtime
...
def verify_intent():
    ...
    model.require(
        ...
        and intent["parent_round"] == 118
        and intent["same_question_branches"] == 38
...
def plan():
    """R119训练与R118运行链均完成后才绑定；父运行层未完成时拒绝占位接入。"""
    intent = verify_intent()
    ...
    context = {
        ...
        "code_hashes": code,
        "artifact_hashes": {
            "round-119/plan.json": trained["plan_hash"],
            "round-119/result.json": base.digest(trained),
            "round-118/plan.json": prior["plan_hash"],
            "round-118/result.json": base.digest(prior),
            "round-118/runtime-plan.json": base.digest(parent_run),
        },
    }

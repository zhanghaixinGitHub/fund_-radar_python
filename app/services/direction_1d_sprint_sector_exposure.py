"""第三十八轮：加入基金自身对四个市场的历史敏感度，预测下一交易日方向。
...
from app.services import direction_1d_sprint_sector as previous_round
from app.services import direction_1d_sprint_sector_data as sector_data
from app.services import direction_1d_sprint_sector_exposure_context as context_source
...
CANDIDATES = ("EXPOSURE_HK23_ERR504",)
LEARNED = CANDIDATES
...
def fingerprint():
    ...
    for name in (
        "app/services/direction_1d_sprint_sector_exposure.py",
        "scripts/direction_1d_sprint_sector_exposure.py",
        "tests/test_direction_1d_sprint_sector_exposure.py",
        "app/services/direction_1d_sprint_sector_exposure_context.py",
        "tests/test_direction_1d_sprint_sector_exposure_context.py",
        ".local-runs/direction-1d-sprint-20260914/round-38/context-feasibility.py",
    ):
...
def live_vector(source, original, markets, points, snapshot):
    ...
    if snapshot["cutoff"] > original["u"]:
        raise ValueError("EXPOSURE_CONTEXT_FUTURE_CUTOFF")
    if original["code"] not in snapshot["contexts"]:
        raise ValueError("EXPOSURE_CONTEXT_FUND_MISSING")
...
def freeze_context(p):
    ...
    if p["current_fit_cutoff"] != proposal["current_context_cutoff"]:
        raise ValueError("EXPOSURE_CONTEXT_CUTOFF_CHANGED")
    wanted = {
    ...
    if path.exists():
        ...
        if any(saved[key] != value for key, value in wanted.items()):
            raise ValueError("EXPOSURE_CURRENT_CONTEXT_CHANGED")
        return saved
...
def current_contexts():
    ...
    if (
        ...
        or snapshot["reference_hash"] != p["input_hashes"]["round-38/current-context-feasibility.json"]
...
def plan():
    ...
    if path.exists():
        ...
        if value["fingerprint"] != fingerprint() or value["calendar_hash"] != base.calendar()[1]:
            raise ValueError("ROUND_38_CODE_OR_CALENDAR_CHANGED")
        return value
    active()
    if base.read(previous_round.root() / "result.json")["fingerprint"] != previous_round.fingerprint():
        raise ValueError("EXPOSURE_PREVIOUS_FINGERPRINT_CHANGED")
    parent, _ = hk.models()
    value = {
        "at": base.now().isoformat(),
        "fingerprint": fingerprint(),
        "candidates": CANDIDATES,
        "hypothesis": "Individual residual sector sensitivity may contextualize common HK12 baseline errors",
        "motivation": "Fund-specific residual5sector exposure; unlike round36broadmarkets and round37commonsectors",
        ...
        "input_features": "HK12+5shrunkenbetas+5beta*sector-relative-returns+contextcount/126; reuse round37 capture",
        "training_dates": 504,
        ...
        | {
            "round-38/" + name: base.digest(base.read(root() / name))
...
def fit(rows, name, cutoff):
    ...
    if name not in CANDIDATES:
        raise ValueError("EXPOSURE_MODEL_RECIPE_INVALID")
    ...
    if len(Counter(y)) != 2 or min(Counter(y).values()) < 20:
        raise ValueError("EXPOSURE_MODEL_ERROR_LABELS_INSUFFICIENT")
...
def fit_checkpoint(rows, name, cutoff, label):
    ...
    if receipt.exists():
        ...
        if (
            ...
            raise ValueError("EXPOSURE_MODEL_CHECKPOINT_CHANGED")
        return joblib.load(path)
    if attempt.exists():
        raise ValueError("EXPOSURE_MODEL_PREVIOUS_FIT_INTERRUPTED")
    ...
    if base.digest([r | {"z": r["z"][:12]} for r in short]) != expected:
        raise ValueError("EXPOSURE_CONTROL_TRAINING_ROWS_CHANGED")
...
def train():
    ...
    for name, expected in p["input_hashes"].items():
        if base.digest(base.read(base.ROOT / name)) != expected:
            raise ValueError("ROUND_38_INPUT_CHANGED")
    rows, proofs = dataset()
    ...
    with threadpool_limits(limits=2):
        ...
        if len(expected) != 5670 or any(
            ...
            raise ValueError("ROUND_38_COMMON_EXAM_CHANGED")
...
def models():
    ...
    if (
        ...
        raise ValueError("ROUND_38_MODEL_OR_CODE_CHANGED")
...
def report():
    ...
    for path in (root() / "forward").glob("*/*.json"):
        ...
        if not np.allclose(
            ...
            raise ValueError("ROUND_38_VECTOR_CHANGED")
        good += 1

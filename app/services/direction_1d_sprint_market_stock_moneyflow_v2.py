"""第121轮：T日沪深订单分类买卖金额，保持原标签、训练样本和对照。"""
...
from app.services import direction_1d_sprint_market_stock_tail as previous
...
from app.services import direction_1d_sprint_stock_moneyflow_data_v2 as data

original, active, reference = previous.original, previous.active, previous.reference
CANDIDATES = ("SIGN3_MONEYFLOW5_LR504",)
CONTROLS = previous.CONTROLS
BRANCHES = CANDIDATES + CONTROLS
FIRST_TARGET = "2026-09-17"
INTENT_HASH = "d48b6ec4770ab4effcd56bffba89513db21b1be4f615c4d1dbc1ac85c6db500c"
PROPOSAL_HASH = "befcd20496621109ad53cabe9e5e008b33f2f2d784b4e1a241db57c588ee2285"


def root():
    return base.ROOT / "round-121"


def require(condition, reason):
    if not condition:
        raise ValueError("STOCK_MONEYFLOW_" + reason)


def fingerprint():
    code = dict(base.read(base.ROOT / "round-116" / "plan.json")["fingerprint"]["code"])
    ...
    for name, expected in source_plan["code"].items():
        ...
        code[name] = expected
    for name in (
        "app/services/direction_1d_sprint_market_stock_moneyflow_v2.py",
        "scripts/direction_1d_sprint_market_stock_moneyflow_v2.py",
        "tests/test_direction_1d_sprint_market_stock_moneyflow_v2.py",
        "app/services/direction_1d_sprint_stock_moneyflow_data_v2.py",
        "tests/test_direction_1d_sprint_stock_moneyflow_data_v2.py",
    ):
...
def plan():
    ...
    require(proposal["r116_result_hash"] == base.digest(base.read(previous.root() / "result.json")), "SOURCE_CHANGED")
    require(
...
def reconstruct_dataset():
    """按共享市场日期缓存新增沪深大单不平衡及供应商净额比例，避免为同日30基金重复解析整份交易日历。"""
    ...
    for row in rows:
        key = row["t"], row["u"]
        if key not in cache:
            cache[key] = data.extend({}, *key, h["snapshot"]["rows"])["stock_moneyflow"]
        extra.append(row | {"market": row["market"] | {"stock_moneyflow": cache[key]}})
    require([data.original_row(r) for r in extra] == rows, "ORIGINAL_ROWS_CHANGED")
    proof = proof | {"stock_moneyflow_history_hash": base.digest(h)}
    return extra, proof
...
def vector(value):
    ...
    feature = value["stock_moneyflow"]
    require(feature["available"], "STOCK_MONEYFLOW_MISSING")
    extra = [feature[k] for k in data.KEYS]
    require(
        all(type(v) in (int, float) and np.isfinite(v) for v in extra) and all(abs(v) <= 1 for v in extra),
        "STOCK_MONEYFLOW_INVALID",
    )
    return [*x, *extra]
...
def fit(rows, name, cutoff):
    ...
    require(
        ...
        and all(
            r["mature"] < cutoff and r["u"] < cutoff and r["market"]["stock_moneyflow"]["available"] for r in chosen
        ),
...
def candidate_answers(values, name, head, fallback):
    ...
    indices = [i for i, v in enumerate(values) if v["available"] and v["stock_moneyflow"]["available"]]
    if indices:

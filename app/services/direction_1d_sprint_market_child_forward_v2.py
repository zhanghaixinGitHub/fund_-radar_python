"""后续市场模型共享第70轮独立输入/实际净值观察；扩展向量独立回读和保存，零额外来源请求。"""
...
def validate(service, value, receipt, p, source, market, manifest, bundle):
    ...
    if (
        ...
        or not runtime.answers_match(value["answers"], service.live_answers(market, p["group"], bundle))
    ):
...
def tick(service):
    ...
    for p in parents.values():
        ...
        value = {k: p[k] for k in ("code", "family", "group", "u", "t")} | {
            ...
            "market": market,
            "answers": service.live_answers(market, p["group"], bundle),

"""第106轮复用 CORE3 输入，复用KURE实收并绑定逐基金冻结反应，不调用已退役的旧实验链。"""
...
from app.services import direction_1d_sprint_health_response_forward as prior_forward
from app.services import direction_1d_sprint_market_health_futures as model
from app.services import direction_1d_sprint_market_health_response as prior_model
...
def validate(value, receipt, parent, source, manifest, bundle, extra, history):
    ...
    model.require(value["r105_forecast_hash"] == base.digest(extra), "R105_PARENT_CHANGED")
    ...
    model.require(
        ...
        "R105_PARENT_READBACK_LATE_OR_CHANGED",
    )
    model.require(
        value["joint_source_hash"] == base.digest(history) and datetime.fromisoformat(history["at"]) <= created,
        "HEALTH_SOURCE_CHANGED_OR_LATE",
    )
    model.require(
        history == model.data.live(parent["t"], parent["u"], extra["plan_hash"]), "ACTUAL_RESPONSE_SOURCE_CHANGED"
    )
    model.require(history["response_source_hash"] == extra["response_source_hash"], "R105_SOURCE_CHANGED")
    intraday = base.read(base.ROOT / "round-96/forward" / parent["u"] / f"{parent['code']}.json")
    model.require(history["intraday_source_hash"] == intraday["intraday_source_hash"], "ACTUAL_IF_SOURCE_CHANGED")
    market = model.data.extend(source["market"], parent["t"], parent["u"], history, parent["code"])
...
def context():
    ...
    for key, p in candidates.items():
        ...
        if p["u"] not in history:
            history[p["u"]] = model.data.live(p["t"], p["u"], prior_manifest["plan_hash"])
        parents[key], extras[key] = p, extra
    return manifest, bundle, ctx, parents, extras, history


def run():
    # R105负责已有分支；本分支联合复用既有医疗及期货实收，不请求新行情。
    prior_forward.run()
    ...
    for key, p in parents.items():
        ...
        market = model.data.extend(source["market"], p["t"], p["u"], history[p["u"]], p["code"])
        ...
        value = {k: p[k] for k in ("code", "family", "group", "t", "u")} | {
            ...
            "r105_forecast_hash": base.digest(extra),
            "joint_source_hash": base.digest(history[p["u"]]),
        }
...
def combined_answers(parent, extra, value):
    """明确合并全部24个同题分支，避免新增中间模型后递归层级漂移丢掉旧对照。"""
    combined = dict(parent["answers"])
    for number in (92, 94, 95, 96, 97, 98, 99, 100, 101, 102, 103, 104):
    ...
    model.require(len(combined) == 24, "FORWARD_BRANCH_COUNT_CHANGED")
    return combined
...
def report(loaded=None):
    ...
    value = {
        ...
        "new_branches": 1,
        "same_question_branches": 24,
        "joint_sources": {
            u: {
                "available": v["available"],
                "received_snapshot_at": v["at"],
                "response_source_hash": v["response_source_hash"],
                "intraday_source_hash": v["intraday_source_hash"],
                "context_manifest_hash": v["context_manifest_hash"],
...
def preflight():
    ...
    history = model.data.historical_inputs()
    checks = 0
    for f in checked:
        actual = model.answers(
            model.data.extend(source["market"], f["t"], f["u"], history, f["code"]),
            f["group"],
            bundle,
        )

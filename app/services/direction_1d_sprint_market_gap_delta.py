"""第73轮：加入开盘缺口与估值差变化，预测仍为相邻交易日原始净值方向。

一个五输入候选与五个冻结/固定对照使用同题、同一SPX回退；不读取2026考试成绩选模。
"""
...
from app.services import direction_1d_sprint_market_margin as previous
...
CANDIDATES = ("EXPANDED_MARKET_GAP_DELTA_LR5_504",)
...
PROPOSAL_HASH = "b7551bb52771799bc6f768e401b4b790628397ca1a696fa7a7af4b37d5338e68"
active = regression.active


def root():
    return base.ROOT / "round-73"


def fingerprint():
    value = previous.fingerprint()
    for name in (
        "app/services/direction_1d_sprint_market_gap_delta.py",
        "app/services/direction_1d_sprint_market_gap_delta_data.py",
        "app/services/direction_1d_sprint_market_child_forward_v2.py",
        "scripts/direction_1d_sprint_market_gap_delta.py",
        "tests/test_direction_1d_sprint_market_gap_delta.py",
        "tests/test_direction_1d_sprint_market_gap_delta_data.py",
        "tests/test_direction_1d_sprint_market_child_forward_v2.py",
    ):
...
def plan():
    ...
    if path.exists():
        ...
        return p
    active()
    if base.digest(previous.models()[0]) != proposal["r72_result_hash"]:
    ...
    paths = [
        "history.json",
        "round-73/proposal-before-implementation.json",
    ...
    p = {
        ...
        "old_questions": 5670,
        "training": (
            "504mature fullmarket dates pergroup; natural date/family weights; LR5 C.1/nointercept/seed17/max1500"
        ),
        "transform": "five raw inputs divided by training scale; threshold.5; quarterlyrefits; controls use first3only",
...
def batch_answers(values, name, head=None):
    ...
    if name == "MARKET_MAJORITY3":
        ...
        width = 5 if name in CANDIDATES else 3
        if head is None or head["signed"] != signed or head["mean"] != [0.0] * width:
            raise ValueError("MARKET_GAP_DELTA_HEAD_SCHEMA_INVALID")
        scores = head["model"].predict_proba(transformed(x[indices, :width], head["scale"], signed))[:, 1]
        ...
        for i, score in zip(indices, scores, strict=True):
            out[i] = {
                ...
                "route": "MARKET_LR5" if width == 5 else "MARKET_LR3",
            }
    return out
...
def tick():
    ...
    from app.services import direction_1d_sprint_market_gap_delta as service

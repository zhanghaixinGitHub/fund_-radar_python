"""第119轮：T日只保留沪市减深市的成交额加权收益差，保持原标签、训练样本和对照。"""
...
from app.services import direction_1d_sprint_market_stock_exchange_spread as previous
...
from app.services import direction_1d_sprint_stock_exchange_spread_data as data

original, active, reference = previous.original, previous.active, previous.reference
CANDIDATES = ("SIGN3_EXCHANGE_RETURN4_LR504",)
CONTROLS = previous.CONTROLS
BRANCHES = CANDIDATES + CONTROLS
FIRST_TARGET = "2026-09-17"
PROPOSAL_HASH = "897ee5579a2bc4bb860396cb88b7884c776c3b36664caf477e658ad5d680d95c"


def root():
    return base.ROOT / "round-119"


def require(condition, reason):
    if not condition:
        raise ValueError("STOCK_EXCHANGE_SPREAD_" + reason)


def fingerprint():
    code = dict(base.read(base.ROOT / "round-118" / "plan.json")["fingerprint"]["code"])
    ...
    for name, expected in source_plan["code"].items():
        ...
        code[name] = expected
    for name in (
        "app/services/direction_1d_sprint_market_stock_exchange_return.py",
        "scripts/direction_1d_sprint_market_stock_exchange_return.py",
        "tests/test_direction_1d_sprint_market_stock_exchange_return.py",
    ):
...
def plan():
    ...
    require(base.digest(intent) == "3684a21a0026df096c0c13b40431d267c593f745c50ddca4b167b8db907900a8", "INTENT_CHANGED")
    proposal = base.read(root() / "proposal-before-implementation.json")
    require(proposal["research_intent_hash"] == base.digest(intent), "PROPOSAL_INTENT_CHANGED")
    ...
    require(proposal["r118_result_hash"] == base.digest(base.read(previous.root() / "result.json")), "SOURCE_CHANGED")
    require(
    ...
    require(base.digest(source) == proposal["source_qualification_hash"], "EXCHANGE_SPREAD_SOURCE_CHANGED")
    coverage = base.read(root() / "feature-feasibility.json")
    require(
        coverage["source_history_hash"] == source["source_proof"]["source_history_hash"]
        and all(x["exchange_spread_available"] == x["questions"] for x in coverage["exam"].values())
...
def reconstruct_dataset():
    """按共享市场日期缓存复用沪深两市场完整来源，向量仅取收益差，避免为同日30基金重复解析整份交易日历。"""
    ...
    for row in rows:
        key = row["t"], row["u"]
        if key not in cache:
            cache[key] = data.extend({}, *key, h["snapshot"]["rows"])["stock_exchange_spread"]
        extra.append(row | {"market": row["market"] | {"stock_exchange_spread": cache[key]}})
    require([data.original_row(r) for r in extra] == rows, "ORIGINAL_ROWS_CHANGED")
    proof = proof | {"stock_exchange_spread_history_hash": base.digest(h)}
    return extra, proof
...
def vector(value):
    ...
    feature = value["stock_exchange_spread"]
    require(feature["available"], "STOCK_EXCHANGE_SPREAD_MISSING")
    extra = [feature["return_spread"]]
    require(
        all(type(v) in (int, float) and np.isfinite(v) for v in extra) and abs(extra[0]) <= 40,
        "STOCK_EXCHANGE_SPREAD_INVALID",
    )
    return [*x, *extra]
...
def fit(rows, name, cutoff):
    ...
    require(
        ...
        and all(
            r["mature"] < cutoff and r["u"] < cutoff and r["market"]["stock_exchange_spread"]["available"]
            for r in chosen
        ),
    ...
    return {
        ...
        "max_feature_date": max(r["market"]["stock_exchange_spread"]["date"] for r in chosen),
...
def candidate_answers(values, name, head, fallback):
    ...
    indices = [i for i, v in enumerate(values) if v["available"] and v["stock_exchange_spread"]["available"]]
    if indices:

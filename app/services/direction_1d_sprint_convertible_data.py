"""可转债基准行情的历史核验和真实提前采集，所有基金共用原始响应。

每目标日三时槽，每槽每指数至多一次，总计九次，不自动重试。
...
from app.integrations.tushare_sprint_convertible import CODES, FIELDS, MAX_BYTES, URL, fetch_convertible
from app.services import direction_1d_sprint as base
from app.services import direction_1d_sprint_market as market

SOURCE = "TUSHARE_CB_BENCHMARK_EXISTING_PERMISSION_RESEARCH"
SLOTS = ("0700", "0730", "0800")


def root():
    return base.ROOT / "round-47" / "benchmark-inputs"
...
def json_rows(raw, code):
    ...
    for row in data["items"]:
        ...
        if day in rows or any(type(v) not in (float, int) or not math.isfinite(v) or v <= 0 for v in row[2:]):
            raise ValueError("CB_DUPLICATE_OR_PRICE_INVALID")
...
def parse(raw, code, start="2021-01-01", end="2026-12-31"):
    ...
    if any(not start <= day <= end for day, _, _ in rows):
        raise ValueError("CB_DATE_OUTSIDE_REQUEST")
...
def required(base_day, target):
    ...
    if base_day not in days:
        raise ValueError("CB_BASE_OUTSIDE_CALENDAR")
    index = days.index(base_day)
    if index < 5 or index + 1 >= len(days) or days[index + 1] != target:
        raise ValueError("CB_TARGET_NOT_ADJACENT")
    return days[index - 5 : index + 1]


def features(x, fund_code, base_day, target, points):
    """三项指数按每天80/10/10加权再连乘，得到1/5日复合参考；不混合不同指数点位。

    005284追加复合参考相对CSI500的1/5日表现，以及基金1/5日减复合参考，单位百分点。
    权重来自2020Q3公开基准，非实际持仓；原始净值与参考可能含不同分红/费用影响。
    其它基金继续原HK判断，不以0输入冒充具有此基准。
    """
    days = required(base_day, target)
    if len(x) != 32 or not np.isfinite(x).all():
        raise ValueError("CB_PARENT_FEATURE_INVALID")
    if fund_code != "005284":
        return [0.0] * 5
    daily = []
    for code in CODES:
        if code not in points or any(day not in points[code] for day in days):
            raise ValueError("CB_REQUIRED_DATE_MISSING")
        ...
        if any(
            ...
            raise ValueError("CB_FEATURE_PRICE_INVALID")
        if any(abs(a["pre_close"] - p["close"]) > 0.001 for p, a in zip(prices[:-1], prices[1:], strict=True)):
            raise ValueError("CB_PRECLOSE_DISCONTINUITY")
        daily.append([a["close"] / p["close"] - 1 for p, a in zip(prices[:-1], prices[1:], strict=True)])
    combined = np.asarray([0.8, 0.1, 0.1]) @ np.asarray(daily)
    r1, r5 = float(combined[-1]), float(np.prod(1 + combined) - 1)
    return [float(np.clip(v * 100, -20, 20)) for v in (r1 - x[23], r5 - x[24], x[7] - r1, x[0] - r5)] + [1.0]


def history():
    """原CSI目录两指数与SSE补查中证800独立溯源，校验23次原请求结果后合并。"""
    combined = {}
    for code in CODES:
        symbol = code.split(".")[0]
        supplement = symbol == "000906"
        folder = base.ROOT / ("convertible-csi800-supplement-v1" if supplement else "convertible-benchmark-data-v1")
        plan = base.read(folder / "plan.json")
        snapshot, result = base.read(folder / "history.json"), base.read(folder / "result.json")
        identities = base.read(folder / ("identity.json" if supplement else "identities.json"))
        identity = identities["row"] if supplement else identities["rows"][symbol]
        names = {"000832": "中证可转换债券指数", "000906": "中证800指数", "H11001": "中证全债指数"}
        if (
            hashlib.sha256((folder / "acquire.py").read_bytes()).hexdigest() != plan["script_sha256"]
            or identities["plan_hash"] != base.digest(plan)
            or identity["ts_code"] != code
            or identity["fullname"] != names[symbol]
            or snapshot["plan_hash"] != base.digest(plan)
            or result["plan_hash"] != base.digest(plan)
            or result["history_hash"] != base.digest(snapshot)
            or (supplement and result["status"] != "ACQUIRED_NOT_TRAINED")
            or (not supplement and result["series"][symbol]["status"] != "ACQUIRED")
        ):
            raise ValueError("CB_HISTORICAL_MANIFEST_CHANGED")
        pieces = [("probe" if supplement else "probe-" + symbol, "2025-01-02", "2025-01-10")]
        pieces += [
            (str(y) if supplement else f"{symbol}-{y}", f"{y}-01-01", f"{y}-12-31" if y < 2026 else "2026-09-14")
        ...
        for label, start, end in pieces:
            ...
            if (
                ...
                raise ValueError("CB_HISTORICAL_RAW_CHANGED")
            parsed = parse(raw, code, start, end)
            if label.startswith("probe"):
                probe = parsed
            else:
                if set(points) & set(parsed):
                    raise ValueError("CB_HISTORICAL_DUPLICATE")
                points.update(parsed)
        expected = snapshot["points"] if supplement else snapshot["points"][code]
        if points != expected or any(points.get(k) != v for k, v in probe.items()):
            raise ValueError("CB_HISTORICAL_PARSED_CHANGED")
        combined[code] = points
    return combined
...
def load_piece(target, base_day, code, slot):
    ...
    if slot not in SLOTS or code not in CODES:
        raise ValueError("CB_LIVE_SLOT_OR_CODE_INVALID")
    folder = root() / target
    ...
    if (
        ...
        raise ValueError("CB_LIVE_RESPONSE_CHANGED_OR_LATE")
    ...
    if meta["bytes"] != len(raw) or meta["body_sha256"] != hashlib.sha256(raw).hexdigest():
        raise ValueError("CB_LIVE_RAW_CHANGED")
...
def load(target):
    ...
    if value["target"] != target or value["source"] != SOURCE or at >= deadline or set(value["rows"]) != set(CODES):
        raise ValueError("CB_LIVE_INPUT_SCOPE_OR_TIME_INVALID")
    for code in CODES:
        ...
        if ref["hash"] != base.digest(meta) or datetime.fromisoformat(meta["received_at"]) > at:
            raise ValueError("CB_LIVE_INPUT_RECEIPT_CHANGED")
        if points != value["rows"][code]:
            raise ValueError("CB_LIVE_PARSED_INPUT_CHANGED")
    features([0.0] * 32, "005284", value["base"], target, value["rows"])
    return value
...
def capture(at):
    ...
    if at >= end or window["status"] != "OPEN" or not time(7) <= at.time() < time(8, 30):
        raise ValueError("CB_LIVE_WINDOW_CLOSED")
    ...
    for code in CODES:
        ...
        try:
            ...
            if (
                ...
                raise ValueError("CB_LIVE_REQUEST_TIME_INVALID")
            if not meta_path.exists():
                if attempt.exists() or len(list((folder / "requests").glob("*.json"))) >= 9:
                    raise ValueError("CB_LIVE_SLOT_OR_BUDGET_EXHAUSTED")
                request = {
                ...
                try:
                    raw = fetch_convertible(code, date.fromisoformat(needed[0]), date.fromisoformat(needed[-1]))
    ...
    if errors:
        ...
        raise ValueError("CB_LIVE_INPUT_INCOMPLETE")
    features([0.0] * 32, "005284", base_day, target, combined)
    if base.now() >= deadline:
        raise ValueError("CB_LIVE_DEADLINE_REACHED")
    value = {
    ...
    checked = load(target)
    if base.now() >= deadline:
        raise ValueError("CB_LIVE_READBACK_LATE")
    return checked

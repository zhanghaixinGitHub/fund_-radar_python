"""第98轮：T日IF方向与成交持仓活跃度交互，保持原标签、训练样本和对照。"""
...
from app.services import direction_1d_sprint_futures_activity_data as data
from app.services import direction_1d_sprint_market_only as baseline
from app.services import direction_1d_sprint_market_option_position as previous
...
CANDIDATES = ("SIGN3_IF_ACTIVITY7_LR504",)
CONTROLS = previous.CONTROLS
BRANCHES = CANDIDATES + CONTROLS
FIRST_TARGET = "2026-09-17"
PROPOSAL_HASH = "869a8b758652b964f66135b2a7fb6830e9de93065c1a9f1d76b0867e40639b5e"


def root():
    return base.ROOT / "round-98"


def require(condition, reason):
    if not condition:
        raise ValueError("IF_ACTIVITY_" + reason)


def fingerprint():
    code = dict(base.read(base.ROOT / "round-97" / "plan.json")["fingerprint"]["code"])
    for name, expected in code.items():
        require(core.sha(base.PROJECT / name) == expected, "OLD_CODE_CHANGED")
    for name in (
        "app/services/direction_1d_sprint_market_futures_activity.py",
        "app/services/direction_1d_sprint_futures_activity_forward.py",
        "scripts/direction_1d_sprint_market_futures_activity.py",
        "tests/test_direction_1d_sprint_market_futures_activity.py",
        "app/services/direction_1d_sprint_futures_activity_data.py",
        "tests/test_direction_1d_sprint_futures_activity_data.py",
    ):
...
def plan():
    ...
    require(proposal["r97_result_hash"] == base.digest(base.read(previous.root() / "result.json")), "SOURCE_CHANGED")
    require(
...
def reconstruct_dataset():
    ...
    for row in rows:
        key = row["t"], row["u"]
        if key not in cache:
            cache[key] = data.extend({}, *key, h["snapshot"]["rows"])["futures_activity"]
        extra.append(row | {"market": row["market"] | {"futures_activity": cache[key]}})
    require([data.original_row(r) for r in extra] == rows, "ORIGINAL_ROWS_CHANGED")
    proof = proof | {"futures_activity_history_hash": base.digest(h)}
    return extra, proof
...
def vector(value):
    ...
    feature = value["futures_activity"]
    require(feature["available"], "INTRADAY_MISSING")
    extra = [feature[k] for k in ("intraday_return_pct", "close_location", "return_activity", "location_activity")]
    require(
...
def fit(rows, name, cutoff):
    ...
    require(
        ...
        and all(
            r["mature"] < cutoff and r["u"] < cutoff and r["market"]["futures_activity"]["available"] for r in chosen
        ),
    ...
    return {
        ...
        "max_feature_date": max(r["market"]["futures_activity"]["date"] for r in chosen),
...
def candidate_answers(values, name, head, fallback):
    ...
    indices = [i for i, v in enumerate(values) if v["available"] and v["futures_activity"]["available"]]
    if indices:
...
def preflight():
    from app.services import direction_1d_sprint_futures_activity_forward as forward

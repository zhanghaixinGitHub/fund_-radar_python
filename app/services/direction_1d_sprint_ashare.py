"""第六十三轮：检验沪深300海外板块隔夜价格能否改善一日方向模型。

固定零截距LR4与原LR3作同成熟题对照，仅增加ASHR美国时段涨跌；该代理不是基金持仓映射。
...
from app.services import direction_1d_sprint_ashare_data as sectors
from app.services import direction_1d_sprint_china_internet_tree as previous_round
...
CANDIDATES = ("SPX_FXI_CNYA_ASHR_LR4_ZERO504", "ASHR_SESSION_SIGN_WITH_PARENT")
...
def fingerprint():
    ...
    for name in (
        "app/services/direction_1d_sprint_ashare.py",
        "scripts/direction_1d_sprint_ashare.py",
        "tests/test_direction_1d_sprint_ashare.py",
        "app/services/direction_1d_sprint_ashare_data.py",
        "tests/test_direction_1d_sprint_ashare_data.py",
        "app/integrations/sina_sprint_ashare_etf.py",
        "app/integrations/sina_sprint_ashare_decode.cjs",
        "tests/test_sina_sprint_ashare_etf.py",
    ):
...
def selected_features(z, name):
    if (
        ...
        raise ValueError("ASHARE_INPUT_INVALID")
...
def design(values, scale, name):
    ...
    if scales.shape != (4,) or not np.isfinite(scales).all() or min(scales) <= 0:
        raise ValueError("ASHARE_SCALE_INVALID")
...
def dataset():
    ...
    if sorted({r["code"] for r in rows}) != list(CODE_ORDER):
        raise ValueError("ASHARE_SCOPE_CHANGED")
    ...
    if any(majority.available(r["z"]) and context[r["t"], r["u"]][1] != 1 for r in rows):
        raise ValueError("ASHARE_COMMON_ROWS_CHANGED")
...
def plan():
    ...
    if path.exists():
        ...
        if value["fingerprint"] != fingerprint() or value["calendar_hash"] != base.calendar()[1]:
            raise ValueError("ROUND_63_CODE_OR_CALENDAR_CHANGED")
        return value
    active()
    ...
    if proposal["candidates"] != list(CANDIDATES) or proposal["budget"] != {
        ...
        raise ValueError("ASHARE_PROPOSAL_CHANGED")
    value = {
        ...
        "features": "R50three +ASHR_USsessionpct;four continuousinputs, nofundinteractions",
        ...
        | {
            "round-63/proposal-before-implementation.json": base.digest(proposal),
            **{
        ...
        "live_budget": "ASHRonly*3slots maximum3GET/target;existingETF/CNYAcaptures reused",
...
def fit(rows, name, cutoff):
    ...
    if name not in CANDIDATES:
        raise ValueError("ASHARE_RECIPE_INVALID")
    ...
    if len({r["u"] for r in chosen}) < 120:
        raise ValueError("ASHARE_TRAINING_SHORT")
    ...
    if len(Counter(y)) != 2 or min(Counter(y).values()) < 20:
        raise ValueError("ASHARE_CLASSES_INSUFFICIENT")
...
def batch_answers(values, name, trained):
    ...
    if indices:
        if name not in LEARNED:
            # 只判断完整美国时段开到收的方向，不搜索阈值，也不读取训练标签。
            ...
            if (
                ...
                raise ValueError("ASHARE_MODEL_SCHEMA_CHANGED")
        ...
        if not np.isfinite(scores).all() or min(scores) < 0 or max(scores) > 1:
            raise ValueError("ASHARE_SCORE_INVALID")
...
def fit_checkpoint(rows, name, cutoff, label):
    ...
    if (
        ...
        raise ValueError("ASHARE_PRIOR_CHECKPOINT_CHANGED")
    ...
    if base.digest([r | {"z": r["z"][:20]} for r in shared]) != prior_head["fit_hash"]:
        raise ValueError("ASHARE_LR3_CONTROL_ROWS_CHANGED")
...
def train():
    ...
    for name, expected in p["input_hashes"].items():
        if base.digest(base.read(base.ROOT / name)) != expected:
            raise ValueError("ROUND_63_INPUT_CHANGED")
    rows, proofs = dataset()
    ...
    with threadpool_limits(limits=2):
        ...
        if len(expected) != 5670 or any(
            ...
            raise ValueError("ROUND_63_COMMON_EXAM_CHANGED")
...
def models():
    ...
    if (
        ...
        raise ValueError("ROUND_63_MODEL_OR_CODE_CHANGED")
...
def report():
    ...
    for path in (root() / "forward").glob("*/*.json"):
        ...
        if (
            ...
            raise ValueError("ROUND_63_PARENT_OR_MODEL_CHANGED")
        ...
        if value["sector_input_hash"] != base.digest(sector_input):
            raise ValueError("ASHARE_INPUT_CHANGED")
        ...
        if not np.allclose(
            ...
            raise ValueError("ROUND_63_VECTOR_CHANGED")
        ...
        if not runtime.answers_match(value["answers"], expected_answers):
            raise ValueError("ROUND_63_SAVED_ANSWER_CHANGED")
        good += 1

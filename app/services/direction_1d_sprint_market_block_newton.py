"""第92轮：固定八个训练时间块的平滑最差损失，目标仍为下一交易日RAW净值方向。"""
...
CANDIDATES = ("INTERVAL_RAW3_BLOCK_NEWTON504", "INTERVAL_SIGN3_BLOCK_NEWTON504")
CONTROLS = previous.CONTROLS
BRANCHES = CANDIDATES + CONTROLS
FIRST_TARGET = "2026-09-17"
PROPOSAL_HASH = "e3612832a87c154965ed46dc1fd428f51a642a756dbb5a710e007a0058289da5"


def root():
    return base.ROOT / "round-92"


def require(condition, reason):
    if not condition:
        raise ValueError("BLOCK_NEWTON_" + reason)


def fingerprint():
    # 对全部前版本冻结文件核对一次，再把本轮四个新文件纳入独立冻结清单。
    previous_spec = base.read(core.root() / "plan.json")
    code = dict(base.read(base.ROOT / "round-91/plan.json")["fingerprint"]["code"])
    for name, expected in code.items():
        require(core.sha(base.PROJECT / name) == expected, "OLD_CODE_CHANGED")
    for name in (
        "app/services/direction_1d_sprint_market_block_newton.py",
        "app/services/direction_1d_sprint_block_newton_forward.py",
        "scripts/direction_1d_sprint_market_block_newton.py",
        "tests/test_direction_1d_sprint_market_block_newton.py",
    ):
...
def hessian(coef, x, y, weights, blocks):
    """平滑最差块风险的精确二阶导；正L2确保3x3线性系统可解。"""
    _, _, _, adversary = loss_gradient(coef, x, y, weights, blocks)
    probability = expit(x @ coef)
    totals = np.bincount(blocks, weights=weights, minlength=8)
    gradients, curvature = [], []
    for block in range(8):
        mask = blocks == block
        w = weights[mask] / totals[block]
        matrix = x[mask]
        gradients.append((w * (probability[mask] - y[mask])) @ matrix)
        curvature.append(matrix.T @ ((w * probability[mask] * (1 - probability[mask]))[:, None] * matrix))
    gradients, curvature = np.asarray(gradients), np.asarray(curvature)
    mean = adversary @ gradients
    covariance = (gradients.T * adversary) @ gradients - np.outer(mean, mean)
    return np.einsum("b,bij->ij", adversary, curvature) + covariance / 0.05 + 10 / weights.sum() * np.eye(3)


def solve_newton(x, y, weights, blocks):
    """零初值、固定Armijo折半；每步保留损失和梯度，不根据开发分数选择解。"""
    coef = np.zeros(3)
    trace = []
    for iteration in range(101):
        value, gradient, _, _ = loss_gradient(coef, x, y, weights, blocks)
        norm = float(np.max(np.abs(gradient)))
        trace.append({"iteration": iteration, "loss": value, "gradient_inf": norm})
        if norm <= 1e-8:
            return SimpleNamespace(x=coef, success=True, nit=iteration, trace=trace)
        if iteration == 100:
            break
        matrix = hessian(coef, x, y, weights, blocks)
        step = np.linalg.solve(matrix, -gradient)
        slope = float(gradient @ step)
        require(np.isfinite(step).all() and slope < 0, "NEWTON_DIRECTION_INVALID")
        accepted = False
        for backtrack in range(40):
            scale = 0.5**backtrack
            next_coef = coef + scale * step
            next_value = loss_gradient(next_coef, x, y, weights, blocks)[0]
            if next_value <= value + 1e-4 * scale * slope + 1e-14 * max(1, abs(value)):
                trace[-1]["backtracks"] = backtrack
                coef, accepted = next_coef, True
                break
        if not accepted:
            break
    return SimpleNamespace(x=coef, success=False, nit=len(trace) - 1, trace=trace)
...
def fit(rows, name, cutoff):
    ...
    with threadpool_limits(limits=2):
        solved = solve_newton(x, y, weights, blocks)
    active()
    value, gradient, means, adversary = loss_gradient(solved.x, x, y, weights, blocks)
    if not solved.success or np.max(np.abs(gradient)) >= 1e-6:
        base.save(
            root() / "solver-errors" / f"{cutoff}-{name}-{base.digest(chosen)[:12]}.json",
            {
        ...
        raise ValueError("BLOCK_NEWTON_OPTIMIZER_FAILED")
    return {
        "coef": solved.x.tolist(),
        "mean": [0.0] * 3,
        "scale": scale,
        ...
        "solver_trace": solved.trace,
        "solver": "DAMPED_NEWTON_EXPLICIT_HESSIAN",
        "objective": value,
...
def preflight():
    from app.services import direction_1d_sprint_block_newton_forward as forward

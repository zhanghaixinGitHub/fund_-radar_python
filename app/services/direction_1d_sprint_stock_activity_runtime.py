"""第115轮前向运行独立绑定：不改已冻结训练代码，待R114完成后一次性冻结接入关系。"""
...
from app.services import direction_1d_sprint_market_stock_interaction as parent
from app.services import direction_1d_sprint_stock_interaction_runtime as parent_runtime
...
def verify_intent():
    ...
    model.require(
        ...
        and intent["parent_round"] == 114
        and intent["same_question_branches"] == 34
        and intent["new_request_budget"] == 0
        and intent["first_target"] == model.FIRST_TARGET,
        "RUNTIME_INTENT_CHANGED",
    )
    model.require(
        intent["baseline_hash"] == base.digest(base.read(model.root() / "future-baseline.json")),
...
def plan():
    """R115训练与R114运行链均完成后才绑定；父运行层未完成时拒绝占位接入。"""
    intent = verify_intent()
    ...
    context = {
        ...
        "code_hashes": code,
        "artifact_hashes": {
            "round-115/future-baseline.json": intent["baseline_hash"],
            "round-115/plan.json": trained["plan_hash"],
            "round-115/result.json": base.digest(trained),
            "round-114/plan.json": prior["plan_hash"],
            "round-114/result.json": base.digest(prior),
            "round-114/runtime-plan.json": base.digest(parent_run),
        },
    }

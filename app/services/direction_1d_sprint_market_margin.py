"""第72轮：线性支持向量机优化分类间隔，预测仍为相邻交易日原始净值方向。
...
from app.services import direction_1d_sprint_market_intercept as previous
...
CANDIDATES = ("EXPANDED_MARKET_LINEAR_SVM3_504",)
...
PROPOSAL_HASH = "dbf0823b29c1d3d1495205171fe60acc39bd9b5da3c229ed387d10ae02906032"
active = regression.active


def root():
    return base.ROOT / "round-72"


def fingerprint():
    value = previous.fingerprint()
    for name in (
        "app/services/direction_1d_sprint_market_margin.py",
        "scripts/direction_1d_sprint_market_margin.py",
        "tests/test_direction_1d_sprint_market_margin.py",
    ):
...
def control_path(name, group, label):
    original = baseline.CANDIDATES[0] if name == CONTROLS[0] else baseline.CANDIDATES[1]
...
def plan():
    ...
    if path.exists():
        ...
        return p
    active()
    if base.digest(previous.models()[0]) != proposal["r71_result_hash"]:
        raise ValueError("MARKET_INTERCEPT_PARENT_RESULT_CHANGED")
    groups = sorted({f["group"] for f in data.scope()})
    paths = [
        "history.json",
        "round-72/proposal-before-implementation.json",
    ...
    p = {
        ...
        "old_questions": 5670,
        "training": (
            "504mature fullmarket dates pergroup; natural date/family weights; "
            "LinearSVC C.1/nointercept/seed17/max10000"
        ),
        "transform": "raw divided by training scale; newSVMmargin>=0; quarterlyrefits; frozenLRcontrolskeep.5",
...
def fit(rows, name, cutoff):
    ...
    with warnings.catch_warnings(), threadpool_limits(limits=2):
        ...
        model = LinearSVC(
            C=0.1, penalty="l2", loss="squared_hinge", dual="auto", max_iter=10000, random_state=17, fit_intercept=False
        )
...
def batch_answers(values, name, head=None):
    ...
    if name == "MARKET_MAJORITY3":
        ...
        x_transformed = transformed(x[indices], head["scale"], signed)
        is_margin = name in CANDIDATES
        scores = (
            head["model"].decision_function(x_transformed)
            if is_margin
            else head["model"].predict_proba(x_transformed)[:, 1]
        )
        if not np.isfinite(scores).all() or (not is_margin and (min(scores) < 0 or max(scores) > 1)):
            raise ValueError("MARKET_MARGIN_SCORE_INVALID")
        for i, score in zip(indices, scores, strict=True):
            out[i] = {
                "prediction": int(score >= (0.0 if is_margin else 0.5)),
                "research_score": float(score),
                "kind": "UNCALIBRATED_DIRECTION_MARGIN" if is_margin else "UNCALIBRATED_UP_SCORE",
                "route": "MARKET_LINEAR_SVM3" if is_margin else "MARKET_LR3",
            }
    return out
...
def tick():
    ...
    from app.services import direction_1d_sprint_market_margin as service

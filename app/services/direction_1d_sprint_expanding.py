"""第二十四轮：全部已有成熟历史与最近504日训练的一日模型对照。

保持原港股12项输入、资产组、树结构、权重及阈值，只扩大截止前成熟训练历史。
回看每条样本的输入口径不变；预测期限始终是原始单位净值的下一交易日。
"""
...
from app.services import direction_1d_sprint_ensemble as previous_round
...
CANDIDATES = ("HK_EXTRA12_ERR_EXPANDING",)
LEARNED = CANDIDATES
...
def fingerprint():
    ...
    for name in (
        "app/services/direction_1d_sprint_expanding.py",
        "scripts/direction_1d_sprint_expanding.py",
        "tests/test_direction_1d_sprint_expanding.py",
    ):
...
def selected_features(z, name):
    if name not in CANDIDATES or len(z) != 12 or not np.isfinite(z).all():
        raise ValueError("EXPANDING_INPUT_INVALID")
    return z
...
def expanding_rows(rows, cutoff):
    """只使用截止前已成熟的标签，保留全部已有历史，不以开发结果挑开始日期。"""
    selected = [r for r in rows if r["u"] < cutoff and r["mature"] < cutoff]
    counts = Counter(r["y"] for r in selected)
    if len({r["u"] for r in selected}) < 120 or len(counts) != 2 or min(counts.values()) < 20:
        raise ValueError("EXPANDING_TRAIN_INSUFFICIENT")
    return selected
...
def plan():
    ...
    if path.exists():
        ...
        if value["fingerprint"] != fingerprint() or value["calendar_hash"] != base.calendar()[1]:
            raise ValueError("ROUND_24_CODE_OR_CALENDAR_CHANGED")
        return value
    active()
    if base.read(previous_round.root() / "result.json")["fingerprint"] != previous_round.fingerprint():
        raise ValueError("EXPANDING_PREVIOUS_FINGERPRINT_CHANGED")
    parent, _ = hk.models()
    value = {
        "at": base.now().isoformat(),
        "fingerprint": fingerprint(),
        "candidates": CANDIDATES,
        "hypothesis": "All available mature history may stabilize response across regimes versus trailing504dates",
        ...
        "input_features": "unchanged originalHK12; no newVIXinputs or source requests",
        "training_dates": "all available u<cutoff and mature<cutoff from frozen2021onward data, no trailing truncation",
        "minimum_training": "120distinctdates, bothoriginaldirectionclasses anderrorclasses>=20rows",
        "sample_audit": "Before fit verify original504subset digest against round17; record fullhistory span/count",
        "extra_trees": {
            "trees": 256,
            "depth": 6,
            "min_leaf": 80,
        ...
        "weights": "date/family natural error prior, no class balance or recency decay",
        ...
        "expected_dates": 199,
        "controls": ["HK_EXTRA12_ERR504", "MEAN3_ERR504", "SPX_SIGN", "ALWAYS_UP"],
        "selection": "one predeclared candidate; no starting-date or threshold search",
...
def fit(rows, name, cutoff):
    """使用所有截止前成熟历史；日期/家族等权规则与原HK12模型一致。"""
    if name not in CANDIDATES:
        raise ValueError("EXPANDING_RECIPE_INVALID")
    chosen = expanding_rows(rows, cutoff)
    ...
    if len(Counter(y)) != 2 or min(Counter(y).values()) < 20:
        raise ValueError("EXPANDING_ERROR_LABELS_INSUFFICIENT")
    ...
    return {
        ...
        "fit_start": min(r["u"] for r in chosen),
        "fit_end": max(r["u"] for r in chosen),
        "original504_rows": len(adaptive.training_rows(rows, cutoff, "MONTHLY_BAL504")),
...
def batch_answers(values, name, trained):
    ...
    if not np.isfinite(x).all():
        raise ValueError("EXPANDING_MODEL_NORMALIZED_INPUT_INVALID")
    scores = trained["model"].predict_proba(x)[:, 1]
    if not np.isfinite(scores).all() or min(scores) < 0 or max(scores) > 1:
        raise ValueError("EXPANDING_MODEL_SCORE_INVALID")
    result = []
...
def fit_checkpoint(rows, name, cutoff, label):
    ...
    if receipt.exists():
        ...
        if (
            ...
            raise ValueError("EXPANDING_MODEL_CHECKPOINT_CHANGED")
        return joblib.load(path)
    if attempt.exists():
        raise ValueError("EXPANDING_MODEL_PREVIOUS_FIT_INTERRUPTED")
    ...
    if base.digest(short) != expected:
        raise ValueError("EXPANDING_CONTROL_TRAINING_ROWS_CHANGED")
...
def train():
    ...
    for name, expected in p["input_hashes"].items():
        if base.digest(base.read(base.ROOT / name)) != expected:
            raise ValueError("ROUND_24_INPUT_CHANGED")
    rows, proofs = dataset()
    ...
    with threadpool_limits(limits=2):
        for q in range(1, 5):
            ...
            for group in groups:
                ...
                output["MEAN3_ERR504"].extend(base.read(previous_round.root() / f"folds/{q}-{group}-MEAN3_ERR504.json"))
        ...
        if len(expected) != 5670 or any(
            ...
            raise ValueError("ROUND_24_COMMON_EXAM_CHANGED")
...
def models():
    ...
    if (
        ...
        raise ValueError("ROUND_24_MODEL_OR_CODE_CHANGED")

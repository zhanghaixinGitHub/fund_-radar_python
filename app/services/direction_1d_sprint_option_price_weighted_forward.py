"""第107轮复用 CORE3 输入，并复用已实际取得T日原始期权行情，不调用已退役的旧实验链。"""
...
from app.services import direction_1d_sprint_health_futures_forward as prior_forward
from app.services import direction_1d_sprint_market_health_futures as prior_model
from app.services import direction_1d_sprint_market_option_price_weighted as model
...
def validate(value, receipt, parent, source, manifest, bundle, extra, history):
    ...
    model.require(value["r106_forecast_hash"] == base.digest(extra), "R106_PARENT_CHANGED")
    ...
    model.require(
        ...
        "R106_PARENT_READBACK_LATE_OR_CHANGED",
    )
    model.require(
        value["option_price_weighted_source_hash"] == base.digest(history)
    ...
    model.require(
        ...
        "ACTUAL_PRICE_WEIGHT_SOURCE_CHANGED",
    )
...
def run():
    # 先完成已有CORE3及R106分支，再在同一实际输入上增加独立期权价格权重模型。
    prior_forward.run()
    ...
    for key, p in parents.items():
        ...
        value = {k: p[k] for k in ("code", "family", "group", "t", "u")} | {
            ...
            "r106_forecast_hash": base.digest(extra),
            "option_price_weighted_source_hash": base.digest(history[p["u"]]),
        }
...
def combined_answers(parent, extra, value):
    """明确合并全部25个同题分支，避免新增中间模型后递归层级漂移丢掉旧对照。"""
    combined = dict(parent["answers"])
    for number in (92, 94, 95, 96, 97, 98, 99, 100, 101, 102, 103, 104, 105):
    ...
    model.require(len(combined) == 25, "FORWARD_BRANCH_COUNT_CHANGED")
    return combined
...
def report(loaded=None):
    ...
    value = {
        ...
        "new_branches": 1,
        "same_question_branches": 25,
        "option_price_weighted_sources": {
            u: {

"""第二十九轮：按独立历史交易日寻找相似市场状态，估计下一日基线错误。

每个资产组内先合并同日同产品家族的标签，再合并家族；每个日期只作为一个邻居。
固定使用32个相似日期，不搜索邻居数或距离权重，预测仍是相邻交易日单位净值方向。
"""
...
from app.services import direction_1d_sprint_boost as previous_round
...
CANDIDATES = ("DAY_KNN9_K32_ERR504",)
MARKET_INDICES = (0, 1, 2, 6, 7, 8, 9, 10, 11)
NEIGHBORS = 32
LEARNED = CANDIDATES
...
def fingerprint():
    ...
    for name in (
        "app/services/direction_1d_sprint_neighbors.py",
        "scripts/direction_1d_sprint_neighbors.py",
        "tests/test_direction_1d_sprint_neighbors.py",
    ):
...
def selected_features(z, name):
    if name not in CANDIDATES or len(z) != 12 or not np.isfinite(z).all():
        raise ValueError("NEIGHBOR_INPUT_INVALID")
    return [z[i] for i in MARKET_INDICES]
...
def plan():
    ...
    if path.exists():
        ...
        if value["fingerprint"] != fingerprint() or value["calendar_hash"] != base.calendar()[1]:
            raise ValueError("ROUND_29_CODE_OR_CALENDAR_CHANGED")
        return value
    active()
    if base.read(previous_round.root() / "result.json")["fingerprint"] != previous_round.fingerprint():
        raise ValueError("NEIGHBOR_PREVIOUS_FINGERPRINT_CHANGED")
    parent, _ = hk.models()
    value = {
        "at": base.now().isoformat(),
        "fingerprint": fingerprint(),
        "candidates": CANDIDATES,
        "hypothesis": "Similar market days may estimate baseline errors without counting repeated fund shares",
        "motivation": "Trees did not improve; test local conditional errors with equal date weight",
        ...
        "input_features": {"parent": "HK12", "selected_indices": MARKET_INDICES, "excludes": "fund-specific momentum"},
        "training_dates": 504,
        ...
        "neighbors": {
            "count": NEIGHBORS,
            "distance": "Euclidean train-date standardized market9",
            "tie_break": "oldest historical target date first",
            "weights": "uniform over32dates",
        },
        "aggregation": "within each asset-group/date, average errors within family then average families",
        "weights": "one historical date one neighbor; no class balance",
        "normalization": "equal train-date weights only; centered-square scale; constantcolumn scale1",
        ...
        "expected_dates": 199,
        "controls": ["HK_EXTRA12_ERR504", "HGB_HK12_ERR504", "SPX_SIGN", "ALWAYS_UP"],
...
class MarketDayNeighbors:
    """固定32个独立日期的均匀邻居估计，不按基金行数重复计票。

    points按日期从早到晚排列，距离相同时稳定排序保留该次序。
    分块计算距离，避免推理题目数与历史日期数乘积造成无界内存占用。
    """

    def __init__(self, points, errors):
        self.points = np.asarray(points, dtype=float).copy()
        self.errors = np.asarray(errors, dtype=float).copy()
        if (
            self.points.ndim != 2
            or self.points.shape[1] != len(MARKET_INDICES)
            or len(self.points) < NEIGHBORS
            or self.errors.shape != (len(self.points),)
            or not np.isfinite(self.points).all()
            or not np.isfinite(self.errors).all()
            or min(self.errors) < 0
            or max(self.errors) > 1
        ):
            raise ValueError("NEIGHBOR_TRAINING_POINTS_INVALID")

    def predict_proba(self, values):
        values = np.asarray(values, dtype=float)
        if values.ndim != 2 or values.shape[1] != self.points.shape[1] or not np.isfinite(values).all():
            raise ValueError("NEIGHBOR_QUERY_INVALID")
        scores = []
        for start in range(0, len(values), 256):
            distance = np.sum((values[start : start + 256, None, :] - self.points[None, :, :]) ** 2, axis=2)
            if not np.isfinite(distance).all():
                raise ValueError("NEIGHBOR_DISTANCE_INVALID")
            nearest = np.argsort(distance, axis=1, kind="stable")[:, :NEIGHBORS]
            scores.extend(self.errors[nearest].mean(axis=1).tolist())
        scores = np.asarray(scores)
        return np.column_stack((1 - scores, scores))


def fit(rows, name, cutoff):
    """仅合并训练截止前成熟的原样本，保留同一504日选取，不删除难预测日期。

    市场输入在同一目标日期必须一致。标签先家族内平均，再家族间平均，
    防止同产品的多个份额或同日多基金把一个日期放大为许多邻居。
    """
    if name not in CANDIDATES:
        raise ValueError("NEIGHBOR_RECIPE_INVALID")
    chosen = training_rows(rows, cutoff)
    labels = [int(int(r["z"][0] >= 0) != r["y"]) for r in chosen]
    if len(Counter(labels)) != 2 or min(Counter(labels).values()) < 20:
        raise ValueError("NEIGHBOR_ERROR_LABELS_INSUFFICIENT")
    daily_inputs, daily_families = {}, defaultdict(lambda: defaultdict(list))
    for row, label in zip(chosen, labels, strict=True):
        x = selected_features(row["z"], name)
        day = row["u"]
        if day in daily_inputs and not np.allclose(daily_inputs[day], x, rtol=0, atol=1e-12):
            raise ValueError("NEIGHBOR_SAME_DATE_MARKET_MISMATCH")
        daily_inputs[day] = x
        daily_families[day][row["family"]].append(label)
    dates = sorted(daily_inputs)
    x = np.asarray([daily_inputs[day] for day in dates])
    errors = np.asarray([np.mean([np.mean(v) for v in daily_families[day].values()]) for day in dates])
    x, mean, scale = sequence.normalize_training(x, np.ones(len(dates)))
    active()
    model = MarketDayNeighbors(x, errors)
    return {
        "model": model,
        "mean": mean,
        "scale": scale,
        ...
        "cutoff": cutoff,
        "weighted_error_rate": float(np.mean(errors)),
        "training_target": "NEXT_DAY_SPX_BASELINE_ERROR",
        "neighbor_dates": dates,
        "neighbors": NEIGHBORS,
        "date_aggregation_hash": base.digest(
            [{"u": d, "x": daily_inputs[d], "error": float(e)} for d, e in zip(dates, errors, strict=True)]
        ),
    }
...
def fit_checkpoint(rows, name, cutoff, label):
    ...
    if receipt.exists():
        ...
        if (
            ...
            raise ValueError("NEIGHBOR_MODEL_CHECKPOINT_CHANGED")
        return joblib.load(path)
    if attempt.exists():
        raise ValueError("NEIGHBOR_MODEL_PREVIOUS_FIT_INTERRUPTED")
    ...
    if base.digest(short) != expected:
        raise ValueError("NEIGHBOR_CONTROL_TRAINING_ROWS_CHANGED")
...
def train():
    ...
    for name, expected in p["input_hashes"].items():
        if base.digest(base.read(base.ROOT / name)) != expected:
            raise ValueError("ROUND_29_INPUT_CHANGED")
    rows, proofs = dataset()
    ...
    with threadpool_limits(limits=2):
        for q in range(1, 5):
            ...
            for group in groups:
                ...
                output["HGB_HK12_ERR504"].extend(
                    base.read(previous_round.root() / f"folds/{q}-{group}-HGB_HK12_ERR504.json")
                )
        ...
        if len(expected) != 5670 or any(
            ...
            raise ValueError("ROUND_29_COMMON_EXAM_CHANGED")
...
def models():
    ...
    if (
        ...
        raise ValueError("ROUND_29_MODEL_OR_CODE_CHANGED")

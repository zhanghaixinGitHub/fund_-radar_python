"""第五十四轮：用部分共享系数学习基金对三项市场信号的不同反应。

固定零截距LR93与既有LR3同训练题比较；基金偏离系数受到更强收缩。
...
from app.services import direction_1d_sprint_margin_balance as previous_round
...
CANDIDATES = ("FUND_PARTIAL_LR93_ZERO504",)
SHRINKAGE = 0.25
CODE_ORDER = (
    "001021",
    "001632",
...
def fingerprint():
    ...
    for name in (
        "app/services/direction_1d_sprint_fund_pooling.py",
        "scripts/direction_1d_sprint_fund_pooling.py",
        "tests/test_direction_1d_sprint_fund_pooling.py",
    ):
...
def fund_index(code):
    ...
    if code not in CODE_ORDER:
        raise ValueError("FUND_POOLING_UNKNOWN_CODE")
    return CODE_ORDER.index(code)


def vector(x, code, t, u, markets, etf_points, cn_points):
    return majority.vector(x, code, t, u, markets, etf_points, cn_points) + [fund_index(code)]


def live_vector(source, original, markets, etf_points, cn_points):
    sequence.live_vector(source, original)
    return vector(source["x"], original["code"], original["base"], original["u"], markets, etf_points, cn_points)
...
def selected_features(z, name):
    if (
        ...
        or isinstance(z[20], bool)
        or not float(z[20]).is_integer()
        or not 0 <= z[20] < len(CODE_ORDER)
    ):
        raise ValueError("FUND_POOLING_INPUT_INVALID")
    return majority.selected_features(z[:20], majority.CANDIDATES[0])


def design(values, scale):
    """全局三项按训练尺度缩放；对应基金三列乘0.25，不再次逐列归一化，保留收缩强度。"""
    scales = np.asarray(scale, dtype=float)
    if scales.shape != (3,) or not np.isfinite(scales).all() or min(scales) <= 0:
        raise ValueError("FUND_POOLING_SCALE_INVALID")
    core = np.asarray([selected_features(z, CANDIDATES[0]) for z in values]) / scales
    out = np.zeros((len(values), 3 + 3 * len(CODE_ORDER)))
    out[:, :3] = core
    for i, z in enumerate(values):
        offset = 3 + 3 * int(z[20])
        out[i, offset : offset + 3] = SHRINKAGE * core[i]
    return out
...
def dataset():
    ...
    if sorted({r["code"] for r in rows}) != list(CODE_ORDER):
        raise ValueError("FUND_POOLING_SCOPE_CHANGED")
    return [r | {"z": r["z"] + [fund_index(r["code"])]} for r in rows], proof | {
        "code_order": CODE_ORDER,
        "extra_provider_requests": 0,
    }
...
def plan():
    ...
    if path.exists():
        ...
        if value["fingerprint"] != fingerprint() or value["calendar_hash"] != base.calendar()[1]:
            raise ValueError("ROUND_54_CODE_OR_CALENDAR_CHANGED")
        return value
    active()
    ...
    if proposal["budget"] != {
        ...
        raise ValueError("FUND_POOLING_PROPOSAL_BUDGET_CHANGED")
    value = {
        "at": base.now().isoformat(),
        "fingerprint": fingerprint(),
        "candidates": CANDIDATES,
        "code_order": CODE_ORDER,
        "hypothesis": "Fund-specific slopes with partial pooling on SPX/FXI/CNYA may outperform uniform-group slopes",
        "target": "Raw unit NAV T to adjacent CN U UP/NON_UP,flat separate",
        "training_target": "raw_NAV_UP",
        "features": "3global +30fund*3interactions=93columns;fundcode selects its own block",
        "shrinkage": SHRINKAGE,
        "normalization": "R50three training scales;no centering;interaction=.25*scaledglobal;no extra scaling",
        "recipe": "Logistic L2 C.1 max_iter1500 lbfgs seed17 nointercept naturaldate/familyweights threshold.5",
        "training": "Last504mature jointavailable dates/group;same30funds and15windows asfrozen R50LR3",
        "control": "Reuse R50LR3 all15checkpoints,exactmature rowhash checks;no controlfit",
        "max_development_fits": 12,
        "max_current_fits": 3,
        "max_reproduction_count": 1,
        "expected_questions": 5670,
        "expected_dates": 199,
        "selection": "One predeclared partial-pooling candidate vsR50LR3/majority;no shrinkage or threshold search",
        "parent_result_hash": base.digest(parent),
        "calendar_hash": base.calendar()[1],
        "input_hashes": majority.plan()["input_hashes"]
        | {
            "round-54/proposal-before-implementation.json": base.digest(proposal),
            "common-direction-ceiling-v1/result.json": base.digest(
                base.read(base.ROOT / "common-direction-ceiling-v1/result.json")
            ),
        },
        ...
        "source_timing": "Exact R50source timing andactualU0830 receipts",
        "live_budget": "Reuse existingETF/CNYA captures;0new providerrequests",
    }
...
def fit(rows, name, cutoff):
    """只给原三输入增加收缩的基金差异，成熟日期和训练尺度沿用同题对照规则。"""
    if name not in CANDIDATES:
        raise ValueError("FUND_POOLING_RECIPE_INVALID")
    ...
    if len({r["u"] for r in chosen}) < 120:
        raise ValueError("FUND_POOLING_TRAINING_SHORT")
...
def batch_answers(values, name, trained):
    ...
    if indices:
        ...
        if (
            ...
            or head["model"].n_features_in_ != 93
            or head["mean"] != [0.0] * 3
        ):
            raise ValueError("FUND_POOLING_MODEL_SCHEMA_CHANGED")
        x = design([values[i] for i in indices], head["scale"])
        scores = head["model"].predict_proba(x)[:, 1]
        if not np.isfinite(scores).all() or min(scores) < 0 or max(scores) > 1:
            raise ValueError("FUND_POOLING_SCORE_INVALID")
...
def fit_checkpoint(rows, name, cutoff, label):
    ...
    if (
        ...
        raise ValueError("FUND_POOLING_PRIOR_CHECKPOINT_CHANGED")
    ...
    if base.digest([r | {"z": r["z"][:20]} for r in shared]) != prior_head["fit_hash"]:
        raise ValueError("FUND_POOLING_LR3_CONTROL_ROWS_CHANGED")
...
def train():
    ...
    for name, expected in p["input_hashes"].items():
        if base.digest(base.read(base.ROOT / name)) != expected:
            raise ValueError("ROUND_54_INPUT_CHANGED")
    rows, proofs = dataset()
    ...
    with threadpool_limits(limits=2):
        ...
        if len(expected) != 5670 or any(
            ...
            raise ValueError("ROUND_54_COMMON_EXAM_CHANGED")
...
def models():
    ...
    if (
        ...
        raise ValueError("ROUND_54_MODEL_OR_CODE_CHANGED")
...
def report():
    ...
    for path in (root() / "forward").glob("*/*.json"):
        ...
        if (
            ...
            raise ValueError("ROUND_54_PARENT_OR_MODEL_CHANGED")
        ...
        if not np.allclose(
            ...
            raise ValueError("ROUND_54_VECTOR_CHANGED")
        ...
        if not runtime.answers_match(value["answers"], expected_answers):
            raise ValueError("ROUND_54_SAVED_ANSWER_CHANGED")
        good += 1

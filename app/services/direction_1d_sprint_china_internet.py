"""第六十一轮：检验中国互联网板块隔夜价格能否改善一日方向模型。

固定零截距LR4与原LR3作同成熟题对照，仅增加KWEB美国时段涨跌；该代理不是基金持仓映射。
...
from app.services import direction_1d_sprint_monthly_direct as prior_fingerprint
...
from app.services import direction_1d_sprint_stacking_monotonic as previous_round
from app.services import direction_1d_sprint_us_etf_data as us_etf_data

CANDIDATES = ("SPX_FXI_CNYA_KWEB_LR4_ZERO504",)
...
def fingerprint():
    value = prior_fingerprint.fingerprint()
    for name in (
        "app/services/direction_1d_sprint_china_internet.py",
        "scripts/direction_1d_sprint_china_internet.py",
        "tests/test_direction_1d_sprint_china_internet.py",
...
def plan():
    ...
    if path.exists():
        ...
        if value["fingerprint"] != fingerprint() or value["calendar_hash"] != base.calendar()[1]:
            raise ValueError("ROUND_61_CODE_OR_CALENDAR_CHANGED")
        return value
    active()
    ...
    value = {
        ...
        "recipe": "LR L2 C.1 lbfgs maxiter1500 seed17 nointercept naturaldatefamilyweights UP>=.5",
        ...
        | {
            "round-61/proposal-before-implementation.json": base.digest(proposal),
            **{
        ...
        "live_budget": "KWEBonly*3slots maximum3GET/target;existingETF/CNYAcaptures reused",
...
def train():
    ...
    for name, expected in p["input_hashes"].items():
        if base.digest(base.read(base.ROOT / name)) != expected:
            raise ValueError("ROUND_61_INPUT_CHANGED")
    rows, proofs = dataset()
    ...
    with threadpool_limits(limits=2):
        ...
        if len(expected) != 5670 or any(
            ...
            raise ValueError("ROUND_61_COMMON_EXAM_CHANGED")
...
def models():
    ...
    if (
        ...
        raise ValueError("ROUND_61_MODEL_OR_CODE_CHANGED")
...
def report():
    ...
    for path in (root() / "forward").glob("*/*.json"):
        ...
        if (
            ...
            raise ValueError("ROUND_61_PARENT_OR_MODEL_CHANGED")
        ...
        if not np.allclose(
            ...
            raise ValueError("ROUND_61_VECTOR_CHANGED")
        ...
        if not runtime.answers_match(value["answers"], expected_answers):
            raise ValueError("ROUND_61_SAVED_ANSWER_CHANGED")
        good += 1

"""第113轮前向运行独立绑定：不改已冻结训练代码，待R112完成后一次性冻结接入关系。"""
...
from app.services import direction_1d_sprint_credit_pair_runtime as parent_runtime
from app.services import direction_1d_sprint_market_credit_pair as parent
...
def verify_intent():
    ...
    model.require(
        ...
        and intent["parent_round"] == 112
        and intent["same_question_branches"] == 32
...
def plan():
    """R113训练与R112运行链均完成后才绑定；父运行层未完成时拒绝占位接入。"""
    intent = verify_intent()
    ...
    context = {
        ...
        "code_hashes": code,
        "artifact_hashes": {
            "round-113/plan.json": trained["plan_hash"],
            "round-113/result.json": base.digest(trained),
            "round-112/plan.json": prior["plan_hash"],
            "round-112/result.json": base.digest(prior),
            "round-112/runtime-plan.json": base.digest(parent_run),
        },
    }

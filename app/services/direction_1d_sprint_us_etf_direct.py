"""第四十九轮：固定简单直接方向模型，检查FXI交易时段价格的独立贡献。

单输入对照和双输入实验使用相同样本及零截距逻辑回归；无新美股收盘或价格质量异常时复用原HK答案。
...
from app.services import direction_1d_sprint_us_etf as previous_round
from app.services import direction_1d_sprint_us_etf_data as us_etf_data

CANDIDATES = ("SPX_LR1_ZERO_AVAILABLE504", "SPX_FXI_SESSION_LR2_ZERO504")
LEARNED = CANDIDATES
...
def fingerprint():
    ...
    for name in (
        "app/services/direction_1d_sprint_us_etf_direct.py",
        "scripts/direction_1d_sprint_us_etf_direct.py",
        "tests/test_direction_1d_sprint_us_etf_direct.py",
        ".local-runs/direction-1d-sprint-20260914/round-49/feature-feasibility.py",
    ):
...
def selected_features(z, name):
    """保留完整证据校验，只让估计器读取SPX；实验组额外读取FXI开盘至收盘百分比。"""
    if name not in CANDIDATES:
        raise ValueError("US_ETF_DIRECT_CANDIDATE_INVALID")
    previous_round.selected_features(z, previous_round.CANDIDATES[0])
    return [z[0]] if name == CANDIDATES[0] else [z[0], z[12]]


def dataset():
    """复用第48轮完全相同的题目、路由、输入与成熟日期，不重新请求行情。"""
    ...
    projected = {n: [selected_features(r["z"], n) for r in rows] for n in CANDIDATES}
    if (
        feasible["proposal_hash"] != base.digest(proposal)
        or feasible["rows_hash"] != base.digest(rows)
        or feasible["projection_hash"] != base.digest(projected)
        ...
        raise ValueError("US_ETF_DIRECT_PROTOTYPE_CHANGED")
    return rows, proof
...
def plan():
    ...
    if path.exists():
        ...
        if value["fingerprint"] != fingerprint() or value["calendar_hash"] != base.calendar()[1]:
            raise ValueError("ROUND_49_CODE_OR_CALENDAR_CHANGED")
        return value
    active()
    parent, _ = hk.models()
    ...
    value = {
        ...
        "hypothesis": (
            "Direct UP target may use FXI session information obscured by error trees; "
            "compare SPX-only with SPX+FXI"
        ),
        "target": "Raw unit NAV T to adjacent CN U, UP vs NON_UP; keep flat separate",
        "feature_order": (
            "Full17 retained; SPX-only index0 versus SPX plus FXI-session indices0,12; "
            "route is not a regressor"
        ),
        "routing": "All30 fund samequestions; source unavailable or no newUSclose reuses exactHK predictions",
        "ablation": "Same direct UP target/available504 dates/recipe; only additional FXI-session input differs",
        "recipe": (
            "LogisticRegression C.1 max_iter1500 lbfgs L2 fit_interceptFalse seed17 "
            "threads2; convergence warning fails"
        ),
        "training": (
            "Last504 mature available dates/group; natural date/family UP weights; "
            "train-only scale, no mean subtraction"
        ),
        ...
        | {
            ...
            for name in (
                ...
                "round-49/proposal-before-training.json",
                "round-49/input-feasibility.json",
            )
        },
        ...
        "live_budget": "Reuse validated round48 ETF input; no additional HTTP requests",
...
def train():
    ...
    for name, expected in p["input_hashes"].items():
        if base.digest(base.read(base.ROOT / name)) != expected:
            raise ValueError("ROUND_49_INPUT_CHANGED")
    rows, proofs = dataset()
    ...
    with threadpool_limits(limits=2):
        ...
        if len(expected) != 5670 or any(
            ...
            raise ValueError("ROUND_49_COMMON_EXAM_CHANGED")
...
def models():
    ...
    if (
        ...
        raise ValueError("ROUND_49_MODEL_OR_CODE_CHANGED")
...
def report():
    ...
    for path in (root() / "forward").glob("*/*.json"):
        ...
        if (
            ...
            raise ValueError("ROUND_49_PARENT_OR_MODEL_CHANGED")

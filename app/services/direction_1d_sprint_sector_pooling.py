"""第五十六轮：新增半导体、金属矿业隔夜信号，比较全局与基金差异模型。

固定零截距LR5与部分共享LR155，同成熟题比较；行业信号不是基金持仓映射。
...
from app.services import direction_1d_sprint_etf_fund_state as previous_round
...
CANDIDATES = ("SPX_FXI_CNYA_SECTOR_LR5_ZERO504", "FUND_SECTOR_PARTIAL_LR155_ZERO504")
...
def fingerprint():
    ...
    for name in (
        "app/services/direction_1d_sprint_sector_pooling.py",
        "scripts/direction_1d_sprint_sector_pooling.py",
        "tests/test_direction_1d_sprint_sector_pooling.py",
        "app/services/direction_1d_sprint_us_sector_etf_data.py",
        "tests/test_direction_1d_sprint_us_sector_etf_data.py",
        "app/integrations/sina_sprint_sector_etf.py",
        "app/integrations/sina_sprint_sector_decode.cjs",
        "tests/test_sina_sprint_sector_etf.py",
    ):
...
def fund_index(code):
    ...
    if code not in CODE_ORDER:
        raise ValueError("SECTOR_POOLING_UNKNOWN_CODE")
...
def selected_features(z, name):
    if (
        ...
        raise ValueError("SECTOR_POOLING_INPUT_INVALID")
...
def design(values, scale, name):
    ...
    if scales.shape != (5,) or not np.isfinite(scales).all() or min(scales) <= 0:
        raise ValueError("SECTOR_POOLING_SCALE_INVALID")
...
def dataset():
    ...
    if sorted({r["code"] for r in rows}) != list(CODE_ORDER):
        raise ValueError("SECTOR_POOLING_SCOPE_CHANGED")
    points = sectors.history()
    context = {key: sectors.features(*key, points) for key in {(r["t"], r["u"]) for r in rows}}
    if any(majority.available(r["z"]) and context[r["t"], r["u"]][2] != 1 for r in rows):
        raise ValueError("SECTOR_POOLING_COMMON_ROWS_CHANGED")
...
def plan():
    ...
    if path.exists():
        ...
        if value["fingerprint"] != fingerprint() or value["calendar_hash"] != base.calendar()[1]:
            raise ValueError("ROUND_56_CODE_OR_CALENDAR_CHANGED")
        return value
    active()
    ...
    if proposal["candidates"] != list(CANDIDATES) or proposal["budget"] != {
        ...
        "new_cost_cny": 0,
        "future_GETs_per_target_max": 6,
    }:
        raise ValueError("SECTOR_POOLING_PROPOSAL_CHANGED")
    value = {
        "at": base.now().isoformat(),
        "fingerprint": fingerprint(),
        "candidates": CANDIDATES,
        "code_order": CODE_ORDER,
        "hypothesis": "Overnight SOXX/XME sector moves may add information beyond broad market,globally orperfund",
        "target": "Raw unit NAV T->adjacentCN U UP/NON_UP,flatseparate",
        "training_target": "raw_NAV_UP",
        "feature_indices": FEATURE_INDICES,
        "features": "R50three +SOXX/XME_USsessionpct;global5 vs5+30*5fundinteractions",
        "shrinkage": SHRINKAGE,
        "normalization": "Same5weightedtraining scales,no centering;interaction .25 andnoextrare-scaling",
        ...
        | {
            "round-56/proposal-before-implementation.json": base.digest(proposal),
            **{
                f"us-sector-etf-source-v1/{name}": base.digest(base.read(base.ROOT / "us-sector-etf-source-v1" / name))
        ...
        "live_budget": "2newsectorquotes*3slots maximum6GET/target;existingETF/CNYAcaptures reused",
        "limits": "Sectorproxies notholdings;historicalpublicationunverified;TANpendingpricecheck",
    }
...
def fit(rows, name, cutoff):
    ...
    if name not in CANDIDATES:
        raise ValueError("SECTOR_POOLING_RECIPE_INVALID")
    ...
    if len({r["u"] for r in chosen}) < 120:
        raise ValueError("SECTOR_POOLING_TRAINING_SHORT")
    ...
    if len(Counter(y)) != 2 or min(Counter(y).values()) < 20:
        raise ValueError("SECTOR_POOLING_CLASSES_INSUFFICIENT")
...
def batch_answers(values, name, trained):
    ...
    if indices:
        ...
        if (
            ...
            raise ValueError("SECTOR_POOLING_MODEL_SCHEMA_CHANGED")
        ...
        if not np.isfinite(scores).all() or min(scores) < 0 or max(scores) > 1:
            raise ValueError("SECTOR_POOLING_SCORE_INVALID")
...
def fit_checkpoint(rows, name, cutoff, label):
    ...
    if (
        ...
        raise ValueError("SECTOR_POOLING_PRIOR_CHECKPOINT_CHANGED")
    ...
    if base.digest([r | {"z": r["z"][:20]} for r in shared]) != prior_head["fit_hash"]:
        raise ValueError("SECTOR_POOLING_LR3_CONTROL_ROWS_CHANGED")
...
def train():
    ...
    for name, expected in p["input_hashes"].items():
        if base.digest(base.read(base.ROOT / name)) != expected:
            raise ValueError("ROUND_56_INPUT_CHANGED")
    rows, proofs = dataset()
    ...
    with threadpool_limits(limits=2):
        ...
        if len(expected) != 5670 or any(
            ...
            raise ValueError("ROUND_56_COMMON_EXAM_CHANGED")
...
def models():
    ...
    if (
        ...
        raise ValueError("ROUND_56_MODEL_OR_CODE_CHANGED")
...
def report():
    ...
    for path in (root() / "forward").glob("*/*.json"):
        ...
        if (
            ...
            raise ValueError("ROUND_56_PARENT_OR_MODEL_CHANGED")
        ...
        if value["sector_input_hash"] != base.digest(sector_input):
            raise ValueError("SECTOR_POOLING_INPUT_CHANGED")
        ...
        if not np.allclose(
            ...
            raise ValueError("ROUND_56_VECTOR_CHANGED")
        ...
        if not runtime.answers_match(value["answers"], expected_answers):
            raise ValueError("ROUND_56_SAVED_ANSWER_CHANGED")
        good += 1

"""第六十四轮：检验包含开盘缺口的沪深300海外价格残差。

固定零截距LR4与原LR3作同成熟题对照，仅增加ASHR现金调整跨时段残差；该代理不是基金持仓映射。
...
from app.services import direction_1d_sprint_ashare as previous_round
from app.services import direction_1d_sprint_ashare_residual_data as sectors
...
CANDIDATES = ("SPX_FXI_CNYA_ASHR_RESIDUAL_LR4_ZERO504", "ASHR_RESIDUAL_SIGN_WITH_PARENT")
FEATURE_INDICES = (0, 12, 17, 20)
CODE_ORDER = (
    "001021",
    "001632",
    "002112",
...
def fingerprint():
    ...
    for name in (
        "app/services/direction_1d_sprint_ashare_residual.py",
        "scripts/direction_1d_sprint_ashare_residual.py",
        "tests/test_direction_1d_sprint_ashare_residual.py",
        "app/services/direction_1d_sprint_ashare_residual_data.py",
        "tests/test_direction_1d_sprint_ashare_residual_data.py",
    ):
...
def fund_index(code):
    ...
    if code not in CODE_ORDER:
        raise ValueError("ASHARE_RESIDUAL_UNKNOWN_CODE")
    return CODE_ORDER.index(code)


def vector(x, code, t, u, markets, etf_points, cn_points, sector_points):
    """原三信号之后追加ASHR现金调整残差及可用标记，最后绑定基金身份。"""
    parent = majority.vector(x, code, t, u, markets, etf_points, cn_points)
    return parent + sectors.features(t, u, sector_points, parent[6]) + [fund_index(code)]
...
def available(z):
    return majority.available(z[:20]) and z[21] == 1
...
def selected_features(z, name):
    if (
        ...
        or isinstance(z[22], bool)
        or not float(z[22]).is_integer()
        or not 0 <= z[22] < len(CODE_ORDER)
        or z[21] not in (0, 1)
        or (z[21] == 0 and any(z[20:21]))
    ):
        raise ValueError("ASHARE_RESIDUAL_INPUT_INVALID")
...
def design(values, scale, name):
    """四项百分比信号仅按已成熟训练题的尺度缩放，不居中、不加基金交互。"""
    scales = np.asarray(scale, dtype=float)
    if scales.shape != (4,) or not np.isfinite(scales).all() or min(scales) <= 0:
        raise ValueError("ASHARE_RESIDUAL_SCALE_INVALID")
    return np.asarray([selected_features(z, name) for z in values]) / scales


def dataset():
    rows, proof = majority.dataset()
    if sorted({r["code"] for r in rows}) != list(CODE_ORDER):
        raise ValueError("ASHARE_RESIDUAL_SCOPE_CHANGED")
    points = sectors.history()
    china = {}
    for row in rows:
        key = (row["t"], row["u"])
        if key in china and abs(china[key] - row["z"][6]) > 1e-12:
            raise ValueError("ASHARE_RESIDUAL_CHINA_CONTEXT_CHANGED")
        china[key] = row["z"][6]
    context = {key: sectors.features(*key, points, value) for key, value in china.items()}
    if any(majority.available(r["z"]) and context[r["t"], r["u"]][1] != 1 for r in rows):
        raise ValueError("ASHARE_RESIDUAL_COMMON_ROWS_CHANGED")
    return [r | {"z": r["z"] + context[r["t"], r["u"]] + [fund_index(r["code"])]} for r in rows], proof | {
        "code_order": CODE_ORDER,
        "sector_hash": base.digest(points),
        "extra_history_requests": 0,
    }


def plan():
    source_receipt = base.read(base.ROOT / "ashr-cash-residual-source-v1/qualification-result.json")
    if source_receipt["status"] != "QUALIFIED_SAME_QUESTION_INPUT":
        raise ValueError("ASHARE_RESIDUAL_SOURCE_NOT_QUALIFIED")
    path = root() / "plan.json"
    if path.exists():
        value = base.read(path)
        if value["fingerprint"] != fingerprint() or value["calendar_hash"] != base.calendar()[1]:
            raise ValueError("ROUND_64_CODE_OR_CALENDAR_CHANGED")
        if any(
            base.digest(base.read(base.ROOT / "ashr-cash-residual-source-v1" / name)) != digest
            for name, digest in value["residual_source_hashes"].items()
        ):
            raise ValueError("ASHARE_RESIDUAL_SOURCE_CHANGED")
        return value
    active()
    ...
    if proposal["candidates"] != list(CANDIDATES) or proposal["budget"] != {
        "development_fits": 12,
        "current_fits": 3,
        "reproductions": 1,
        "new_history_requests": 0,
        "new_cost_cny": 0,
        "future_GETs_per_target_max": 0,
    }:
        raise ValueError("ASHARE_RESIDUAL_PROPOSAL_CHANGED")
    value = {
        ...
        "target": "Raw unit NAV T->adjacentCN U UP/NON_UP,flatseparate",
        "training_target": "raw_NAV_UP",
        "feature_indices": FEATURE_INDICES,
        "features": proposal["formula"],
        "normalization": "Fourweightedtraining scales,no centering",
        "recipe": proposal["recipe"],
        "training": "Last504mature jointavailable dates/group;same15windows asR50LR3",
        ...
        "input_hashes": majority.plan()["input_hashes"]
        | {
            "round-64/proposal-before-implementation.json": base.digest(proposal),
            **{
                f"ashare-etf-source-v1/{name}": base.digest(base.read(base.ROOT / "ashare-etf-source-v1" / name))
                for name in (
                    "qualification-plan.json",
                    "qualification-result.json",
                    "history.json",
                    "primary-price-context.json",
                )
            },
        },
        "residual_source_hashes": {
            name: base.digest(base.read(base.ROOT / "ashr-cash-residual-source-v1" / name))
            for name in ("cash-distributions.json", "qualification-plan.json", "qualification-result.json")
        },
        "current_fit_cutoff": majority.plan()["current_fit_cutoff"],
        "first_forward_target": FIRST_TARGET,
        "new_cost_cny": 0,
        "this_round_2026_scores_read": False,
        "source_timing": (
            "AnchorlastUSclosebeforeT15 through lastUSclosebeforeU0830,subtractknownCSI300Treturn;"
            "actualfutureHTTP+readbackreceipts"
        ),
        "live_budget": "ReuseR63 ASHR capture and full raw response; zero additional GETs",
        "limits": proposal["source_limit"],
    }
...
def fit(rows, name, cutoff):
    """四项市场输入使用与原三项模型相同的成熟训练题；不根据历史答案删除难预测基金。"""
    if name not in CANDIDATES:
        raise ValueError("ASHARE_RESIDUAL_RECIPE_INVALID")
    if name not in LEARNED:
        return None  # 无参数规则不拟合，不能将检查点文件数量当作训练次数。
    ...
    if len({r["u"] for r in chosen}) < 120:
        raise ValueError("ASHARE_RESIDUAL_TRAINING_SHORT")
    ...
    if len(Counter(y)) != 2 or min(Counter(y).values()) < 20:
        raise ValueError("ASHARE_RESIDUAL_CLASSES_INSUFFICIENT")
...
def batch_answers(values, name, trained):
    ...
    if indices:
        if name not in LEARNED:
            # 只判断现金调整后残差的方向，不搜索阈值，也不读取训练标签。
            scores = np.asarray([float(values[i][20] >= 0) for i in indices])
            kind = "FIXED_DIRECTION_RULE"
        else:
            head = trained["us_etf"]
            if (
                head is None
                ...
                raise ValueError("ASHARE_RESIDUAL_MODEL_SCHEMA_CHANGED")
        ...
        if not np.isfinite(scores).all() or min(scores) < 0 or max(scores) > 1:
            raise ValueError("ASHARE_RESIDUAL_SCORE_INVALID")
...
def fit_checkpoint(rows, name, cutoff, label):
    """沿用旧HK，另校验LR3对照训练题一致；本轮仅拟合新增四输入模型一次。"""
    ...
    if (
        ...
        raise ValueError("ASHARE_RESIDUAL_PRIOR_CHECKPOINT_CHANGED")
    ...
    if base.digest([r | {"z": r["z"][:20]} for r in shared]) != prior_head["fit_hash"]:
        raise ValueError("ASHARE_RESIDUAL_LR3_CONTROL_ROWS_CHANGED")
...
def train():
    ...
    for name, expected in p["input_hashes"].items():
        if base.digest(base.read(base.ROOT / name)) != expected:
            raise ValueError("ROUND_64_INPUT_CHANGED")
    rows, proofs = dataset()
    ...
    with threadpool_limits(limits=2):
        for q in range(1, 5):
            ...
            for group in groups:
                ...
                parent_candidate = majority.CANDIDATES[0]
                output[parent_candidate].extend(
                    base.read(majority.root() / f"folds/{q}-{group}-{parent_candidate}.json")
                )
        ...
        if len(expected) != 5670 or any(
            ...
            raise ValueError("ROUND_64_COMMON_EXAM_CHANGED")
...
def models():
    ...
    if (
        ...
        raise ValueError("ROUND_64_MODEL_OR_CODE_CHANGED")
...
def report():
    ...
    for path in (root() / "forward").glob("*/*.json"):
        ...
        if (
            ...
            raise ValueError("ROUND_64_PARENT_OR_MODEL_CHANGED")
        ...
        if value["sector_input_hash"] != base.digest(sector_input):
            raise ValueError("ASHARE_RESIDUAL_INPUT_CHANGED")
        ...
        if not np.allclose(
            ...
            raise ValueError("ROUND_64_VECTOR_CHANGED")
        ...
        if not runtime.answers_match(value["answers"], expected_answers):
            raise ValueError("ROUND_64_SAVED_ANSWER_CHANGED")
        good += 1

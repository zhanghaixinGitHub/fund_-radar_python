"""第二十五轮：直接学习一日涨跌，并固定比较自然方向权重和涨跌平衡权重。

保持原港股12项输入、资产组、504成熟日期和树结构；预测原始方向而非SPX错误。
...
from app.services import direction_1d_sprint_ensemble as ensemble
from app.services import direction_1d_sprint_expanding as previous_round
...
CANDIDATES = ("DIRECT_HK12_NAT504", "DIRECT_HK12_BAL504")
LEARNED = CANDIDATES
...
def fingerprint():
    ...
    for name in (
        "app/services/direction_1d_sprint_direct.py",
        "scripts/direction_1d_sprint_direct.py",
        "tests/test_direction_1d_sprint_direct.py",
    ):
...
def selected_features(z, name):
    if name not in CANDIDATES or len(z) != 12 or not np.isfinite(z).all():
        raise ValueError("DIRECT_INPUT_INVALID")
    return z
...
def plan():
    ...
    if path.exists():
        ...
        if value["fingerprint"] != fingerprint() or value["calendar_hash"] != base.calendar()[1]:
            raise ValueError("ROUND_25_CODE_OR_CALENDAR_CHANGED")
        return value
    active()
    if base.read(previous_round.root() / "result.json")["fingerprint"] != previous_round.fingerprint():
        raise ValueError("DIRECT_PREVIOUS_FINGERPRINT_CHANGED")
    parent, _ = hk.models()
    value = {
        ...
        "hypothesis": (
            "Direct raw direction learning may recover signals suppressed by baseline-error prediction "
            "and its fixed flip gate; compare two predeclared weighting policies"
        ),
        ...
        "training_dates": 504,
        "minimum_training": "120distinctdates, bothoriginaldirectionclasses>=20rows",
        ...
        "weights": {
            CANDIDATES[0]: "date/family natural direction prior",
            CANDIDATES[1]: "date/family then equal total weight for UP and NON_UP",
        },
        "normalization": "train-only weighted centered-square scale, same round12 implementation",
        "up_threshold": UP_THRESHOLD,
        "score_semantics": "Direct uncalibrated UP score; not baseline error probability",
        ...
        "selection": "two fixed weight policies; best2025same-question metric; both kept forward; no threshold search",
...
def fit(rows, name, cutoff):
    """直接学习相邻交易日的涨跌标签，保留自然先验或使用事先固定的两类平衡。

    两个候选使用完全相同的成熟样本。平衡权重只读取训练标签，不读开发题目；
    分数大于0.5预测上涨，等于0.5归入不涨，不能沿用纠错模型的0.55门槛。
    """
    if name not in CANDIDATES:
        raise ValueError("DIRECT_RECIPE_INVALID")
    chosen = training_rows(rows, cutoff)
    x = np.asarray([selected_features(r["z"], name) for r in chosen])
    y = np.asarray([r["y"] for r in chosen])
    weights = regression.weights(chosen) if name == CANDIDATES[0] else sparse.training_weights(chosen)
    ...
    return {
        ...
        "training_target": "DIRECT_NEXT_DAY_RAW_NAV_UP",
        "weight_policy": "NATURAL" if name == CANDIDATES[0] else "BALANCED_UP_NON_UP",
    }
...
def fit_checkpoint(rows, name, cutoff, label):
    ...
    if receipt.exists():
        ...
        if (
            ...
            raise ValueError("DIRECT_MODEL_CHECKPOINT_CHANGED")
        return joblib.load(path)
    if attempt.exists():
        raise ValueError("DIRECT_MODEL_PREVIOUS_FIT_INTERRUPTED")
    ...
    if base.digest(short) != expected:
        raise ValueError("DIRECT_CONTROL_TRAINING_ROWS_CHANGED")
...
def train():
    ...
    for name, expected in p["input_hashes"].items():
        if base.digest(base.read(base.ROOT / name)) != expected:
            raise ValueError("ROUND_25_INPUT_CHANGED")
    rows, proofs = dataset()
    ...
    with threadpool_limits(limits=2):
        for q in range(1, 5):
            ...
            for group in groups:
                ...
                output["MEAN3_ERR504"].extend(base.read(ensemble.root() / f"folds/{q}-{group}-MEAN3_ERR504.json"))
        ...
        if len(expected) != 5670 or any(
            ...
            raise ValueError("ROUND_25_COMMON_EXAM_CHANGED")
...
def models():
    ...
    if (
        ...
        raise ValueError("ROUND_25_MODEL_OR_CODE_CHANGED")

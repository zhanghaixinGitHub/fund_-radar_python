"""第95轮：T日IF期现价差追加到原三个市场输入，保持原标签、训练样本和对照。"""
...
from app.services import direction_1d_sprint_current_basis_data as data
from app.services import direction_1d_sprint_market_futures_basis_v2 as previous
...
CANDIDATES = ("SIGN3_CURRENT_IF_BASIS_LR4_504",)
CONTROLS = previous.CONTROLS
BRANCHES = CANDIDATES + CONTROLS
FIRST_TARGET = "2026-09-17"
PROPOSAL_HASH = "d8768e3c8a0ad3b3452dd9c6d5b0fd3de17813829dcd8082fcaa74e4c604e8e3"


def root():
    return base.ROOT / "round-95"
...
def fingerprint():
    code = dict(base.read(base.ROOT / "round-94" / "plan.json")["fingerprint"]["code"])
    for name, expected in code.items():
        require(core.sha(base.PROJECT / name) == expected, "OLD_CODE_CHANGED")
    for name in (
        "app/services/direction_1d_sprint_market_current_basis.py",
        "app/services/direction_1d_sprint_current_basis_forward.py",
        "scripts/direction_1d_sprint_market_current_basis.py",
        "tests/test_direction_1d_sprint_market_current_basis.py",
        "app/services/direction_1d_sprint_current_basis_data.py",
        "tests/test_direction_1d_sprint_current_basis_data.py",
    ):
...
def plan():
    ...
    require(proposal["r94_result_hash"] == base.digest(base.read(previous.root() / "result.json")), "SOURCE_CHANGED")
    require(
...
def preflight():
    from app.services import direction_1d_sprint_current_basis_forward as forward

"""第113轮复用 CORE3 输入，复用IF/UUP/HYG/IEF实际响应，不调用已退役的旧实验链。"""
...
from app.services import direction_1d_sprint_credit_pair_forward as prior_forward
from app.services import direction_1d_sprint_joint_capacity_live as live
from app.services import direction_1d_sprint_joint_capacity_runtime as runtime
from app.services import direction_1d_sprint_market_credit_pair as prior_model
from app.services import direction_1d_sprint_market_joint_capacity as model
...
def validate(value, receipt, parent, source, manifest, bundle, extra, history):
    ...
    model.require(value["r112_forecast_hash"] == base.digest(extra), "R112_PARENT_CHANGED")
    ...
    model.require(
        ...
        "R112_PARENT_READBACK_LATE_OR_CHANGED",
    )
    model.require(
        value["joint_source_hash"] == base.digest(history) and datetime.fromisoformat(history["at"]) <= created,
        "JOINT_SOURCE_CHANGED_OR_LATE",
    )
    model.require(
        history == live.load_live(parent["t"], parent["u"], manifest["plan_hash"]), "ACTUAL_JOINT_SOURCE_CHANGED"
    )
    model.require(
        live.in_window(parent["t"], parent["u"], datetime.fromisoformat(history["at"])),
        "JOINT_SOURCE_NOT_ACTUALLY_CAPTURED_IN_WINDOW",
    )
...
def run():
    ...
    for key, p in parents.items():
        ...
        value = {k: p[k] for k in ("code", "family", "group", "t", "u")} | {
            ...
            "r112_forecast_hash": base.digest(extra),
...
def combined_answers(parent, extra, value):
    """明确合并全部32个同题分支，避免新增中间模型后递归层级漂移丢掉旧对照。"""
    combined = dict(parent["answers"])
    for number in (92, 94, 95, 96, 97, 98, 99, 100, 101, 102, 103, 104, 105, 106, 107, 108, 109, 110, 111):
    ...
    model.require(len(combined) == 32, "FORWARD_BRANCH_COUNT_CHANGED")
    return combined

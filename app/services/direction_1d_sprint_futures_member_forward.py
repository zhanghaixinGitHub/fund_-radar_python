"""第120轮复用 CORE3 输入，独立读取实际IF主力合约公开会员排名，不调用已退役的旧实验链。"""
...
from app.services import direction_1d_sprint_futures_member_runtime as runtime
from app.services import direction_1d_sprint_market_futures_member as model
from app.services import direction_1d_sprint_market_stock_exchange_return as prior_model
from app.services import direction_1d_sprint_stock_exchange_return_forward as prior_forward
...
def validate(value, receipt, parent, source, manifest, bundle, extra, history):
    ...
    model.require(value["r119_forecast_hash"] == base.digest(extra), "R119_PARENT_CHANGED")
    ...
    model.require(
        ...
        "R119_PARENT_READBACK_LATE_OR_CHANGED",
    )
    model.require(
        value["futures_member_source_hash"] == base.digest(history)
        and datetime.fromisoformat(history["at"]) <= created,
        "FUTURES_MEMBER_SOURCE_CHANGED_OR_LATE",
    )
    model.require(
        history == live.load_live(parent["t"], parent["u"], manifest["plan_hash"]),
        "ACTUAL_FUTURES_MEMBER_SOURCE_CHANGED",
    )
    model.require(
        live.in_window(parent["t"], parent["u"], datetime.fromisoformat(history["at"])),
        "FUTURES_MEMBER_SOURCE_NOT_ACTUALLY_CAPTURED_IN_WINDOW",
    )
...
def run():
    # 先复用R95原预算取得实际主力映射，再读取同合约排名；父链复用同一快照。
    rp = runtime.plan()
    ...
    if today >= model.FIRST_TARGET and today in days and days.index(today) > 0:
        ...
        live.quote_live.capture(t, today, base.digest(base.read(base.ROOT / "round-95/plan.json")))
        if (live.quote_live.live_root() / today / "snapshot.json").exists():
    ...
    for key, p in parents.items():
        ...
        value = {k: p[k] for k in ("code", "family", "group", "t", "u")} | {
            ...
            "r119_forecast_hash": base.digest(extra),
            "futures_member_source_hash": base.digest(history[p["u"]]),
        }
...
def combined_answers(parent, extra, value):
    """明确合并全部39个同题分支，避免新增中间模型后递归层级漂移丢掉旧对照。"""
    ...
    model.require(len(combined) == 39, "FORWARD_BRANCH_COUNT_CHANGED")
    return combined

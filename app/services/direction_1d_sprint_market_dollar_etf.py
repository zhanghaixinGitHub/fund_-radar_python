"""第111轮：新增美国会话美元相关ETF同日对数收益，保持原标签、训练样本和对照。"""
...
CANDIDATES = ("SIGN3_UUP_INTRADAY4_LR504",)
CONTROLS = previous.CONTROLS
BRANCHES = CANDIDATES + CONTROLS
FIRST_TARGET = "2026-09-17"
PROPOSAL_HASH = "6ad3186b67d6f7037ec9a283f76b02e8c59120e98bd1397885ceaac9c7a4b741"


def root():
    return base.ROOT / "round-111"


def require(condition, reason):
    if not condition:
        raise ValueError("UUP_MODEL_" + reason)
...
def fingerprint():
    ...
    for name, expected in source["code_hashes"].items():
        ...
        code[name] = expected
    for name in (
        "app/services/direction_1d_sprint_market_dollar_etf.py",
        "scripts/direction_1d_sprint_market_dollar_etf.py",
        "tests/test_direction_1d_sprint_market_dollar_etf.py",
    ):
...
def plan():
    ...
    require(base.digest(source) == proposal["source_qualification_hash"], "DOLLAR_SOURCE_CHANGED")
    ...
    require(
        ...
        and all(x["dates"] == 504 and x["dollar_available"] == x["questions"] for x in coverage["training_windows"]),
...
def reconstruct_dataset():
    """按共享市场日期缓存美元相关ETF同日收益，避免为同日30基金重复解析整份交易日历。"""
    ...
    for row in rows:
        key = row["t"], row["u"]
        if key not in cache:
            cache[key] = data.extend({}, *key, h["points"])["dollar_etf"]
        extra.append(row | {"market": row["market"] | {"dollar_etf": cache[key]}})
    require([data.original_row(r) for r in extra] == rows, "ORIGINAL_ROWS_CHANGED")
    proof = proof | {"dollar_etf_history_hash": base.digest(h)}
    return extra, proof
...
def fit(rows, name, cutoff):
    ...
    require(
        ...
        and all(r["mature"] < cutoff and r["u"] < cutoff and r["market"]["dollar_etf"]["available"] for r in chosen),
    ...
    return {
        ...
        "max_feature_date": max(d for r in chosen for d in r["market"]["dollar_etf"]["new_us_dates"]),
        "training_feature_mean_not_subtracted": mean,
        "masked_training_rows": sum(not r["market"]["dollar_etf"]["available"] for r in chosen),
...
def candidate_answers(values, name, head, fallback):
    ...
    indices = [i for i, v in enumerate(values) if v["available"] and v["dollar_etf"]["available"]]
    if indices:

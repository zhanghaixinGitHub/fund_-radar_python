"""第114轮：T日沪深涨跌范围与隔夜市场方向的固定交互，保持原标签、训练样本和对照。"""
...
from app.services import direction_1d_sprint_market_joint_capacity as previous
...
from app.services import direction_1d_sprint_stock_breadth_data as data

original, active, reference = previous.original, previous.active, previous.reference
CANDIDATES = ("SIGN3_STOCK_BREADTH_INTERACT9_LR504",)
CONTROLS = previous.CONTROLS
BRANCHES = CANDIDATES + CONTROLS
FIRST_TARGET = "2026-09-17"
INTENT_HASH = "5facb9b2cb91de0e5301c4afa52c9d933f323532372ac419809c4e73777b062a"


def root():
    return base.ROOT / "round-114"


def require(condition, reason):
    if not condition:
        raise ValueError("STOCK_BREADTH_" + reason)


def fingerprint():
    code = dict(base.read(base.ROOT / "round-113" / "plan.json")["fingerprint"]["code"])
    ...
    source_code = dict(source_plan["code"])
    source_code.update(base.read(data.root() / "plan.json")["code"])
    for name, expected in source_code.items():
        ...
        code[name] = expected
    for name in (
        "app/services/direction_1d_sprint_market_stock_interaction.py",
        "scripts/direction_1d_sprint_market_stock_interaction.py",
        "tests/test_direction_1d_sprint_market_stock_interaction.py",
    ):
...
def plan():
    # 配方及来源核验规则在补数完成前固定；到齐后只补绑定实际输入摘要，不再选择配方。
    ...
    require(
        proposal["research_intent_hash"] == INTENT_HASH and all(proposal[k] == v for k, v in intent.items()),
        "PROPOSAL_CHANGED",
    )
    proposal_hash = base.digest(proposal)
    require(proposal["design_sha256"] == core.sha(root() / "design-before-implementation.md"), "DESIGN_CHANGED")
    require(proposal["r113_result_hash"] == base.digest(base.read(previous.root() / "result.json")), "SOURCE_CHANGED")
    require(
    ...
    require(base.digest(source) == proposal["source_qualification_hash"], "STOCK_SOURCE_CHANGED")
    require(
        base.digest(base.read(data.root() / "qualification-plan.json")) == intent["qualification_plan_hash"],
        "QUALIFICATION_PLAN_CHANGED",
    )
    coverage = base.read(root() / "feature-feasibility.json")
    require(
        coverage["source_history_hash"] == source["history_hash"]
        and all(x["stock_available"] == x["questions"] for x in coverage["exam"].values())
...
def reconstruct_dataset():
    """按共享市场日期缓存新增沪深股票分布，避免为同日30基金重复解析整份交易日历。"""
    ...
    for row in rows:
        key = row["t"], row["u"]
        if key not in cache:
            cache[key] = data.extend({}, *key, h["snapshot"]["rows"])["stock_breadth"]
        extra.append(row | {"market": row["market"] | {"stock_breadth": cache[key]}})
    require([data.original_row(r) for r in extra] == rows, "ORIGINAL_ROWS_CHANGED")
    proof = proof | {"stock_breadth_history_hash": base.digest(h)}
    return extra, proof
...
def vector(value):
    ...
    feature = value["stock_breadth"]
    require(feature["available"], "STOCK_BREADTH_MISSING")
    ...
    require(
        ...
        and abs(extra[1]) <= 20
        and 0 <= extra[2] <= 40,
        "STOCK_BREADTH_INVALID",
    )
    # 只新增三个预先指定的交互：T日上涨减下跌比例 × 各隔夜市场方向。
    # 全球市场的同向信号可随国内普涨/普跌状态改变强弱；不借用U日国内涨跌。
    interaction = (np.sign(np.asarray(x, dtype=float)) * extra[0]).tolist()
    return [*x, *extra, *interaction]
...
def fit(rows, name, cutoff):
    ...
    require(
        ...
        and all(r["mature"] < cutoff and r["u"] < cutoff and r["market"]["stock_breadth"]["available"] for r in chosen),
    ...
    return {
        ...
        "max_feature_date": max(r["market"]["stock_breadth"]["date"] for r in chosen),
...
def validate_head(head, name, cutoff=None):
    ...
    require(
        ...
        and model.n_features_in_ == 9
        and model.coef_.shape == (1, 9)
...
def candidate_answers(values, name, head, fallback):
    ...
    indices = [i for i, v in enumerate(values) if v["available"] and v["stock_breadth"]["available"]]
    if indices:

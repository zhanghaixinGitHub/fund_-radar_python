"""第103轮：T日不同股指期货相对日内变化，保持原标签、训练样本和对照。"""
...
from app.services import direction_1d_sprint_futures_style_data as data
from app.services import direction_1d_sprint_market_futures_curve as previous
...
CANDIDATES = ("SIGN3_IF_STYLE7_LR504",)
CONTROLS = previous.CONTROLS
BRANCHES = CANDIDATES + CONTROLS
FIRST_TARGET = "2026-09-17"
PROPOSAL_HASH = "a1e9d25b0cabd453825a45dec774cafc003afdbce412bab0b23b77f41ef11248"


def root():
    return base.ROOT / "round-103"


def require(condition, reason):
    if not condition:
        raise ValueError("IF_STYLE_" + reason)


def fingerprint():
    code = dict(base.read(base.ROOT / "round-102" / "plan.json")["fingerprint"]["code"])
    for name, expected in code.items():
        require(core.sha(base.PROJECT / name) == expected, "OLD_CODE_CHANGED")
    for name in (
        "app/services/direction_1d_sprint_market_futures_style.py",
        "app/services/direction_1d_sprint_futures_style_forward.py",
        "scripts/direction_1d_sprint_market_futures_style.py",
        "tests/test_direction_1d_sprint_market_futures_style.py",
        "app/services/direction_1d_sprint_futures_style_data.py",
        "tests/test_direction_1d_sprint_futures_style_data.py",
        "app/integrations/tushare_sprint_futures_style.py",
        "tests/test_tushare_sprint_futures_style.py",
        ".local-runs/direction-1d-sprint-20260914/china-futures-style-feasibility-v1/probe.py",
        ".local-runs/direction-1d-sprint-20260914/china-futures-style-history-v1/acquire.py",
        ".local-runs/direction-1d-sprint-20260914/china-futures-style-history-v1/qualify.py",
    ):
...
def plan():
    ...
    require(proposal["r102_result_hash"] == base.digest(base.read(previous.root() / "result.json")), "SOURCE_CHANGED")
    require(
...
def reconstruct_dataset():
    ...
    for row in rows:
        key = row["t"], row["u"]
        if key not in cache:
            cache[key] = data.extend({}, *key, h["snapshot"]["rows"])["futures_style"]
        extra.append(row | {"market": row["market"] | {"futures_style": cache[key]}})
    require([data.original_row(r) for r in extra] == rows, "ORIGINAL_ROWS_CHANGED")
    proof = proof | {"futures_style_history_hash": base.digest(h)}
    return extra, proof
...
def vector(value):
    ...
    feature = value["futures_style"]
    require(feature["available"], "INTRADAY_MISSING")
    extra = [
        feature[k]
        for k in ("intraday_return_pct", "close_location", "ic_relative_intraday_pct", "ih_relative_intraday_pct")
    ]
    require(
...
def fit(rows, name, cutoff):
    ...
    require(
        ...
        and all(r["mature"] < cutoff and r["u"] < cutoff and r["market"]["futures_style"]["available"] for r in chosen),
    ...
    return {
        ...
        "max_feature_date": max(r["market"]["futures_style"]["date"] for r in chosen),
...
def candidate_answers(values, name, head, fallback):
    ...
    indices = [i for i, v in enumerate(values) if v["available"] and v["futures_style"]["available"]]
    if indices:
...
def preflight():
    from app.services import direction_1d_sprint_futures_style_forward as forward

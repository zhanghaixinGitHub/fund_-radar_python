"""第122轮：资金流与基金过去成熟方向的个体反应，保持原标签、训练样本和对照。"""
...
from app.services import direction_1d_sprint_market_stock_moneyflow_v2 as previous
...
CANDIDATES = ("SIGN3_FLOW_PERSONAL10_LR504",)
CONTROLS = previous.CONTROLS
BRANCHES = CANDIDATES + CONTROLS
FIRST_TARGET = "2026-09-17"
INTENT_HASH = "65e22e8309a42686c9a6e080533bc7eb98d537693d5585cede813e475d8280da"
PROPOSAL_HASH = "2ed7a85b0b43c2d506afd94203252064020392584c308f474e3b97e2ecf970c0"


def root():
    return base.ROOT / "round-122"


def require(condition, reason):
    if not condition:
        raise ValueError("FUND_MONEYFLOW_" + reason)


def fingerprint():
    code = dict(base.read(base.ROOT / "round-121" / "plan.json")["fingerprint"]["code"])
    for name, expected in code.items():
        require(core.sha(base.PROJECT / name) == expected, "OLD_CODE_CHANGED")
    source_plan = base.read(data.root() / "context-plan.json")
    for name, expected in source_plan["code_hashes"].items():
        ...
        code[name] = expected
    for name in (
        "app/services/direction_1d_sprint_market_fund_moneyflow.py",
        "scripts/direction_1d_sprint_market_fund_moneyflow.py",
        "tests/test_direction_1d_sprint_market_fund_moneyflow.py",
        "app/services/direction_1d_sprint_fund_moneyflow_data.py",
        "tests/test_direction_1d_sprint_fund_moneyflow_data.py",
    ):
...
def plan():
    intent = base.read(root() / "research-intent.json")
    require(base.digest(intent) == INTENT_HASH, "RESEARCH_INTENT_CHANGED")
    proposal = base.read(root() / "proposal-before-implementation.json")
    require(
        base.digest(proposal) == PROPOSAL_HASH and proposal["research_intent_hash"] == INTENT_HASH, "PROPOSAL_CHANGED"
    )
    require(proposal["design_sha256"] == core.sha(root() / "design-before-implementation.md"), "DESIGN_CHANGED")
    require(proposal["r121_result_hash"] == base.digest(base.read(previous.root() / "result.json")), "SOURCE_CHANGED")
    require(
    ...
    source = base.read(data.root() / "context-feasibility.json")
    data.reference()
    require(
        base.digest(base.read(data.root() / "context-reference.json")) == proposal["context_reference_hash"],
        "CONTEXT_REFERENCE_CHANGED",
    )
    require(base.digest(source) == proposal["source_qualification_hash"], "MONEYFLOW_SOURCE_CHANGED")
    coverage = base.read(root() / "feature-feasibility.json")
    require(
        coverage["source_history_hash"] == source["snapshot_hash"]
        and all(x["moneyflow_available"] == x["questions"] for x in coverage["exam"].values())
...
def vector(value):
    ...
    flow = value["stock_moneyflow"]
    personal = value["fund_moneyflow"]
    require(flow["available"] and personal["available"], "FUND_MONEYFLOW_MISSING")
    extra = [flow[k] for k in data.KEYS]
    require(all(type(v) in (int, float) and np.isfinite(v) and abs(v) <= 1 for v in extra), "FUND_MONEYFLOW_INVALID")
    data.validate_feature(personal, flow)
    require(len(personal["extra"]) == 5, "INDIVIDUAL_FEATURE_COUNT")
    return [*x, *extra, *personal["extra"]]
...
def fit(rows, name, cutoff):
    ...
    require(
        ...
        and all(
            r["mature"] < cutoff
            and r["u"] < cutoff
            and r["market"]["stock_moneyflow"]["available"]
            and r["market"]["fund_moneyflow"]["available"]
            for r in chosen
        ),
    ...
    return {
        ...
        "max_feature_date": max(r["market"]["stock_moneyflow"]["date"] for r in chosen),
        "max_context_cutoff": max(r["market"]["fund_moneyflow"]["cutoff"] for r in chosen),
        "max_context_mature_date": max(
            (
                r["market"]["fund_moneyflow"]["prior"]["max_mature"]
                for r in chosen
                if r["market"]["fund_moneyflow"]["prior"]["count"]
            ),
            default=None,
        ),
...
def validate_head(head, name, cutoff=None):
    ...
    require(
        ...
        and model.n_features_in_ == 10
        and model.coef_.shape == (1, 10)
    ...
    require(
        head["max_context_cutoff"] == head["fit_end"]
        and (head["max_context_mature_date"] is None or head["max_context_mature_date"] < head["fit_end"]),
        "HEAD_CONTEXT_TIME_INVALID",
    )
...
def candidate_answers(values, name, head, fallback):
    ...
    indices = [
        ...
        if v["available"] and v["stock_moneyflow"]["available"] and v["fund_moneyflow"]["available"]
    ]
    if indices:

"""第100轮：T日期货日内方向与期权比例变化联合，保持原标签、训练样本和对照。"""
...
from app.services import direction_1d_sprint_market_option_change as previous
from app.services import direction_1d_sprint_option_futures_data as data
...
CANDIDATES = ("SIGN3_IF_IO_CHANGE7_LR504",)
CONTROLS = previous.CONTROLS
BRANCHES = CANDIDATES + CONTROLS
FIRST_TARGET = "2026-09-17"
PROPOSAL_HASH = "1b26b1264039b71e7cc360c64992de1cd406deef298514958c95756ad19682db"


def root():
    return base.ROOT / "round-100"


def require(condition, reason):
    if not condition:
        raise ValueError("IO_FUTURES_" + reason)


def fingerprint():
    code = dict(base.read(base.ROOT / "round-99" / "plan.json")["fingerprint"]["code"])
    for name, expected in code.items():
        require(core.sha(base.PROJECT / name) == expected, "OLD_CODE_CHANGED")
    for name in (
        "app/services/direction_1d_sprint_market_option_futures.py",
        "app/services/direction_1d_sprint_option_futures_forward.py",
        "scripts/direction_1d_sprint_market_option_futures.py",
        "tests/test_direction_1d_sprint_market_option_futures.py",
        "app/services/direction_1d_sprint_option_futures_data.py",
        "tests/test_direction_1d_sprint_option_futures_data.py",
    ):
...
def plan():
    ...
    require(proposal["r99_result_hash"] == base.digest(base.read(previous.root() / "result.json")), "SOURCE_CHANGED")
    require(
...
def reconstruct_dataset():
    ...
    for row in rows:
        key = row["t"], row["u"]
        if key not in cache:
            cache[key] = data.extend({}, *key, h["snapshot"]["rows"])["option_futures"]
        extra.append(row | {"market": row["market"] | {"option_futures": cache[key]}})
    require([data.original_row(r) for r in extra] == rows, "ORIGINAL_ROWS_CHANGED")
    proof = proof | {"option_futures_history_hash": base.digest(h)}
    return extra, proof
...
def vector(value):
    ...
    feature = value["option_futures"]
    require(feature["available"], "INTRADAY_MISSING")
    extra = [
        feature[k]
        for k in ("intraday_return_pct", "close_location", "log_put_call_volume_change", "log_put_call_oi_change")
    ]
    require(
...
def fit(rows, name, cutoff):
    ...
    require(
        ...
        and all(
            r["mature"] < cutoff and r["u"] < cutoff and r["market"]["option_futures"]["available"] for r in chosen
        ),
    ...
    return {
        ...
        "max_feature_date": max(r["market"]["option_futures"]["date"] for r in chosen),
...
def candidate_answers(values, name, head, fallback):
    ...
    indices = [i for i, v in enumerate(values) if v["available"] and v["option_futures"]["available"]]
    if indices:
...
def preflight():
    from app.services import direction_1d_sprint_option_futures_forward as forward

...
from app.services import direction_1d_sprint_sector as sector
...
def market_returns(row):
    """读取第37轮固定五行业相对中证500的百分数收益，不还原宽基或港股收益。"""
    z = row["z"]
    if len(z) != 17 or not np.isfinite(z).all():
        raise ValueError("JOINT_MARKET_INPUT_INVALID")
    return np.asarray(z[12:17], dtype=float)


def context(rows, cutoff):
    """只取同基金、目标和成熟日期均早于截止日的最近126条记录，不读取y。

    基金收益先减中证500收益。使用五行业总体协方差矩阵C，求解(C+0.25I)beta=cov，
    再乘n/(n+126)并限幅±3；联合求解用于处理行业信号之间的相关性。
    未成熟记录先排除，其输入即使缺失或修订也不得影响当前上下文。
    """
    date.fromisoformat(cutoff)
    if len({r["code"] for r in rows}) > 1:
        raise ValueError("JOINT_CONTEXT_MULTIPLE_FUNDS")
    ...
    if len({r["u"] for r in eligible}) != len(eligible):
        raise ValueError("JOINT_CONTEXT_DUPLICATE_DATE")
    markets = np.asarray([market_returns(r) for r in eligible])
    funds = np.asarray([(r["x"][7] - r["x"][23]) * 100 for r in eligible])
    if not np.isfinite(markets).all() or not np.isfinite(funds).all():
        raise ValueError("JOINT_CONTEXT_NONFINITE")
    centered = markets - markets.mean(axis=0)
    covariance = (centered * (funds - funds.mean())[:, None]).mean(axis=0)
    market_covariance = centered.T @ centered / len(eligible)
    beta = np.clip(
        np.linalg.solve(market_covariance + 0.25 * np.eye(5), covariance) * len(eligible) / (len(eligible) + 126), -3, 3
    )
    return {
        "beta": beta.tolist(),
        "count": len(eligible),
...
def validate_context(prior, cutoff):
    ...
    if (
        ...
        raise ValueError("JOINT_CONTEXT_VALUES_INVALID")
    if count == 0:
        if beta != [0.0] * 5 or prior["max_u"] is not None or prior["max_mature"] is not None:
            raise ValueError("JOINT_EMPTY_CONTEXT_INVALID")
    else:
        for name in ("max_u", "max_mature"):
            day = prior[name]
            if not isinstance(day, str) or str(date.fromisoformat(day)) != day or day >= cutoff:
                raise ValueError("JOINT_CONTEXT_NOT_MATURE")
...
def reference():
    ...
    if (
        ...
        or proposal["source_hashes"] != sector.plan()["input_hashes"]
    ):
        raise ValueError("JOINT_REFERENCE_CHANGED")
    for name, expected in proposal["source_hashes"].items():
        if base.digest(base.read(base.ROOT / name)) != expected:
            raise ValueError("JOINT_HISTORY_INPUT_CHANGED")
...
def dataset():
    ...
    for row in rows:
        ...
        output.append(row | {"z": row["z"][:12] + extra})
    ...
    if base.digest(derived) != expected["feature_hash"] or actual_contexts != snapshot["contexts"]:
        raise ValueError("JOINT_PROTOTYPE_RESULT_CHANGED")
    return output, proof | {
        "sector_joint_feature_hash": base.digest(derived),

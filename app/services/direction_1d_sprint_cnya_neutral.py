"""第四十五轮：以SPX与CNYA口径差的零截距关系预测下一日原始净值方向。

估计器只读取两项输入；完整15项仍保存供原始数据与时点核验，不另取供应商数据。
直接学习UP标签并固定0.5分类边界，区别于冻结的0.55纠错分支，分数不作正式概率。
"""
...
from app.services import direction_1d_sprint_cnya_direct as previous_round
...
CANDIDATES = ("CNYA_SPX_LR2_ZERO504",)
LEARNED = CANDIDATES
...
def fingerprint():
    ...
    for name in (
        "app/services/direction_1d_sprint_cnya_neutral.py",
        "scripts/direction_1d_sprint_cnya_neutral.py",
        "tests/test_direction_1d_sprint_cnya_neutral.py",
        ".local-runs/direction-1d-sprint-20260914/round-45/feature-feasibility.py",
    ):
...
def vector(x, t, u, markets, points):
    return hk.vector(x, t, u, markets) + cnya_data.features(t, u, points)


def live_vector(source, original, markets, points):
    """复用父答案的净值和港股输入；CNYA只取目标日08:30前的两个美股现金收盘，真实响应必须在目标日截止前取得。"""
...
def selected_features(z, name):
    """完整输入验证后，仅投影SPX百分比、CNYA口径差百分比两个字段。"""
    if name not in CANDIDATES or len(z) != 15 or not np.isfinite(z).all():
        raise ValueError("CNYA_DIRECT_MODEL_INPUT_INVALID")
    return [z[i] for i in (0, 12)]


def dataset():
    """继承同一题目和成熟标签，校验投影原型，没有按新模型表现增删题目。"""
    ...
    projected = [{"code": r["code"], "u": r["u"], "selected": selected_features(r["z"], CANDIDATES[0])} for r in rows]
    if (
        feasible["proposal_hash"] != base.digest(proposal)
        or proposal["parent_plan_hash"] != base.digest(previous_round.plan())
        or feasible["full_rows_hash"] != base.digest(rows)
        or feasible["selected_feature_hash"] != base.digest(projected)
        or hashlib.sha256((root() / "feature-feasibility.py").read_bytes()).hexdigest() != proposal["script_sha256"]
    ):
        raise ValueError("CNYA_DIRECT_PROTOTYPE_CHANGED")
    return rows, proof
...
def plan():
    ...
    if path.exists():
        ...
        if value["fingerprint"] != fingerprint() or value["calendar_hash"] != base.calendar()[1]:
            raise ValueError("ROUND_45_CODE_OR_CALENDAR_CHANGED")
        return value
    active()
    if base.read(previous_round.root() / "result.json")["fingerprint"] != previous_round.fingerprint():
        raise ValueError("CNYA_PREVIOUS_FINGERPRINT_CHANGED")
    parent, _ = hk.models()
    cnya_data.history()
    value = {
        ...
        "hypothesis": (
            "Remove raw bias after round44; neutral zero-move boundary may avoid historical directional-prior drift"
        ),
        ...
        "feature_order": (
            "Full15 evidence retained; estimator selects indices0,12: SPXpercent,CNYAgapPercent; no constant indicator"
        ),
        "cnya_source": cnya_data.SOURCE,
        "cnya_url": cnya_data.URL,
        "source_timing": "LatestcompletedUSdates; historicalfirstpublicationunverified; actualarrival/readback<U08:30",
        "future_evidence": "Raw XML/request/receive/required2rows/reasons/hashes; input/predictionreadback<U08:30",
        "missingness": "-- explicitmissing, zero invalid; unavailable. Absentrow/negative/NaN still fail",
        "live_budget": {"reuse_validated_round43_input": True, "max_additional_requests": 0},
        "logistic_regression": {
            "C": 0.1,
            "max_iter": 1500,
            "seed": 17,
            ...
            "penalty": "default L2",
            "class_balance": False,
            "convergence_warning": "fail without retry",
            "fit_intercept": False,
        },
        "weights": "Natural date/family direction prior; no class balance",
        "normalization": "Divide train-only centered-square scale without subtracting mean; twoinputs and no intercept",
        "up_threshold": UP_THRESHOLD,
        "score_semantics": "Direct uncalibrated UP score, UP if >=.5; no baseline error gate",
        "training_dates": 504,
        ...
        | {
            ...
            for name in (
                "cnya-data-feasibility-v1/plan.json",
                "cnya-data-feasibility-v1/history.json",
                "cnya-data-feasibility-v1/result.json",
                "cnya-data-feasibility-v1/capture.json",
                "cnya-data-feasibility-v1/request.json",
                "round-45/proposal-before-training.json",
                "round-45/input-feasibility.json",
            )
        },
...
def fit(rows, name, cutoff):
    """双输入零截距直接学习原始UP，保留原成熟样本和自然权重；不收敛时明确失败，不自动调参。"""
    if name not in CANDIDATES:
        raise ValueError("CNYA_MODEL_RECIPE_INVALID")
    ...
    if len(Counter(y)) != 2 or min(Counter(y).values()) < 20:
        raise ValueError("CNYA_DIRECT_UP_LABELS_INSUFFICIENT")
    weights = regression.weights(chosen)
    # 仅估计训练内尺度，不减均值；否则即使关闭截距也会重新引入原始单位下的常量偏移。
...
def batch_answers(values, name, trained):
    ...
    if not np.isfinite(x).all():
        raise ValueError("CNYA_MODEL_NORMALIZED_INPUT_INVALID")
    scores = trained["model"].predict_proba(x)[:, 1]
    if not np.isfinite(scores).all() or min(scores) < 0 or max(scores) > 1:
        raise ValueError("CNYA_MODEL_SCORE_INVALID")
    result = []
    for z, score in zip(values, scores, strict=True):
        baseline, prediction = int(z[0] >= 0), int(score >= UP_THRESHOLD)
        result.append(
            {
...
def fit_checkpoint(rows, name, cutoff, label):
    ...
    if receipt.exists():
        ...
        if (
            ...
            raise ValueError("CNYA_MODEL_CHECKPOINT_CHANGED")
        return joblib.load(path)
    if attempt.exists():
        raise ValueError("CNYA_MODEL_PREVIOUS_FIT_INTERRUPTED")
    ...
    if base.digest([r | {"z": r["z"][:12]} for r in chosen]) != expected:
        raise ValueError("CNYA_CONTROL_TRAINING_ROWS_CHANGED")
...
def train():
    ...
    for name, expected in p["input_hashes"].items():
        if base.digest(base.read(base.ROOT / name)) != expected:
            raise ValueError("ROUND_45_INPUT_CHANGED")
    rows, proofs = dataset()
    ...
    with threadpool_limits(limits=2):
        ...
        if len(expected) != 5670 or any(
            ...
            raise ValueError("ROUND_45_COMMON_EXAM_CHANGED")
...
def models():
    ...
    if (
        ...
        raise ValueError("ROUND_45_MODEL_OR_CODE_CHANGED")
...
def tick():
    ...
    cnya_input = cnya_data.capture(base.now())
    ...
    with threadpool_limits(limits=2):
        for path in paths:
            ...
            z = live_vector(source, original, hk_input["rows"], cnya_input["rows"])
            ...
            value = {
                ...
                "cnya_input_hash": base.digest(cnya_input),
...
def report():
    ...
    for path in (root() / "forward").glob("*/*.json"):
        ...
        if (
            ...
            raise ValueError("ROUND_45_PARENT_OR_MODEL_CHANGED")
        hk_input = hk_live.load(value["u"])
        cnya_input = cnya_data.load(value["u"])
        if value["cnya_input_hash"] != base.digest(cnya_input):
            raise ValueError("CNYA_MODEL_LIVE_INPUT_CHANGED")
        if value["hk_input_hash"] != base.digest(hk_input):
            raise ValueError("CNYA_MODEL_LIVE_INPUT_CHANGED")
        if not np.allclose(
            value["z"], live_vector(source, original, hk_input["rows"], cnya_input["rows"]), rtol=0, atol=1e-12
        ):
            raise ValueError("ROUND_45_VECTOR_CHANGED")
        good += 1

"""第三十六轮：加入基金自身对四个市场的历史敏感度，预测下一交易日方向。
...
from app.services import direction_1d_sprint_linear_error as previous_round
...
from app.services import direction_1d_sprint_sensitivity_context as context_source
...
CANDIDATES = ("SENSITIVITY_HK21_ERR504",)
LEARNED = CANDIDATES
...
def fingerprint():
    ...
    for name in (
        "app/services/direction_1d_sprint_sensitivity.py",
        "scripts/direction_1d_sprint_sensitivity.py",
        "tests/test_direction_1d_sprint_sensitivity.py",
        "app/services/direction_1d_sprint_sensitivity_context.py",
        "tests/test_direction_1d_sprint_sensitivity_context.py",
        ".local-runs/direction-1d-sprint-20260914/round-36/context-feasibility.py",
    ):
        value["code"][name] = hashlib.sha256((base.PROJECT / name).read_bytes()).hexdigest()
    return value


def vector(x, t, u, markets, prior):
    z = hk.vector(x, t, u, markets)
    return z + context_source.extra_features({"z": z, "u": u}, prior)


def live_vector(source, original, markets, snapshot):
    """按原基金代码查找冻结上下文；拒绝未来截止日或找不到该基金的快照。"""
    sequence.live_vector(source, original)
    if snapshot["cutoff"] > original["u"]:
        raise ValueError("SENSITIVITY_CONTEXT_FUTURE_CUTOFF")
    if original["code"] not in snapshot["contexts"]:
        raise ValueError("SENSITIVITY_CONTEXT_FUND_MISSING")
    prior = snapshot["contexts"][original["code"]]
    context_source.validate_context(prior, snapshot["cutoff"])
    return vector(source["x"], original["base"], original["u"], markets, prior)
...
def freeze_context(p):
    ...
    if p["current_fit_cutoff"] != proposal["current_context_cutoff"]:
        raise ValueError("SENSITIVITY_CONTEXT_CUTOFF_CHANGED")
    wanted = {
    ...
    if path.exists():
        ...
        if any(saved[key] != value for key, value in wanted.items()):
            raise ValueError("SENSITIVITY_CURRENT_CONTEXT_CHANGED")
        return saved
...
def current_contexts():
    ...
    if (
        ...
        or snapshot["reference_hash"] != p["input_hashes"]["round-36/current-context-feasibility.json"]
...
def selected_features(z, name):
    if name not in CANDIDATES or len(z) != 21 or not np.isfinite(z).all():
        raise ValueError("SENSITIVITY_INPUT_INVALID")
    return z
...
def plan():
    ...
    if path.exists():
        ...
        if value["fingerprint"] != fingerprint() or value["calendar_hash"] != base.calendar()[1]:
            raise ValueError("ROUND_36_CODE_OR_CALENDAR_CHANGED")
        return value
    active()
    if base.read(previous_round.root() / "result.json")["fingerprint"] != previous_round.fingerprint():
        raise ValueError("SENSITIVITY_PREVIOUS_FINGERPRINT_CHANGED")
    parent, _ = hk.models()
    value = {
        "at": base.now().isoformat(),
        "fingerprint": fingerprint(),
        "candidates": CANDIDATES,
        "hypothesis": ("Individual past market sensitivity may contextualize common HK12 baseline errors"),
        "motivation": "Fund-specific contemporaneous4market sensitivity; unlike round09 forward SPX-only response",
        ...
        "input_features": "HK12+4shrunkenbetas+4beta*base-market-returns+contextcount/126; no new requests",
        "training_dates": 504,
        ...
        | {
            "round-36/" + name: base.digest(base.read(root() / name))
...
def fit(rows, name, cutoff):
    """严格沿用第17轮树配方；只追加九项个体市场上下文信息，记录原训练样本摘要。"""
    if name not in CANDIDATES:
        raise ValueError("SENSITIVITY_MODEL_RECIPE_INVALID")
    ...
    if len(Counter(y)) != 2 or min(Counter(y).values()) < 20:
        raise ValueError("SENSITIVITY_MODEL_ERROR_LABELS_INSUFFICIENT")
...
def batch_answers(values, name, trained):
    ...
    if not np.isfinite(x).all():
        raise ValueError("SENSITIVITY_MODEL_NORMALIZED_INPUT_INVALID")
    with threadpool_limits(limits=2):
        scores = trained["model"].predict_proba(x)[:, 1]
    if not np.isfinite(scores).all() or min(scores) < 0 or max(scores) > 1:
        raise ValueError("SENSITIVITY_MODEL_SCORE_INVALID")
    result = []
...
def fit_checkpoint(rows, name, cutoff, label):
    ...
    if receipt.exists():
        ...
        if (
            ...
            raise ValueError("SENSITIVITY_MODEL_CHECKPOINT_CHANGED")
        return joblib.load(path)
    if attempt.exists():
        raise ValueError("SENSITIVITY_MODEL_PREVIOUS_FIT_INTERRUPTED")
    ...
    if base.digest([r | {"z": r["z"][:12]} for r in short]) != expected:
        raise ValueError("SENSITIVITY_CONTROL_TRAINING_ROWS_CHANGED")
...
def train():
    ...
    for name, expected in p["input_hashes"].items():
        if base.digest(base.read(base.ROOT / name)) != expected:
            raise ValueError("ROUND_36_INPUT_CHANGED")
    rows, proofs = dataset()
    ...
    with threadpool_limits(limits=2):
        ...
        if len(expected) != 5670 or any(
            ...
            raise ValueError("ROUND_36_COMMON_EXAM_CHANGED")
...
def models():
    ...
    if (
        ...
        raise ValueError("ROUND_36_MODEL_OR_CODE_CHANGED")
...
def tick():
    ...
    with threadpool_limits(limits=2):
        for path in paths:
            ...
            z = live_vector(source, original, hk_input["rows"], context_snapshot)
...
def report():
    ...
    for path in (root() / "forward").glob("*/*.json"):
        ...
        if (
            ...
            raise ValueError("ROUND_36_PARENT_OR_MODEL_CHANGED")
        hk_input = hk_live.load(value["u"])
        if value["hk_input_hash"] != base.digest(hk_input):
            raise ValueError("SENSITIVITY_MODEL_LIVE_INPUT_CHANGED")
        if not np.allclose(
            value["z"], live_vector(source, original, hk_input["rows"], context_snapshot), rtol=0, atol=1e-12
        ):
            raise ValueError("ROUND_36_VECTOR_CHANGED")
        good += 1

"""第107轮：T日期权收盘价加权成交持仓比例，保持原标签、训练样本和对照。"""
...
from app.services import direction_1d_sprint_market_health_futures as previous
from app.services import direction_1d_sprint_market_only as baseline
from app.services import direction_1d_sprint_option_price_weighted_data as data
...
CANDIDATES = ("SIGN3_IO_CLOSE_WEIGHT5_LR504",)
CONTROLS = previous.CONTROLS
BRANCHES = CANDIDATES + CONTROLS
FIRST_TARGET = "2026-09-17"
PROPOSAL_HASH = "d9428c7f7f6954d1259f1b62a1cd2df79143f3a608330bb1ba94f300835cf00f"


def root():
    return base.ROOT / "round-107"


def require(condition, reason):
    if not condition:
        raise ValueError("IO_CLOSE_WEIGHT_" + reason)


def fingerprint():
    code = dict(base.read(base.ROOT / "round-106" / "plan.json")["fingerprint"]["code"])
    for name, expected in code.items():
        require(core.sha(base.PROJECT / name) == expected, "OLD_CODE_CHANGED")
    for name in (
        "app/services/direction_1d_sprint_market_option_price_weighted.py",
        "app/services/direction_1d_sprint_option_price_weighted_forward.py",
        "scripts/direction_1d_sprint_market_option_price_weighted.py",
        "tests/test_direction_1d_sprint_market_option_price_weighted.py",
        "app/services/direction_1d_sprint_option_price_weighted_data.py",
        "tests/test_direction_1d_sprint_option_price_weighted_data.py",
        ".local-runs/direction-1d-sprint-20260914/option-price-weighted-feasibility-v1/check.py",
    ):
...
def plan():
    ...
    require(proposal["r106_result_hash"] == base.digest(base.read(previous.root() / "result.json")), "SOURCE_CHANGED")
    require(
    ...
    require(
        ...
        and all(x["price_available"] == x["questions"] for x in coverage["exam"].values())
...
def reconstruct_dataset():
    """按共享市场日期缓存新增期权价格权重，避免为同日30基金重复解析整份交易日历。"""
    ...
    for row in rows:
        key = row["t"], row["u"]
        if key not in cache:
            cache[key] = data.extend({}, *key, h["snapshot"]["rows"])["option_price_weighted"]
        extra.append(row | {"market": row["market"] | {"option_price_weighted": cache[key]}})
    require([data.original_row(r) for r in extra] == rows, "ORIGINAL_ROWS_CHANGED")
    proof = proof | {"option_price_weighted_history_hash": base.digest(h)}
    return extra, proof
...
def vector(value):
    ...
    feature = value["option_price_weighted"]
    require(feature["available"], "OPTION_PRICE_WEIGHTED_MISSING")
    extra = [feature[k] for k in ("log_put_call_close_volume", "log_put_call_close_oi")]
    require(
        all(type(v) in (int, float) and np.isfinite(v) for v in extra) and abs(extra[0]) <= 20 and abs(extra[1]) <= 20,
        "OPTION_PRICE_WEIGHTED_INVALID",
    )
    return [*x, *extra]
...
def fit(rows, name, cutoff):
    ...
    require(
        ...
        and all(
            r["mature"] < cutoff and r["u"] < cutoff and r["market"]["option_price_weighted"]["available"]
            for r in chosen
        ),
    ...
    return {
        ...
        "max_feature_date": max(r["market"]["option_price_weighted"]["date"] for r in chosen),
...
def candidate_answers(values, name, head, fallback):
    ...
    indices = [i for i, v in enumerate(values) if v["available"] and v["option_price_weighted"]["available"]]
    if indices:

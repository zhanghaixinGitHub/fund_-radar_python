"""第112轮：新增美国信用债相对国债ETF同日价格变化，保持原标签、训练样本和对照。"""
...
from app.services import direction_1d_sprint_market_dollar_etf as previous
...
CANDIDATES = ("SIGN3_CREDIT_PAIR5_LR504",)
CONTROLS = previous.CONTROLS
BRANCHES = CANDIDATES + CONTROLS
FIRST_TARGET = "2026-09-17"
PROPOSAL_HASH = "c55070b07c8cb7a9e106c6f44554021563678103146453ce865cfc422b4d70d6"


def root():
    return base.ROOT / "round-112"


def require(condition, reason):
    if not condition:
        raise ValueError("CREDIT_PAIR_MODEL_" + reason)


def fingerprint():
    code = dict(base.read(base.ROOT / "round-111" / "plan.json")["fingerprint"]["code"])
    ...
    for name, expected in source["code_hashes"].items():
        ...
        code[name] = expected
    for name in (
        "app/services/direction_1d_sprint_market_credit_pair.py",
        "scripts/direction_1d_sprint_market_credit_pair.py",
        "tests/test_direction_1d_sprint_market_credit_pair.py",
    ):
...
def plan():
    ...
    require(proposal["r111_result_hash"] == base.digest(base.read(previous.root() / "result.json")), "SOURCE_CHANGED")
    require(
    ...
    require(base.digest(source) == proposal["source_qualification_hash"], "CREDIT_SOURCE_CHANGED")
    ...
    require(
        ...
        and all(x["dates"] == 504 and x["credit_available"] == x["questions"] for x in coverage["training_windows"]),
...
def reconstruct_dataset():
    """按共享市场日期缓存信用债相对国债ETF同日收益，避免为同日30基金重复解析整份交易日历。"""
    ...
    for row in rows:
        key = row["t"], row["u"]
        if key not in cache:
            cache[key] = data.extend({}, *key, h["points"])["credit_pair"]
        extra.append(row | {"market": row["market"] | {"credit_pair": cache[key]}})
    require([data.original_row(r) for r in extra] == rows, "ORIGINAL_ROWS_CHANGED")
    proof = proof | {"credit_pair_history_hash": base.digest(h)}
    return extra, proof
...
def vector(value):
    ...
    feature = value["credit_pair"]
    require(type(feature["available"]) is bool, "INTRADAY_INVALID")
    extra = [feature[k] for k in ("relative_log_pct", "treasury_log_pct")]
    require(all(type(v) in (int, float) and np.isfinite(v) and abs(v) <= 20 for v in extra), "INTRADAY_INVALID")
...
def fit(rows, name, cutoff):
    ...
    require(
        ...
        and all(r["mature"] < cutoff and r["u"] < cutoff and r["market"]["credit_pair"]["available"] for r in chosen),
    ...
    return {
        ...
        "max_feature_date": max(d for r in chosen for d in r["market"]["credit_pair"]["new_us_dates"]),
        "training_feature_mean_not_subtracted": mean,
        "masked_training_rows": sum(not r["market"]["credit_pair"]["available"] for r in chosen),
...
def candidate_answers(values, name, head, fallback):
    ...
    indices = [i for i, v in enumerate(values) if v["available"] and v["credit_pair"]["available"]]
    if indices:

"""第97轮：T日IO成交持仓分布追加到原三个市场输入，保持原标签、训练样本和对照。"""
...
from app.services import direction_1d_sprint_market_futures_intraday as previous
from app.services import direction_1d_sprint_market_only as baseline
from app.services import direction_1d_sprint_option_position_data as data
...
CANDIDATES = ("SIGN3_IO_POSITION5_LR504",)
CONTROLS = previous.CONTROLS
BRANCHES = CANDIDATES + CONTROLS
FIRST_TARGET = "2026-09-17"
PROPOSAL_HASH = "8b99ca75d54deb20b46d9fead1441618c4a42668f9a2000d99fd3a3415eb2401"


def root():
    return base.ROOT / "round-97"
...
def fingerprint():
    code = dict(base.read(base.ROOT / "round-96" / "plan.json")["fingerprint"]["code"])
    for name, expected in code.items():
        require(core.sha(base.PROJECT / name) == expected, "OLD_CODE_CHANGED")
    for name in (
        "app/services/direction_1d_sprint_market_option_position.py",
        "app/services/direction_1d_sprint_option_position_forward.py",
        "scripts/direction_1d_sprint_market_option_position.py",
        "tests/test_direction_1d_sprint_market_option_position.py",
        "app/services/direction_1d_sprint_option_position_data.py",
        "tests/test_direction_1d_sprint_option_position_data.py",
        "app/integrations/tushare_sprint_index_options.py",
        "tests/test_tushare_sprint_index_options.py",
    ):
...
def plan():
    ...
    require(proposal["r96_result_hash"] == base.digest(base.read(previous.root() / "result.json")), "SOURCE_CHANGED")
    require(
    ...
    require(base.digest(source) == proposal["source_qualification_hash"], "OPTION_SOURCE_CHANGED")
    coverage = base.read(root() / "feature-feasibility.json")
    require(
        coverage["history_hash"] == source["history_hash"]
        and all(x["missing_options"] == 0 for x in coverage["exam"].values())
        and len(coverage["training_windows"]) == 27
        and all(x["missing_options"] == 0 for x in coverage["training_windows"]),
...
def reconstruct_dataset():
    """按共享市场日期缓存新增期权比率，避免为同日30基金重复解析整份交易日历。"""
    ...
    for row in rows:
        key = row["t"], row["u"]
        if key not in cache:
            cache[key] = data.extend({}, *key, h["snapshot"]["rows"])["option_position"]
        extra.append(row | {"market": row["market"] | {"option_position": cache[key]}})
    require([data.original_row(r) for r in extra] == rows, "ORIGINAL_ROWS_CHANGED")
    proof = proof | {"option_history_hash": base.digest(h)}
    return extra, proof
...
def vector(value):
    ...
    feature = value["option_position"]
    require(feature["available"], "OPTION_MISSING")
    extra = [feature["log_put_call_volume"], feature["log_put_call_oi"]]
    require(
...
def fit(rows, name, cutoff):
    ...
    require(
        ...
        and all(
            r["mature"] < cutoff and r["u"] < cutoff and r["market"]["option_position"]["available"] for r in chosen
        ),
    ...
    return {
        ...
        "max_feature_date": max(r["market"]["option_position"]["date"] for r in chosen),
...
def candidate_answers(values, name, head, fallback):
    ...
    indices = [i for i, v in enumerate(values) if v["available"] and v["option_position"]["available"]]
    if indices:

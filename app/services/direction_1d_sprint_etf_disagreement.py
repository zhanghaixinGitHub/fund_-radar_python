"""第五十一轮：只在SPX与FXI方向冲突时学习下一日方向。

同向时保持父轮固定多数规则；缺少任一来源时仍回退HK，不删除任何评估题目。
...
from app.services import direction_1d_sprint_etf_joint as previous_round
...
CANDIDATES = ("ETF_DISAGREEMENT_LR3_ZERO504",)
LEARNED = CANDIDATES
...
def fingerprint():
    ...
    for name in (
        "app/services/direction_1d_sprint_etf_disagreement.py",
        "scripts/direction_1d_sprint_etf_disagreement.py",
        "tests/test_direction_1d_sprint_etf_disagreement.py",
        ".local-runs/direction-1d-sprint-20260914/round-51/route-feasibility.py",
        ".local-runs/direction-1d-sprint-20260914/round-51/freeze-proposal.py",
    ):
        value["code"][name] = hashlib.sha256((base.PROJECT / name).read_bytes()).hexdigest()
    return value


vector = previous_round.vector
live_vector = previous_round.live_vector
available = previous_round.available


def disagreement(z):
    """仅来源有效且SPX、FXI时段收益正负相反时进入新模型；零按正号处理。"""
    return available(z) and (z[0] >= 0) != (z[12] >= 0)


def selected_features(z, name):
    if name not in CANDIDATES:
        raise ValueError("ETF_DISAGREEMENT_CANDIDATE_INVALID")
    return previous_round.selected_features(z, previous_round.LEARNED[0])


def dataset():
    """沿用父轮所有输入，核对已冻结的路由可行性；不在不同模型间改变题目。"""
    rows, proof = previous_round.dataset()
    proposal = base.read(root() / "proposal-before-training.json")
    route_plan = base.read(root() / "feasibility-plan.json")
    feasible = base.read(root() / "input-feasibility.json")
    if (
        feasible["plan_hash"] != base.digest(route_plan)
        or feasible["rows_hash"] != base.digest(rows)
        or not feasible["all_training_windows_eligible"]
        or proposal["route_result_hash"] != base.digest(feasible)
        or proposal["parent_plan_hash"] != base.digest(previous_round.plan())
        or proposal["script_sha256"] != hashlib.sha256((root() / "freeze-proposal.py").read_bytes()).hexdigest()
        or route_plan["script_sha256"] != hashlib.sha256((root() / "route-feasibility.py").read_bytes()).hexdigest()
    ):
        raise ValueError("ETF_DISAGREEMENT_PROTOTYPE_CHANGED")
    return rows, proof
...
def plan():
    ...
    if path.exists():
        ...
        if value["fingerprint"] != fingerprint() or value["calendar_hash"] != base.calendar()[1]:
            raise ValueError("ROUND_51_CODE_OR_CALENDAR_CHANGED")
        return value
    active()
    parent, _ = hk.models()
    ...
    value = {
        ...
        "hypothesis": "Fit only SPX/FXI disagreement; preserve R50 majority elsewhere",
        "target": "Raw unit NAV T to adjacent CN U UP/NON_UP, flat separate",
        "features": "Same SPX,FXI_USsession,CNYA_gap; route fixed by source availability and SPX/FXI sign",
        "recipe": "LR3 C.1 max_iter1500 lbfgs L2 seed17 no intercept; scale only; convergence warning fails",
        "training": "Last504 mature joint-available dates/group THEN conflict filter; >=120 dates, eachclass>=20",
        "weights": "Natural date/productfamily weights on conditional training rows, no class balancing",
        "routing": "Agreement and missing-source questions keep exact frozen R50 majority/HK fallback",
        ...
        "expected_dates": 199,
        "selection": "One conditional model versus frozen R50 majority; reuse-only oldLR3 rejected before final plan",
        ...
        | {
            ...
            for name in (
                ...
                "runtime-etf-v2/plan.json",
                "cnya-data-feasibility-v1/history.json",
                "etf-cross-signal-diagnostic-v1/result.json",
                "round-51/proposal-before-training.json",
                "round-51/input-feasibility.json",
                "round-51/feasibility-plan.json",
                "round-50/result.json",
            )
        },
        ...
        "live_budget": "Reuse round48 ETF and round43 CNYA validated inputs; no additional HTTP requests",
...
def fit(rows, name, cutoff):
    ...
    window_hash = base.digest(chosen)
    window_dates = len({r["u"] for r in chosen})
    chosen = [r for r in chosen if disagreement(r["z"])]
    if len({r["u"] for r in chosen}) < 120 or any(not disagreement(r["z"]) for r in chosen):
    ...
    return {
        ...
        "parent_window_hash": window_hash,
        "parent_window_dates": window_dates,
        "fit_rows": len(chosen),
        ...
        "source_route": "SPX_FXI_DISAGREE_WITH_BOTH_SOURCES_AVAILABLE",
    }


def batch_answers(values, name, trained):
    """默认保留父轮多数规则及HK回退，只覆盖来源可用且SPX/FXI方向冲突的行。"""
    if not values:
        return []
    selected = [selected_features(z, name) for z in values]
    choices = previous_round.batch_answers(values, previous_round.CANDIDATES[0], trained)
    indices = [i for i, z in enumerate(values) if disagreement(z)]
    if indices:
        head = trained["us_etf"]
        if head is None:
            raise ValueError("ETF_DISAGREEMENT_MODEL_MISSING")
        x = (np.asarray([selected[i] for i in indices]) - head["mean"]) / head["scale"]
        if not np.isfinite(x).all():
            raise ValueError("ETF_DISAGREEMENT_NORMALIZED_INPUT_INVALID")
        ...
        if not np.isfinite(scores).all() or min(scores) < 0 or max(scores) > 1:
            raise ValueError("ETF_JOINT_SCORE_INVALID")
...
def train():
    ...
    for name, expected in p["input_hashes"].items():
        if base.digest(base.read(base.ROOT / name)) != expected:
            raise ValueError("ROUND_51_INPUT_CHANGED")
    rows, proofs = dataset()
    ...
    with threadpool_limits(limits=2):
        ...
        if len(expected) != 5670 or any(
            ...
            raise ValueError("ROUND_51_COMMON_EXAM_CHANGED")
...
def models():
    ...
    if (
        ...
        raise ValueError("ROUND_51_MODEL_OR_CODE_CHANGED")
...
def report():
    ...
    for path in (root() / "forward").glob("*/*.json"):
        ...
        if (
            ...
            raise ValueError("ROUND_51_PARENT_OR_MODEL_CHANGED")
        ...
        if not np.allclose(
            ...
            raise ValueError("ROUND_51_VECTOR_CHANGED")
        ...
        if not runtime.answers_match(value["answers"], expected_answers):
            raise ValueError("ROUND_51_SAVED_ANSWER_CHANGED")
        good += 1

"""第五十八轮：用严格按时间留出的幅度预测与三票差额训练一日组合模型。
...
CANDIDATES = ("VOTE_RIDGE_OOF_LR2_ZERO504",)
...
def fingerprint():
    ...
    for name in (
        "app/services/direction_1d_sprint_stacking.py",
        "scripts/direction_1d_sprint_stacking.py",
        "tests/test_direction_1d_sprint_stacking.py",
        "app/services/direction_1d_sprint_stacking_oof.py",
        "tests/test_direction_1d_sprint_stacking_oof.py",
    ):
...
def plan():
    ...
    value = prior | {
        ...
        "recipe": proposal["meta_recipe"],
        "inner_recipe": proposal["base_recipe"],
        "max_development_fits": 12,
        "max_current_fits": 3,
        "new_inner_base_fits": 39,
        "reused_inner_checkpoints": 12,
        "total_new_fits": 54,
        "input_hashes": prior["input_hashes"]
        | {
            "round-58/proposal-before-implementation.json": base.digest(proposal),
            "stacking-oof-feasibility-v1/result.json": base.digest(feasible),
        },
        "control": "R50majority andR57ridge155;noin-samplebasefeatures",
...
def fit(rows, name, cutoff):
    """只在成熟题的时间留出预测上训练组合权重，基础模型不能看过该题的目标。"""
    if name not in CANDIDATES:
...
def train():
    ...
    for name, expected in p["input_hashes"].items():
        if base.digest(base.read(base.ROOT / name)) != expected:
            raise ValueError("ROUND_58_INPUT_CHANGED")
    rows, proofs = dataset()
    oof_manifest = oof.prepare(rows)
    ...
    with threadpool_limits(limits=2):
        ...
        if len(expected) != 5670 or any(
            ...
            raise ValueError("ROUND_58_COMMON_EXAM_CHANGED")
...
def models():
    ...
    if (
        ...
        raise ValueError("ROUND_58_MODEL_OR_CODE_CHANGED")
...
def report():
    ...
    for path in (root() / "forward").glob("*/*.json"):
        ...
        if (
            ...
            raise ValueError("ROUND_58_PARENT_OR_MODEL_CHANGED")
        ...
        if not np.allclose(
            ...
            raise ValueError("ROUND_58_VECTOR_CHANGED")
        ...
        if not runtime.answers_match(value["answers"], expected_answers):
            raise ValueError("ROUND_58_SAVED_ANSWER_CHANGED")
        good += 1

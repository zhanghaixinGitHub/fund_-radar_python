"""US ETF成交价格的一日输入，历史开发与真实08:30前采集各自留证。"""
...
from app.integrations.sina_sprint_etf import MAX_BYTES, SYMBOLS, fetch_etf, parse, url_for
...
SOURCE = "SINA_US_ETF_UNADJUSTED_QUOTE_V1"
...
def features(base_day, target, points):
    ...
    for symbol in SYMBOLS:
        ...
        for row in (prior, latest):
            ...
            available &= valid
        values += [100 * (latest["close"] / latest["open"] - 1), 100 * (latest["close"] / prior["close"] - 1)]
    ...
    return np.clip(values, -20, 20).tolist() + [1.0] if available else [0.0] * 5


def history():
    """以原始HTTP字节重新解码并核对既有来源验收，不能改解析表绕过原始证据。"""
    folder = base.ROOT / "sina-etf-source-v1"

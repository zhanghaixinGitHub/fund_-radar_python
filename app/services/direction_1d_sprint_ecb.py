"""第二十六轮：ECB人民币与美元参考汇率变化对一日港股纠错模型的增量影响。

保留原港股12项输入，增加基准日及以前ECB参考汇率的人民币和美元变化。
...
from app.services import direction_1d_sprint_direct as previous_round
from app.services import direction_1d_sprint_dual_us as dual
from app.services import direction_1d_sprint_ecb_data as ecb_data
...
def fingerprint():
    ...
    for name in (
        "app/services/direction_1d_sprint_ecb.py",
        "scripts/direction_1d_sprint_ecb.py",
        "tests/test_direction_1d_sprint_ecb.py",
        "app/integrations/ecb_sprint_fx.py",
        "app/services/direction_1d_sprint_ecb_data.py",
        "tests/test_direction_1d_sprint_ecb_data.py",
        "app/data/calendars/ecb_target_2021_2026_sprint_v1.json",
    ):
...
def vector(x, t, u, markets, points):
    return hk.vector(x, t, u, markets) + ecb_data.features(t, u, points)
...
def dataset():
    ...
    points = ecb_data.history()
    cache, output = {}, []
    for row in rows:
        key = (row["t"], row["u"])
        if key not in cache:
            cache[key] = ecb_data.features(*key, points)
...
def plan():
    ...
    if path.exists():
        ...
        if value["fingerprint"] != fingerprint() or value["calendar_hash"] != base.calendar()[1]:
            raise ValueError("ROUND_26_CODE_OR_CALENDAR_CHANGED")
        return value
    active()
    ...
    value = {
        ...
        "hypothesis": "Reference CNYperUSD and USDperEUR changes may add currency information beyond SPX and HK",
        ...
        "feature_order": "HK12; CNYperUSD change%; USDperEUR change%; each clipped+-20; anchor ECBdate<=T",
        "ecb_source": ecb_data.SOURCE,
        "ecb_raw_url": ecb_data.URL,
        "source_timing": (
        ...
        | {
            ...
            for name in (
                ...
                "ecb-fx-feasibility-v1/feature-feasibility-result.json",
            )
        },
...
def fit(rows, name, cutoff):
    """严格沿用第17轮树配方；只追加两个已知ECB_FX输入，记录原训练样本摘要。"""
...
def train():
    ...
    for name, expected in p["input_hashes"].items():
        if base.digest(base.read(base.ROOT / name)) != expected:
            raise ValueError("ROUND_26_INPUT_CHANGED")
    rows, proofs = dataset()
    ...
    with threadpool_limits(limits=2):
        ...
        if len(expected) != 5670 or any(
            ...
            raise ValueError("ROUND_26_COMMON_EXAM_CHANGED")
...
def models():
    ...
    if (
        ...
        raise ValueError("ROUND_26_MODEL_OR_CODE_CHANGED")
...
def tick():
    ...
    ecb_input = ecb_data.capture(base.now())
...
def report():
    ...
    for path in (root() / "forward").glob("*/*.json"):
        ...
        if (
            ...
            raise ValueError("ROUND_26_PARENT_OR_MODEL_CHANGED")
        hk_input = hk_live.load(value["u"])
        ecb_input = ecb_data.load(value["u"])
        ...
        if not np.allclose(
            ...
            raise ValueError("ROUND_26_VECTOR_CHANGED")
        good += 1

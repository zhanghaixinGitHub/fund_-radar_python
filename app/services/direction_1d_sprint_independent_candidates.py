"""把三个已训练候选独立接到R116同题输入，不依赖未训练的R117。
...
def root():
    return b.ROOT / "independent-candidates-r118-r120-v1"
...
def bindings():
    ...
    require(
        ...
        and plan["new_fits"] == 0
        and plan["new_source_request_budget"] == 1
...
def candidate_answers(source, question, histories, loaded):
    """只组合三个原answers的候选分支，不选最高分、不改缺失回退或概率数值。"""
    result = {}
    ...
    require(len(result) == 3, "CANDIDATE_COUNT_CHANGED")
    return result


def combine(old, new):
    require(len(old) == 35 and len(new) == 3 and set(old).isdisjoint(new), "BRANCH_COLLISION_OR_MISSING")
    value = old | new
...
def validate(value, receipt, question, source, tail, old_answers, histories, loaded, plan):
    """保存及以后核对均按同一契约重验时点、输入、父回读、来源、模型和38个完整答案。"""
...
def write_and_report(plan, loaded, parents):
    """同一题一次性保存38分支，回读后才可计数；后续只核对成熟的真实结果。"""
    histories_by_day = {}
...
def execute(action):
    ...
    if action == "preflight":
        ...
        require(result["checks"] == 90, "PREFLIGHT_COUNT_CHANGED")
    else:

"""把五个已训练候选独立接到R116同题输入，不依赖未训练的R117。
...
from app.services import direction_1d_sprint_fund_moneyflow_binding as fund_runtime
from app.services import direction_1d_sprint_fund_moneyflow_live as fund_live
...
from app.services import direction_1d_sprint_market_fund_moneyflow as fund_model
...
SPECS = {
    ...
    122: (fund_model, fund_live, fund_runtime),
}
BRANCHES = 40
...
def root():
    return b.ROOT / "independent-candidates-r118-r122-v3"
...
def candidate_market(number, source, question, history):
    """个体候选必须用已核验的基金code选择冻结统计，其他候选保持原市场向量。"""
    model = SPECS[number][0]
    args = (source["market"], question["t"], question["u"], history["points"])
    if number == 122:
        return model.data.extend(*args, code=question["code"], frozen_current=history["frozen_current"])
    return model.data.extend(*args)


def candidate_answers(source, question, histories, loaded):
    """只组合五个原answers的候选分支，不选最高分、不改缺失回退或概率数值。"""
    result = {}
    for number, (model, _, _) in SPECS.items():
        market = candidate_market(number, source, question, histories[number])
    ...
    require(len(result) == 5, "CANDIDATE_COUNT_CHANGED")
    return result


def combine(old, new):
    require(len(old) == 35 and len(new) == 5 and set(old).isdisjoint(new), "BRANCH_COLLISION_OR_MISSING")
    value = old | new
...
def validate(value, receipt, question, source, tail, old_answers, histories, loaded, plan):
    """保存及以后核对均按同一契约重验时点、输入、父回读、来源、模型和40个完整答案。"""
...
def write_and_report(plan, loaded, parents):
    """同一题一次性保存40分支，回读后才可计数；后续只核对成熟的真实结果。"""
    histories_by_day = {}
...
def preflight(loaded):
    ...
    histories[122]["frozen_current"] = fund_model.data.reference()[3]
    checks = []
    for question in questions:
        combined = candidate_answers(source, question, histories, loaded)
        for n, (model, _, _) in SPECS.items():
            market = candidate_market(n, source, question, histories[n])
...
def execute(action):
    ...
    if action == "preflight":
        ...
        require(result["checks"] == 150, "PREFLIGHT_COUNT_CHANGED")
    else:

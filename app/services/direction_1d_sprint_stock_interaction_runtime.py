"""第114轮前向运行独立绑定：不改已冻结训练代码，待R113完成后一次性冻结接入关系。"""
...
from app.services import direction_1d_sprint_joint_capacity_runtime as parent_runtime
from app.services import direction_1d_sprint_market_joint_capacity as parent
...
def verify_intent():
    ...
    model.require(
        ...
        and intent["parent_round"] == 113
        and intent["same_question_branches"] == 33
...
def plan():
    """R114训练与R113运行链均完成后才绑定；父运行层未完成时拒绝占位接入。"""
    intent = verify_intent()
    ...
    context = {
        ...
        "code_hashes": code,
        "artifact_hashes": {
            "round-114/plan.json": trained["plan_hash"],
            "round-114/result.json": base.digest(trained),
            "round-113/plan.json": prior["plan_hash"],
            "round-113/result.json": base.digest(prior),
            "round-113/runtime-plan.json": base.digest(parent_run),
        },
    }

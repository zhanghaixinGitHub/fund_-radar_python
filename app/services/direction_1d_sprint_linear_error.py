"""第三十五轮：使用低复杂度线性关系学习一日SPX方向规则的错误。

第33、34轮存在训练期能区分错误、后续历史区分接近随机的落差，故检验线性纠错。
沿用第13轮逻辑回归配方，仅将其输入扩展为原HK12；不平衡错误类别，不搜索参数。
"""
...
from app.services import direction_1d_sprint_forest as previous_round
...
def fingerprint():
    ...
    for name in (
        "app/services/direction_1d_sprint_linear_error.py",
        "scripts/direction_1d_sprint_linear_error.py",
        "tests/test_direction_1d_sprint_linear_error.py",
    ):
...
def selected_features(z, name):
    if name not in CANDIDATES or len(z) != 12 or not np.isfinite(z).all():
        raise ValueError("LINEAR_ERROR_INPUT_INVALID")
    return z
...
def plan():
    ...
    if path.exists():
        ...
        if value["fingerprint"] != fingerprint() or value["calendar_hash"] != base.calendar()[1]:
            raise ValueError("ROUND_35_CODE_OR_CALENDAR_CHANGED")
        return value
    active()
    if base.read(previous_round.root() / "result.json")["fingerprint"] != previous_round.fingerprint():
        raise ValueError("LINEAR_ERROR_PREVIOUS_FINGERPRINT_CHANGED")
    parent, _ = hk.models()
    value = {
        "at": base.now().isoformat(),
        "fingerprint": fingerprint(),
        "candidates": CANDIDATES,
        "hypothesis": ("Regularized linear HK12 error relations may generalize better than tree interactions"),
        "motivation": "Round33/34 generalization gap; apply original LR8 regularized recipe to HK12",
...
def fit(rows, name, cutoff):
    """沿用第13轮逻辑回归训练配方，只将原8项纠错输入扩展为HK12，不用开发成绩调参。"""
    if name not in CANDIDATES:
        raise ValueError("LINEAR_ERROR_MODEL_RECIPE_INVALID")
    ...
    if len(Counter(y)) != 2 or min(Counter(y).values()) < 20:
        raise ValueError("LINEAR_ERROR_MODEL_ERROR_LABELS_INSUFFICIENT")
...
def batch_answers(values, name, trained):
    ...
    if not np.isfinite(x).all():
        raise ValueError("LINEAR_ERROR_MODEL_NORMALIZED_INPUT_INVALID")
    with threadpool_limits(limits=2):
        scores = trained["model"].predict_proba(x)[:, 1]
    if not np.isfinite(scores).all() or min(scores) < 0 or max(scores) > 1:
        raise ValueError("LINEAR_ERROR_MODEL_SCORE_INVALID")
    result = []
...
def fit_checkpoint(rows, name, cutoff, label):
    ...
    if receipt.exists():
        ...
        if (
            ...
            raise ValueError("LINEAR_ERROR_MODEL_CHECKPOINT_CHANGED")
        return joblib.load(path)
    if attempt.exists():
        raise ValueError("LINEAR_ERROR_MODEL_PREVIOUS_FIT_INTERRUPTED")
    ...
    if base.digest(short) != expected:
        raise ValueError("LINEAR_ERROR_CONTROL_TRAINING_ROWS_CHANGED")
...
def train():
    ...
    for name, expected in p["input_hashes"].items():
        if base.digest(base.read(base.ROOT / name)) != expected:
            raise ValueError("ROUND_35_INPUT_CHANGED")
    rows, proofs = dataset()
    ...
    with threadpool_limits(limits=2):
        ...
        if len(expected) != 5670 or any(
            ...
            raise ValueError("ROUND_35_COMMON_EXAM_CHANGED")
...
def models():
    ...
    if (
        ...
        raise ValueError("ROUND_35_MODEL_OR_CODE_CHANGED")

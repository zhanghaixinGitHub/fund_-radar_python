"""第109轮：T日沪深股票涨跌范围与分布，保持原标签、训练样本和对照。"""
...
from app.services import direction_1d_sprint_market_fund_if_response as previous
...
CANDIDATES = ("SIGN3_STOCK_BREADTH6_LR504",)
CONTROLS = previous.CONTROLS
BRANCHES = CANDIDATES + CONTROLS
FIRST_TARGET = "2026-09-17"
INTENT_HASH = "85a4aab263e95a4ceb8c653ff22d3639f01cd9d785748aee410ff6a3436de5d3"


def root():
    return base.ROOT / "round-109"
...
def fingerprint():
    code = dict(base.read(base.ROOT / "round-108" / "plan.json")["fingerprint"]["code"])
    ...
    for name, expected in source_code.items():
        ...
        code[name] = expected
    for name in (
        "app/services/direction_1d_sprint_market_stock_breadth.py",
        "app/services/direction_1d_sprint_stock_breadth_forward.py",
        "app/services/direction_1d_sprint_stock_breadth_live.py",
        "scripts/direction_1d_sprint_market_stock_breadth.py",
        "tests/test_direction_1d_sprint_market_stock_breadth.py",
        "tests/test_direction_1d_sprint_stock_breadth_live.py",
    ):
...
def plan():
    ...
    require(proposal["r108_result_hash"] == base.digest(base.read(previous.root() / "result.json")), "SOURCE_CHANGED")
    require(

"""第三十一轮：发行方FXI净值口径差对一日港股纠错模型的增量影响。
...
from app.services import direction_1d_sprint_fxi_gap_data as fxi_gap_data
...
from app.services import direction_1d_sprint_shibor as previous_round
from app.services import direction_1d_sprint_sparse as sparse

CANDIDATES = ("FXI_GAP_HK15_ERR504",)
LEARNED = CANDIDATES
...
def fingerprint():
    ...
    for name in (
        "app/services/direction_1d_sprint_fxi_gap.py",
        "scripts/direction_1d_sprint_fxi_gap.py",
        "tests/test_direction_1d_sprint_fxi_gap.py",
        "app/services/direction_1d_sprint_fxi_gap_data.py",
        "tests/test_direction_1d_sprint_fxi_gap_data.py",
        "app/integrations/ishares_sprint_fxi.py",
    ):
...
def vector(x, t, u, markets, points):
    return hk.vector(x, t, u, markets) + fxi_gap_data.features(t, u, points)


def live_vector(source, original, markets, points):
    """复用父答案的净值和港股输入；FXI只取基准日15:00前的两个美股现金收盘，真实响应必须在目标日截止前取得。"""
...
def dataset():
    ...
    points = fxi_gap_data.history()
    cache, output = {}, []
    for row in rows:
        key = (row["t"], row["u"])
        if key not in cache:
            cache[key] = fxi_gap_data.features(*key, points)
    ...
    return output, proof | {
        ...
        "fxi_input_time": "latest US cash close<=T15:00 and previous cash date",
    }
...
def plan():
    ...
    if path.exists():
        ...
        if value["fingerprint"] != fingerprint() or value["calendar_hash"] != base.calendar()[1]:
            raise ValueError("ROUND_31_CODE_OR_CALENDAR_CHANGED")
        return value
    active()
    ...
    parent, _ = hk.models()
    fxi_gap_data.history()
    value = {
        "at": base.now().isoformat(),
        "fingerprint": fingerprint(),
        "candidates": CANDIDATES,
        "hypothesis": "Issuer NAV versus Non-FV NAV gap may add information beyond broad market direction",
        ...
        "fxi_source": fxi_gap_data.SOURCE,
        "fxi_url": fxi_gap_data.URL,
        ...
        "live_budget": {
            ...
            "max_bytes": fxi_gap_data.MAX_BYTES,
            "redirects": False,
            "automatic_retries": 0,
        },
        "extra_trees": {
...
def train():
    ...
    for name, expected in p["input_hashes"].items():
        if base.digest(base.read(base.ROOT / name)) != expected:
            raise ValueError("ROUND_31_INPUT_CHANGED")
    rows, proofs = dataset()
    ...
    with threadpool_limits(limits=2):
        ...
        if len(expected) != 5670 or any(
            ...
            raise ValueError("ROUND_31_COMMON_EXAM_CHANGED")
...
def models():
    ...
    if (
        ...
        raise ValueError("ROUND_31_MODEL_OR_CODE_CHANGED")
...
def tick():
    ...
    fxi_input = fxi_gap_data.capture(base.now())
...
def report():
    ...
    for path in (root() / "forward").glob("*/*.json"):
        ...
        if (
            ...
            raise ValueError("ROUND_31_PARENT_OR_MODEL_CHANGED")
        hk_input = hk_live.load(value["u"])
        fxi_input = fxi_gap_data.load(value["u"])
        ...
        if not np.allclose(
            ...
            raise ValueError("ROUND_31_VECTOR_CHANGED")
        good += 1

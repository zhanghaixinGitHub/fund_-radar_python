"""后续市场模型共享第70轮独立输入/实际净值观察；各自保存提前答案，零额外来源请求。"""
...
def validate(service, value, receipt, p, source, manifest, bundle):
    ...
    if (
        ...
        or not runtime.answers_match(value["answers"], service.answers(source["market"], p["group"], bundle))
    ):
...
def tick(service):
    ...
    for p in parents.values():
        ...
        value = {k: p[k] for k in ("code", "family", "group", "u", "t")} | {
            ...
            "answers": service.answers(source["market"], p["group"], bundle),
        ...
        validate(service, value, receipt, p, source, manifest, bundle)
...
def report(service, manifest=None, bundle=None, parents=None):
    ...
    if parents is None:
        parents = parent_context()
    inputs, observations, paired = {}, {}, defaultdict(list)
    ...
    for path in sorted((service.root() / "forward").glob("*/*.json")):
        try:
            ...
            validate(service, value, receipt, p, inputs[value["u"]], manifest, bundle)
            valid.append(value)

"""第五十三轮：检验滞后融资余额变化是否增加下一日方向信息。

固定零截距LR7与既有LR3同训练题比较；不使用与原表有差异的偿还额。
...
from app.services import direction_1d_sprint_margin_data as rates
...
from app.services import direction_1d_sprint_us_yield as previous_round

CANDIDATES = ("MARGIN_BALANCE_LR7_ZERO504",)
LEARNED = CANDIDATES
...
def fingerprint():
    ...
    for name in (
        "app/services/direction_1d_sprint_margin_balance.py",
        "scripts/direction_1d_sprint_margin_balance.py",
        "tests/test_direction_1d_sprint_margin_balance.py",
        "app/services/direction_1d_sprint_margin_data.py",
        "app/integrations/tushare_sprint_margin.py",
        "tests/test_direction_1d_sprint_margin_data.py",
        ".local-runs/direction-1d-sprint-20260914/margin-source-v1/qualify.py",
        ".local-runs/direction-1d-sprint-20260914/margin-source-v1/fetch-history.py",
    ):
...
def available(z):
    return majority.available(z[:20])


def selected_features(z, name):
    if name not in CANDIDATES or len(z) != 24 or not np.isfinite(z).all():
        raise ValueError("MARGIN_MODEL_INPUT_INVALID")
    return majority.selected_features(z[:20], majority.CANDIDATES[0]) + z[20:24]
...
def plan():
    ...
    if path.exists():
        ...
        if value["fingerprint"] != fingerprint() or value["calendar_hash"] != base.calendar()[1]:
            raise ValueError("ROUND_53_CODE_OR_CALENDAR_CHANGED")
        return value
    active()
    ...
    value = {
        ...
        "hypothesis": "Lagged financing balance changes may add funding pressure information beyond3equitysignals",
        "target": "Raw unit NAV T to adjacent CN U UP/NON_UP,flat separate",
        "training_target": "raw_NAV_UP",
        "features": "SPXpct,FXI_USsessionpct,CNYAgappct;SSE financingbalance1d/5dpct,SZSEsame,clip+-20%",
        "excluded_source_fields": "Provider rzche disagrees with primary source;no repayment/netflow field used",
        "recipe": "Logistic C.1 max_iter1500 seed17 L2 lbfgs no intercept; scale only; natural weights",
        "training": "Last504mature equity-sourceavailable dates/group;identical to frozenR50LR3 control",
        "control": "Reuse R50LR3 frozen checkpoints, verify all15 mature training row hashes;0duplicate control fits",
        "routing": "Equitysources unavailable retain R50majority/HK; exact6 laggedCNdates for all questions",
        ...
        "expected_dates": 199,
        "selection": "One newLR7,compare oldLR3 andbestmajority;no tuning",
        ...
        "source_timing": "6CNdates endpriorCNday(T);T record can be publishedafterU0830 so excluded",
        "live_budget": "3shared bounded margin POSTs max/target,real receipt/readback beforeU0830;reuseETF/CNYA",
    }
...
def fit(rows, name, cutoff):
    """同一成熟窗口下给旧三输入模型增加四个余额变化，不把迟到数据或偿还差异纳入训练。"""
    if name not in CANDIDATES:
        raise ValueError("MARGIN_MODEL_RECIPE_INVALID")
    ...
    if len({r["u"] for r in chosen}) < 120:
        raise ValueError("MARGIN_MODEL_TRAINING_SHORT")
    ...
    if len(Counter(y)) != 2 or min(Counter(y).values()) < 20:
        raise ValueError("MARGIN_MODEL_UP_CLASSES_INSUFFICIENT")
...
def fit_checkpoint(rows, name, cutoff, label):
    """沿用旧HK，另校验LR3对照训练题一致；本轮仅拟合新增LR7一次。"""
    ...
    if (
        ...
        raise ValueError("MARGIN_MODEL_PRIOR_CHECKPOINT_CHANGED")
    ...
    if base.digest([r | {"z": r["z"][:20]} for r in shared]) != prior_head["fit_hash"]:
        raise ValueError("MARGIN_MODEL_LR3_CONTROL_ROWS_CHANGED")
...
def train():
    ...
    for name, expected in p["input_hashes"].items():
        if base.digest(base.read(base.ROOT / name)) != expected:
            raise ValueError("ROUND_53_INPUT_CHANGED")
    rows, proofs = dataset()
    ...
    with threadpool_limits(limits=2):
        ...
        if len(expected) != 5670 or any(
            ...
            raise ValueError("ROUND_53_COMMON_EXAM_CHANGED")
...
def models():
    ...
    if (
        ...
        raise ValueError("ROUND_53_MODEL_OR_CODE_CHANGED")
...
def report():
    ...
    for path in (root() / "forward").glob("*/*.json"):
        ...
        if (
            ...
            raise ValueError("ROUND_53_PARENT_OR_MODEL_CHANGED")
        ...
        if value["rate_input_hash"] != base.digest(rate_input):
            raise ValueError("MARGIN_MODEL_RATE_INPUT_CHANGED")
        ...
        if not np.allclose(
            ...
            raise ValueError("ROUND_53_VECTOR_CHANGED")
        ...
        if not runtime.answers_match(value["answers"], expected_answers):
            raise ValueError("ROUND_53_SAVED_ANSWER_CHANGED")
        good += 1

"""第108轮：组公共IF走势与逐基金IF历史反应联合，保持原标签、训练样本和对照。"""
...
from app.services import direction_1d_sprint_fund_if_response_data as data
from app.services import direction_1d_sprint_market_futures_intraday as fallback_model
from app.services import direction_1d_sprint_market_only as baseline
from app.services import direction_1d_sprint_market_option_price_weighted as previous
...
CANDIDATES = ("SIGN3_IF_RESPONSE7_LR504",)
CONTROLS = previous.CONTROLS
BRANCHES = CANDIDATES + CONTROLS
FALLBACK_KEY = "FROZEN_R96_IF_INTRADAY5"
FIRST_TARGET = "2026-09-17"
PROPOSAL_HASH = "970befde624f0b1d3d5b0154642b8e7b33a65677c0e3d48cb832d6aaaa41db57"


def root():
    return base.ROOT / "round-108"
...
def fingerprint():
    code = dict(base.read(base.ROOT / "round-107" / "plan.json")["fingerprint"]["code"])
    for name, expected in code.items():
        require(core.sha(base.PROJECT / name) == expected, "OLD_CODE_CHANGED")
    for name in (
        "app/services/direction_1d_sprint_market_fund_if_response.py",
        "app/services/direction_1d_sprint_fund_if_response_forward.py",
        "scripts/direction_1d_sprint_market_fund_if_response.py",
        "tests/test_direction_1d_sprint_market_fund_if_response.py",
        "app/services/direction_1d_sprint_fund_if_response_data.py",
        "tests/test_direction_1d_sprint_fund_if_response_data.py",
        "app/services/direction_1d_sprint_fund_if_response_context.py",
        "tests/test_direction_1d_sprint_fund_if_response_context.py",
        ".local-runs/direction-1d-sprint-20260914/fund-if-response-feasibility-v1/check.py",
    ):
...
def plan():
    ...
    require(proposal["r107_result_hash"] == base.digest(base.read(previous.root() / "result.json")), "SOURCE_CHANGED")
    require(
    ...
    source = base.read(data.root() / "result.json")
    require(base.digest(source) == proposal["source_qualification_hash"], "FUTURES_SOURCE_CHANGED")
    coverage = base.read(root() / "feature-feasibility.json")
    require(
        coverage["source_features_hash"] == source["features_hash"]
        and len(coverage["training_windows"]) == 27
        and all(x["dates"] == 504 for x in coverage["training_windows"]),
    ...
    context = {
        ...
        "current_context_hash": base.digest(data.current()),
    }
    path = root() / "plan.json"
    if path.exists():
        p = base.read(path)
...
def vector(value):
    """前三维公共方向，接两项IF行情及两项个体反应；未知个体反应训练置零但保留标记。"""
    x = baseline.market_vector(value)
    future, feature = value["futures_intraday"], value["fund_if_response"]
    require(future["available"], "IF_TRAINING_INPUT_MISSING")
    extra = [
        future["intraday_return_pct"],
        future["close_location"],
        feature["response_return"],
        feature["response_location"],
    ]
    require(
        all(
            type(v) in (int, float) and np.isfinite(v) and abs(v) <= bound
            for v, bound in zip(extra, [20, 1, 60, 3], strict=True)
        ),
        "RESPONSE_INPUT_INVALID",
    )
    require(feature["available"] or extra[2:] == [0, 0], "MISSING_RESPONSE_NOT_ZERO")
    return [*x, *extra]
...
def fit(rows, name, cutoff):
    ...
    return {
        ...
        "max_feature_date": max(r["market"]["futures_intraday"]["date"] for r in chosen),
        "training_feature_mean_not_subtracted": mean,
        "max_prior_mature": max(
            r["market"]["fund_if_response"]["max_prior_mature"]
            for r in chosen
            if r["market"]["fund_if_response"]["max_prior_mature"] is not None
        ),
        "latest_context_cutoff": max(r["market"]["fund_if_response"]["context_cutoff"] for r in chosen),
        "masked_training_rows": sum(not r["market"]["fund_if_response"]["available"] for r in chosen),
...
def validate_head(head, name, cutoff=None):
    ...
    require(
        ...
        and model.n_features_in_ == 7
        and model.coef_.shape == (1, 7)
    ...
    require(
        ...
        and head["max_prior_mature"] < head["latest_context_cutoff"] == head["fit_end"],
        "HEAD_TIME_INVALID",
    )
...
def candidate_answers(values, name, head, fallback):
    ...
    indices = [i for i, v in enumerate(values) if v["available"] and v["fund_if_response"]["available"]]
    if indices:
...
def batch(values, heads, control_heads, fallback_head):
    controls = reference.batch(values, control_heads)
    output = {n: controls[n] for n in CONTROLS}
    fallback = fallback_model.batch(values, {fallback_model.CANDIDATES[0]: fallback_head}, control_heads)[
        fallback_model.CANDIDATES[0]
    ]
    for name in CANDIDATES:
        output[name] = candidate_answers(values, name, heads.get(name), fallback)
    return output
...
def answers(market, group, bundle):
    ...
    return {n: v[0] for n, v in batch([market], heads, controls, bundle[FALLBACK_KEY][group]).items()}


def historical_fallback(cutoff, group):
    """只读取已完成R96历史头和原始拟合回执，绝不触发旧模型重新训练。"""
    name = fallback_model.CANDIDATES[0]
    path = fallback_model.root() / "checkpoints" / f"{cutoff}-{group}-{name}.joblib"
    receipt = base.read(path.with_suffix(".json"))
    require(
        receipt["plan_hash"] == base.digest(base.read(fallback_model.root() / "plan.json"))
        and receipt["sha256"] == core.sha(path)
        and receipt["group"] == group
        and receipt["cutoff"] == cutoff
        and receipt["name"] == name,
        "FALLBACK_HEAD_RECEIPT_CHANGED",
    )
    head = joblib.load(path)
    fallback_model.validate_head(head, name, cutoff)
    return head
...
def prepare():
    ...
    for year in p["years"]:
        for q in range(1, 5):
            ...
            for group in groups:
                ...
                values = batch([r["market"] for r in exam], heads, control_heads, historical_fallback(start, group))
    ...
    bundle = {n: {} for n in CANDIDATES + CONTROLS[:2] + (FALLBACK_KEY,)}
    _, fallback_bundle = fallback_model.models()
    for group in groups:
        bundle[FALLBACK_KEY][group] = fallback_bundle[fallback_model.CANDIDATES[0]][group]
        for name in CANDIDATES:
            head = checkpoint(
...
def models():
    ...
    require(
        set(bundle) == set(CANDIDATES + CONTROLS[:2] + (FALLBACK_KEY,))
        and all(set(v) == groups for v in bundle.values()),
        "MODEL_GROUP_MISSING",
    )
    for name in CANDIDATES:
        for head in bundle[name].values():
            validate_head(head, name, p["current_cutoff"])
    for head in bundle[FALLBACK_KEY].values():
        fallback_model.validate_head(head, fallback_model.CANDIDATES[0], p["current_cutoff"])

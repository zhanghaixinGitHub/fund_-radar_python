"""逐份重验已冻结沪深日线并派生尾部比例；无供应商请求、无基金评分。"""

from app.services import direction_1d_sprint as base
from app.services import direction_1d_sprint_stock_breadth_data as old
from app.services import direction_1d_sprint_stock_tail_features as features

KEYS, scope, original = features.KEYS, old.scope, old.original


def root():
    return base.ROOT / "stock-tail-feasibility-v1"


def require(condition, reason):
    if not condition:
        raise ValueError("STOCK_TAIL_DATA_" + reason)
...
def history():
    ...
    require(
        result["plan_hash"] == base.digest(p) and result["status"] == "QUALIFIED_STOCK_TAIL_WITH_LIMITS",
...
def extend(market, t, u, points):
    return market | {"stock_tail": features.aligned(t, u, points, list(map(str, base.calendar()[0])))}


def original_row(row):
    return row | {"market": {k: v for k, v in row["market"].items() if k != "stock_tail"}}

"""对既有完整股票来源独立重验并派生活跃程度输入；不请求网络、不读取新基金成绩。"""

from app.services import direction_1d_sprint as base
from app.services import direction_1d_sprint_stock_activity_features as features
from app.services import direction_1d_sprint_stock_turnover_data as old

KEYS, scope, original = features.KEYS, old.scope, old.original


def root():
    return base.ROOT / "stock-activity-feasibility-v1"


def require(condition, reason):
    if not condition:
        raise ValueError("STOCK_ACTIVITY_DATA_" + reason)


def reconstruct():
    """重新验证父历史、每份原文和派生来源身份；不依赖后台进度或不完整采集目录。"""
    source = old.history()
    seed = base.read(old.old.root() / "reused-history.json")
    points, hashes = {}, {}
    for day, expected in source["snapshot"]["rows"].items():
        path = (
            base.ROOT / seed["sources"][day]["raw_path"]
            if day in seed["sources"]
            else old.old.root() / "raw" / (day + ".json")
        )
        value = features.parse(path.read_bytes(), day)
        require(value["source_turnover_hash"] == base.digest(expected), "PARENT_DISTRIBUTION_CHANGED")
        require(value["included_set_sha256"] == expected["included_set_sha256"], "SOURCE_UNIVERSE_CHANGED")
        points[day], hashes[day] = value, old.old.sha(path)
...
def history():
    ...
    for name, expected in p["code"].items():
        require(old.old.sha(base.PROJECT / name) == expected, "QUALIFICATION_CODE_CHANGED")
    require(old.old.sha(root() / "design-before-analysis.md") == p["design_sha256"], "DESIGN_CHANGED")
    ...
    require(
        result["plan_hash"] == base.digest(p) and result["status"] == "QUALIFIED_STOCK_ACTIVITY_WITH_LIMITS",
...
def extend(market, t, u, points):
    return market | {"stock_activity": features.rolling(t, u, points, list(map(str, base.calendar()[0])))}


def original_row(row):
    return row | {"market": {k: v for k, v in row["market"].items() if k != "stock_activity"}}

"""第111轮前向运行独立绑定：不改已冻结训练代码，待R110完成后一次性冻结接入关系。"""
...
from app.services import direction_1d_sprint_market_stock_turnover as parent
...
def verify_intent():
    ...
    model.require(
        ...
        and intent["parent_round"] == 110
        and intent["same_question_branches"] == 29
...
def plan():
    """只能在R110与R111真实模型均完成后冻结；父实验未完成时明确失败，不绑定占位参数。"""
    intent = verify_intent()
    trained, _ = model.models()
    prior, _ = parent.models()
    code = dict(trained["fingerprint"]["code"])
    for mapping in (prior["fingerprint"]["code"], intent["code_hashes"]):
    ...
    context = {
        ...
        "code_hashes": code,
        "artifact_hashes": {
            "round-111/plan.json": trained["plan_hash"],
            "round-111/result.json": base.digest(trained),
            "round-110/plan.json": prior["plan_hash"],
            "round-110/result.json": base.digest(prior),
        },
    }

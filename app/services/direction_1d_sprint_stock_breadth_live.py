"""沪深股票分布一次实收：保留当时请求、原文及失败，禁止历史回填和请求重试。"""
...
def root():
    return b.ROOT / "round-109/live-stock-breadth"


def require(condition, reason):
    if not condition:
        raise ValueError("STOCK_BREADTH_LIVE_" + reason)
...
def query(t):
    return {"api_name": "daily", "params": {"trade_date": t.replace("-", "")}, "fields": ",".join(parser.FIELDS)}


def fetch(t):
    """只读一次已有权限daily接口；凭据仅留内存，异常不落请求体或供应商错误文本。"""
    cfg = get_settings()
...
def reconstruct(t, u, ph):
    ...
    feature = data.extend({}, t, u, points)["stock_breadth"]
    return {
        "t": t,
        "u": u,
        "plan_hash": ph,

"""第五十二轮：检验美国利率是否能纠正三信号多数方向的错误。

同题比较三信号纠错对照与增加八项美债特征；无新利率时保留父答案。
...
from app.services import direction_1d_sprint_etf_disagreement as previous_round
...
from app.services import direction_1d_sprint_us_tycr_data as rates

CANDIDATES = ("MAJORITY_ERR3_504", "MAJORITY_USYIELD_ERR11_504")
LEARNED = CANDIDATES
...
def fingerprint():
    ...
    for name in (
        "app/services/direction_1d_sprint_us_yield.py",
        "scripts/direction_1d_sprint_us_yield.py",
        "tests/test_direction_1d_sprint_us_yield.py",
        "app/services/direction_1d_sprint_us_tycr_data.py",
        "app/integrations/tushare_sprint_us_tycr.py",
        "tests/test_direction_1d_sprint_us_tycr_data.py",
        ".local-runs/direction-1d-sprint-20260914/us-treasury-source-v1/qualify.py",
        ".local-runs/direction-1d-sprint-20260914/us-treasury-source-v1/fetch-history.py",
    ):
        value["code"][name] = hashlib.sha256((base.PROJECT / name).read_bytes()).hexdigest()
    return value


def vector(x, code, t, u, markets, etf_points, cn_points, rate_points):
    return majority.vector(x, code, t, u, markets, etf_points, cn_points) + rates.features(t, u, rate_points)


def live_vector(source, original, markets, etf_points, cn_points, rate_points):
    sequence.live_vector(source, original)
    return vector(
        source["x"], original["code"], original["base"], original["u"], markets, etf_points, cn_points, rate_points
    )


def available(z):
    """必须有三项父信号以及国内收盘后新增的利率，才使用本轮纠错。"""
    return majority.available(z[:20]) and z[28] == 1.0


def parent_sign(z):
    return int(sum(z[i] >= 0 for i in (0, 12, 17)) >= 2)


def selected_features(z, name):
    if name not in CANDIDATES or len(z) != 29 or not np.isfinite(z).all() or z[28] not in (0, 1):
        raise ValueError("US_YIELD_INPUT_INVALID")
    majority.selected_features(z[:20], majority.CANDIDATES[0])
    if z[28] == 0 and any(z[20:28]):
        raise ValueError("US_YIELD_UNAVAILABLE_VALUES_INVALID")
    equity = [z[i] for i in (0, 12, 17)]
    return equity if name == CANDIDATES[0] else equity + z[20:28]
...
def dataset():
    ...
    for row in rows:
        key = (row["t"], row["u"])
        if key not in cache:
            cache[key] = rates.features(*key, points)
        output.append(row | {"z": row["z"] + cache[key]})
    return output, proof | {
        "rates_history_hash": base.digest(base.read(rates.source_folder() / "history.json")),
        "rate_rows": len(points),
        "new_2026_scores_read": False,
    }
...
def plan():
    ...
    if path.exists():
        ...
        if value["fingerprint"] != fingerprint() or value["calendar_hash"] != base.calendar()[1]:
            raise ValueError("ROUND_52_CODE_OR_CALENDAR_CHANGED")
        return value
    active()
    ...
    value = {
        ...
        "hypothesis": "US overnight yields may improve detection of majority3 errors beyond equity signals",
        "target": "Raw unit NAV T to adjacent CN U UP/NON_UP, flat separate",
        "training_target": "Frozen majority3 direction differs from raw_NAV_UP",
        "features": "3 SPX/FXI/CNYA inputs vs same3+5USyield changesbp+2curve spreadsbp+10ylevel%",
        "recipe": "ExtraTrees256 max_depth6 min_leaf80 max_features1 bootstrapFalse seed17 threads2",
        "training": "Last504 mature joint-source-available dates/group; natural date/family error weights",
        "normalization": "Training-only weighted mean and centered-square scale",
        "routing": "Both candidates use identical source availability and preserve majority3/HK elsewhere",
        ...
        "expected_dates": 199,
        "selection": "Prespecified control vs rates ablation; no parameter/threshold search or 2026 score use",
        "parent_result_hash": base.digest(parent),
        "calendar_hash": base.calendar()[1],
        "input_hashes": majority.plan()["input_hashes"]
        | {
            str(path.relative_to(base.ROOT)): base.digest(base.read(path))
            for path in (
                rates.source_folder() / name
                for name in (
                    "history.json",
                    "calendar.json",
        ...
        "source_timing": "18ET historical publication assumption; actual request/receipt/readback<U08:30 mandatory",
        "live_budget": "3shared rate POSTs max/target,existing permissions, no retry; reuseETF/CNYA sources",
    }
...
def fit(rows, name, cutoff):
    """同一成熟窗口比较两组特征，学习父规则错误而非将全体上涨先验当作模型进步。"""
    if name not in CANDIDATES:
        raise ValueError("US_YIELD_RECIPE_INVALID")
    ...
    if len({r["u"] for r in chosen}) < 120:
        raise ValueError("US_YIELD_TRAINING_DATES_INSUFFICIENT")
    x = np.asarray([selected_features(r["z"], name) for r in chosen])
    y = np.asarray([int(parent_sign(r["z"]) != r["y"]) for r in chosen])
    if len(Counter(y)) != 2 or min(Counter(y).values()) < 20:
        raise ValueError("US_YIELD_ERROR_CLASSES_INSUFFICIENT")
    ...
    return {
        ...
        "training_target": "MAJORITY3_ERROR",
    }


def batch_answers(values, name, trained):
    """先保留父答案；错误分数严格大于0.55才反向，分数并非校准概率。"""
    if not values:
        return []
    ...
    if indices:
        head = trained["us_etf"]
        if head is None:
            raise ValueError("US_YIELD_MODEL_MISSING")
        x = (np.asarray([selected[i] for i in indices]) - head["mean"]) / head["scale"]
        if not np.isfinite(x).all():
            raise ValueError("US_YIELD_NORMALIZED_INPUT_INVALID")
        scores = head["model"].predict_proba(x)[:, 1]
        if not np.isfinite(scores).all() or min(scores) < 0 or max(scores) > 1:
            raise ValueError("US_YIELD_SCORE_INVALID")
        for i, score in zip(indices, scores, strict=True):
            baseline = parent_sign(values[i])
            flipped = bool(score > FLIP_THRESHOLD)
            choices[i] = {
                "research_score": float(score),
                "kind": "UNCALIBRATED_MAJORITY_ERROR_SCORE",
...
def train():
    ...
    for name, expected in p["input_hashes"].items():
        if base.digest(base.read(base.ROOT / name)) != expected:
            raise ValueError("ROUND_52_INPUT_CHANGED")
    rows, proofs = dataset()
    ...
    with threadpool_limits(limits=2):
        ...
        if len(expected) != 5670 or any(
            ...
            raise ValueError("ROUND_52_COMMON_EXAM_CHANGED")
...
def models():
    ...
    if (
        ...
        raise ValueError("ROUND_52_MODEL_OR_CODE_CHANGED")
...
def tick():
    ...
    rate_input = rates.capture(base.now())
    cn_input_path = cnya_data.root() / target / "input.json"
    cn_input = cnya_data.load(target) if cn_input_path.exists() else None
    if hk_input is None or us_etf_input is None or cn_input is None or rate_input is None:
        return report()
    ...
    with threadpool_limits(limits=2):
        for path in paths:
            ...
            z = live_vector(
                source, original, hk_input["rows"], us_etf_input["rows"], cn_input["rows"], rate_input["rows"]
            )
            ...
            value = {
                ...
                "rate_input_hash": base.digest(rate_input),
...
def report():
    ...
    for path in (root() / "forward").glob("*/*.json"):
        ...
        if (
            ...
            raise ValueError("ROUND_52_PARENT_OR_MODEL_CHANGED")
        hk_input = hk_live.load(value["u"])
        us_etf_input = us_etf_data.load(value["u"])
        cn_input = cnya_data.load(value["u"])
        rate_input = rates.load(value["u"])
        if value["rate_input_hash"] != base.digest(rate_input):
            raise ValueError("US_YIELD_RATE_INPUT_CHANGED")
        ...
        if not np.allclose(
            value["z"],
            live_vector(source, original, hk_input["rows"], us_etf_input["rows"], cn_input["rows"], rate_input["rows"]),
            rtol=0,
            atol=1e-12,
        ):
            raise ValueError("ROUND_52_VECTOR_CHANGED")
        ...
        if not runtime.answers_match(value["answers"], expected_answers):
            raise ValueError("ROUND_52_SAVED_ANSWER_CHANGED")
        good += 1

"""第四十二轮：改用预测早晨前最新美股现金日期的FXI净值口径差。

原HK12增加口径差、差值变化及可用标记，不将净值当作市场价格，不丢弃缺失题目。
只比较该固定特征组合，沿用原树结构、自然错误权重和门槛，仍预测相邻交易日单位净值方向。
"""
...
from app.services import direction_1d_sprint_fxi_overnight_data as fxi_overnight_data
from app.services import direction_1d_sprint_fxi_shares as previous_round
...
CANDIDATES = ("FXI_OVERNIGHT_HK15_ERR504",)
LEARNED = CANDIDATES
...
def fingerprint():
    ...
    for name in (
        "app/services/direction_1d_sprint_fxi_overnight.py",
        "scripts/direction_1d_sprint_fxi_overnight.py",
        "tests/test_direction_1d_sprint_fxi_overnight.py",
        "app/services/direction_1d_sprint_fxi_overnight_data.py",
        "tests/test_direction_1d_sprint_fxi_overnight_data.py",
        ".local-runs/direction-1d-sprint-20260914/round-42/feature-feasibility.py",
    ):
...
def vector(x, t, u, markets, points):
    return hk.vector(x, t, u, markets) + fxi_overnight_data.features(t, u, points)


def live_vector(source, original, markets, points):
    """复用父答案的净值和港股输入；FXI只取目标日08:30前的两个美股现金收盘，真实响应必须在目标日截止前取得。"""
...
def selected_features(z, name):
    if name not in CANDIDATES or len(z) != 15 or not np.isfinite(z).all():
        raise ValueError("FXI_MODEL_INPUT_INVALID")
    return z


def dataset():
    rows, proof = hk.dataset()
    points = fxi_overnight_data.history()
    cache, output = {}, []
    for row in rows:
        key = (row["t"], row["u"])
        if key not in cache:
            cache[key] = fxi_overnight_data.features(*key, points)
        output.append(row | {"z": row["z"] + cache[key]})
    expected = base.read(base.ROOT / "round-42/input-feasibility.json")["feature_hash"]
    if base.digest([{k: r[k] for k in ("code", "u", "z")} for r in output]) != expected:
        raise ValueError("FXI_OVERNIGHT_PROTOTYPE_FEATURES_CHANGED")
    return output, proof | {
        "at": base.now().isoformat(),
        "fxi_rows": len(points),
        "fxi_history_hash": base.digest(base.read(base.ROOT / "fxi-masked-feasibility-v2/history.json")),
        "fxi_input_time": "latest US cash close<=U08:30 and previous cash date",
    }
...
def plan():
    ...
    if path.exists():
        ...
        if value["fingerprint"] != fingerprint() or value["calendar_hash"] != base.calendar()[1]:
            raise ValueError("ROUND_42_CODE_OR_CALENDAR_CHANGED")
        return value
    active()
    if base.read(previous_round.root() / "result.json")["fingerprint"] != previous_round.fingerprint():
        raise ValueError("FXI_PREVIOUS_FINGERPRINT_CHANGED")
    parent, _ = hk.models()
    fxi_overnight_data.history()
    value = {
        "at": base.now().isoformat(),
        "fingerprint": fingerprint(),
        "candidates": CANDIDATES,
        "hypothesis": (
            "Samegapfeaturesasround31 but latestUSclosesbeforeU08:30 ratherthanbaseT15 may addfreshinformation"
        ),
        ...
        "feature_order": "HK12;100*(NAV/NonFV-1),change versus prior USdate,pair available flag;gap/change clip+-20",
        "fxi_source": fxi_overnight_data.SOURCE,
        "fxi_url": fxi_overnight_data.URL,
        ...
        "live_budget": {
            ...
            "max_additional_requests_per_target": 3,
            "prefer_reuse_round31_raw": True,
            "shared_across_funds": True,
            "max_bytes": fxi_overnight_data.MAX_BYTES,
            "redirects": False,
            "automatic_retries": 0,
        },
        "extra_trees": {
        ...
        | {
            ...
            for name in (
                "fxi-masked-feasibility-v2/plan.json",
                "fxi-masked-feasibility-v2/history.json",
                "fxi-masked-feasibility-v2/result.json",
                "round-42/proposal-before-training.json",
                "round-42/input-feasibility.json",
            )
        },
...
def fit(rows, name, cutoff):
    """严格沿用第17轮树配方；只追加三项发行方口径差信息，记录原训练样本摘要。"""
    if name not in CANDIDATES:
        raise ValueError("FXI_MODEL_RECIPE_INVALID")
    ...
    if len(Counter(y)) != 2 or min(Counter(y).values()) < 20:
        raise ValueError("FXI_MODEL_ERROR_LABELS_INSUFFICIENT")
...
def fit_checkpoint(rows, name, cutoff, label):
    ...
    if receipt.exists():
        ...
        if (
            ...
            raise ValueError("FXI_MODEL_CHECKPOINT_CHANGED")
        return joblib.load(path)
    if attempt.exists():
        raise ValueError("FXI_MODEL_PREVIOUS_FIT_INTERRUPTED")
    ...
    if base.digest([r | {"z": r["z"][:12]} for r in chosen]) != expected:
        raise ValueError("FXI_CONTROL_TRAINING_ROWS_CHANGED")
...
def train():
    ...
    for name, expected in p["input_hashes"].items():
        if base.digest(base.read(base.ROOT / name)) != expected:
            raise ValueError("ROUND_42_INPUT_CHANGED")
    rows, proofs = dataset()
    ...
    with threadpool_limits(limits=2):
        ...
        if len(expected) != 5670 or any(
            ...
            raise ValueError("ROUND_42_COMMON_EXAM_CHANGED")
...
def models():
    ...
    if (
        ...
        raise ValueError("ROUND_42_MODEL_OR_CODE_CHANGED")
...
def tick():
    ...
    fxi_input = fxi_overnight_data.capture(base.now())
    ...
    with threadpool_limits(limits=2):
        for path in paths:
            ...
            z = live_vector(source, original, hk_input["rows"], fxi_input["rows"])
            ...
            value = {
                ...
                "fxi_input_hash": base.digest(fxi_input),
...
def report():
    ...
    for path in (root() / "forward").glob("*/*.json"):
        ...
        if (
            ...
            raise ValueError("ROUND_42_PARENT_OR_MODEL_CHANGED")
        hk_input = hk_live.load(value["u"])
        fxi_input = fxi_overnight_data.load(value["u"])
        if value["fxi_input_hash"] != base.digest(fxi_input):
            raise ValueError("FXI_MODEL_LIVE_INPUT_CHANGED")
        if value["hk_input_hash"] != base.digest(hk_input):
            raise ValueError("FXI_MODEL_LIVE_INPUT_CHANGED")
        if not np.allclose(
            value["z"], live_vector(source, original, hk_input["rows"], fxi_input["rows"]), rtol=0, atol=1e-12
        ):
            raise ValueError("ROUND_42_VECTOR_CHANGED")
        good += 1

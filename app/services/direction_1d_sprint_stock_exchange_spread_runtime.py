"""第118轮前向运行独立绑定：不改已冻结训练代码，待R117完成后一次性冻结接入关系。"""
...
from app.services import direction_1d_sprint_market_stock_moneyflow as parent
from app.services import direction_1d_sprint_stock_moneyflow_runtime as parent_runtime
...
def verify_intent():
    ...
    model.require(
        ...
        and intent["parent_round"] == 117
        and intent["same_question_branches"] == 37
...
def plan():
    """R118训练与R117运行链均完成后才绑定；父运行层未完成时拒绝占位接入。"""
    intent = verify_intent()
    ...
    context = {
        ...
        "code_hashes": code,
        "artifact_hashes": {
            "round-118/plan.json": trained["plan_hash"],
            "round-118/result.json": base.digest(trained),
            "round-117/plan.json": prior["plan_hash"],
            "round-117/result.json": base.digest(prior),
            "round-117/runtime-plan.json": base.digest(parent_run),
        },
    }

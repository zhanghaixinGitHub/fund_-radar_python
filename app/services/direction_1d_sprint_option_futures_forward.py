"""第100轮复用 CORE3 输入，并复用已实际取得T日原始期货行情，不调用已退役的旧实验链。"""
...
from app.services import direction_1d_sprint_market_option_change as prior_model
from app.services import direction_1d_sprint_market_option_futures as model
from app.services import direction_1d_sprint_option_change_forward as prior_forward
...
def validate(value, receipt, parent, source, manifest, bundle, extra, history):
    ...
    model.require(value["r99_forecast_hash"] == base.digest(extra), "R99_PARENT_CHANGED")
    ...
    model.require(
        ...
        "R99_PARENT_READBACK_LATE_OR_CHANGED",
    )
    model.require(
        value["joint_source_hash"] == base.digest(history) and datetime.fromisoformat(history["at"]) <= created,
        "OPTION_SOURCE_CHANGED_OR_LATE",
    )
    source_parent = base.read(base.ROOT / "round-99" / "forward" / parent["u"] / f"{parent['code']}.json")
    model.require(
        history["t"] == parent["t"]
        and history["u"] == parent["u"]
        and history["change_source_hash"] == source_parent["option_change_source_hash"],
        "OPTION_SOURCE_SCOPE_CHANGED",
    )
    ...
    model.require(
        model.data.change.old.in_window(parent["t"], parent["u"], datetime.fromisoformat(history["at"])),
        "OPTION_SOURCE_NOT_ACTUALLY_CAPTURED_IN_WINDOW",
    )
    model.require(
        datetime.fromisoformat(history["previous_history_at"]) < datetime.fromisoformat(history["at"]),
        "PREVIOUS_HISTORY_LATE",
    )
    ...
    for key, p in candidates.items():
        ...
        if p["u"] not in history:
            history[p["u"]] = model.data.live(
                p["t"],
                p["u"],
                base.digest(base.read(base.ROOT / "round-97" / "plan.json")),
                base.digest(base.read(base.ROOT / "round-95" / "plan.json")),
            )
...
def run():
    # 先完成已有CORE3及R99分支，再在同一实际输入上增加期货与期权变化联合模型。
    prior_forward.run()
    ...
    for key, p in parents.items():
        ...
        value = {k: p[k] for k in ("code", "family", "group", "t", "u")} | {
            ...
            "r99_forecast_hash": base.digest(extra),
...
def combined_answers(parent, extra, value):
    """明确合并全部18个同题分支，避免新增中间模型后递归层级漂移丢掉旧对照。"""
    combined = dict(parent["answers"])
    for number in (92, 94, 95, 96, 97, 98):
    ...
    model.require(len(combined) == 18, "FORWARD_BRANCH_COUNT_CHANGED")
    return combined
...
def report(loaded=None):
    ...
    value = {
        ...
        "new_branches": 1,
        "same_question_branches": 18,
        "joint_sources": {
            u: {
                "available": v["available"],
                "received_snapshot_at": v["at"],
                "change_source_hash": v["change_source_hash"],

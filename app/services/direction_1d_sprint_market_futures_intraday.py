"""第96轮：T日IF日内走势追加到原三个市场输入，保持原标签、训练样本和对照。"""
...
from app.services import direction_1d_sprint_futures_intraday_data as data
from app.services import direction_1d_sprint_market_current_basis as previous
...
CANDIDATES = ("SIGN3_IF_INTRADAY5_LR504",)
CONTROLS = previous.CONTROLS
BRANCHES = CANDIDATES + CONTROLS
FIRST_TARGET = "2026-09-17"
PROPOSAL_HASH = "31a5b01b9fa96b410493643f2409e4cb08c369a35de09d0c843b751f4734f845"


def root():
    return base.ROOT / "round-96"


def require(condition, reason):
    if not condition:
        raise ValueError("IF_INTRADAY_" + reason)


def fingerprint():
    code = dict(base.read(base.ROOT / "round-95" / "plan.json")["fingerprint"]["code"])
    for name, expected in code.items():
        require(core.sha(base.PROJECT / name) == expected, "OLD_CODE_CHANGED")
    for name in (
        "app/services/direction_1d_sprint_market_futures_intraday.py",
        "app/services/direction_1d_sprint_futures_intraday_forward.py",
        "scripts/direction_1d_sprint_market_futures_intraday.py",
        "tests/test_direction_1d_sprint_market_futures_intraday.py",
        "app/services/direction_1d_sprint_futures_intraday_data.py",
        "tests/test_direction_1d_sprint_futures_intraday_data.py",
    ):
...
def plan():
    ...
    require(proposal["r95_result_hash"] == base.digest(base.read(previous.root() / "result.json")), "SOURCE_CHANGED")
    require(
...
def reconstruct_dataset():
    ...
    for row in rows:
        key = row["t"], row["u"]
        if key not in cache:
            cache[key] = data.extend({}, *key, h["snapshot"]["rows"])["futures_intraday"]
        extra.append(row | {"market": row["market"] | {"futures_intraday": cache[key]}})
    require([data.original_row(r) for r in extra] == rows, "ORIGINAL_ROWS_CHANGED")
    proof = proof | {"futures_intraday_history_hash": base.digest(h)}
    return extra, proof
...
def vector(value):
    ...
    feature = value["futures_intraday"]
    require(feature["available"], "INTRADAY_MISSING")
    extra = [feature["intraday_return_pct"], feature["close_location"]]
    require(
        all(type(v) in (int, float) and np.isfinite(v) for v in extra) and abs(extra[0]) <= 20 and abs(extra[1]) <= 1,
        "INTRADAY_INVALID",
    )
    return [*x, *extra]
...
def fit(rows, name, cutoff):
    ...
    require(
        ...
        and all(
            r["mature"] < cutoff and r["u"] < cutoff and r["market"]["futures_intraday"]["available"] for r in chosen
        ),
...
def candidate_answers(values, name, head, fallback):
    ...
    indices = [i for i, v in enumerate(values) if v["available"] and v["futures_intraday"]["available"]]
    if indices:
...
def preflight():
    from app.services import direction_1d_sprint_futures_intraday_forward as forward

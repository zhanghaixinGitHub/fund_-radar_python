"""第118轮复用 CORE3 输入，复用R109的实际沪深股票响应，不调用已退役的旧实验链。"""
...
from app.services import direction_1d_sprint_market_stock_exchange_spread as model
from app.services import direction_1d_sprint_market_stock_moneyflow as prior_model
from app.services import direction_1d_sprint_stock_exchange_spread_live as live
from app.services import direction_1d_sprint_stock_exchange_spread_runtime as runtime
from app.services import direction_1d_sprint_stock_moneyflow_forward as prior_forward
...
def validate(value, receipt, parent, source, manifest, bundle, extra, history):
    ...
    model.require(value["r117_forecast_hash"] == base.digest(extra), "R117_PARENT_CHANGED")
    ...
    model.require(
        ...
        "R117_PARENT_READBACK_LATE_OR_CHANGED",
    )
    model.require(
...
def run():
    ...
    for key, p in parents.items():
        ...
        value = {k: p[k] for k in ("code", "family", "group", "t", "u")} | {
            ...
            "r117_forecast_hash": base.digest(extra),
...
def combined_answers(parent, extra, value):
    """明确合并全部37个同题分支，避免新增中间模型后递归层级漂移丢掉旧对照。"""
    ...
    model.require(len(combined) == 37, "FORWARD_BRANCH_COUNT_CHANGED")
    return combined

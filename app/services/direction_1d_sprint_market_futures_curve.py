"""第102轮：T日期货日内方向与具体合约期限价差联合，保持原标签、训练样本和对照。"""
...
from app.services import direction_1d_sprint_futures_curve_data as data
from app.services import direction_1d_sprint_market_futures_equal as previous
...
CANDIDATES = ("SIGN3_IF_CURVE6_LR504",)
CONTROLS = previous.intraday.CONTROLS
BRANCHES = CANDIDATES + CONTROLS
FIRST_TARGET = "2026-09-17"
PROPOSAL_HASH = "22c13475a3e0c2838f1e68bb5dad252934378141cd91dc53970eff52eb9e4758"


def root():
    return base.ROOT / "round-102"


def require(condition, reason):
    if not condition:
        raise ValueError("IF_CURVE_" + reason)


def fingerprint():
    code = dict(base.read(base.ROOT / "round-101" / "plan.json")["fingerprint"]["code"])
    for name, expected in code.items():
        require(core.sha(base.PROJECT / name) == expected, "OLD_CODE_CHANGED")
    for name in (
        "app/services/direction_1d_sprint_market_futures_curve.py",
        "app/services/direction_1d_sprint_futures_curve_forward.py",
        "scripts/direction_1d_sprint_market_futures_curve.py",
        "tests/test_direction_1d_sprint_market_futures_curve.py",
        "app/services/direction_1d_sprint_futures_curve_data.py",
        "tests/test_direction_1d_sprint_futures_curve_data.py",
        "app/integrations/tushare_sprint_futures_curve.py",
        "tests/test_tushare_sprint_futures_curve.py",
        "app/integrations/tushare_sprint_futures_curve_v2.py",
        "tests/test_tushare_sprint_futures_curve_v2.py",
        "app/services/direction_1d_sprint_futures_curve_history.py",
        "tests/test_direction_1d_sprint_futures_curve_history.py",
        ".local-runs/direction-1d-sprint-20260914/futures-curve-history-v1/acquire.py",
        ".local-runs/direction-1d-sprint-20260914/futures-curve-history-v2/acquire.py",
        ".local-runs/direction-1d-sprint-20260914/futures-curve-history-v2/qualify.py",
    ):
...
def plan():
    ...
    require(proposal["r101_result_hash"] == base.digest(base.read(previous.root() / "result.json")), "SOURCE_CHANGED")
    require(
    ...
    require(
        ...
        and all(x["missing_joint"] == 0 for x in coverage["exam"].values())
        and len(coverage["training_windows"]) == 27
        and all(x["missing_joint"] == 0 for x in coverage["training_windows"]),
...
def reconstruct_dataset():
    """按共享市场日期缓存新增期货走势，避免为同日30基金重复解析整份交易日历。"""
    ...
    for row in rows:
        key = row["t"], row["u"]
        if key not in cache:
            cache[key] = data.extend({}, *key, h["snapshot"]["rows"])["futures_curve"]
        extra.append(row | {"market": row["market"] | {"futures_curve": cache[key]}})
    require([data.original_row(r) for r in extra] == rows, "ORIGINAL_ROWS_CHANGED")
    proof = proof | {"futures_curve_history_hash": base.digest(h)}
    return extra, proof
...
def vector(value):
    ...
    feature = value["futures_curve"]
    require(feature["available"], "INTRADAY_MISSING")
    extra = [feature[k] for k in ("intraday_return_pct", "close_location", "annualized_spread_pct")]
    require(
        all(type(v) in (int, float) and np.isfinite(v) for v in extra)
        and abs(extra[0]) <= 20
        and abs(extra[1]) <= 1
        and abs(extra[2]) <= 50,
        "INTRADAY_INVALID",
...
def fit(rows, name, cutoff):
    ...
    require(
        ...
        and all(r["mature"] < cutoff and r["u"] < cutoff and r["market"]["futures_curve"]["available"] for r in chosen),
    ...
    return {
        ...
        "max_feature_date": max(r["market"]["futures_curve"]["date"] for r in chosen),
...
def validate_head(head, name, cutoff=None):
    ...
    require(
        ...
        and model.n_features_in_ == 6
        and model.coef_.shape == (1, 6)
...
def candidate_answers(values, name, head, fallback):
    ...
    indices = [i for i, v in enumerate(values) if v["available"] and v["futures_curve"]["available"]]
    if indices:
...
def preflight():
    from app.services import direction_1d_sprint_futures_curve_forward as forward

"""中国互联网ETF成交价格的一日输入，历史开发与真实08:30前采集各自留证。"""
...
from app.integrations.sina_sprint_china_internet_etf import MAX_BYTES, fetch_etf, parse, url_for
...
SYMBOLS = ("KWEB",)
SOURCE = "SINA_CHINA_INTERNET_ETF_QUOTE_V1"
...
def root():
    return base.ROOT / "round-61/internet-inputs"


def features(base_day, target, points):
    """各ETF只追加最新完整美股日开到收百分数，最后标记该源是否可用。

    美股最近一次收盘若早于基准日T的15:00，则没有新的隔夜现金市场信息，回退原三票规则。
    ...
    return np.clip(values, -20, 20).tolist() + [1.0] if available else [0.0] * 2


def history():
    """以原始HTTP字节重新解码并核对既有来源验收，不能改解析表绕过原始证据。"""
    folder = base.ROOT / "china-internet-etf-source-v1"

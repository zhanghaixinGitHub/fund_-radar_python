"""第111轮复用 CORE3 输入，独立保存UUP实际响应，不调用已退役的旧实验链。"""
...
from app.services import direction_1d_sprint_dollar_etf_live as live
from app.services import direction_1d_sprint_dollar_etf_runtime as runtime
from app.services import direction_1d_sprint_market_dollar_etf as model
from app.services import direction_1d_sprint_market_stock_turnover as prior_model
from app.services import direction_1d_sprint_stock_turnover_forward as prior_forward
...
def validate(value, receipt, parent, source, manifest, bundle, extra, history):
    ...
    model.require(value["r110_forecast_hash"] == base.digest(extra), "R110_PARENT_CHANGED")
    ...
    model.require(
        ...
        "R110_PARENT_READBACK_LATE_OR_CHANGED",
    )
    model.require(
        value["dollar_etf_source_hash"] == base.digest(history) and datetime.fromisoformat(history["at"]) <= created,
        "DOLLAR_SOURCE_CHANGED_OR_LATE",
    )
    model.require(
        history == live.load_live(parent["t"], parent["u"], manifest["plan_hash"]), "ACTUAL_DOLLAR_SOURCE_CHANGED"
    )
    model.require(
        live.in_window(parent["t"], parent["u"], datetime.fromisoformat(history["at"])),
        "DOLLAR_SOURCE_NOT_ACTUALLY_CAPTURED_IN_WINDOW",
    )
...
def run():
    # 先在实际时窗内只读美元相关ETF行情，再续接原比较链；不以历史文件充当新实收。
    rp = runtime.plan()
    p = model.plan()
    ...
    for key, p in parents.items():
        ...
        value = {k: p[k] for k in ("code", "family", "group", "t", "u")} | {
            ...
            "r110_forecast_hash": base.digest(extra),
            "dollar_etf_source_hash": base.digest(history[p["u"]]),
        }
...
def combined_answers(parent, extra, value):
    """明确合并全部29个同题分支，避免新增中间模型后递归层级漂移丢掉旧对照。"""
    combined = dict(parent["answers"])
    for number in (92, 94, 95, 96, 97, 98, 99, 100, 101, 102, 103, 104, 105, 106, 107, 108, 109):
    ...
    model.require(len(combined) == 29, "FORWARD_BRANCH_COUNT_CHANGED")
    return combined

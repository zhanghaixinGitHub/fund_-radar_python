"""第四十一轮：发行方FXI份额变化对一日港股纠错模型的增量影响。

原HK12增加一日和五日份额变化、结构跳变及可用标记，不将份额变化当成资金流入。
...
from app.services import direction_1d_sprint_fxi_shares_data as fxi_shares_data
...
from app.services import direction_1d_sprint_partial_fund as previous_round
...
CANDIDATES = ("FXI_SHARES_HK16_ERR504",)
LEARNED = CANDIDATES
...
def fingerprint():
    ...
    for name in (
        "app/services/direction_1d_sprint_fxi_shares.py",
        "scripts/direction_1d_sprint_fxi_shares.py",
        "tests/test_direction_1d_sprint_fxi_shares.py",
        "app/services/direction_1d_sprint_fxi_shares_data.py",
        "tests/test_direction_1d_sprint_fxi_shares_data.py",
        ".local-runs/direction-1d-sprint-20260914/round-41/shares-feasibility.py",
    ):
...
def vector(x, t, u, markets, points):
    return hk.vector(x, t, u, markets) + fxi_shares_data.features(t, u, points)


def live_vector(source, original, markets, points):
    """复用父答案的净值和港股输入；FXI只取基准日15:00前的六个美股现金收盘，真实响应必须在目标日截止前取得。"""
...
def dataset():
    ...
    points = fxi_shares_data.history()
    cache, output = {}, []
    for row in rows:
        key = (row["t"], row["u"])
        if key not in cache:
            cache[key] = fxi_shares_data.features(*key, points)
        output.append(row | {"z": row["z"] + cache[key]})
    expected = base.read(base.ROOT / "round-41/input-feasibility.json")["feature_hash"]
    if base.digest([{k: r[k] for k in ("code", "u", "z")} for r in output]) != expected:
        raise ValueError("SHARES_PROTOTYPE_FEATURES_CHANGED")
    return output, proof | {
        "at": base.now().isoformat(),
        "fxi_rows": len(points),
        "fxi_history_hash": base.digest(base.read(base.ROOT / "round-41/shares-history.json")),
        "fxi_input_time": "six US cashdates,latestclose<=T15:00",
    }
...
def plan():
    ...
    if path.exists():
        ...
        if value["fingerprint"] != fingerprint() or value["calendar_hash"] != base.calendar()[1]:
            raise ValueError("ROUND_41_CODE_OR_CALENDAR_CHANGED")
        return value
    active()
    ...
    parent, _ = hk.models()
    fxi_shares_data.history()
    value = {
        "at": base.now().isoformat(),
        "fingerprint": fingerprint(),
        "candidates": CANDIDATES,
        "hypothesis": "Issuer share supply changes may add information beyond broad price direction; not cashflow",
        ...
        "feature_order": "HK12;1day/5day shares changespercentclip+-50,structuralJumpFlag,validFlag",
        "fxi_source": fxi_shares_data.SOURCE,
        "fxi_url": fxi_shares_data.URL,
        "source_timing": "Lagged US dates; firstversions assumed; actual arrival andreadback<U08:30",
        "future_evidence": (
            "Reuse validated round31 rawfile/input; sixexactdates/reasons/hashes; input/predictionreadback<U08:30"
        ),
        "missingness": (
            "--/zero unavailable. Any dailychangeabs>=50pct gives jump1/valid0. "
            "Absentrow/malformed/negative/nonfinite fail"
        ),
        "live_budget": {
            "slots": ["0700", "0730", "0800"],
            "max_additional_requests_per_target": 0,
            "reuse_round31_raw_only": True,
            "shared_across_funds": True,
            "max_bytes": fxi_shares_data.MAX_BYTES,
            "redirects": False,
            "automatic_retries": 0,
        },
        "extra_trees": {
        ...
        | {
            ...
            for name in (
                "round-41/proposal-before-training.json",
                "round-41/shares-history.json",
                "round-41/input-feasibility.json",
            )
        },
...
def fit(rows, name, cutoff):
    """严格沿用第17轮树配方；只追加四项发行方份额信息，记录原训练样本摘要。"""
...
def train():
    ...
    for name, expected in p["input_hashes"].items():
        if base.digest(base.read(base.ROOT / name)) != expected:
            raise ValueError("ROUND_41_INPUT_CHANGED")
    rows, proofs = dataset()
    ...
    with threadpool_limits(limits=2):
        ...
        if len(expected) != 5670 or any(
            ...
            raise ValueError("ROUND_41_COMMON_EXAM_CHANGED")
...
def models():
    ...
    if (
        ...
        raise ValueError("ROUND_41_MODEL_OR_CODE_CHANGED")
...
def tick():
    ...
    fxi_input = fxi_shares_data.capture(base.now())
...
def report():
    ...
    for path in (root() / "forward").glob("*/*.json"):
        ...
        if (
            ...
            raise ValueError("ROUND_41_PARENT_OR_MODEL_CHANGED")
        hk_input = hk_live.load(value["u"])
        fxi_input = fxi_shares_data.load(value["u"])
        ...
        if not np.allclose(
            ...
            raise ValueError("ROUND_41_VECTOR_CHANGED")
        good += 1

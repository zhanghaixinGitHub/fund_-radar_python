"""第105轮复用 CORE3 输入，复用KURE实收并绑定逐基金冻结反应，不调用已退役的旧实验链。"""

from collections import defaultdict
from datetime import datetime

from app.services import direction_1d_sprint as base
from app.services import direction_1d_sprint_china_health_forward as prior_forward
from app.services import direction_1d_sprint_core_forward as core
from app.services import direction_1d_sprint_market_china_health as prior_model
from app.services import direction_1d_sprint_market_health_response as model
...
def validate(value, receipt, parent, source, manifest, bundle, extra, history):
    ...
    model.require(value["r104_forecast_hash"] == base.digest(extra), "R104_PARENT_CHANGED")
    ...
    model.require(
        ...
        "R104_PARENT_READBACK_LATE_OR_CHANGED",
    )
    model.require(
        value["response_source_hash"] == base.digest(history) and datetime.fromisoformat(history["at"]) <= created,
        "HEALTH_SOURCE_CHANGED_OR_LATE",
    )
    model.require(
        history == model.data.live(parent["t"], parent["u"], extra["plan_hash"]), "ACTUAL_RESPONSE_SOURCE_CHANGED"
    )
    model.require(history["health_source_hash"] == extra["health_source_hash"], "R104_SOURCE_CHANGED")
    market = model.data.extend(
        source["market"], parent["t"], parent["u"], history["points"], parent["code"], history["contexts"]
    )
...
def run():
    # R104负责既有一次采集；本分支仅增加已冻结的基金历史反应，不请求新行情。
    prior_forward.run()
    ...
    for key, p in parents.items():
        ...
        market = model.data.extend(
            source["market"], p["t"], p["u"], history[p["u"]]["points"], p["code"], history[p["u"]]["contexts"]
        )
        ...
        value = {k: p[k] for k in ("code", "family", "group", "t", "u")} | {
            ...
            "r104_forecast_hash": base.digest(extra),
            "response_source_hash": base.digest(history[p["u"]]),
        }
...
def combined_answers(parent, extra, value):
    """明确合并全部23个同题分支，避免新增中间模型后递归层级漂移丢掉旧对照。"""
    combined = dict(parent["answers"])
    for number in (92, 94, 95, 96, 97, 98, 99, 100, 101, 102, 103):
    ...
    model.require(len(combined) == 23, "FORWARD_BRANCH_COUNT_CHANGED")
    return combined
...
def report(loaded=None):
    ...
    value = {
        ...
        "new_branches": 1,
        "same_question_branches": 23,
        "joint_sources": {
            u: {
                "available": v["available"],
                "received_snapshot_at": v["at"],
                "health_source_hash": v["health_source_hash"],
...
def preflight():
    ...
    for f in checked:
        actual = model.answers(
            model.data.extend(source["market"], f["t"], f["u"], history["points"], f["code"], contexts),
            f["group"],
            bundle,
        )

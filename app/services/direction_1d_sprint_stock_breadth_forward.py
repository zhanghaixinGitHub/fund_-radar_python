"""第109轮复用 CORE3 输入，独立保存沪深股票分布实际响应，不调用已退役的旧实验链。"""
...
from app.services import direction_1d_sprint_fund_if_response_forward as prior_forward
from app.services import direction_1d_sprint_market_fund_if_response as prior_model
from app.services import direction_1d_sprint_market_stock_breadth as model
from app.services import direction_1d_sprint_stock_breadth_live as live
...
def validate(value, receipt, parent, source, manifest, bundle, extra, history):
    ...
    model.require(value["r108_forecast_hash"] == base.digest(extra), "R108_PARENT_CHANGED")
    ...
    model.require(
        ...
        "R108_PARENT_READBACK_LATE_OR_CHANGED",
    )
    model.require(
        value["stock_breadth_source_hash"] == base.digest(history) and datetime.fromisoformat(history["at"]) <= created,
        "STOCK_BREADTH_SOURCE_CHANGED_OR_LATE",
    )
    model.require(
        history["t"] == parent["t"] and history["u"] == parent["u"] and history["plan_hash"] == manifest["plan_hash"],
        "STOCK_BREADTH_SOURCE_SCOPE_CHANGED",
    )
    model.require(
        live.in_window(parent["t"], parent["u"], datetime.fromisoformat(history["at"])),
        "STOCK_BREADTH_SOURCE_NOT_ACTUALLY_CAPTURED_IN_WINDOW",
    )
    model.require(
        history == live.load_live(parent["t"], parent["u"], manifest["plan_hash"]),
        "ACTUAL_STOCK_BREADTH_SOURCE_CHANGED",
    )
...
def run():
    # 先在实际时窗内只读T日沪深股票日行情，再续接原比较链；不以历史文件充当新实收。
    p = model.plan()
    ...
    for key, p in parents.items():
        ...
        value = {k: p[k] for k in ("code", "family", "group", "t", "u")} | {
            ...
            "r108_forecast_hash": base.digest(extra),
            "stock_breadth_source_hash": base.digest(history[p["u"]]),
        }
...
def combined_answers(parent, extra, value):
    """明确合并全部27个同题分支，避免新增中间模型后递归层级漂移丢掉旧对照。"""
    combined = dict(parent["answers"])
    for number in (92, 94, 95, 96, 97, 98, 99, 100, 101, 102, 103, 104, 105, 106, 107):
    ...
    model.require(len(combined) == 27, "FORWARD_BRANCH_COUNT_CHANGED")
    return combined
...
def report(loaded=None):
    ...
    value = {
        ...
        "new_branches": 1,
        "same_question_branches": 27,
        "stock_distribution_sources": {
            u: {
        ...
        "reserved_source_requests": len(list(live.root().glob("*/request.json"))),
        "confirmed_source_responses": sum(v["new_source_requests"] == 1 for v in history.values()),
        "new_cost_cny": 0,
        "status": "MODEL_NOT_RELEASED",
    }

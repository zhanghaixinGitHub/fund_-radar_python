"""第106轮：IF日内走势与基金个体医疗反应联合，保持原标签、训练样本和对照。"""
...
from app.services import direction_1d_sprint_health_futures_data as data
...
CANDIDATES = ("SIGN3_IF_KURE_RESPONSE6_LR504",)
CONTROLS = previous.CONTROLS
BRANCHES = CANDIDATES + CONTROLS
FIRST_TARGET = "2026-09-17"
PROPOSAL_HASH = "16a9bbbd5638787ecf2cae65671a46e9fe05884c396edbe51dec3d3c56b54736"


def root():
    return base.ROOT / "round-106"


def require(condition, reason):
    if not condition:
        raise ValueError("HEALTH_FUTURES_" + reason)


def fingerprint():
    code = dict(base.read(base.ROOT / "round-105" / "plan.json")["fingerprint"]["code"])
    for name, expected in code.items():
        require(core.sha(base.PROJECT / name) == expected, "OLD_CODE_CHANGED")
    for name in (
        "app/services/direction_1d_sprint_market_health_futures.py",
        "app/services/direction_1d_sprint_health_futures_forward.py",
        "scripts/direction_1d_sprint_market_health_futures.py",
        "tests/test_direction_1d_sprint_market_health_futures.py",
        "app/services/direction_1d_sprint_health_futures_data.py",
        "tests/test_direction_1d_sprint_health_futures_data.py",
        ".local-runs/direction-1d-sprint-20260914/health-response-futures-feasibility-v1/check.py",
    ):
...
def plan():
    ...
    require(proposal["r105_result_hash"] == base.digest(base.read(previous.root() / "result.json")), "SOURCE_CHANGED")
    require(
...
def vector(value):
    ...
    feature = value["health_futures"]
    extra = [feature[k] for k in ("intraday_return_pct", "close_location", "response_signal")]
    require(
    ...
    require(feature["response_available"] or extra[2] == 0, "MISSING_INPUT_NOT_ZERO")
    require(feature["if_available"] or extra[:2] == [0, 0], "MISSING_IF_NOT_ZERO")
    require(feature["available"] is (feature["if_available"] and feature["response_available"]), "JOINT_FLAG_INVALID")
    return [*x, *extra]
...
def fit(rows, name, cutoff):
    ...
    return {
        ...
        "max_feature_date": max(d for r in chosen for d in r["market"]["health_futures"]["new_us_dates"]),
        "training_feature_mean_not_subtracted": mean,
        "max_prior_mature": max(
            r["market"]["health_futures"]["max_prior_mature"]
            for r in chosen
            if r["market"]["health_futures"]["max_prior_mature"] is not None
        ),
        "latest_context_cutoff": max(r["market"]["health_futures"]["context_cutoff"] for r in chosen),
        "masked_training_rows": sum(not r["market"]["health_futures"]["available"] for r in chosen),
...
def candidate_answers(values, name, head, fallback):
    ...
    indices = [i for i, v in enumerate(values) if v["available"] and v["health_futures"]["available"]]
    if indices:
...
def preflight():
    from app.services import direction_1d_sprint_health_futures_forward as forward

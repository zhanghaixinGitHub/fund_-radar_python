"""第118轮：T日沪市减深市的相对强弱，保持原标签、训练样本和对照。"""
...
CANDIDATES = ("SIGN3_EXCHANGE_SPREAD5_LR504",)
CONTROLS = previous.CONTROLS
BRANCHES = CANDIDATES + CONTROLS
FIRST_TARGET = "2026-09-17"
PROPOSAL_HASH = "a4cbe7b6b93559e343e3cec7ce0b93f4c926175a6de89a1278c0d03f98b88751"


def root():
    return base.ROOT / "round-118"
...
def fingerprint():
    ...
    for name, expected in source_plan["code"].items():
        ...
        code[name] = expected
    for name in (
        "app/services/direction_1d_sprint_market_stock_exchange_spread.py",
        "scripts/direction_1d_sprint_market_stock_exchange_spread.py",
        "tests/test_direction_1d_sprint_market_stock_exchange_spread.py",
    ):
...
def plan():
    ...
    require(base.digest(intent) == "f69f75e048e432dc0c20aaecece85f9ec7d8760e31b6c6e549c9b393386469ca", "INTENT_CHANGED")
...
def reconstruct_dataset():
    """按共享市场日期缓存新增沪深两市场的广度与收益差，避免为同日30基金重复解析整份交易日历。"""
...
def vector(value):
    ...
    require(
        all(type(v) in (int, float) and np.isfinite(v) for v in extra) and abs(extra[0]) <= 2 and abs(extra[1]) <= 40,

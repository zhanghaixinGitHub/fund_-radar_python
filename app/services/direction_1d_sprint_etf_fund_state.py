"""第五十五轮：直接预测下一日方向，检验自身净值状态对三市场信号的增量。

固定三输入与六输入树模型同题消融；旧市场基线、成熟时间和覆盖范围保持一致。
...
from app.services import direction_1d_sprint_fund_pooling as previous_round
...
CANDIDATES = ("DIRECT_ETF3_ET504", "DIRECT_ETF_NAV6_ET504")
FEATURE_INDICES = {CANDIDATES[0]: (0, 12, 17), CANDIDATES[1]: (0, 12, 17, 3, 4, 5)}
LEARNED = CANDIDATES
...
def fingerprint():
    ...
    for name in (
        "app/services/direction_1d_sprint_etf_fund_state.py",
        "scripts/direction_1d_sprint_etf_fund_state.py",
        "tests/test_direction_1d_sprint_etf_fund_state.py",
    ):
...
def vector(x, code, t, u, markets, etf_points, cn_points):
    """沿用R50完整20项输入，基金状态在T日前原始净值中计算，不增采新来源。"""
    return majority.vector(x, code, t, u, markets, etf_points, cn_points)
...
def available(z):
    return majority.available(z)


def selected_features(z, name):
    """同架构消融：三项市场信号对照，新增三项自身1/5/20日风险归一涨跌。"""
    if name not in CANDIDATES or len(z) != 20 or not np.isfinite(z).all() or max(abs(v) for v in z[3:6]) > 5:
        raise ValueError("ETF_FUND_STATE_INPUT_INVALID")
    majority.selected_features(z, majority.CANDIDATES[0])
    return [float(z[i]) for i in FEATURE_INDICES[name]]


def dataset():
    rows, proof = majority.dataset()
    return rows, proof | {"extra_provider_requests": 0, "state_source": "unchangedHK z3/4/5,originalfundNAV"}
...
def plan():
    ...
    if path.exists():
        ...
        if value["fingerprint"] != fingerprint() or value["calendar_hash"] != base.calendar()[1]:
            raise ValueError("ROUND_55_CODE_OR_CALENDAR_CHANGED")
        return value
    active()
    ...
    if proposal["budget"] != {
        ...
        "new_cost_cny": 0,
    } or proposal["candidates"] != list(CANDIDATES):
        raise ValueError("ETF_FUND_STATE_PROPOSAL_CHANGED")
    value = {
        "at": base.now().isoformat(),
        "fingerprint": fingerprint(),
        "candidates": CANDIDATES,
        "hypothesis": "Fund NAV state may add nonlinear interactions to overnight market signals",
        "target": "Raw unit NAV T to adjacent CN U UP/NON_UP,flat separate",
        "training_target": "raw_NAV_UP",
        "feature_indices": FEATURE_INDICES,
        "features": "SameR50z20;3market signals plus ownNAV1/5/20day volatilitynormalized returns clipped5",
        "recipe": "ET256 depth6 leaf80 max_features1.0 class_weightNone seed17 n_jobs2 naturalweights threshold.5",
        "training": "Last504mature jointavailable dates/group;same30funds and15windows asfrozen R50LR3",
        "control": "New3featureET vs6featureET;also reuse R50LR3 same15windows andmajority/HK",
        "max_development_fits": 24,
        "max_current_fits": 6,
        "max_reproduction_count": 1,
        "expected_questions": 5670,
        "expected_dates": 199,
        "selection": "Two predeclared candidates;no threshold/architecture/feature search withinround",
        "parent_result_hash": base.digest(parent),
        "calendar_hash": base.calendar()[1],
        "input_hashes": majority.plan()["input_hashes"]
        | {
            "round-55/proposal-before-implementation.json": base.digest(proposal),
            "round-54/analysis-result.json": base.digest(base.read(previous_round.root() / "analysis-result.json")),
        },
        "current_fit_cutoff": str(base.now().date()),
        "first_forward_target": FIRST_TARGET,
        "new_cost_cny": 0,
        "this_round_2026_scores_read": False,
        "source_timing": "Exact R50source timing andactualU0830 receipts;NAVinputs throughT",
...
def fit(rows, name, cutoff):
    """直接拟合下一日是否上涨；基金状态消融与三市场信号对照使用完全相同成熟题。"""
    if name not in CANDIDATES:
        raise ValueError("ETF_FUND_STATE_RECIPE_INVALID")
    ...
    if len({r["u"] for r in chosen}) < 120:
        raise ValueError("ETF_FUND_STATE_TRAINING_SHORT")
    ...
    if len(Counter(y)) != 2 or min(Counter(y).values()) < 20:
        raise ValueError("ETF_FUND_STATE_CLASSES_INSUFFICIENT")
...
def batch_answers(values, name, trained):
    ...
    choices = majority.batch_answers(values, majority.CANDIDATES[0], trained)
    indices = [i for i, z in enumerate(values) if available(z)]
    if indices:
        head = trained["us_etf"]
        if (
            head is None
            or tuple(head["feature_indices"]) != FEATURE_INDICES[name]
            or head["model"].n_features_in_ != len(FEATURE_INDICES[name])
        ):
            raise ValueError("ETF_FUND_STATE_MODEL_SCHEMA_CHANGED")
        x = np.asarray([selected_features(values[i], name) for i in indices])
        scores = head["model"].predict_proba(x)[:, 1]
        if not np.isfinite(scores).all() or min(scores) < 0 or max(scores) > 1:
            raise ValueError("ETF_FUND_STATE_SCORE_INVALID")
...
def fit_checkpoint(rows, name, cutoff, label):
    """沿用旧HK，另校验LR3对照训练题一致；本轮每个树候选仅拟合一次。"""
    ...
    if (
        ...
        raise ValueError("ETF_FUND_STATE_PRIOR_CHECKPOINT_CHANGED")
    ...
    if base.digest([r | {"z": r["z"][:20]} for r in shared]) != prior_head["fit_hash"]:
        raise ValueError("ETF_FUND_STATE_LR3_CONTROL_ROWS_CHANGED")
...
def train():
    ...
    for name, expected in p["input_hashes"].items():
        if base.digest(base.read(base.ROOT / name)) != expected:
            raise ValueError("ROUND_55_INPUT_CHANGED")
    rows, proofs = dataset()
    ...
    with threadpool_limits(limits=2):
        ...
        if len(expected) != 5670 or any(
            ...
            raise ValueError("ROUND_55_COMMON_EXAM_CHANGED")
...
def models():
    ...
    if (
        ...
        raise ValueError("ROUND_55_MODEL_OR_CODE_CHANGED")
...
def report():
    ...
    for path in (root() / "forward").glob("*/*.json"):
        ...
        if (
            ...
            raise ValueError("ROUND_55_PARENT_OR_MODEL_CHANGED")
        ...
        if not np.allclose(
            ...
            raise ValueError("ROUND_55_VECTOR_CHANGED")
        ...
        if not runtime.answers_match(value["answers"], expected_answers):
            raise ValueError("ROUND_55_SAVED_ANSWER_CHANGED")
        good += 1

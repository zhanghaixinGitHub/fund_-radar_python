"""第104轮复用 CORE3 输入，独立保存KURE实际响应，不调用已退役的旧实验链。"""

from collections import defaultdict
from datetime import datetime

from app.services import direction_1d_sprint as base
from app.services import direction_1d_sprint_china_health_live as live
from app.services import direction_1d_sprint_core_forward as core
from app.services import direction_1d_sprint_futures_style_forward as prior_forward
from app.services import direction_1d_sprint_market_china_health as model
from app.services import direction_1d_sprint_market_futures_style as prior_model
...
def validate(value, receipt, parent, source, manifest, bundle, extra, history):
    ...
    model.require(value["r103_forecast_hash"] == base.digest(extra), "R103_PARENT_CHANGED")
    ...
    model.require(
        ...
        "R103_PARENT_READBACK_LATE_OR_CHANGED",
    )
    model.require(
        value["health_source_hash"] == base.digest(history) and datetime.fromisoformat(history["at"]) <= created,
        "HEALTH_SOURCE_CHANGED_OR_LATE",
    )
    model.require(
        history == live.load_live(parent["t"], parent["u"], manifest["plan_hash"]), "ACTUAL_HEALTH_SOURCE_CHANGED"
    )
...
def run():
    # 先在实际时窗内只读新医疗行情，再续接原比较链；不以历史文件充当新实收。
    p = model.plan()
    ...
    for key, p in parents.items():
        ...
        value = {k: p[k] for k in ("code", "family", "group", "t", "u")} | {
            ...
            "r103_forecast_hash": base.digest(extra),
            "health_source_hash": base.digest(history[p["u"]]),
        }
...
def combined_answers(parent, extra, value):
    """明确合并全部22个同题分支，避免新增中间模型后递归层级漂移丢掉旧对照。"""
    combined = dict(parent["answers"])
    for number in (92, 94, 95, 96, 97, 98, 99, 100, 101, 102):
    ...
    model.require(len(combined) == 22, "FORWARD_BRANCH_COUNT_CHANGED")
    return combined
...
def report(loaded=None):
    ...
    value = {
        ...
        "new_source_requests": len(list(live.root().glob("*/request.json"))),
        "new_cost_cny": 0,
        "status": "MODEL_NOT_RELEASED",
    }

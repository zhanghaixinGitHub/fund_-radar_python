"""第97轮复用 CORE3 输入，并独立有界取得T日IO成交持仓行情，不调用已退役的旧实验链。"""
...
from app.services import direction_1d_sprint_futures_intraday_forward as prior_forward
from app.services import direction_1d_sprint_market_futures_intraday as prior_model
from app.services import direction_1d_sprint_market_option_position as model
...
def validate(value, receipt, parent, source, manifest, bundle, extra, history):
    ...
    model.require(value["r96_forecast_hash"] == base.digest(extra), "R96_PARENT_CHANGED")
    ...
    model.require(
        ...
        "R96_PARENT_READBACK_LATE_OR_CHANGED",
    )
    model.require(
        value["option_source_hash"] == base.digest(history) and datetime.fromisoformat(history["at"]) <= created,
...
def context():
    ...
    for key, p in candidates.items():
        ...
        if p["u"] not in history:
            if (model.data.live_root() / p["u"] / "snapshot.json").exists():
...
def run():
    # 先完成已有CORE3及R96分支，再在同一实际输入上增加独立期权分布模型。
    prior_forward.run()
    manifest, bundle, ctx, parents, extras, history = context()
    sources = {}
    for p in {p["u"]: p for p in parents.values()}.values():
        if p["u"] not in history:
            snapshot = model.data.capture(p["t"], p["u"], manifest["plan_hash"])
            if snapshot is not None:
                history[p["u"]] = snapshot
    for key, p in parents.items():
        ...
        value = {k: p[k] for k in ("code", "family", "group", "t", "u")} | {
            ...
            "r96_forecast_hash": base.digest(extra),
            "option_source_hash": base.digest(history[p["u"]]),
        }
...
def report(loaded=None):
    ...
    for path in sorted((model.root() / "forward").glob("*/*.json")):
        ...
        # 父上下文已逐层验证这些答案；报告统一保留全部15分支同题对照。
        combined = dict(p["answers"])
        for round_number in (92, 94, 95):
            record = base.read(base.ROOT / f"round-{round_number:02d}" / "forward" / p["u"] / path.name)
            combined.update(record["answers"])
        combined.update(extras[p["code"], p["u"]]["answers"])
        combined.update(value["answers"])
        model.require(len(combined) == 15, "FORWARD_BRANCH_COUNT_CHANGED")
        for name, answer in combined.items():
    ...
    value = {
        ...
        "new_source_requests": len(history),
        "new_cost_cny": 0,
        "status": "MODEL_NOT_RELEASED",
    }

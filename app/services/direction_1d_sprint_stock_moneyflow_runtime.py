"""第117轮前向运行独立绑定：不改已冻结训练代码，待R116完成后一次性冻结接入关系。"""
...
from app.services import direction_1d_sprint_market_stock_tail as parent
from app.services import direction_1d_sprint_stock_tail_runtime as parent_runtime
...
def verify_intent():
    ...
    model.require(
        ...
        and intent["parent_round"] == 116
        and intent["same_question_branches"] == 36
...
def plan():
    """R117训练与R116运行链均完成后才绑定；父运行层未完成时拒绝占位接入。"""
    intent = verify_intent()
    ...
    context = {
        ...
        "code_hashes": code,
        "artifact_hashes": {
            "round-117/plan.json": trained["plan_hash"],
            "round-117/result.json": base.digest(trained),
            "round-116/plan.json": prior["plan_hash"],
            "round-116/result.json": base.digest(prior),
            "round-116/runtime-plan.json": base.digest(parent_run),
        },
    }

"""第112轮前向运行独立绑定：不改已冻结训练代码，待R111完成后一次性冻结接入关系。"""
...
from app.services import direction_1d_sprint_dollar_etf_runtime as parent_runtime
from app.services import direction_1d_sprint_market_credit_pair as model
from app.services import direction_1d_sprint_market_dollar_etf as parent
...
def verify_intent():
    ...
    model.require(
        ...
        and intent["parent_round"] == 111
        and intent["same_question_branches"] == 30
        and intent["new_request_budget"] == 2
...
def plan():
    """R112训练与R111运行链均完成后才绑定；父运行层未完成时拒绝占位接入。"""
    intent = verify_intent()
    ...
    context = {
        ...
        "code_hashes": code,
        "artifact_hashes": {
            "round-112/plan.json": trained["plan_hash"],
            "round-112/result.json": base.digest(trained),
            "round-111/plan.json": prior["plan_hash"],
            "round-111/result.json": base.digest(prior),
            "round-111/runtime-plan.json": base.digest(parent_run),
        },
    }

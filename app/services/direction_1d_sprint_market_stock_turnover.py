"""第110轮：T日沪深股票成交额权重的方向与涨幅，保持原标签、训练样本和对照。"""
...
from app.services import direction_1d_sprint_market_stock_breadth as previous
...
from app.services import direction_1d_sprint_stock_turnover_data as data

original, active, reference = previous.original, previous.active, previous.reference
CANDIDATES = ("SIGN3_STOCK_TURNOVER5_LR504",)
CONTROLS = previous.CONTROLS
BRANCHES = CANDIDATES + CONTROLS
FIRST_TARGET = "2026-09-17"
PROPOSAL_HASH = "61307992c5df0bdc7fe551385b439fb1fd9a3568d38aba0ec5f8104bab34933e"


def root():
    return base.ROOT / "round-110"


def require(condition, reason):
    if not condition:
        raise ValueError("STOCK_TURNOVER_" + reason)


def fingerprint():
    code = dict(base.read(base.ROOT / "round-109" / "plan.json")["fingerprint"]["code"])
    ...
    for name, expected in source_plan["code"].items():
        ...
        code[name] = expected
    for name in (
        "app/services/direction_1d_sprint_market_stock_turnover.py",
        "app/services/direction_1d_sprint_stock_turnover_forward.py",
        "scripts/direction_1d_sprint_market_stock_turnover.py",
        "tests/test_direction_1d_sprint_market_stock_turnover.py",
    ):
...
def plan():
    ...
    require(proposal["r109_result_hash"] == base.digest(base.read(previous.root() / "result.json")), "SOURCE_CHANGED")
    require(
    ...
    require(base.digest(source) == proposal["source_qualification_hash"], "TURNOVER_SOURCE_CHANGED")
    coverage = base.read(root() / "feature-feasibility.json")
    require(
        coverage["source_history_hash"] == source["source_proof"]["source_history_hash"]
        and all(x["turnover_available"] == x["questions"] for x in coverage["exam"].values())
...
def reconstruct_dataset():
    """按共享市场日期缓存新增沪深成交额分布，避免为同日30基金重复解析整份交易日历。"""
    ...
    for row in rows:
        key = row["t"], row["u"]
        if key not in cache:
            cache[key] = data.extend({}, *key, h["snapshot"]["rows"])["stock_turnover"]
        extra.append(row | {"market": row["market"] | {"stock_turnover": cache[key]}})
    require([data.original_row(r) for r in extra] == rows, "ORIGINAL_ROWS_CHANGED")
    proof = proof | {"stock_turnover_history_hash": base.digest(h)}
    return extra, proof
...
def vector(value):
    ...
    feature = value["stock_turnover"]
    require(feature["available"], "STOCK_TURNOVER_MISSING")
    extra = [feature[k] for k in data.KEYS]
    require(
        all(type(v) in (int, float) and np.isfinite(v) for v in extra) and abs(extra[0]) <= 1 and abs(extra[1]) <= 20,
        "STOCK_TURNOVER_INVALID",
    )
    return [*x, *extra]
...
def fit(rows, name, cutoff):
    ...
    require(
        ...
        and all(
            r["mature"] < cutoff and r["u"] < cutoff and r["market"]["stock_turnover"]["available"] for r in chosen
        ),
    ...
    return {
        ...
        "max_feature_date": max(r["market"]["stock_turnover"]["date"] for r in chosen),
...
def candidate_answers(values, name, head, fallback):
    ...
    indices = [i for i, v in enumerate(values) if v["available"] and v["stock_turnover"]["available"]]
    if indices:

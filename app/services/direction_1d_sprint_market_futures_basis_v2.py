"""第94轮：前一交易日IF期现价差追加到原三个市场输入，保持原标签、训练样本和对照。"""
...
PROPOSAL_HASH = "1934c7815cd7c7dcabf47efb278accc807932c8e6bdebd3341c479d7494bb79a"


def root():
    return base.ROOT / "round-94"
...
def fingerprint():
    code = dict(base.read(base.ROOT / "round-93" / "plan.json")["fingerprint"]["code"])
    for name, expected in code.items():
        require(core.sha(base.PROJECT / name) == expected, "OLD_CODE_CHANGED")
    for name in (
        "app/services/direction_1d_sprint_market_futures_basis_v2.py",
        "app/services/direction_1d_sprint_futures_basis_forward_v2.py",
        "scripts/direction_1d_sprint_market_futures_basis_v2.py",
        "tests/test_direction_1d_sprint_market_futures_basis_v2.py",
    ):
...
def preflight():
    from app.services import direction_1d_sprint_futures_basis_forward_v2 as forward

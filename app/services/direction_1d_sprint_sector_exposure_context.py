...
def market_returns(row):
    ...
    if len(z) != 17 or not np.isfinite(z).all():
        raise ValueError("EXPOSURE_MARKET_INPUT_INVALID")
...
def context(rows, cutoff):
    ...
    基金收益先减中证500收益，斜率以该残差与行业相对收益的百分数协方差除以市场方差加0.25，再乘n/(n+126)并限幅±3。
    未成熟记录先排除，其输入即使缺失或修订也不得影响当前上下文。
    """
    date.fromisoformat(cutoff)
    if len({r["code"] for r in rows}) > 1:
        raise ValueError("EXPOSURE_CONTEXT_MULTIPLE_FUNDS")
    ...
    if len({r["u"] for r in eligible}) != len(eligible):
        raise ValueError("EXPOSURE_CONTEXT_DUPLICATE_DATE")
    ...
    if not np.isfinite(markets).all() or not np.isfinite(funds).all():
        raise ValueError("EXPOSURE_CONTEXT_NONFINITE")
...
def dataset():
    ...
    if base.digest(derived) != expected["feature_hash"] or actual_contexts != snapshot["contexts"]:
        raise ValueError("EXPOSURE_PROTOTYPE_RESULT_CHANGED")
    return output, proof | {
        "sector_exposure_feature_hash": base.digest(derived),

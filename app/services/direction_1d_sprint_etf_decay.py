"""第六十五轮：相同三项市场输入和504日期，仅提高近期训练题的损失权重。

尺度仍用原自然权重计算，保持特征正则化口径；只检验固定63个观察日期的半衰期。
"""
...
from app.services import direction_1d_sprint_ashare_residual as previous_round
...
CANDIDATES = ("SPX_FXI_CNYA_DECAY63_LR3_504",)
LEARNED = CANDIDATES
...
def fingerprint():
    ...
    for name in (
        "app/services/direction_1d_sprint_etf_decay.py",
        "scripts/direction_1d_sprint_etf_decay.py",
        "tests/test_direction_1d_sprint_etf_decay.py",
    ):
...
def selected_features(z, name):
    if name not in CANDIDATES:
        raise ValueError("ETF_DECAY_RECIPE_INVALID")
    return reference.selected_features(z, reference.LEARNED[0])


def decay_weights(rows):
    """沿用已验证的交易观察日年龄与产品家族去重，半衰期固定63日，不平衡涨跌。"""
    return adaptive.weights(rows, "MONTHLY_NAT252")
...
def plan():
    ...
    if path.exists():
        ...
        if value["fingerprint"] != fingerprint() or value["calendar_hash"] != base.calendar()[1]:
            raise ValueError("ROUND_65_CODE_OR_CALENDAR_CHANGED")
        return value
    active()
    ...
    if (
        ...
        raise ValueError("ETF_DECAY_PROPOSAL_CHANGED")
    ...
    value = {
        ...
        "features": "ExactR50raw20, selected0/12/17, original naturalweighted3scales; only lossweights decay",
        "training": "Same504maturejointavailabledates,quarterlyrefresh; half-life63observeddates,nonbalanced",
        ...
        | {"round-65/proposal-before-implementation.json": base.digest(proposal)},
...
def fit(rows, name, cutoff):
    ...
    return {
        ...
        "loss_half_life_observed_dates": 63,
...
def batch_answers(values, name, trained):
    """推断及缺源回退完全复用原LR3路径，权重差异仅存在于训练后的系数。"""
    if name not in CANDIDATES:
        raise ValueError("ETF_DECAY_RECIPE_INVALID")
...
def train():
    ...
    for name, expected in p["input_hashes"].items():
        if base.digest(base.read(base.ROOT / name)) != expected:
            raise ValueError("ROUND_65_INPUT_CHANGED")
    rows, proofs = dataset()
    ...
    with threadpool_limits(limits=2):
        ...
        if len(expected) != 5670 or any(
            ...
            raise ValueError("ROUND_65_COMMON_EXAM_CHANGED")
...
def models():
    ...
    if (
        ...
        raise ValueError("ROUND_65_MODEL_OR_CODE_CHANGED")
...
def report():
    ...
    for path in (root() / "forward").glob("*/*.json"):
        ...
        if (
            ...
            raise ValueError("ROUND_65_PARENT_OR_MODEL_CHANGED")
        ...
        if not np.allclose(
            ...
            raise ValueError("ROUND_65_VECTOR_CHANGED")
        ...
        if not runtime.answers_match(value["answers"], expected_answers):
            raise ValueError("ROUND_65_SAVED_ANSWER_CHANGED")
        good += 1

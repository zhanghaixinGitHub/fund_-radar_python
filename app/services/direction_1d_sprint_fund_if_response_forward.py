"""第108轮复用 CORE3 输入，并复用已实际取得T日原始IF行情，不调用已退役的旧实验链。"""
...
from app.services import direction_1d_sprint_market_fund_if_response as model
from app.services import direction_1d_sprint_market_option_price_weighted as prior_model
from app.services import direction_1d_sprint_option_price_weighted_forward as prior_forward
...
def validate(value, receipt, parent, source, manifest, bundle, extra, history):
    ...
    model.require(value["r107_forecast_hash"] == base.digest(extra), "R107_PARENT_CHANGED")
    ...
    model.require(
        ...
        "R107_PARENT_READBACK_LATE_OR_CHANGED",
    )
    model.require(
        value["fund_if_source_hash"] == base.digest(history) and datetime.fromisoformat(history["at"]) <= created,
        "OPTION_SOURCE_CHANGED_OR_LATE",
    )
    source_parent = base.read(base.ROOT / "round-96" / "forward" / parent["u"] / f"{parent['code']}.json")
    model.require(
        history["t"] == parent["t"]
        and history["u"] == parent["u"]
        and history["parent_plan_hash"] == source_parent["plan_hash"]
        and history["parent_source_hash"] == source_parent["intraday_source_hash"],
    ...
    model.require(
        ...
        "ACTUAL_FUND_IF_SOURCE_CHANGED",
    )
    market = model.data.extend(
        source["market"], parent["t"], parent["u"], history["snapshot"]["points"], parent["code"], history["contexts"]
    )
    ...
    for key, p in candidates.items():
        ...
        if p["u"] not in history:
            history[p["u"]] = model.data.live(
                p["t"], p["u"], base.digest(base.read(base.ROOT / "round-96" / "plan.json"))
            )
...
def run():
    # 先完成已有CORE3及R107分支，再增加基金各自过去反应；复用真实IF源。
    prior_forward.run()
    ...
    for key, p in parents.items():
        ...
        market = model.data.extend(
            ...
            p["t"],
            p["u"],
            history[p["u"]]["snapshot"]["points"],
            p["code"],
            history[p["u"]]["contexts"],
        )
        ...
        value = {k: p[k] for k in ("code", "family", "group", "t", "u")} | {
            ...
            "r107_forecast_hash": base.digest(extra),
            "fund_if_source_hash": base.digest(history[p["u"]]),
        }
...
def combined_answers(parent, extra, value):
    """明确合并全部26个同题分支，避免新增中间模型后递归层级漂移丢掉旧对照。"""
    combined = dict(parent["answers"])
    for number in (92, 94, 95, 96, 97, 98, 99, 100, 101, 102, 103, 104, 105, 106):
    ...
    model.require(len(combined) == 26, "FORWARD_BRANCH_COUNT_CHANGED")
    return combined
...
def preflight():
    ...
    history = model.data.source.data.history()
    contexts = model.data.current()["contexts"]
    checks = 0
    for f in checked:
        actual = model.answers(
            model.data.extend(source["market"], f["t"], f["u"], history["snapshot"]["rows"], f["code"], contexts),
            f["group"],
            bundle,
        )

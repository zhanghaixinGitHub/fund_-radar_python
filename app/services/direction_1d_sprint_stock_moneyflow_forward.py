"""第117轮：提前采集一次订单分类金额，复用R109报价验覆盖，保留36个同题分支。"""
...
from app.services import direction_1d_sprint_market_stock_moneyflow as model
from app.services import direction_1d_sprint_market_stock_tail as prior_model
from app.services import direction_1d_sprint_stock_moneyflow_live as live
from app.services import direction_1d_sprint_stock_moneyflow_runtime as runtime
from app.services import direction_1d_sprint_stock_tail_forward as prior_forward
...
def validate(value, receipt, parent, source, manifest, bundle, extra, history):
    ...
    model.require(value["r116_forecast_hash"] == base.digest(extra), "R116_PARENT_CHANGED")
    ...
    model.require(
        ...
        "R116_PARENT_READBACK_LATE_OR_CHANGED",
    )
    model.require(
        value["stock_moneyflow_source_hash"] == base.digest(history)
        and datetime.fromisoformat(history["at"]) <= created,
        "STOCK_MONEYFLOW_SOURCE_CHANGED_OR_LATE",
    )
    model.require(
        history == live.load_live(parent["t"], parent["u"], manifest["plan_hash"]),
        "ACTUAL_STOCK_MONEYFLOW_SOURCE_CHANGED",
    )
    model.require(
        live.in_window(parent["t"], parent["u"], datetime.fromisoformat(history["at"])),
        "STOCK_MONEYFLOW_SOURCE_NOT_ACTUALLY_CAPTURED_IN_WINDOW",
    )
...
def run():
    # 先采集两个有实际时限的当日输入；R109原入口会复用同一请求，不增加其预算。
    rp = runtime.plan()
    ...
    if today >= model.FIRST_TARGET and today in days and days.index(today) > 0:
        t = days[days.index(today) - 1]
        live.quote_live.capture(t, today, base.digest(base.read(base.ROOT / "round-109/plan.json")))
        live.capture(t, today, base.digest(base.read(model.root() / "plan.json")))
    prior_forward.run()
    ...
    for key, p in parents.items():
        ...
        value = {k: p[k] for k in ("code", "family", "group", "t", "u")} | {
            ...
            "r116_forecast_hash": base.digest(extra),
            "stock_moneyflow_source_hash": base.digest(history[p["u"]]),
        }
...
def combined_answers(parent, extra, value):
    """明确合并全部36个同题分支，避免新增中间模型后递归层级漂移丢掉旧对照。"""
    ...
    model.require(len(combined) == 36, "FORWARD_BRANCH_COUNT_CHANGED")
    return combined
...
def report(loaded=None):
    ...
    value = {
        ...
        "reserved_request_slots": sum(v["reserved_request_slots"] for v in history.values()),

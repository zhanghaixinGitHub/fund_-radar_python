"""第104轮：新增美国会话医疗行业同日对数收益，保持原标签、训练样本和对照。"""
...
from app.services import direction_1d_sprint_market_futures_style as previous
...
CANDIDATES = ("SIGN3_KURE_INTRADAY4_LR504",)
CONTROLS = previous.CONTROLS
BRANCHES = CANDIDATES + CONTROLS
FIRST_TARGET = "2026-09-17"
PROPOSAL_HASH = "ab91864a2c36c2a5d5ee6b3d80c57610604631ee357d52d354fc6db4cebd6ff8"


def root():
    return base.ROOT / "round-104"


def require(condition, reason):
    if not condition:
        raise ValueError("KURE_MODEL_" + reason)


def fingerprint():
    code = dict(base.read(base.ROOT / "round-103" / "plan.json")["fingerprint"]["code"])
    for name, expected in code.items():
        require(core.sha(base.PROJECT / name) == expected, "OLD_CODE_CHANGED")
    for name in (
        "app/services/direction_1d_sprint_market_china_health.py",
        "app/services/direction_1d_sprint_china_health_forward.py",
        "app/services/direction_1d_sprint_china_health_live.py",
        "scripts/direction_1d_sprint_market_china_health.py",
        "tests/test_direction_1d_sprint_market_china_health.py",
        "tests/test_direction_1d_sprint_china_health_live.py",
        "app/services/direction_1d_sprint_china_health_data.py",
        "tests/test_direction_1d_sprint_china_health_data.py",
        "app/integrations/sina_sprint_china_health_etf.py",
        "app/integrations/sina_sprint_china_health_decode.cjs",
        "tests/test_sina_sprint_china_health_etf.py",
        ".local-runs/direction-1d-sprint-20260914/china-health-etf-feasibility-v1/probe.py",
        ".local-runs/direction-1d-sprint-20260914/china-health-etf-feasibility-v1/qualify.py",
        ".local-runs/direction-1d-sprint-20260914/china-health-etf-feasibility-v1/fetch_primary.py",
        ".local-runs/direction-1d-sprint-20260914/china-health-etf-feasibility-v1/fetch_factsheet.py",
    ):
...
def plan():
    ...
    require(proposal["r103_result_hash"] == base.digest(base.read(previous.root() / "result.json")), "SOURCE_CHANGED")
    require(
...
def reconstruct_dataset():
    """按共享市场日期缓存医疗同日收益，避免为同日30基金重复解析整份交易日历。"""
    ...
    for row in rows:
        key = row["t"], row["u"]
        if key not in cache:
            cache[key] = data.extend({}, *key, h["points"])["china_health"]
        extra.append(row | {"market": row["market"] | {"china_health": cache[key]}})
    require([data.original_row(r) for r in extra] == rows, "ORIGINAL_ROWS_CHANGED")
    proof = proof | {"china_health_history_hash": base.digest(h)}
    return extra, proof
...
def vector(value):
    ...
    feature = value["china_health"]
    require(type(feature["available"]) is bool, "INTRADAY_INVALID")
    extra = feature["intraday_log_pct"]
    require(type(extra) in (int, float) and np.isfinite(extra) and abs(extra) <= 20, "INTRADAY_INVALID")
    require(feature["available"] or extra == 0.0, "MISSING_INPUT_NOT_ZERO")
    return [*x, extra]
...
def fit(rows, name, cutoff):
    ...
    return {
        ...
        "max_feature_date": max(d for r in chosen for d in r["market"]["china_health"]["new_us_dates"]),
        "training_feature_mean_not_subtracted": mean,
        "masked_training_rows": sum(not r["market"]["china_health"]["available"] for r in chosen),
...
def validate_head(head, name, cutoff=None):
    ...
    require(
        ...
        and model.n_features_in_ == 4
        and model.coef_.shape == (1, 4)
...
def candidate_answers(values, name, head, fallback):
    ...
    indices = [i for i, v in enumerate(values) if v["available"] and v["china_health"]["available"]]
    if indices:
...
def preflight():
    from app.services import direction_1d_sprint_china_health_forward as forward

"""第93轮：前一交易日IF期现价差追加到原三个市场输入，保持原标签、训练样本和对照。"""
...
from app.services import direction_1d_sprint_futures_basis_data as data
from app.services import direction_1d_sprint_market_block_newton as previous
...
CANDIDATES = ("INTERVAL_RAW3_IF_BASIS_LR4_504", "INTERVAL_SIGN3_IF_BASIS_LR4_504")
CONTROLS = previous.CONTROLS
BRANCHES = CANDIDATES + CONTROLS
FIRST_TARGET = "2026-09-17"
PROPOSAL_HASH = "967e7109b191bdcb0adccc030fb84dbf6cea4b4f69f2fe46a9174dfbf59e479b"


def root():
    return base.ROOT / "round-93"


def require(condition, reason):
    if not condition:
        raise ValueError("IF_BASIS_" + reason)
...
def fingerprint():
    ...
    for name in (
        "app/integrations/tushare_sprint_futures.py",
        "tests/test_tushare_sprint_futures.py",
        "app/services/direction_1d_sprint_futures_basis_data.py",
        "tests/test_direction_1d_sprint_futures_basis_data.py",
        "app/services/direction_1d_sprint_market_futures_basis.py",
        "app/services/direction_1d_sprint_futures_basis_forward.py",
        "scripts/direction_1d_sprint_market_futures_basis.py",
        "tests/test_direction_1d_sprint_market_futures_basis.py",
    ):
...
def plan():
    ...
    require(proposal["r92_result_hash"] == base.digest(base.read(previous.root() / "result.json")), "SOURCE_CHANGED")
    ...
    coverage = base.read(data.root() / "feature-feasibility.json")
    require(
        coverage["history_hash"] == source["history_hash"]
        and all(x["missing_futures"] == 0 for x in coverage["exam"].values())
        and len(coverage["training_windows"]) == 27
        and all(x["missing_futures"] == 0 for x in coverage["training_windows"]),
...
def dataset():
    """按共享市场日期缓存新增价差，避免为同日30基金重复解析整份交易日历。"""
    ...
    for row in rows:
        key = row["t"], row["u"]
        if key not in cache:
            cache[key] = data.extend({}, *key, h["snapshot"]["rows"])["futures_basis"]
        extra.append(row | {"market": row["market"] | {"futures_basis": cache[key]}})
    require([data.original_row(r) for r in extra] == rows, "ORIGINAL_ROWS_CHANGED")
    proof = proof | {"futures_history_hash": base.digest(h)}
    require(
        base.digest(proof) == base.read(data.root() / "feature-feasibility.json")["question_proof_hash"],
        "QUESTION_PROOF_CHANGED",
    )
    return extra, proof
...
def vector(value):
    ...
    feature = value["futures_basis"]
    require(
        feature["available"]
        and type(feature["basis_pct"]) in (int, float)
        and np.isfinite(feature["basis_pct"])
        and -20 <= feature["basis_pct"] <= 20,
        "BASIS_MISSING_OR_INVALID",
    )
    return [*x, feature["basis_pct"]]
...
def fit(rows, name, cutoff):
    ...
    require(
        ...
        and all(r["mature"] < cutoff and r["u"] < cutoff and r["market"]["futures_basis"]["available"] for r in chosen),
    ...
    return {
        ...
        "max_basis_date": max(r["market"]["futures_basis"]["date"] for r in chosen),
...
def validate_head(head, name, cutoff=None):
    ...
    require(
        ...
        and head["max_basis_date"] < head["fit_end"],
        "HEAD_TIME_INVALID",
    )
...
def candidate_answers(values, name, head, fallback):
    """新增信息缺失时保留匹配原模型答案，不能删除该题或把缺失价差当成零。"""
    require(name in CANDIDATES and len(values) == len(fallback), "UNKNOWN_BRANCH_OR_ROWS")
    output = list(fallback)
    indices = [i for i, v in enumerate(values) if v["available"] and v["futures_basis"]["available"]]
    if indices:
...
def batch(values, heads, control_heads):
    ...
    for i, name in enumerate(CANDIDATES):
        output[name] = candidate_answers(values, name, heads.get(name), controls[CONTROLS[i]])
    return output

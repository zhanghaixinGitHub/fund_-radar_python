"""第112轮复用 CORE3 输入，独立保存HYG/IEF实际响应，不调用已退役的旧实验链。"""
...
from app.services import direction_1d_sprint_credit_pair_live as live
from app.services import direction_1d_sprint_credit_pair_runtime as runtime
from app.services import direction_1d_sprint_dollar_etf_forward as prior_forward
from app.services import direction_1d_sprint_market_credit_pair as model
from app.services import direction_1d_sprint_market_dollar_etf as prior_model
...
def validate(value, receipt, parent, source, manifest, bundle, extra, history):
    ...
    model.require(value["r111_forecast_hash"] == base.digest(extra), "R111_PARENT_CHANGED")
    ...
    model.require(
        ...
        "R111_PARENT_READBACK_LATE_OR_CHANGED",
    )
    model.require(
        value["credit_pair_source_hash"] == base.digest(history) and datetime.fromisoformat(history["at"]) <= created,
        "CREDIT_SOURCE_CHANGED_OR_LATE",
    )
    model.require(
        history == live.load_live(parent["t"], parent["u"], manifest["plan_hash"]), "ACTUAL_CREDIT_SOURCE_CHANGED"
    )
    model.require(
        live.in_window(parent["t"], parent["u"], datetime.fromisoformat(history["at"])),
        "CREDIT_SOURCE_NOT_ACTUALLY_CAPTURED_IN_WINDOW",
    )
...
def context():
    ...
    for key, p in candidates.items():
        ...
        if not (live.root() / p["u"] / "snapshot.json").exists():
            continue
...
def run():
    # 先在实际时窗内只读HYG/IEF双腿行情，再续接原比较链；不以历史文件充当新实收。
    rp = runtime.plan()
    p = model.plan()
    today = str(base.now().astimezone(base.ZONE).date())
    days = [str(d) for d in base.calendar()[0]]
    if today >= model.FIRST_TARGET and today in days and days.index(today) > 0:
        live.capture(days[days.index(today) - 1], today, base.digest(p))
    prior_forward.run()
    ...
    for key, p in parents.items():
        ...
        value = {k: p[k] for k in ("code", "family", "group", "t", "u")} | {
            ...
            "r111_forecast_hash": base.digest(extra),
            "credit_pair_source_hash": base.digest(history[p["u"]]),
        }
...
def combined_answers(parent, extra, value):
    """明确合并全部30个同题分支，避免新增中间模型后递归层级漂移丢掉旧对照。"""
    combined = dict(parent["answers"])
    for number in (92, 94, 95, 96, 97, 98, 99, 100, 101, 102, 103, 104, 105, 106, 107, 108, 109, 110):
    ...
    model.require(len(combined) == 30, "FORWARD_BRANCH_COUNT_CHANGED")
    return combined
...
def report(loaded=None):
    ...
    value = {
        ...
        "reserved_request_slots": len(list(live.root().glob("*/*/request.json"))),
...
def preflight():
    ...
    for f in checked:
        actual = model.answers(
            model.data.extend(source["market"], f["t"], f["u"], history["points"]), f["group"], bundle
        )

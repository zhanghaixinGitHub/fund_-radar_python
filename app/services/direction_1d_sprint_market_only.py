"""第70轮：按纯市场模型的实际依赖扩充训练，标签仍为相邻交易日原始净值方向。
...
from app.services import direction_1d_sprint_etf_fundstate_linear as previous
from app.services import direction_1d_sprint_etf_joint as r50
from app.services import direction_1d_sprint_etf_transform as r67
...
CANDIDATES = ("EXPANDED_MARKET_LR3_504", "EXPANDED_MARKET_SIGN_LR3_504")
CONTROLS = ("FROZEN_R50_MARKET_LR3", "FROZEN_R67_MARKET_SIGN_LR3", "MARKET_MAJORITY3", "SPX_SIGN", "ALWAYS_UP")
BRANCHES = CANDIDATES + CONTROLS
FIRST_TARGET = "2026-09-16"
PROPOSAL_HASH = "cc3e6d515b91558411bd73562ab21f67ad0fff872007c994665ea1a99f6fa9c9"
active = regression.active


def root():
    return base.ROOT / "round-70"


def fingerprint():
    value = previous.fingerprint()
    for name in (
        "app/services/direction_1d_sprint_market_only.py",
        "app/services/direction_1d_sprint_market_only_data.py",
        "app/services/direction_1d_sprint_market_only_forward.py",
        "scripts/direction_1d_sprint_market_only.py",
        "tests/test_direction_1d_sprint_market_only.py",
        "tests/test_direction_1d_sprint_market_only_data.py",
        "tests/test_direction_1d_sprint_market_only_forward.py",
    ):
...
def control_path(name, group, label):
    service, original = (r50, r50.LEARNED[0]) if name == CONTROLS[0] else (r67, r67.CANDIDATES[1])
...
def plan():
    ...
    paths = [
        "history.json",
        "round-70/proposal-before-implementation.json",
    ...
    p = {
        ...
        "old_questions": 5670,
        "training": "504mature fullmarket dates pergroup; natural date/family weights; C.1/nointercept/seed17/max1500",
...
def frozen_head(name, group, label, cutoff):
    """只复用已冻结的三输入头；原HK回退不进入新比较，旧模型和答案保持不变。"""
...
def tick():
    from app.services import direction_1d_sprint_market_only_forward as forward

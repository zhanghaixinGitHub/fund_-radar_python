"""第二十八轮：固定梯度提升算法学习下一日SPX方向规则的错误。

使用第17轮相同HK12输入和504个已成熟目标日期，检验顺序提升树能否改善随机树的纠错。
沿用第13轮事前固定的梯度提升配方，不搜索参数；预测仍是相邻交易日原始单位净值方向。
"""
...
from app.services import direction_1d_sprint_fx_basket as previous_round
...
def fingerprint():
    ...
    for name in (
        "app/services/direction_1d_sprint_boost.py",
        "scripts/direction_1d_sprint_boost.py",
        "tests/test_direction_1d_sprint_boost.py",
    ):
...
def selected_features(z, name):
    if name not in CANDIDATES or len(z) != 12 or not np.isfinite(z).all():
        raise ValueError("BOOST_INPUT_INVALID")
    return z
...
def plan():
    ...
    if path.exists():
        ...
        if value["fingerprint"] != fingerprint() or value["calendar_hash"] != base.calendar()[1]:
            raise ValueError("ROUND_28_CODE_OR_CALENDAR_CHANGED")
        return value
    active()
    if base.read(previous_round.root() / "result.json")["fingerprint"] != previous_round.fingerprint():
        raise ValueError("BOOST_PREVIOUS_FINGERPRINT_CHANGED")
    parent, _ = hk.models()
    value = {
        ...
        "hypothesis": (
            "Fixed sequential boosting on HK12 may learn baseline-error interactions differently from ExtraTrees"
        ),
        "motivation": "Round13 HGB used8features; hold that algorithm fixed and use round17 exactHK12 samples",
...
def fit(rows, name, cutoff):
    """只学习已成熟样本的基线错误；固定120轮提升，不根据开发成绩提前停训。

    SPX非负时基线为上涨，错误标签为基线方向与原始下一日方向不一致。
    自然日期/家族权重保留错误率先验，不使用涨跌平衡权重。模型配置与第13轮相同。
    """
    if name not in CANDIDATES:
        raise ValueError("BOOST_RECIPE_INVALID")
    ...
    if len(Counter(y)) != 2 or min(Counter(y).values()) < 20:
        raise ValueError("BOOST_ERROR_LABELS_INSUFFICIENT")
    ...
    active()
    if model.n_iter_ != 120:
        raise ValueError("BOOST_FIT_ITERATIONS_CHANGED")
    return {
        "model": model,
        "mean": mean,
        "scale": scale,
...
def fit_checkpoint(rows, name, cutoff, label):
    ...
    if receipt.exists():
        ...
        if (
            ...
            raise ValueError("BOOST_MODEL_CHECKPOINT_CHANGED")
        return joblib.load(path)
    if attempt.exists():
        raise ValueError("BOOST_MODEL_PREVIOUS_FIT_INTERRUPTED")
    ...
    if base.digest(short) != expected:
        raise ValueError("BOOST_CONTROL_TRAINING_ROWS_CHANGED")
...
def train():
    ...
    for name, expected in p["input_hashes"].items():
        if base.digest(base.read(base.ROOT / name)) != expected:
            raise ValueError("ROUND_28_INPUT_CHANGED")
    rows, proofs = dataset()
    ...
    with threadpool_limits(limits=2):
        ...
        if len(expected) != 5670 or any(
            ...
            raise ValueError("ROUND_28_COMMON_EXAM_CHANGED")
...
def models():
    ...
    if (
        ...
        raise ValueError("ROUND_28_MODEL_OR_CODE_CHANGED")

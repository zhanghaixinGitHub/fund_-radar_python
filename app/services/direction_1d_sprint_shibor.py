"""第三十轮：基准日资金利率对一日港股纠错模型的增量影响。

保留原HK12输入，加八项Shibor水平、变化和期限差；单位与期限由独立数据模块约束。
只比较这一固定特征组合，沿用原树结构、自然错误权重和纠错门槛。
"""
...
from app.services import direction_1d_sprint_neighbors as previous_round
...
from app.services import direction_1d_sprint_shibor_data as shibor_data
from app.services import direction_1d_sprint_sparse as sparse

CANDIDATES = ("SHIBOR_HK20_ERR504",)
LEARNED = CANDIDATES
...
def fingerprint():
    ...
    for name in (
        "app/services/direction_1d_sprint_shibor.py",
        "scripts/direction_1d_sprint_shibor.py",
        "tests/test_direction_1d_sprint_shibor.py",
        "app/services/direction_1d_sprint_shibor_data.py",
        "tests/test_direction_1d_sprint_shibor_data.py",
        "app/integrations/tushare_sprint_shibor.py",
    ):
...
def vector(x, t, u, markets, points):
    return hk.vector(x, t, u, markets) + shibor_data.features(t, u, points)


def live_vector(source, original, markets, points):
    """复用父答案的净值和港股输入；Shibor日期必须等于基准日及前一国内交易日，真实响应必须在目标日截止前取得。"""
...
def selected_features(z, name):
    if name not in CANDIDATES or len(z) != 20 or not np.isfinite(z).all():
        raise ValueError("SHIBOR_MODEL_INPUT_INVALID")
    return z


def dataset():
    rows, proof = hk.dataset()
    points = shibor_data.history()
    cache, output = {}, []
    for row in rows:
        key = (row["t"], row["u"])
        if key not in cache:
            cache[key] = shibor_data.features(*key, points)
        output.append(row | {"z": row["z"] + cache[key]})
    return output, proof | {
        "at": base.now().isoformat(),
        "shibor_rows": len(points),
        "shibor_history_hash": base.digest(base.read(base.ROOT / "shibor-feasibility-v1/history.json")),
        "shibor_input_time": "exact base CN trading day T and previous CN trading day",
    }
...
def plan():
    ...
    if path.exists():
        ...
        if value["fingerprint"] != fingerprint() or value["calendar_hash"] != base.calendar()[1]:
            raise ValueError("ROUND_30_CODE_OR_CALENDAR_CHANGED")
        return value
    active()
    if base.read(previous_round.root() / "result.json")["fingerprint"] != previous_round.fingerprint():
        raise ValueError("SHIBOR_PREVIOUS_FINGERPRINT_CHANGED")
    parent, _ = hk.models()
    shibor_data.history()
    value = {
        "at": base.now().isoformat(),
        ...
        "hypothesis": "Domestic funding rate moves and term spreads may add information beyond existing price inputs",
        ...
        "feature_order": "HK12; ON/1W/3M/1Y changes bp; ONlevel%; 1W-ON/3M-1W/1Y-3M spreads bp; bpclip+-1000",
        "shibor_source": shibor_data.SOURCE,
        "shibor_url": shibor_data.URL,
        "source_timing": "Documented11:00 publication; historical firstreceipt/version assumed; actual must be<U08:30",
        ...
        "live_budget": {
            ...
            "max_bytes": shibor_data.MAX_BYTES,
            "redirects": False,
            "automatic_retries": 0,
        },
        "extra_trees": {
        ...
        | {
            ...
            for name in (
                "shibor-feasibility-v1/history-plan.json",
                "shibor-feasibility-v1/history.json",
                "shibor-feasibility-v1/coverage-result.json",
            )
        },
...
def fit(rows, name, cutoff):
    """严格沿用第17轮树配方；只追加八项已知利率输入，记录原训练样本摘要。"""
    if name not in CANDIDATES:
        raise ValueError("SHIBOR_MODEL_RECIPE_INVALID")
    ...
    if len(Counter(y)) != 2 or min(Counter(y).values()) < 20:
        raise ValueError("SHIBOR_MODEL_ERROR_LABELS_INSUFFICIENT")
...
def fit_checkpoint(rows, name, cutoff, label):
    ...
    if receipt.exists():
        ...
        if (
            ...
            raise ValueError("SHIBOR_MODEL_CHECKPOINT_CHANGED")
        return joblib.load(path)
    if attempt.exists():
        raise ValueError("SHIBOR_MODEL_PREVIOUS_FIT_INTERRUPTED")
    ...
    if base.digest([r | {"z": r["z"][:12]} for r in chosen]) != expected:
        raise ValueError("SHIBOR_CONTROL_TRAINING_ROWS_CHANGED")
...
def train():
    ...
    for name, expected in p["input_hashes"].items():
        if base.digest(base.read(base.ROOT / name)) != expected:
            raise ValueError("ROUND_30_INPUT_CHANGED")
    rows, proofs = dataset()
    ...
    with threadpool_limits(limits=2):
        ...
        if len(expected) != 5670 or any(
            ...
            raise ValueError("ROUND_30_COMMON_EXAM_CHANGED")
...
def models():
    ...
    if (
        ...
        raise ValueError("ROUND_30_MODEL_OR_CODE_CHANGED")
...
def tick():
    ...
    shibor_input = shibor_data.capture(base.now())
    ...
    with threadpool_limits(limits=2):
        for path in paths:
            ...
            z = live_vector(source, original, hk_input["rows"], shibor_input["rows"])
            ...
            value = {
                ...
                "shibor_input_hash": base.digest(shibor_input),
...
def report():
    ...
    for path in (root() / "forward").glob("*/*.json"):
        ...
        if (
            ...
            raise ValueError("ROUND_30_PARENT_OR_MODEL_CHANGED")
        hk_input = hk_live.load(value["u"])
        shibor_input = shibor_data.load(value["u"])
        if value["shibor_input_hash"] != base.digest(shibor_input):
            raise ValueError("SHIBOR_MODEL_LIVE_INPUT_CHANGED")
        if value["hk_input_hash"] != base.digest(hk_input):
            raise ValueError("SHIBOR_MODEL_LIVE_INPUT_CHANGED")
        if not np.allclose(
            value["z"], live_vector(source, original, hk_input["rows"], shibor_input["rows"]), rtol=0, atol=1e-12
        ):
            raise ValueError("ROUND_30_VECTOR_CHANGED")
        good += 1
